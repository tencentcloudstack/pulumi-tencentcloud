// *** WARNING: this file was generated by pulumi-language-dotnet. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace TencentCloudIAC.PulumiPackage.Tencentcloud.Waf
{
    /// <summary>
    /// Provides a resource to create a waf custom rule
    /// 
    /// &gt; **NOTE:** If `JobType` is `TimedJob`, Then `ExpireTime` must select the maximum time value of the `EndDateTime` in the parameter list `Timed`.
    /// 
    /// ## Example Usage
    /// 
    /// ### Create a standard custom rule
    /// 
    /// ```csharp
    /// using System.Collections.Generic;
    /// using System.Linq;
    /// using Pulumi;
    /// using Tencentcloud = TencentCloudIAC.PulumiPackage.Tencentcloud;
    /// 
    /// return await Deployment.RunAsync(() =&gt; 
    /// {
    ///     var example = new Tencentcloud.Waf.CustomRule("example", new()
    ///     {
    ///         Name = "tf-example",
    ///         SortId = "50",
    ///         Redirect = "/",
    ///         ExpireTime = "0",
    ///         Strategies = new[]
    ///         {
    ///             new Tencentcloud.Waf.Inputs.CustomRuleStrategyArgs
    ///             {
    ///                 Field = "IP",
    ///                 CompareFunc = "ipmatch",
    ///                 Content = "2.2.2.2",
    ///                 Arg = "",
    ///             },
    ///             new Tencentcloud.Waf.Inputs.CustomRuleStrategyArgs
    ///             {
    ///                 Field = "QUERY_STRING",
    ///                 CompareFunc = "rematch",
    ///                 Content = "need query string",
    ///                 Arg = "",
    ///             },
    ///         },
    ///         Status = "1",
    ///         Domain = "test.com",
    ///         ActionType = "1",
    ///     });
    /// 
    /// });
    /// ```
    /// 
    /// ### Create a timed resource for execution
    /// 
    /// ```csharp
    /// using System.Collections.Generic;
    /// using System.Linq;
    /// using Pulumi;
    /// using Tencentcloud = TencentCloudIAC.PulumiPackage.Tencentcloud;
    /// 
    /// return await Deployment.RunAsync(() =&gt; 
    /// {
    ///     var example = new Tencentcloud.Waf.CustomRule("example", new()
    ///     {
    ///         Name = "tf-example",
    ///         SortId = "50",
    ///         Redirect = "/",
    ///         ExpireTime = "1740672000",
    ///         Strategies = new[]
    ///         {
    ///             new Tencentcloud.Waf.Inputs.CustomRuleStrategyArgs
    ///             {
    ///                 Field = "IP",
    ///                 CompareFunc = "ipmatch",
    ///                 Content = "2.2.2.2",
    ///                 Arg = "",
    ///             },
    ///             new Tencentcloud.Waf.Inputs.CustomRuleStrategyArgs
    ///             {
    ///                 Field = "Referer",
    ///                 CompareFunc = "strprefix",
    ///                 Content = "https://www.demo.com",
    ///                 Arg = "",
    ///                 CaseNotSensitive = 1,
    ///             },
    ///         },
    ///         Status = "1",
    ///         Domain = "test.com",
    ///         ActionType = "1",
    ///         JobType = "TimedJob",
    ///         JobDateTime = new Tencentcloud.Waf.Inputs.CustomRuleJobDateTimeArgs
    ///         {
    ///             Timeds = new[]
    ///             {
    ///                 new Tencentcloud.Waf.Inputs.CustomRuleJobDateTimeTimedArgs
    ///                 {
    ///                     StartDateTime = 1740585600,
    ///                     EndDateTime = 1740672000,
    ///                 },
    ///             },
    ///             TimeTZone = "UTC+8",
    ///         },
    ///     });
    /// 
    /// });
    /// ```
    /// 
    /// ### Create a cron resource for execution
    /// 
    /// ```csharp
    /// using System.Collections.Generic;
    /// using System.Linq;
    /// using Pulumi;
    /// using Tencentcloud = TencentCloudIAC.PulumiPackage.Tencentcloud;
    /// 
    /// return await Deployment.RunAsync(() =&gt; 
    /// {
    ///     var example = new Tencentcloud.Waf.CustomRule("example", new()
    ///     {
    ///         Name = "tf-example",
    ///         SortId = "50",
    ///         Redirect = "/",
    ///         ExpireTime = "0",
    ///         Strategies = new[]
    ///         {
    ///             new Tencentcloud.Waf.Inputs.CustomRuleStrategyArgs
    ///             {
    ///                 Field = "IP",
    ///                 CompareFunc = "ipmatch",
    ///                 Content = "2.2.2.2",
    ///                 Arg = "",
    ///             },
    ///             new Tencentcloud.Waf.Inputs.CustomRuleStrategyArgs
    ///             {
    ///                 Field = "Referer",
    ///                 CompareFunc = "strprefix",
    ///                 Content = "https://www.demo.com",
    ///                 Arg = "",
    ///                 CaseNotSensitive = 1,
    ///             },
    ///         },
    ///         Status = "1",
    ///         Domain = "test.com",
    ///         ActionType = "1",
    ///         JobType = "CronJob",
    ///         JobDateTime = new Tencentcloud.Waf.Inputs.CustomRuleJobDateTimeArgs
    ///         {
    ///             Crons = new[]
    ///             {
    ///                 new Tencentcloud.Waf.Inputs.CustomRuleJobDateTimeCronArgs
    ///                 {
    ///                     WDays = new[]
    ///                     {
    ///                         0,
    ///                         1,
    ///                         2,
    ///                         3,
    ///                         4,
    ///                         5,
    ///                         6,
    ///                     },
    ///                     StartTime = "01:00:00",
    ///                     EndTime = "03:00:00",
    ///                 },
    ///             },
    ///             TimeTZone = "UTC+8",
    ///         },
    ///     });
    /// 
    /// });
    /// ```
    /// 
    /// ## Import
    /// 
    /// waf custom rule can be imported using the id, e.g.
    /// 
    /// ```sh
    /// $ pulumi import tencentcloud:Waf/customRule:CustomRule example test.com#1100310609
    /// ```
    /// </summary>
    [TencentcloudResourceType("tencentcloud:Waf/customRule:CustomRule")]
    public partial class CustomRule : global::Pulumi.CustomResource
    {
        /// <summary>
        /// Action type, 1(Block), 2(Captcha), 3(log), 4(Redirect).
        /// </summary>
        [Output("actionType")]
        public Output<string> ActionType { get; private set; } = null!;

        /// <summary>
        /// Domain.
        /// </summary>
        [Output("domain")]
        public Output<string> Domain { get; private set; } = null!;

        /// <summary>
        /// Expiration time in second-level timestamp, for example, 1677254399 indicates the expiration time is 2023-02-24 23:59:59; 0 indicates it will never expire.
        /// </summary>
        [Output("expireTime")]
        public Output<string> ExpireTime { get; private set; } = null!;

        /// <summary>
        /// Rule execution time.
        /// </summary>
        [Output("jobDateTime")]
        public Output<Outputs.CustomRuleJobDateTime> JobDateTime { get; private set; } = null!;

        /// <summary>
        /// Rule execution mode: TimedJob indicates scheduled execution. CronJob indicates periodic execution.
        /// </summary>
        [Output("jobType")]
        public Output<string> JobType { get; private set; } = null!;

        /// <summary>
        /// Rule Name.
        /// </summary>
        [Output("name")]
        public Output<string> Name { get; private set; } = null!;

        /// <summary>
        /// If the action is a Redirect, it represents the redirect address; Other situations can be left blank.
        /// </summary>
        [Output("redirect")]
        public Output<string?> Redirect { get; private set; } = null!;

        /// <summary>
        /// rule ID.
        /// </summary>
        [Output("ruleId")]
        public Output<string> RuleId { get; private set; } = null!;

        /// <summary>
        /// Priority, value range 0-100.
        /// </summary>
        [Output("sortId")]
        public Output<string> SortId { get; private set; } = null!;

        /// <summary>
        /// The status of the rule, 1(open), 0(close).
        /// </summary>
        [Output("status")]
        public Output<string?> Status { get; private set; } = null!;

        /// <summary>
        /// Strategies detail.
        /// </summary>
        [Output("strategies")]
        public Output<ImmutableArray<Outputs.CustomRuleStrategy>> Strategies { get; private set; } = null!;


        /// <summary>
        /// Create a CustomRule resource with the given unique name, arguments, and options.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resource</param>
        /// <param name="args">The arguments used to populate this resource's properties</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public CustomRule(string name, CustomRuleArgs args, CustomResourceOptions? options = null)
            : base("tencentcloud:Waf/customRule:CustomRule", name, args ?? new CustomRuleArgs(), MakeResourceOptions(options, ""))
        {
        }

        private CustomRule(string name, Input<string> id, CustomRuleState? state = null, CustomResourceOptions? options = null)
            : base("tencentcloud:Waf/customRule:CustomRule", name, state, MakeResourceOptions(options, id))
        {
        }

        private static CustomResourceOptions MakeResourceOptions(CustomResourceOptions? options, Input<string>? id)
        {
            var defaultOptions = new CustomResourceOptions
            {
                Version = Utilities.Version,
                PluginDownloadURL = "github://api.github.com/tencentcloudstack",
            };
            var merged = CustomResourceOptions.Merge(defaultOptions, options);
            // Override the ID if one was specified for consistency with other language SDKs.
            merged.Id = id ?? merged.Id;
            return merged;
        }
        /// <summary>
        /// Get an existing CustomRule resource's state with the given name, ID, and optional extra
        /// properties used to qualify the lookup.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resulting resource.</param>
        /// <param name="id">The unique provider ID of the resource to lookup.</param>
        /// <param name="state">Any extra arguments used during the lookup.</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public static CustomRule Get(string name, Input<string> id, CustomRuleState? state = null, CustomResourceOptions? options = null)
        {
            return new CustomRule(name, id, state, options);
        }
    }

    public sealed class CustomRuleArgs : global::Pulumi.ResourceArgs
    {
        /// <summary>
        /// Action type, 1(Block), 2(Captcha), 3(log), 4(Redirect).
        /// </summary>
        [Input("actionType", required: true)]
        public Input<string> ActionType { get; set; } = null!;

        /// <summary>
        /// Domain.
        /// </summary>
        [Input("domain", required: true)]
        public Input<string> Domain { get; set; } = null!;

        /// <summary>
        /// Expiration time in second-level timestamp, for example, 1677254399 indicates the expiration time is 2023-02-24 23:59:59; 0 indicates it will never expire.
        /// </summary>
        [Input("expireTime", required: true)]
        public Input<string> ExpireTime { get; set; } = null!;

        /// <summary>
        /// Rule execution time.
        /// </summary>
        [Input("jobDateTime")]
        public Input<Inputs.CustomRuleJobDateTimeArgs>? JobDateTime { get; set; }

        /// <summary>
        /// Rule execution mode: TimedJob indicates scheduled execution. CronJob indicates periodic execution.
        /// </summary>
        [Input("jobType")]
        public Input<string>? JobType { get; set; }

        /// <summary>
        /// Rule Name.
        /// </summary>
        [Input("name")]
        public Input<string>? Name { get; set; }

        /// <summary>
        /// If the action is a Redirect, it represents the redirect address; Other situations can be left blank.
        /// </summary>
        [Input("redirect")]
        public Input<string>? Redirect { get; set; }

        /// <summary>
        /// Priority, value range 0-100.
        /// </summary>
        [Input("sortId", required: true)]
        public Input<string> SortId { get; set; } = null!;

        /// <summary>
        /// The status of the rule, 1(open), 0(close).
        /// </summary>
        [Input("status")]
        public Input<string>? Status { get; set; }

        [Input("strategies", required: true)]
        private InputList<Inputs.CustomRuleStrategyArgs>? _strategies;

        /// <summary>
        /// Strategies detail.
        /// </summary>
        public InputList<Inputs.CustomRuleStrategyArgs> Strategies
        {
            get => _strategies ?? (_strategies = new InputList<Inputs.CustomRuleStrategyArgs>());
            set => _strategies = value;
        }

        public CustomRuleArgs()
        {
        }
        public static new CustomRuleArgs Empty => new CustomRuleArgs();
    }

    public sealed class CustomRuleState : global::Pulumi.ResourceArgs
    {
        /// <summary>
        /// Action type, 1(Block), 2(Captcha), 3(log), 4(Redirect).
        /// </summary>
        [Input("actionType")]
        public Input<string>? ActionType { get; set; }

        /// <summary>
        /// Domain.
        /// </summary>
        [Input("domain")]
        public Input<string>? Domain { get; set; }

        /// <summary>
        /// Expiration time in second-level timestamp, for example, 1677254399 indicates the expiration time is 2023-02-24 23:59:59; 0 indicates it will never expire.
        /// </summary>
        [Input("expireTime")]
        public Input<string>? ExpireTime { get; set; }

        /// <summary>
        /// Rule execution time.
        /// </summary>
        [Input("jobDateTime")]
        public Input<Inputs.CustomRuleJobDateTimeGetArgs>? JobDateTime { get; set; }

        /// <summary>
        /// Rule execution mode: TimedJob indicates scheduled execution. CronJob indicates periodic execution.
        /// </summary>
        [Input("jobType")]
        public Input<string>? JobType { get; set; }

        /// <summary>
        /// Rule Name.
        /// </summary>
        [Input("name")]
        public Input<string>? Name { get; set; }

        /// <summary>
        /// If the action is a Redirect, it represents the redirect address; Other situations can be left blank.
        /// </summary>
        [Input("redirect")]
        public Input<string>? Redirect { get; set; }

        /// <summary>
        /// rule ID.
        /// </summary>
        [Input("ruleId")]
        public Input<string>? RuleId { get; set; }

        /// <summary>
        /// Priority, value range 0-100.
        /// </summary>
        [Input("sortId")]
        public Input<string>? SortId { get; set; }

        /// <summary>
        /// The status of the rule, 1(open), 0(close).
        /// </summary>
        [Input("status")]
        public Input<string>? Status { get; set; }

        [Input("strategies")]
        private InputList<Inputs.CustomRuleStrategyGetArgs>? _strategies;

        /// <summary>
        /// Strategies detail.
        /// </summary>
        public InputList<Inputs.CustomRuleStrategyGetArgs> Strategies
        {
            get => _strategies ?? (_strategies = new InputList<Inputs.CustomRuleStrategyGetArgs>());
            set => _strategies = value;
        }

        public CustomRuleState()
        {
        }
        public static new CustomRuleState Empty => new CustomRuleState();
    }
}
