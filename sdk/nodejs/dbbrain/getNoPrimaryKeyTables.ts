// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of dbbrain noPrimaryKeyTables
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const noPrimaryKeyTables = tencentcloud.Dbbrain.getNoPrimaryKeyTables({
 *     instanceId: "",
 *     date: "",
 *     product: "",
 * });
 * ```
 */
export function getNoPrimaryKeyTables(args: GetNoPrimaryKeyTablesArgs, opts?: pulumi.InvokeOptions): Promise<GetNoPrimaryKeyTablesResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Dbbrain/getNoPrimaryKeyTables:getNoPrimaryKeyTables", {
        "date": args.date,
        "instanceId": args.instanceId,
        "product": args.product,
        "resultOutputFile": args.resultOutputFile,
    }, opts);
}

/**
 * A collection of arguments for invoking getNoPrimaryKeyTables.
 */
export interface GetNoPrimaryKeyTablesArgs {
    /**
     * Query date, such as 2021-05-27, the earliest date is 30 days ago.
     */
    date: string;
    /**
     * instance id.
     */
    instanceId: string;
    /**
     * Service product type, supported values: `mysql` - ApsaraDB for MySQL, the default is `mysql`.
     */
    product?: string;
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
}

/**
 * A collection of values returned by getNoPrimaryKeyTables.
 */
export interface GetNoPrimaryKeyTablesResult {
    readonly date: string;
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    readonly instanceId: string;
    /**
     * The difference with yesterday&amp;#39;s scan of the table without a primary key. A positive number means an increase, a negative number means a decrease, and 0 means no change.
     */
    readonly noPrimaryKeyTableCountDiff: number;
    /**
     * A list of tables without primary keys.
     */
    readonly noPrimaryKeyTables: outputs.Dbbrain.GetNoPrimaryKeyTablesNoPrimaryKeyTable[];
    readonly product?: string;
    readonly resultOutputFile?: string;
    /**
     * Collection timestamp (seconds).
     */
    readonly timestamp: number;
}
/**
 * Use this data source to query detailed information of dbbrain noPrimaryKeyTables
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const noPrimaryKeyTables = tencentcloud.Dbbrain.getNoPrimaryKeyTables({
 *     instanceId: "",
 *     date: "",
 *     product: "",
 * });
 * ```
 */
export function getNoPrimaryKeyTablesOutput(args: GetNoPrimaryKeyTablesOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetNoPrimaryKeyTablesResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Dbbrain/getNoPrimaryKeyTables:getNoPrimaryKeyTables", {
        "date": args.date,
        "instanceId": args.instanceId,
        "product": args.product,
        "resultOutputFile": args.resultOutputFile,
    }, opts);
}

/**
 * A collection of arguments for invoking getNoPrimaryKeyTables.
 */
export interface GetNoPrimaryKeyTablesOutputArgs {
    /**
     * Query date, such as 2021-05-27, the earliest date is 30 days ago.
     */
    date: pulumi.Input<string>;
    /**
     * instance id.
     */
    instanceId: pulumi.Input<string>;
    /**
     * Service product type, supported values: `mysql` - ApsaraDB for MySQL, the default is `mysql`.
     */
    product?: pulumi.Input<string>;
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
}
