// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of ssl describeManagers
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const describeManagers = tencentcloud.Ssl.getDescribeManagers({
 *     companyId: 11772,
 * });
 * ```
 */
export function getDescribeManagers(args: GetDescribeManagersArgs, opts?: pulumi.InvokeOptions): Promise<GetDescribeManagersResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Ssl/getDescribeManagers:getDescribeManagers", {
        "companyId": args.companyId,
        "managerMail": args.managerMail,
        "managerName": args.managerName,
        "resultOutputFile": args.resultOutputFile,
        "searchKey": args.searchKey,
        "status": args.status,
    }, opts);
}

/**
 * A collection of arguments for invoking getDescribeManagers.
 */
export interface GetDescribeManagersArgs {
    /**
     * Company ID.
     */
    companyId: number;
    /**
     * Vague query manager email (will be abandoned), please use Searchkey.
     */
    managerMail?: string;
    /**
     * Manager&amp;#39;s name (will be abandoned), please use Searchkey.
     */
    managerName?: string;
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
    /**
     * Manager&amp;#39;s surname/Manager name/mailbox/department precise matching.
     */
    searchKey?: string;
    /**
     * Filter according to the status of the manager, and the value is available&amp;#39;None&amp;#39; Unable to submit review&amp;#39;Audit&amp;#39;, Asian Credit Review&amp;#39;Caaudit&amp;#39; CA review&amp;#39;OK&amp;#39; has been reviewed&amp;#39;Invalid&amp;#39; review failed&amp;#39;Expiring&amp;#39; is about to expire&amp;#39;Expired&amp;#39; expired.
     */
    status?: string;
}

/**
 * A collection of values returned by getDescribeManagers.
 */
export interface GetDescribeManagersResult {
    readonly companyId: number;
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    /**
     * Manager mailbox.
     */
    readonly managerMail?: string;
    readonly managerName?: string;
    /**
     * Company Manager List.
     */
    readonly managers: outputs.Ssl.GetDescribeManagersManager[];
    readonly resultOutputFile?: string;
    readonly searchKey?: string;
    /**
     * Status: Audit: OK during the review: review passed inValid: expired expiRing: is about to expire Expired: expired.
     */
    readonly status?: string;
}
/**
 * Use this data source to query detailed information of ssl describeManagers
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const describeManagers = tencentcloud.Ssl.getDescribeManagers({
 *     companyId: 11772,
 * });
 * ```
 */
export function getDescribeManagersOutput(args: GetDescribeManagersOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetDescribeManagersResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Ssl/getDescribeManagers:getDescribeManagers", {
        "companyId": args.companyId,
        "managerMail": args.managerMail,
        "managerName": args.managerName,
        "resultOutputFile": args.resultOutputFile,
        "searchKey": args.searchKey,
        "status": args.status,
    }, opts);
}

/**
 * A collection of arguments for invoking getDescribeManagers.
 */
export interface GetDescribeManagersOutputArgs {
    /**
     * Company ID.
     */
    companyId: pulumi.Input<number>;
    /**
     * Vague query manager email (will be abandoned), please use Searchkey.
     */
    managerMail?: pulumi.Input<string>;
    /**
     * Manager&amp;#39;s name (will be abandoned), please use Searchkey.
     */
    managerName?: pulumi.Input<string>;
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
    /**
     * Manager&amp;#39;s surname/Manager name/mailbox/department precise matching.
     */
    searchKey?: pulumi.Input<string>;
    /**
     * Filter according to the status of the manager, and the value is available&amp;#39;None&amp;#39; Unable to submit review&amp;#39;Audit&amp;#39;, Asian Credit Review&amp;#39;Caaudit&amp;#39; CA review&amp;#39;OK&amp;#39; has been reviewed&amp;#39;Invalid&amp;#39; review failed&amp;#39;Expiring&amp;#39; is about to expire&amp;#39;Expired&amp;#39; expired.
     */
    status?: pulumi.Input<string>;
}
