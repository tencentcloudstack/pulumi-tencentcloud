// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of scf functionAddress
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const functionAddress = tencentcloud.Scf.getFunctionAddress({
 *     functionName: "keep-1676351130",
 *     namespace: "default",
 *     qualifier: "$LATEST",
 * });
 * ```
 */
export function getFunctionAddress(args: GetFunctionAddressArgs, opts?: pulumi.InvokeOptions): Promise<GetFunctionAddressResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Scf/getFunctionAddress:getFunctionAddress", {
        "functionName": args.functionName,
        "namespace": args.namespace,
        "qualifier": args.qualifier,
        "resultOutputFile": args.resultOutputFile,
    }, opts);
}

/**
 * A collection of arguments for invoking getFunctionAddress.
 */
export interface GetFunctionAddressArgs {
    /**
     * Function name.
     */
    functionName: string;
    /**
     * Function namespace.
     */
    namespace?: string;
    /**
     * Function version.
     */
    qualifier?: string;
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
}

/**
 * A collection of values returned by getFunctionAddress.
 */
export interface GetFunctionAddressResult {
    /**
     * SHA256 code of the function.
     */
    readonly codeSha256: string;
    readonly functionName: string;
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    readonly namespace?: string;
    readonly qualifier?: string;
    readonly resultOutputFile?: string;
    /**
     * Cos address of the function.
     */
    readonly url: string;
}
/**
 * Use this data source to query detailed information of scf functionAddress
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const functionAddress = tencentcloud.Scf.getFunctionAddress({
 *     functionName: "keep-1676351130",
 *     namespace: "default",
 *     qualifier: "$LATEST",
 * });
 * ```
 */
export function getFunctionAddressOutput(args: GetFunctionAddressOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetFunctionAddressResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Scf/getFunctionAddress:getFunctionAddress", {
        "functionName": args.functionName,
        "namespace": args.namespace,
        "qualifier": args.qualifier,
        "resultOutputFile": args.resultOutputFile,
    }, opts);
}

/**
 * A collection of arguments for invoking getFunctionAddress.
 */
export interface GetFunctionAddressOutputArgs {
    /**
     * Function name.
     */
    functionName: pulumi.Input<string>;
    /**
     * Function namespace.
     */
    namespace?: pulumi.Input<string>;
    /**
     * Function version.
     */
    qualifier?: pulumi.Input<string>;
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
}
