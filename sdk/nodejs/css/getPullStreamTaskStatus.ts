// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of css pullStreamTaskStatus
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const pullStreamTaskStatus = tencentcloud.Css.getPullStreamTaskStatus({
 *     taskId: "63229997",
 * });
 * ```
 */
export function getPullStreamTaskStatus(args: GetPullStreamTaskStatusArgs, opts?: pulumi.InvokeOptions): Promise<GetPullStreamTaskStatusResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Css/getPullStreamTaskStatus:getPullStreamTaskStatus", {
        "resultOutputFile": args.resultOutputFile,
        "taskId": args.taskId,
    }, opts);
}

/**
 * A collection of arguments for invoking getPullStreamTaskStatus.
 */
export interface GetPullStreamTaskStatusArgs {
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
    /**
     * Task ID.
     */
    taskId: string;
}

/**
 * A collection of values returned by getPullStreamTaskStatus.
 */
export interface GetPullStreamTaskStatusResult {
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    readonly resultOutputFile?: string;
    readonly taskId: string;
    /**
     * Task status info.
     */
    readonly taskStatusInfos: outputs.Css.GetPullStreamTaskStatusTaskStatusInfo[];
}
/**
 * Use this data source to query detailed information of css pullStreamTaskStatus
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const pullStreamTaskStatus = tencentcloud.Css.getPullStreamTaskStatus({
 *     taskId: "63229997",
 * });
 * ```
 */
export function getPullStreamTaskStatusOutput(args: GetPullStreamTaskStatusOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetPullStreamTaskStatusResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Css/getPullStreamTaskStatus:getPullStreamTaskStatus", {
        "resultOutputFile": args.resultOutputFile,
        "taskId": args.taskId,
    }, opts);
}

/**
 * A collection of arguments for invoking getPullStreamTaskStatus.
 */
export interface GetPullStreamTaskStatusOutputArgs {
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
    /**
     * Task ID.
     */
    taskId: pulumi.Input<string>;
}
