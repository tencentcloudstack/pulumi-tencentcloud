// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of vpc snapshotFiles
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const snapshotFiles = tencentcloud.Vpc.getSnapshotFiles({
 *     businessType: "securitygroup",
 *     instanceId: "sg-902tl7t7",
 *     startDate: "2022-10-10 00:00:00",
 *     endDate: "2023-10-30 19:00:00",
 * });
 * ```
 */
export function getSnapshotFiles(args: GetSnapshotFilesArgs, opts?: pulumi.InvokeOptions): Promise<GetSnapshotFilesResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Vpc/getSnapshotFiles:getSnapshotFiles", {
        "businessType": args.businessType,
        "endDate": args.endDate,
        "instanceId": args.instanceId,
        "resultOutputFile": args.resultOutputFile,
        "startDate": args.startDate,
    }, opts);
}

/**
 * A collection of arguments for invoking getSnapshotFiles.
 */
export interface GetSnapshotFilesArgs {
    /**
     * Business type, currently supports security group:securitygroup.
     */
    businessType: string;
    /**
     * End date in the format %Y-%m-%d %H:%M:%S.
     */
    endDate: string;
    /**
     * InstanceId.
     */
    instanceId: string;
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
    /**
     * Start date in the format %Y-%m-%d %H:%M:%S.
     */
    startDate: string;
}

/**
 * A collection of values returned by getSnapshotFiles.
 */
export interface GetSnapshotFilesResult {
    readonly businessType: string;
    readonly endDate: string;
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    /**
     * instance id.
     */
    readonly instanceId: string;
    readonly resultOutputFile?: string;
    /**
     * snap shot file set.
     */
    readonly snapshotFileSets: outputs.Vpc.GetSnapshotFilesSnapshotFileSet[];
    readonly startDate: string;
}
/**
 * Use this data source to query detailed information of vpc snapshotFiles
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const snapshotFiles = tencentcloud.Vpc.getSnapshotFiles({
 *     businessType: "securitygroup",
 *     instanceId: "sg-902tl7t7",
 *     startDate: "2022-10-10 00:00:00",
 *     endDate: "2023-10-30 19:00:00",
 * });
 * ```
 */
export function getSnapshotFilesOutput(args: GetSnapshotFilesOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetSnapshotFilesResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Vpc/getSnapshotFiles:getSnapshotFiles", {
        "businessType": args.businessType,
        "endDate": args.endDate,
        "instanceId": args.instanceId,
        "resultOutputFile": args.resultOutputFile,
        "startDate": args.startDate,
    }, opts);
}

/**
 * A collection of arguments for invoking getSnapshotFiles.
 */
export interface GetSnapshotFilesOutputArgs {
    /**
     * Business type, currently supports security group:securitygroup.
     */
    businessType: pulumi.Input<string>;
    /**
     * End date in the format %Y-%m-%d %H:%M:%S.
     */
    endDate: pulumi.Input<string>;
    /**
     * InstanceId.
     */
    instanceId: pulumi.Input<string>;
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
    /**
     * Start date in the format %Y-%m-%d %H:%M:%S.
     */
    startDate: pulumi.Input<string>;
}
