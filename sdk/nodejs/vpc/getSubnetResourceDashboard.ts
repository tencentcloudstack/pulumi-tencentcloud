// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of vpc subnetResourceDashboard
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const subnetResourceDashboard = tencentcloud.Vpc.getSubnetResourceDashboard({
 *     subnetIds: ["subnet-i9tpf6hq"],
 * });
 * ```
 */
export function getSubnetResourceDashboard(args: GetSubnetResourceDashboardArgs, opts?: pulumi.InvokeOptions): Promise<GetSubnetResourceDashboardResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Vpc/getSubnetResourceDashboard:getSubnetResourceDashboard", {
        "resultOutputFile": args.resultOutputFile,
        "subnetIds": args.subnetIds,
    }, opts);
}

/**
 * A collection of arguments for invoking getSubnetResourceDashboard.
 */
export interface GetSubnetResourceDashboardArgs {
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
    /**
     * Subnet instance ID, such as `subnet-f1xjkw1b`.
     */
    subnetIds: string[];
}

/**
 * A collection of values returned by getSubnetResourceDashboard.
 */
export interface GetSubnetResourceDashboardResult {
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    /**
     * Information of resources returned.
     */
    readonly resourceStatisticsSets: outputs.Vpc.GetSubnetResourceDashboardResourceStatisticsSet[];
    readonly resultOutputFile?: string;
    readonly subnetIds: string[];
}
/**
 * Use this data source to query detailed information of vpc subnetResourceDashboard
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const subnetResourceDashboard = tencentcloud.Vpc.getSubnetResourceDashboard({
 *     subnetIds: ["subnet-i9tpf6hq"],
 * });
 * ```
 */
export function getSubnetResourceDashboardOutput(args: GetSubnetResourceDashboardOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetSubnetResourceDashboardResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Vpc/getSubnetResourceDashboard:getSubnetResourceDashboard", {
        "resultOutputFile": args.resultOutputFile,
        "subnetIds": args.subnetIds,
    }, opts);
}

/**
 * A collection of arguments for invoking getSubnetResourceDashboard.
 */
export interface GetSubnetResourceDashboardOutputArgs {
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
    /**
     * Subnet instance ID, such as `subnet-f1xjkw1b`.
     */
    subnetIds: pulumi.Input<pulumi.Input<string>[]>;
}
