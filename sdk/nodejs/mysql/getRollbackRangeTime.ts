// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of mysql rollbackRangeTime
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const rollbackRangeTime = tencentcloud.Mysql.getRollbackRangeTime({
 *     instanceIds: ["cdb-fitq5t9h"],
 * });
 * ```
 */
export function getRollbackRangeTime(args: GetRollbackRangeTimeArgs, opts?: pulumi.InvokeOptions): Promise<GetRollbackRangeTimeResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Mysql/getRollbackRangeTime:getRollbackRangeTime", {
        "backupRegion": args.backupRegion,
        "instanceIds": args.instanceIds,
        "isRemoteZone": args.isRemoteZone,
        "resultOutputFile": args.resultOutputFile,
    }, opts);
}

/**
 * A collection of arguments for invoking getRollbackRangeTime.
 */
export interface GetRollbackRangeTimeArgs {
    /**
     * If the clone instance is not in the same region as the source instance, fill in the region where the clone instance is located, for example: ap-guangzhou.
     */
    backupRegion?: string;
    /**
     * A list of instance IDs, the format of a single instance ID is: cdb-c1nl9rpv. Same instance ID as displayed in the ApsaraDB for Console page.
     */
    instanceIds: string[];
    /**
     * Whether the clone instance is in the same zone as the source instance, yes: `false`, no: `true`.
     */
    isRemoteZone?: string;
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
}

/**
 * A collection of values returned by getRollbackRangeTime.
 */
export interface GetRollbackRangeTimeResult {
    readonly backupRegion?: string;
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    readonly instanceIds: string[];
    readonly isRemoteZone?: string;
    /**
     * Returned parameter information.
     */
    readonly items: outputs.Mysql.GetRollbackRangeTimeItem[];
    readonly resultOutputFile?: string;
}
/**
 * Use this data source to query detailed information of mysql rollbackRangeTime
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const rollbackRangeTime = tencentcloud.Mysql.getRollbackRangeTime({
 *     instanceIds: ["cdb-fitq5t9h"],
 * });
 * ```
 */
export function getRollbackRangeTimeOutput(args: GetRollbackRangeTimeOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetRollbackRangeTimeResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Mysql/getRollbackRangeTime:getRollbackRangeTime", {
        "backupRegion": args.backupRegion,
        "instanceIds": args.instanceIds,
        "isRemoteZone": args.isRemoteZone,
        "resultOutputFile": args.resultOutputFile,
    }, opts);
}

/**
 * A collection of arguments for invoking getRollbackRangeTime.
 */
export interface GetRollbackRangeTimeOutputArgs {
    /**
     * If the clone instance is not in the same region as the source instance, fill in the region where the clone instance is located, for example: ap-guangzhou.
     */
    backupRegion?: pulumi.Input<string>;
    /**
     * A list of instance IDs, the format of a single instance ID is: cdb-c1nl9rpv. Same instance ID as displayed in the ApsaraDB for Console page.
     */
    instanceIds: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * Whether the clone instance is in the same zone as the source instance, yes: `false`, no: `true`.
     */
    isRemoteZone?: pulumi.Input<string>;
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
}
