// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of mysql slowLogData
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const slowLogData = tencentcloud.Mysql.getSlowLogData({
 *     instanceId: "cdb-fitq5t9h",
 *     startTime: 1682664459,
 *     endTime: 1684392459,
 *     userHosts: ["169.254.128.158"],
 *     userNames: ["keep_dts"],
 *     dataBases: ["tf_ci_test"],
 *     sortBy: "Timestamp",
 *     orderBy: "ASC",
 *     instType: "slave",
 * });
 * ```
 */
export function getSlowLogData(args: GetSlowLogDataArgs, opts?: pulumi.InvokeOptions): Promise<GetSlowLogDataResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Mysql/getSlowLogData:getSlowLogData", {
        "dataBases": args.dataBases,
        "endTime": args.endTime,
        "instType": args.instType,
        "instanceId": args.instanceId,
        "orderBy": args.orderBy,
        "resultOutputFile": args.resultOutputFile,
        "sortBy": args.sortBy,
        "startTime": args.startTime,
        "userHosts": args.userHosts,
        "userNames": args.userNames,
    }, opts);
}

/**
 * A collection of arguments for invoking getSlowLogData.
 */
export interface GetSlowLogDataArgs {
    /**
     * List of databases accessed.
     */
    dataBases?: string[];
    /**
     * End timestamp. For example 1585142640.
     */
    endTime: number;
    /**
     * Only valid when the instance is the master instance or disaster recovery instance, the optional value: slave, which means to pull the log of the slave machine.
     */
    instType?: string;
    /**
     * instance id.
     */
    instanceId: string;
    /**
     * Sort in ascending or descending order. Currently supported: ASC,DESC.
     */
    orderBy?: string;
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
    /**
     * Sort field. Currently supported: Timestamp, QueryTime, LockTime, RowsExamined, RowsSent.
     */
    sortBy?: string;
    /**
     * Start timestamp. For example 1585142640.
     */
    startTime: number;
    /**
     * List of client hosts.
     */
    userHosts?: string[];
    /**
     * A list of client usernames.
     */
    userNames?: string[];
}

/**
 * A collection of values returned by getSlowLogData.
 */
export interface GetSlowLogDataResult {
    readonly dataBases?: string[];
    readonly endTime: number;
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    readonly instType?: string;
    readonly instanceId: string;
    /**
     * Query records.
     */
    readonly items: outputs.Mysql.GetSlowLogDataItem[];
    readonly orderBy?: string;
    readonly resultOutputFile?: string;
    readonly sortBy?: string;
    readonly startTime: number;
    readonly userHosts?: string[];
    readonly userNames?: string[];
}
/**
 * Use this data source to query detailed information of mysql slowLogData
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const slowLogData = tencentcloud.Mysql.getSlowLogData({
 *     instanceId: "cdb-fitq5t9h",
 *     startTime: 1682664459,
 *     endTime: 1684392459,
 *     userHosts: ["169.254.128.158"],
 *     userNames: ["keep_dts"],
 *     dataBases: ["tf_ci_test"],
 *     sortBy: "Timestamp",
 *     orderBy: "ASC",
 *     instType: "slave",
 * });
 * ```
 */
export function getSlowLogDataOutput(args: GetSlowLogDataOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetSlowLogDataResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Mysql/getSlowLogData:getSlowLogData", {
        "dataBases": args.dataBases,
        "endTime": args.endTime,
        "instType": args.instType,
        "instanceId": args.instanceId,
        "orderBy": args.orderBy,
        "resultOutputFile": args.resultOutputFile,
        "sortBy": args.sortBy,
        "startTime": args.startTime,
        "userHosts": args.userHosts,
        "userNames": args.userNames,
    }, opts);
}

/**
 * A collection of arguments for invoking getSlowLogData.
 */
export interface GetSlowLogDataOutputArgs {
    /**
     * List of databases accessed.
     */
    dataBases?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * End timestamp. For example 1585142640.
     */
    endTime: pulumi.Input<number>;
    /**
     * Only valid when the instance is the master instance or disaster recovery instance, the optional value: slave, which means to pull the log of the slave machine.
     */
    instType?: pulumi.Input<string>;
    /**
     * instance id.
     */
    instanceId: pulumi.Input<string>;
    /**
     * Sort in ascending or descending order. Currently supported: ASC,DESC.
     */
    orderBy?: pulumi.Input<string>;
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
    /**
     * Sort field. Currently supported: Timestamp, QueryTime, LockTime, RowsExamined, RowsSent.
     */
    sortBy?: pulumi.Input<string>;
    /**
     * Start timestamp. For example 1585142640.
     */
    startTime: pulumi.Input<number>;
    /**
     * List of client hosts.
     */
    userHosts?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * A list of client usernames.
     */
    userNames?: pulumi.Input<pulumi.Input<string>[]>;
}
