// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of clickhouse backup job detail
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const backupJobDetail = tencentcloud.Clickhouse.getBackupJobDetail({
 *     instanceId: "cdwch-xxxxxx",
 *     backUpJobId: 1234,
 * });
 * ```
 */
export function getBackupJobDetail(args: GetBackupJobDetailArgs, opts?: pulumi.InvokeOptions): Promise<GetBackupJobDetailResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Clickhouse/getBackupJobDetail:getBackupJobDetail", {
        "backUpJobId": args.backUpJobId,
        "instanceId": args.instanceId,
        "resultOutputFile": args.resultOutputFile,
    }, opts);
}

/**
 * A collection of arguments for invoking getBackupJobDetail.
 */
export interface GetBackupJobDetailArgs {
    /**
     * Back up job id.
     */
    backUpJobId: number;
    /**
     * Instance id.
     */
    instanceId: string;
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
}

/**
 * A collection of values returned by getBackupJobDetail.
 */
export interface GetBackupJobDetailResult {
    readonly backUpJobId: number;
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    readonly instanceId: string;
    readonly resultOutputFile?: string;
    /**
     * Back up tables.
     */
    readonly tableContents: outputs.Clickhouse.GetBackupJobDetailTableContent[];
}
/**
 * Use this data source to query detailed information of clickhouse backup job detail
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const backupJobDetail = tencentcloud.Clickhouse.getBackupJobDetail({
 *     instanceId: "cdwch-xxxxxx",
 *     backUpJobId: 1234,
 * });
 * ```
 */
export function getBackupJobDetailOutput(args: GetBackupJobDetailOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetBackupJobDetailResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Clickhouse/getBackupJobDetail:getBackupJobDetail", {
        "backUpJobId": args.backUpJobId,
        "instanceId": args.instanceId,
        "resultOutputFile": args.resultOutputFile,
    }, opts);
}

/**
 * A collection of arguments for invoking getBackupJobDetail.
 */
export interface GetBackupJobDetailOutputArgs {
    /**
     * Back up job id.
     */
    backUpJobId: pulumi.Input<number>;
    /**
     * Instance id.
     */
    instanceId: pulumi.Input<string>;
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
}
