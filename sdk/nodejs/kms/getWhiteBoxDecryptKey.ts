// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of kms whiteBoxDecryptKey
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const example = tencentcloud.Kms.getWhiteBoxDecryptKey({
 *     keyId: "244dab8c-6dad-11ea-80c6-5254006d0810",
 * });
 * ```
 */
export function getWhiteBoxDecryptKey(args: GetWhiteBoxDecryptKeyArgs, opts?: pulumi.InvokeOptions): Promise<GetWhiteBoxDecryptKeyResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Kms/getWhiteBoxDecryptKey:getWhiteBoxDecryptKey", {
        "keyId": args.keyId,
        "resultOutputFile": args.resultOutputFile,
    }, opts);
}

/**
 * A collection of arguments for invoking getWhiteBoxDecryptKey.
 */
export interface GetWhiteBoxDecryptKeyArgs {
    /**
     * Globally unique identifier for the white box key.
     */
    keyId: string;
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
}

/**
 * A collection of values returned by getWhiteBoxDecryptKey.
 */
export interface GetWhiteBoxDecryptKeyResult {
    /**
     * White box decryption key, base64 encoded.
     */
    readonly decryptKey: string;
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    readonly keyId: string;
    readonly resultOutputFile?: string;
}
/**
 * Use this data source to query detailed information of kms whiteBoxDecryptKey
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const example = tencentcloud.Kms.getWhiteBoxDecryptKey({
 *     keyId: "244dab8c-6dad-11ea-80c6-5254006d0810",
 * });
 * ```
 */
export function getWhiteBoxDecryptKeyOutput(args: GetWhiteBoxDecryptKeyOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetWhiteBoxDecryptKeyResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Kms/getWhiteBoxDecryptKey:getWhiteBoxDecryptKey", {
        "keyId": args.keyId,
        "resultOutputFile": args.resultOutputFile,
    }, opts);
}

/**
 * A collection of arguments for invoking getWhiteBoxDecryptKey.
 */
export interface GetWhiteBoxDecryptKeyOutputArgs {
    /**
     * Globally unique identifier for the white box key.
     */
    keyId: pulumi.Input<string>;
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
}
