// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of cam accountSummary
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const accountSummary = tencentcloud.Cam.getAccountSummary({});
 * ```
 */
export function getAccountSummary(args?: GetAccountSummaryArgs, opts?: pulumi.InvokeOptions): Promise<GetAccountSummaryResult> {
    args = args || {};
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Cam/getAccountSummary:getAccountSummary", {
        "resultOutputFile": args.resultOutputFile,
    }, opts);
}

/**
 * A collection of arguments for invoking getAccountSummary.
 */
export interface GetAccountSummaryArgs {
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
}

/**
 * A collection of values returned by getAccountSummary.
 */
export interface GetAccountSummaryResult {
    /**
     * The number of Group.
     */
    readonly group: number;
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    /**
     * The number of identity provider.
     */
    readonly identityProviders: number;
    /**
     * The number of grouped users.
     */
    readonly member: number;
    /**
     * The number of policy.
     */
    readonly policies: number;
    readonly resultOutputFile?: string;
    /**
     * The number of role.
     */
    readonly roles: number;
    /**
     * The number of Sub-user.
     */
    readonly user: number;
}
/**
 * Use this data source to query detailed information of cam accountSummary
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const accountSummary = tencentcloud.Cam.getAccountSummary({});
 * ```
 */
export function getAccountSummaryOutput(args?: GetAccountSummaryOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetAccountSummaryResult> {
    args = args || {};
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Cam/getAccountSummary:getAccountSummary", {
        "resultOutputFile": args.resultOutputFile,
    }, opts);
}

/**
 * A collection of arguments for invoking getAccountSummary.
 */
export interface GetAccountSummaryOutputArgs {
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
}
