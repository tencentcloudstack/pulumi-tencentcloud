// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of oceanus resourceRelatedJob
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const example = tencentcloud.Oceanus.getResourceRelatedJob({
 *     resourceId: "resource-8y9lzcuz",
 *     descByJobConfigCreateTime: 0,
 *     resourceConfigVersion: 1,
 *     workSpaceId: "space-2idq8wbr",
 * });
 * ```
 */
export function getResourceRelatedJob(args: GetResourceRelatedJobArgs, opts?: pulumi.InvokeOptions): Promise<GetResourceRelatedJobResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Oceanus/getResourceRelatedJob:getResourceRelatedJob", {
        "descByJobConfigCreateTime": args.descByJobConfigCreateTime,
        "resourceConfigVersion": args.resourceConfigVersion,
        "resourceId": args.resourceId,
        "resultOutputFile": args.resultOutputFile,
        "workSpaceId": args.workSpaceId,
    }, opts);
}

/**
 * A collection of arguments for invoking getResourceRelatedJob.
 */
export interface GetResourceRelatedJobArgs {
    /**
     * Default:0; 1:sort by job version creation time in descending order.
     */
    descByJobConfigCreateTime?: number;
    /**
     * Resource version number.
     */
    resourceConfigVersion?: number;
    /**
     * Resource ID.
     */
    resourceId: string;
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
    /**
     * Workspace SerialId.
     */
    workSpaceId?: string;
}

/**
 * A collection of values returned by getResourceRelatedJob.
 */
export interface GetResourceRelatedJobResult {
    readonly descByJobConfigCreateTime?: number;
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    /**
     * Associated job information.
     */
    readonly refJobInfos: outputs.Oceanus.GetResourceRelatedJobRefJobInfo[];
    readonly resourceConfigVersion?: number;
    readonly resourceId: string;
    readonly resultOutputFile?: string;
    readonly workSpaceId?: string;
}
/**
 * Use this data source to query detailed information of oceanus resourceRelatedJob
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const example = tencentcloud.Oceanus.getResourceRelatedJob({
 *     resourceId: "resource-8y9lzcuz",
 *     descByJobConfigCreateTime: 0,
 *     resourceConfigVersion: 1,
 *     workSpaceId: "space-2idq8wbr",
 * });
 * ```
 */
export function getResourceRelatedJobOutput(args: GetResourceRelatedJobOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetResourceRelatedJobResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Oceanus/getResourceRelatedJob:getResourceRelatedJob", {
        "descByJobConfigCreateTime": args.descByJobConfigCreateTime,
        "resourceConfigVersion": args.resourceConfigVersion,
        "resourceId": args.resourceId,
        "resultOutputFile": args.resultOutputFile,
        "workSpaceId": args.workSpaceId,
    }, opts);
}

/**
 * A collection of arguments for invoking getResourceRelatedJob.
 */
export interface GetResourceRelatedJobOutputArgs {
    /**
     * Default:0; 1:sort by job version creation time in descending order.
     */
    descByJobConfigCreateTime?: pulumi.Input<number>;
    /**
     * Resource version number.
     */
    resourceConfigVersion?: pulumi.Input<number>;
    /**
     * Resource ID.
     */
    resourceId: pulumi.Input<string>;
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
    /**
     * Workspace SerialId.
     */
    workSpaceId?: pulumi.Input<string>;
}
