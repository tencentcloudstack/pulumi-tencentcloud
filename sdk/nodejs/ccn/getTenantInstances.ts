// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "../utilities";

/**
 * Use this data source to query detailed information of vpc tenantCcn
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const tenantCcn = tencentcloud.Ccn.getTenantInstances({
 *     ccnIds: ["ccn-39lqkygf"],
 *     isSecurityLocks: ["true"],
 * });
 * ```
 */
export function getTenantInstances(args?: GetTenantInstancesArgs, opts?: pulumi.InvokeOptions): Promise<GetTenantInstancesResult> {
    args = args || {};
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("tencentcloud:Ccn/getTenantInstances:getTenantInstances", {
        "ccnIds": args.ccnIds,
        "isSecurityLocks": args.isSecurityLocks,
        "resultOutputFile": args.resultOutputFile,
        "userAccountIds": args.userAccountIds,
    }, opts);
}

/**
 * A collection of arguments for invoking getTenantInstances.
 */
export interface GetTenantInstancesArgs {
    /**
     * filter by ccn ids, like: ['ccn-12345678'].
     */
    ccnIds?: string[];
    /**
     * filter by locked, like ['true'].
     */
    isSecurityLocks?: string[];
    /**
     * Used to save results.
     */
    resultOutputFile?: string;
    /**
     * filter by ccn ids, like: ['12345678'].
     */
    userAccountIds?: string[];
}

/**
 * A collection of values returned by getTenantInstances.
 */
export interface GetTenantInstancesResult {
    readonly ccnIds?: string[];
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
    readonly isSecurityLocks?: string[];
    readonly resultOutputFile?: string;
    readonly userAccountIds?: string[];
}
/**
 * Use this data source to query detailed information of vpc tenantCcn
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as tencentcloud from "@tencentcloud_iac/pulumi";
 *
 * const tenantCcn = tencentcloud.Ccn.getTenantInstances({
 *     ccnIds: ["ccn-39lqkygf"],
 *     isSecurityLocks: ["true"],
 * });
 * ```
 */
export function getTenantInstancesOutput(args?: GetTenantInstancesOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetTenantInstancesResult> {
    args = args || {};
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("tencentcloud:Ccn/getTenantInstances:getTenantInstances", {
        "ccnIds": args.ccnIds,
        "isSecurityLocks": args.isSecurityLocks,
        "resultOutputFile": args.resultOutputFile,
        "userAccountIds": args.userAccountIds,
    }, opts);
}

/**
 * A collection of arguments for invoking getTenantInstances.
 */
export interface GetTenantInstancesOutputArgs {
    /**
     * filter by ccn ids, like: ['ccn-12345678'].
     */
    ccnIds?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * filter by locked, like ['true'].
     */
    isSecurityLocks?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * Used to save results.
     */
    resultOutputFile?: pulumi.Input<string>;
    /**
     * filter by ccn ids, like: ['12345678'].
     */
    userAccountIds?: pulumi.Input<pulumi.Input<string>[]>;
}
