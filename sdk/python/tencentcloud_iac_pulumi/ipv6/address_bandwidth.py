# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["AddressBandwidth", "AddressBandwidthArgs"]


@pulumi.input_type
class AddressBandwidthArgs:
    def __init__(
        __self__,
        *,
        ipv6_address: pulumi.Input[_builtins.str],
        bandwidth_package_id: pulumi.Input[_builtins.str | None] = None,
        internet_charge_type: pulumi.Input[_builtins.str | None] = None,
        internet_max_bandwidth_out: pulumi.Input[_builtins.int | None] = None
    ):
        """
        The set of arguments for constructing a AddressBandwidth resource.

        :param pulumi.Input[_builtins.str] ipv6_address: IPV6 address that needs to be enabled for public network access.
        :param pulumi.Input[_builtins.str] bandwidth_package_id: The bandwidth package id, the Legacy account and the ipv6 address to apply for the bandwidth package charge type need to be passed in.
        :param pulumi.Input[_builtins.str] internet_charge_type: Network billing mode. IPV6 currently supports: `TRAFFIC_POSTPAID_BY_HOUR`, for standard account types; `BANDWIDTH_PACKAGE`, for traditional account types. The default network billing mode is: `TRAFFIC_POSTPAID_BY_HOUR`.
        :param pulumi.Input[_builtins.int] internet_max_bandwidth_out: Bandwidth, in Mbps. The default is 1Mbps.
        """
        pulumi.set(__self__, "ipv6_address", ipv6_address)
        if bandwidth_package_id is not None:
            pulumi.set(__self__, "bandwidth_package_id", bandwidth_package_id)
        if internet_charge_type is not None:
            pulumi.set(__self__, "internet_charge_type", internet_charge_type)
        if internet_max_bandwidth_out is not None:
            pulumi.set(
                __self__, "internet_max_bandwidth_out", internet_max_bandwidth_out
            )

    @_builtins.property
    @pulumi.getter(name="ipv6Address")
    def ipv6_address(self) -> pulumi.Input[_builtins.str]:
        """
        IPV6 address that needs to be enabled for public network access.
        """
        return pulumi.get(self, "ipv6_address")

    @ipv6_address.setter
    def ipv6_address(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "ipv6_address", value)

    @_builtins.property
    @pulumi.getter(name="bandwidthPackageId")
    def bandwidth_package_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        The bandwidth package id, the Legacy account and the ipv6 address to apply for the bandwidth package charge type need to be passed in.
        """
        return pulumi.get(self, "bandwidth_package_id")

    @bandwidth_package_id.setter
    def bandwidth_package_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "bandwidth_package_id", value)

    @_builtins.property
    @pulumi.getter(name="internetChargeType")
    def internet_charge_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Network billing mode. IPV6 currently supports: `TRAFFIC_POSTPAID_BY_HOUR`, for standard account types; `BANDWIDTH_PACKAGE`, for traditional account types. The default network billing mode is: `TRAFFIC_POSTPAID_BY_HOUR`.
        """
        return pulumi.get(self, "internet_charge_type")

    @internet_charge_type.setter
    def internet_charge_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "internet_charge_type", value)

    @_builtins.property
    @pulumi.getter(name="internetMaxBandwidthOut")
    def internet_max_bandwidth_out(self) -> pulumi.Input[_builtins.int | None]:
        """
        Bandwidth, in Mbps. The default is 1Mbps.
        """
        return pulumi.get(self, "internet_max_bandwidth_out")

    @internet_max_bandwidth_out.setter
    def internet_max_bandwidth_out(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "internet_max_bandwidth_out", value)


@pulumi.input_type
class _AddressBandwidthState:
    def __init__(
        __self__,
        *,
        bandwidth_package_id: pulumi.Input[_builtins.str | None] = None,
        internet_charge_type: pulumi.Input[_builtins.str | None] = None,
        internet_max_bandwidth_out: pulumi.Input[_builtins.int | None] = None,
        ipv6_address: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering AddressBandwidth resources.

        :param pulumi.Input[_builtins.str] bandwidth_package_id: The bandwidth package id, the Legacy account and the ipv6 address to apply for the bandwidth package charge type need to be passed in.
        :param pulumi.Input[_builtins.str] internet_charge_type: Network billing mode. IPV6 currently supports: `TRAFFIC_POSTPAID_BY_HOUR`, for standard account types; `BANDWIDTH_PACKAGE`, for traditional account types. The default network billing mode is: `TRAFFIC_POSTPAID_BY_HOUR`.
        :param pulumi.Input[_builtins.int] internet_max_bandwidth_out: Bandwidth, in Mbps. The default is 1Mbps.
        :param pulumi.Input[_builtins.str] ipv6_address: IPV6 address that needs to be enabled for public network access.
        """
        if bandwidth_package_id is not None:
            pulumi.set(__self__, "bandwidth_package_id", bandwidth_package_id)
        if internet_charge_type is not None:
            pulumi.set(__self__, "internet_charge_type", internet_charge_type)
        if internet_max_bandwidth_out is not None:
            pulumi.set(
                __self__, "internet_max_bandwidth_out", internet_max_bandwidth_out
            )
        if ipv6_address is not None:
            pulumi.set(__self__, "ipv6_address", ipv6_address)

    @_builtins.property
    @pulumi.getter(name="bandwidthPackageId")
    def bandwidth_package_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        The bandwidth package id, the Legacy account and the ipv6 address to apply for the bandwidth package charge type need to be passed in.
        """
        return pulumi.get(self, "bandwidth_package_id")

    @bandwidth_package_id.setter
    def bandwidth_package_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "bandwidth_package_id", value)

    @_builtins.property
    @pulumi.getter(name="internetChargeType")
    def internet_charge_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Network billing mode. IPV6 currently supports: `TRAFFIC_POSTPAID_BY_HOUR`, for standard account types; `BANDWIDTH_PACKAGE`, for traditional account types. The default network billing mode is: `TRAFFIC_POSTPAID_BY_HOUR`.
        """
        return pulumi.get(self, "internet_charge_type")

    @internet_charge_type.setter
    def internet_charge_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "internet_charge_type", value)

    @_builtins.property
    @pulumi.getter(name="internetMaxBandwidthOut")
    def internet_max_bandwidth_out(self) -> pulumi.Input[_builtins.int | None]:
        """
        Bandwidth, in Mbps. The default is 1Mbps.
        """
        return pulumi.get(self, "internet_max_bandwidth_out")

    @internet_max_bandwidth_out.setter
    def internet_max_bandwidth_out(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "internet_max_bandwidth_out", value)

    @_builtins.property
    @pulumi.getter(name="ipv6Address")
    def ipv6_address(self) -> pulumi.Input[_builtins.str | None]:
        """
        IPV6 address that needs to be enabled for public network access.
        """
        return pulumi.get(self, "ipv6_address")

    @ipv6_address.setter
    def ipv6_address(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "ipv6_address", value)


@pulumi.type_token("tencentcloud:Ipv6/addressBandwidth:AddressBandwidth")
class AddressBandwidth(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        bandwidth_package_id: pulumi.Input[_builtins.str | None] = None,
        internet_charge_type: pulumi.Input[_builtins.str | None] = None,
        internet_max_bandwidth_out: pulumi.Input[_builtins.int | None] = None,
        ipv6_address: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a ipv6_address_bandwidth

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        ipv6_address_bandwidth = tencentcloud.ipv6.AddressBandwidth("ipv6_address_bandwidth",
            ipv6_address="2402:4e00:1019:9400:0:9905:a90b:2ef0",
            internet_max_bandwidth_out=6,
            internet_charge_type="TRAFFIC_POSTPAID_BY_HOUR")
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] bandwidth_package_id: The bandwidth package id, the Legacy account and the ipv6 address to apply for the bandwidth package charge type need to be passed in.
        :param pulumi.Input[_builtins.str] internet_charge_type: Network billing mode. IPV6 currently supports: `TRAFFIC_POSTPAID_BY_HOUR`, for standard account types; `BANDWIDTH_PACKAGE`, for traditional account types. The default network billing mode is: `TRAFFIC_POSTPAID_BY_HOUR`.
        :param pulumi.Input[_builtins.int] internet_max_bandwidth_out: Bandwidth, in Mbps. The default is 1Mbps.
        :param pulumi.Input[_builtins.str] ipv6_address: IPV6 address that needs to be enabled for public network access.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: AddressBandwidthArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a ipv6_address_bandwidth

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        ipv6_address_bandwidth = tencentcloud.ipv6.AddressBandwidth("ipv6_address_bandwidth",
            ipv6_address="2402:4e00:1019:9400:0:9905:a90b:2ef0",
            internet_max_bandwidth_out=6,
            internet_charge_type="TRAFFIC_POSTPAID_BY_HOUR")
        ```


        :param str resource_name: The name of the resource.
        :param AddressBandwidthArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            AddressBandwidthArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        bandwidth_package_id: pulumi.Input[_builtins.str | None] = None,
        internet_charge_type: pulumi.Input[_builtins.str | None] = None,
        internet_max_bandwidth_out: pulumi.Input[_builtins.int | None] = None,
        ipv6_address: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = AddressBandwidthArgs.__new__(AddressBandwidthArgs)

            __props__.__dict__["bandwidth_package_id"] = bandwidth_package_id
            __props__.__dict__["internet_charge_type"] = internet_charge_type
            __props__.__dict__["internet_max_bandwidth_out"] = (
                internet_max_bandwidth_out
            )
            if ipv6_address is None and not opts.urn:
                raise TypeError("Missing required property 'ipv6_address'")
            __props__.__dict__["ipv6_address"] = ipv6_address
        super().__init__(
            "tencentcloud:Ipv6/addressBandwidth:AddressBandwidth",
            resource_name,
            __props__,
            opts,
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        bandwidth_package_id: pulumi.Input[_builtins.str | None] = None,
        internet_charge_type: pulumi.Input[_builtins.str | None] = None,
        internet_max_bandwidth_out: pulumi.Input[_builtins.int | None] = None,
        ipv6_address: pulumi.Input[_builtins.str | None] = None,
    ) -> "AddressBandwidth":
        """
        Get an existing AddressBandwidth resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] bandwidth_package_id: The bandwidth package id, the Legacy account and the ipv6 address to apply for the bandwidth package charge type need to be passed in.
        :param pulumi.Input[_builtins.str] internet_charge_type: Network billing mode. IPV6 currently supports: `TRAFFIC_POSTPAID_BY_HOUR`, for standard account types; `BANDWIDTH_PACKAGE`, for traditional account types. The default network billing mode is: `TRAFFIC_POSTPAID_BY_HOUR`.
        :param pulumi.Input[_builtins.int] internet_max_bandwidth_out: Bandwidth, in Mbps. The default is 1Mbps.
        :param pulumi.Input[_builtins.str] ipv6_address: IPV6 address that needs to be enabled for public network access.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _AddressBandwidthState.__new__(_AddressBandwidthState)

        __props__.__dict__["bandwidth_package_id"] = bandwidth_package_id
        __props__.__dict__["internet_charge_type"] = internet_charge_type
        __props__.__dict__["internet_max_bandwidth_out"] = internet_max_bandwidth_out
        __props__.__dict__["ipv6_address"] = ipv6_address
        return AddressBandwidth(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="bandwidthPackageId")
    def bandwidth_package_id(self) -> pulumi.Output[_builtins.str | None]:
        """
        The bandwidth package id, the Legacy account and the ipv6 address to apply for the bandwidth package charge type need to be passed in.
        """
        return pulumi.get(self, "bandwidth_package_id")

    @_builtins.property
    @pulumi.getter(name="internetChargeType")
    def internet_charge_type(self) -> pulumi.Output[_builtins.str | None]:
        """
        Network billing mode. IPV6 currently supports: `TRAFFIC_POSTPAID_BY_HOUR`, for standard account types; `BANDWIDTH_PACKAGE`, for traditional account types. The default network billing mode is: `TRAFFIC_POSTPAID_BY_HOUR`.
        """
        return pulumi.get(self, "internet_charge_type")

    @_builtins.property
    @pulumi.getter(name="internetMaxBandwidthOut")
    def internet_max_bandwidth_out(self) -> pulumi.Output[_builtins.int | None]:
        """
        Bandwidth, in Mbps. The default is 1Mbps.
        """
        return pulumi.get(self, "internet_max_bandwidth_out")

    @_builtins.property
    @pulumi.getter(name="ipv6Address")
    def ipv6_address(self) -> pulumi.Output[_builtins.str]:
        """
        IPV6 address that needs to be enabled for public network access.
        """
        return pulumi.get(self, "ipv6_address")
