# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
import types

from .. import _utilities

__config__ = pulumi.Config("tencentcloud")


class _ExportableConfig(types.ModuleType):
    @_builtins.property
    def allowed_account_ids(self) -> str | None:
        """
        List of allowed TencentCloud account IDs to prevent you from mistakenly using the wrong one (and potentially end up destroying a live environment). Conflicts with `forbidden_account_ids`, If use `assume_role_with_saml` or `assume_role_with_web_identity`, it is not supported.
        """
        return __config__.get("allowedAccountIds")

    @_builtins.property
    def assume_role(self) -> str | None:
        """
        The `assume_role` block. If provided, terraform will attempt to assume this role using the supplied credentials.
        """
        return __config__.get("assumeRole")

    @_builtins.property
    def assume_role_with_saml(self) -> str | None:
        """
        The `assume_role_with_saml` block. If provided, terraform will attempt to assume this role using the supplied credentials.
        """
        return __config__.get("assumeRoleWithSaml")

    @_builtins.property
    def assume_role_with_web_identity(self) -> str | None:
        """
        The `assume_role_with_web_identity` block. If provided, terraform will attempt to assume this role using the supplied credentials.
        """
        return __config__.get("assumeRoleWithWebIdentity")

    @_builtins.property
    def cam_role_name(self) -> str | None:
        """
        The name of the CVM instance CAM role. It can be sourced from the `TENCENTCLOUD_CAM_ROLE_NAME` environment variable.
        """
        return __config__.get("camRoleName")

    @_builtins.property
    def cos_domain(self) -> str | None:
        """
        The cos domain of the API request, Default is `https://cos.{region}.myqcloud.com`, Other Examples: `https://cluster-123456.cos-cdc.ap-guangzhou.myqcloud.com`.
        """
        return __config__.get("cosDomain")

    @_builtins.property
    def domain(self) -> str | None:
        """
        The root domain of the API request, Default is `tencentcloudapi.com`.
        """
        return __config__.get("domain")

    @_builtins.property
    def enable_pod_oidc(self) -> bool | None:
        """
        Whether to enable pod oidc.
        """
        return __config__.get_bool("enablePodOidc")

    @_builtins.property
    def forbidden_account_ids(self) -> str | None:
        """
        List of forbidden TencentCloud account IDs to prevent you from mistakenly using the wrong one (and potentially end up destroying a live environment). Conflicts with `allowed_account_ids`, If use `assume_role_with_saml` or `assume_role_with_web_identity`, it is not supported.
        """
        return __config__.get("forbiddenAccountIds")

    @_builtins.property
    def mfa_certification(self) -> str | None:
        """
        The `mfa_certification` block. If provided, terraform will attempt to use the provided credentials for MFA authentication.
        """
        return __config__.get("mfaCertification")

    @_builtins.property
    def profile(self) -> str | None:
        """
        The profile name as set in the shared credentials. It can also be sourced from the `TENCENTCLOUD_PROFILE` environment variable. If not set, the default profile created with `tccli configure` will be used.
        """
        return __config__.get("profile")

    @_builtins.property
    def protocol(self) -> str | None:
        """
        The protocol of the API request. Valid values: `HTTP` and `HTTPS`. Default is `HTTPS`.
        """
        return __config__.get("protocol")

    @_builtins.property
    def region(self) -> str | None:
        """
        This is the TencentCloud region. It can also be sourced from the `TENCENTCLOUD_REGION` environment variables. The default input value is ap-guangzhou.
        """
        return __config__.get("region") or _utilities.get_env("TENCENTCLOUD_REGION")

    @_builtins.property
    def secret_id(self) -> str | None:
        """
        This is the TencentCloud access key. It can also be sourced from the `TENCENTCLOUD_SECRET_ID` environment variable.
        """
        return __config__.get("secretId") or _utilities.get_env(
            "TENCENTCLOUD_SECRET_ID"
        )

    @_builtins.property
    def secret_key(self) -> str | None:
        """
        This is the TencentCloud secret key. It can also be sourced from the `TENCENTCLOUD_SECRET_KEY` environment variable.
        """
        return __config__.get("secretKey") or _utilities.get_env(
            "TENCENTCLOUD_SECRET_KEY"
        )

    @_builtins.property
    def security_token(self) -> str | None:
        """
        TencentCloud Security Token of temporary access credentials. It can be sourced from the `TENCENTCLOUD_SECURITY_TOKEN` environment variable. Notice: for supported products, please refer to: [temporary key supported products](https://intl.cloud.tencent.com/document/product/598/10588).
        """
        return __config__.get("securityToken") or _utilities.get_env(
            "TENCENTCLOUD_SECURITY_TOKEN"
        )

    @_builtins.property
    def shared_credentials_dir(self) -> str | None:
        """
        The directory of the shared credentials. It can also be sourced from the `TENCENTCLOUD_SHARED_CREDENTIALS_DIR` environment variable. If not set this defaults to ~/.tccli.
        """
        return __config__.get("sharedCredentialsDir")
