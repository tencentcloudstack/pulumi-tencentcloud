# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence
from typing import Union

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = [
    "AwaitableGetInvocationTaskResult",
    "GetInvocationTaskResult",
    "get_invocation_task",
    "get_invocation_task_output",
]


@pulumi.output_type
class GetInvocationTaskResult:
    """
    A collection of values returned by getInvocationTask.
    """

    def __init__(
        __self__,
        filters=None,
        hide_output=None,
        id=None,
        invocation_task_ids=None,
        invocation_task_sets=None,
        result_output_file=None,
    ):
        if filters and not isinstance(filters, list):
            raise TypeError("Expected argument 'filters' to be a list")
        pulumi.set(__self__, "filters", filters)
        if hide_output and not isinstance(hide_output, bool):
            raise TypeError("Expected argument 'hide_output' to be a bool")
        pulumi.set(__self__, "hide_output", hide_output)
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)
        if invocation_task_ids and not isinstance(invocation_task_ids, list):
            raise TypeError("Expected argument 'invocation_task_ids' to be a list")
        pulumi.set(__self__, "invocation_task_ids", invocation_task_ids)
        if invocation_task_sets and not isinstance(invocation_task_sets, list):
            raise TypeError("Expected argument 'invocation_task_sets' to be a list")
        pulumi.set(__self__, "invocation_task_sets", invocation_task_sets)
        if result_output_file and not isinstance(result_output_file, str):
            raise TypeError("Expected argument 'result_output_file' to be a str")
        pulumi.set(__self__, "result_output_file", result_output_file)

    @_builtins.property
    @pulumi.getter
    def filters(self) -> Sequence["outputs.GetInvocationTaskFilterResult"] | None:
        return pulumi.get(self, "filters")

    @_builtins.property
    @pulumi.getter(name="hideOutput")
    def hide_output(self) -> _builtins.bool | None:
        return pulumi.get(self, "hide_output")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The provider-assigned unique ID for this managed resource.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="invocationTaskIds")
    def invocation_task_ids(self) -> Sequence[_builtins.str] | None:
        return pulumi.get(self, "invocation_task_ids")

    @_builtins.property
    @pulumi.getter(name="invocationTaskSets")
    def invocation_task_sets(
        self,
    ) -> Sequence["outputs.GetInvocationTaskInvocationTaskSetResult"]:
        """
        List of execution tasks.
        """
        return pulumi.get(self, "invocation_task_sets")

    @_builtins.property
    @pulumi.getter(name="resultOutputFile")
    def result_output_file(self) -> _builtins.str | None:
        return pulumi.get(self, "result_output_file")


class AwaitableGetInvocationTaskResult(GetInvocationTaskResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetInvocationTaskResult(
            filters=self.filters,
            hide_output=self.hide_output,
            id=self.id,
            invocation_task_ids=self.invocation_task_ids,
            invocation_task_sets=self.invocation_task_sets,
            result_output_file=self.result_output_file,
        )


def get_invocation_task(
    filters: (
        Sequence[
            Union["GetInvocationTaskFilterArgs", "GetInvocationTaskFilterArgsDict"]
        ]
        | None
    ) = None,
    hide_output: _builtins.bool | None = None,
    invocation_task_ids: Sequence[_builtins.str] | None = None,
    result_output_file: _builtins.str | None = None,
    opts: pulumi.InvokeOptions | None = None,
) -> AwaitableGetInvocationTaskResult:
    """
    Use this data source to query detailed information of tat invocation_task

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    invocation_task = tencentcloud.tat.get_invocation_task(filters=[{
            "name": "instance-id",
            "values": ["ins-p4pq4gaq"],
        }],
        hide_output=True)
    ```


    :param Sequence[Union['GetInvocationTaskFilterArgs', 'GetInvocationTaskFilterArgsDict']] filters: Filter conditions.invocation-id - String - Required: No - (Filter condition) Filter by the execution activity ID.invocation-task-id - String - Required: No - (Filter condition) Filter by the execution task ID.instance-id - String - Required: No - (Filter condition) Filter by the instance ID.command-id - String - Required: No - (Filter condition) Filter by the command ID.Up to 10 Filters are allowed for each request. Each filter can have up to five Filter.Values. InvocationTaskIds and Filters cannot be specified at the same time.
    :param _builtins.bool hide_output: Whether to hide the output. Valid values:True (default): Hide the outputFalse: Show the output.
    :param Sequence[_builtins.str] invocation_task_ids: List of execution task IDs. Up to 100 IDs are allowed for each request. InvocationTaskIds and Filters cannot be specified at the same time.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["filters"] = filters
    __args__["hideOutput"] = hide_output
    __args__["invocationTaskIds"] = invocation_task_ids
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke(
        "tencentcloud:Tat/getInvocationTask:getInvocationTask",
        __args__,
        opts=opts,
        typ=GetInvocationTaskResult,
    ).value

    return AwaitableGetInvocationTaskResult(
        filters=pulumi.get(__ret__, "filters"),
        hide_output=pulumi.get(__ret__, "hide_output"),
        id=pulumi.get(__ret__, "id"),
        invocation_task_ids=pulumi.get(__ret__, "invocation_task_ids"),
        invocation_task_sets=pulumi.get(__ret__, "invocation_task_sets"),
        result_output_file=pulumi.get(__ret__, "result_output_file"),
    )


def get_invocation_task_output(
    filters: pulumi.Input[
        Sequence[
            Union["GetInvocationTaskFilterArgs", "GetInvocationTaskFilterArgsDict"]
        ]
        | None
    ] = None,
    hide_output: pulumi.Input[_builtins.bool | None] = None,
    invocation_task_ids: pulumi.Input[Sequence[_builtins.str] | None] = None,
    result_output_file: pulumi.Input[_builtins.str | None] = None,
    opts: pulumi.InvokeOptions | pulumi.InvokeOutputOptions | None = None,
) -> pulumi.Output[GetInvocationTaskResult]:
    """
    Use this data source to query detailed information of tat invocation_task

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    invocation_task = tencentcloud.tat.get_invocation_task(filters=[{
            "name": "instance-id",
            "values": ["ins-p4pq4gaq"],
        }],
        hide_output=True)
    ```


    :param Sequence[Union['GetInvocationTaskFilterArgs', 'GetInvocationTaskFilterArgsDict']] filters: Filter conditions.invocation-id - String - Required: No - (Filter condition) Filter by the execution activity ID.invocation-task-id - String - Required: No - (Filter condition) Filter by the execution task ID.instance-id - String - Required: No - (Filter condition) Filter by the instance ID.command-id - String - Required: No - (Filter condition) Filter by the command ID.Up to 10 Filters are allowed for each request. Each filter can have up to five Filter.Values. InvocationTaskIds and Filters cannot be specified at the same time.
    :param _builtins.bool hide_output: Whether to hide the output. Valid values:True (default): Hide the outputFalse: Show the output.
    :param Sequence[_builtins.str] invocation_task_ids: List of execution task IDs. Up to 100 IDs are allowed for each request. InvocationTaskIds and Filters cannot be specified at the same time.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["filters"] = filters
    __args__["hideOutput"] = hide_output
    __args__["invocationTaskIds"] = invocation_task_ids
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output(
        "tencentcloud:Tat/getInvocationTask:getInvocationTask",
        __args__,
        opts=opts,
        typ=GetInvocationTaskResult,
    )
    return __ret__.apply(
        lambda __response__: GetInvocationTaskResult(
            filters=pulumi.get(__response__, "filters"),
            hide_output=pulumi.get(__response__, "hide_output"),
            id=pulumi.get(__response__, "id"),
            invocation_task_ids=pulumi.get(__response__, "invocation_task_ids"),
            invocation_task_sets=pulumi.get(__response__, "invocation_task_sets"),
            result_output_file=pulumi.get(__response__, "result_output_file"),
        )
    )
