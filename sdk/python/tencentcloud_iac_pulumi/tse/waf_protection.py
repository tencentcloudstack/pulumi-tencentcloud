# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["WafProtection", "WafProtectionArgs"]


@pulumi.input_type
class WafProtectionArgs:
    def __init__(
        __self__,
        *,
        gateway_id: pulumi.Input[_builtins.str],
        operate: pulumi.Input[_builtins.str],
        type: pulumi.Input[_builtins.str],
        lists: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None
    ):
        """
        The set of arguments for constructing a WafProtection resource.

        :param pulumi.Input[_builtins.str] gateway_id: Gateway ID.
        :param pulumi.Input[_builtins.str] operate: `open`: open the protection, `close`: close the protection.
        :param pulumi.Input[_builtins.str] type: The type of protection resource. Reference value: `Global`: instance, `Service`: service, `Route`: route, `Object`: obejct (This interface does not currently support this type).
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] lists: Means the list of services or routes when the resource type `Type` is `Service` or `Route`.
        """
        pulumi.set(__self__, "gateway_id", gateway_id)
        pulumi.set(__self__, "operate", operate)
        pulumi.set(__self__, "type", type)
        if lists is not None:
            pulumi.set(__self__, "lists", lists)

    @_builtins.property
    @pulumi.getter(name="gatewayId")
    def gateway_id(self) -> pulumi.Input[_builtins.str]:
        """
        Gateway ID.
        """
        return pulumi.get(self, "gateway_id")

    @gateway_id.setter
    def gateway_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "gateway_id", value)

    @_builtins.property
    @pulumi.getter
    def operate(self) -> pulumi.Input[_builtins.str]:
        """
        `open`: open the protection, `close`: close the protection.
        """
        return pulumi.get(self, "operate")

    @operate.setter
    def operate(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "operate", value)

    @_builtins.property
    @pulumi.getter
    def type(self) -> pulumi.Input[_builtins.str]:
        """
        The type of protection resource. Reference value: `Global`: instance, `Service`: service, `Route`: route, `Object`: obejct (This interface does not currently support this type).
        """
        return pulumi.get(self, "type")

    @type.setter
    def type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "type", value)

    @_builtins.property
    @pulumi.getter
    def lists(self) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        Means the list of services or routes when the resource type `Type` is `Service` or `Route`.
        """
        return pulumi.get(self, "lists")

    @lists.setter
    def lists(self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]):
        pulumi.set(self, "lists", value)


@pulumi.input_type
class _WafProtectionState:
    def __init__(
        __self__,
        *,
        gateway_id: pulumi.Input[_builtins.str | None] = None,
        global_status: pulumi.Input[_builtins.str | None] = None,
        lists: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        operate: pulumi.Input[_builtins.str | None] = None,
        type: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering WafProtection resources.

        :param pulumi.Input[_builtins.str] gateway_id: Gateway ID.
        :param pulumi.Input[_builtins.str] global_status: Global protection status.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] lists: Means the list of services or routes when the resource type `Type` is `Service` or `Route`.
        :param pulumi.Input[_builtins.str] operate: `open`: open the protection, `close`: close the protection.
        :param pulumi.Input[_builtins.str] type: The type of protection resource. Reference value: `Global`: instance, `Service`: service, `Route`: route, `Object`: obejct (This interface does not currently support this type).
        """
        if gateway_id is not None:
            pulumi.set(__self__, "gateway_id", gateway_id)
        if global_status is not None:
            pulumi.set(__self__, "global_status", global_status)
        if lists is not None:
            pulumi.set(__self__, "lists", lists)
        if operate is not None:
            pulumi.set(__self__, "operate", operate)
        if type is not None:
            pulumi.set(__self__, "type", type)

    @_builtins.property
    @pulumi.getter(name="gatewayId")
    def gateway_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Gateway ID.
        """
        return pulumi.get(self, "gateway_id")

    @gateway_id.setter
    def gateway_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "gateway_id", value)

    @_builtins.property
    @pulumi.getter(name="globalStatus")
    def global_status(self) -> pulumi.Input[_builtins.str | None]:
        """
        Global protection status.
        """
        return pulumi.get(self, "global_status")

    @global_status.setter
    def global_status(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "global_status", value)

    @_builtins.property
    @pulumi.getter
    def lists(self) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        Means the list of services or routes when the resource type `Type` is `Service` or `Route`.
        """
        return pulumi.get(self, "lists")

    @lists.setter
    def lists(self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]):
        pulumi.set(self, "lists", value)

    @_builtins.property
    @pulumi.getter
    def operate(self) -> pulumi.Input[_builtins.str | None]:
        """
        `open`: open the protection, `close`: close the protection.
        """
        return pulumi.get(self, "operate")

    @operate.setter
    def operate(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "operate", value)

    @_builtins.property
    @pulumi.getter
    def type(self) -> pulumi.Input[_builtins.str | None]:
        """
        The type of protection resource. Reference value: `Global`: instance, `Service`: service, `Route`: route, `Object`: obejct (This interface does not currently support this type).
        """
        return pulumi.get(self, "type")

    @type.setter
    def type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "type", value)


@pulumi.type_token("tencentcloud:Tse/wafProtection:WafProtection")
class WafProtection(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        gateway_id: pulumi.Input[_builtins.str | None] = None,
        lists: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        operate: pulumi.Input[_builtins.str | None] = None,
        type: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a tse waf_protection

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        waf_protection = tencentcloud.tse.WafProtection("waf_protection",
            gateway_id="gateway-ed63e957",
            type="Route",
            lists=["7324a769-9d87-48ce-a904-48c3defc4abd"],
            operate="open")
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] gateway_id: Gateway ID.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] lists: Means the list of services or routes when the resource type `Type` is `Service` or `Route`.
        :param pulumi.Input[_builtins.str] operate: `open`: open the protection, `close`: close the protection.
        :param pulumi.Input[_builtins.str] type: The type of protection resource. Reference value: `Global`: instance, `Service`: service, `Route`: route, `Object`: obejct (This interface does not currently support this type).
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: WafProtectionArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a tse waf_protection

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        waf_protection = tencentcloud.tse.WafProtection("waf_protection",
            gateway_id="gateway-ed63e957",
            type="Route",
            lists=["7324a769-9d87-48ce-a904-48c3defc4abd"],
            operate="open")
        ```


        :param str resource_name: The name of the resource.
        :param WafProtectionArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            WafProtectionArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        gateway_id: pulumi.Input[_builtins.str | None] = None,
        lists: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        operate: pulumi.Input[_builtins.str | None] = None,
        type: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = WafProtectionArgs.__new__(WafProtectionArgs)

            if gateway_id is None and not opts.urn:
                raise TypeError("Missing required property 'gateway_id'")
            __props__.__dict__["gateway_id"] = gateway_id
            __props__.__dict__["lists"] = lists
            if operate is None and not opts.urn:
                raise TypeError("Missing required property 'operate'")
            __props__.__dict__["operate"] = operate
            if type is None and not opts.urn:
                raise TypeError("Missing required property 'type'")
            __props__.__dict__["type"] = type
            __props__.__dict__["global_status"] = None
        super().__init__(
            "tencentcloud:Tse/wafProtection:WafProtection",
            resource_name,
            __props__,
            opts,
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        gateway_id: pulumi.Input[_builtins.str | None] = None,
        global_status: pulumi.Input[_builtins.str | None] = None,
        lists: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        operate: pulumi.Input[_builtins.str | None] = None,
        type: pulumi.Input[_builtins.str | None] = None,
    ) -> "WafProtection":
        """
        Get an existing WafProtection resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] gateway_id: Gateway ID.
        :param pulumi.Input[_builtins.str] global_status: Global protection status.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] lists: Means the list of services or routes when the resource type `Type` is `Service` or `Route`.
        :param pulumi.Input[_builtins.str] operate: `open`: open the protection, `close`: close the protection.
        :param pulumi.Input[_builtins.str] type: The type of protection resource. Reference value: `Global`: instance, `Service`: service, `Route`: route, `Object`: obejct (This interface does not currently support this type).
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _WafProtectionState.__new__(_WafProtectionState)

        __props__.__dict__["gateway_id"] = gateway_id
        __props__.__dict__["global_status"] = global_status
        __props__.__dict__["lists"] = lists
        __props__.__dict__["operate"] = operate
        __props__.__dict__["type"] = type
        return WafProtection(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="gatewayId")
    def gateway_id(self) -> pulumi.Output[_builtins.str]:
        """
        Gateway ID.
        """
        return pulumi.get(self, "gateway_id")

    @_builtins.property
    @pulumi.getter(name="globalStatus")
    def global_status(self) -> pulumi.Output[_builtins.str]:
        """
        Global protection status.
        """
        return pulumi.get(self, "global_status")

    @_builtins.property
    @pulumi.getter
    def lists(self) -> pulumi.Output[Sequence[_builtins.str] | None]:
        """
        Means the list of services or routes when the resource type `Type` is `Service` or `Route`.
        """
        return pulumi.get(self, "lists")

    @_builtins.property
    @pulumi.getter
    def operate(self) -> pulumi.Output[_builtins.str]:
        """
        `open`: open the protection, `close`: close the protection.
        """
        return pulumi.get(self, "operate")

    @_builtins.property
    @pulumi.getter
    def type(self) -> pulumi.Output[_builtins.str]:
        """
        The type of protection resource. Reference value: `Global`: instance, `Service`: service, `Route`: route, `Object`: obejct (This interface does not currently support this type).
        """
        return pulumi.get(self, "type")
