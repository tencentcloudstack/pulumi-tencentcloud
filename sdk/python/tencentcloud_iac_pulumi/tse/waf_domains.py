# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["WafDomains", "WafDomainsArgs"]


@pulumi.input_type
class WafDomainsArgs:
    def __init__(
        __self__,
        *,
        domain: pulumi.Input[_builtins.str],
        gateway_id: pulumi.Input[_builtins.str]
    ):
        """
        The set of arguments for constructing a WafDomains resource.

        :param pulumi.Input[_builtins.str] domain: The waf protected domain name.
        :param pulumi.Input[_builtins.str] gateway_id: Gateway ID.
        """
        pulumi.set(__self__, "domain", domain)
        pulumi.set(__self__, "gateway_id", gateway_id)

    @_builtins.property
    @pulumi.getter
    def domain(self) -> pulumi.Input[_builtins.str]:
        """
        The waf protected domain name.
        """
        return pulumi.get(self, "domain")

    @domain.setter
    def domain(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "domain", value)

    @_builtins.property
    @pulumi.getter(name="gatewayId")
    def gateway_id(self) -> pulumi.Input[_builtins.str]:
        """
        Gateway ID.
        """
        return pulumi.get(self, "gateway_id")

    @gateway_id.setter
    def gateway_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "gateway_id", value)


@pulumi.input_type
class _WafDomainsState:
    def __init__(
        __self__,
        *,
        domain: pulumi.Input[_builtins.str | None] = None,
        gateway_id: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering WafDomains resources.

        :param pulumi.Input[_builtins.str] domain: The waf protected domain name.
        :param pulumi.Input[_builtins.str] gateway_id: Gateway ID.
        """
        if domain is not None:
            pulumi.set(__self__, "domain", domain)
        if gateway_id is not None:
            pulumi.set(__self__, "gateway_id", gateway_id)

    @_builtins.property
    @pulumi.getter
    def domain(self) -> pulumi.Input[_builtins.str | None]:
        """
        The waf protected domain name.
        """
        return pulumi.get(self, "domain")

    @domain.setter
    def domain(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "domain", value)

    @_builtins.property
    @pulumi.getter(name="gatewayId")
    def gateway_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Gateway ID.
        """
        return pulumi.get(self, "gateway_id")

    @gateway_id.setter
    def gateway_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "gateway_id", value)


@pulumi.type_token("tencentcloud:Tse/wafDomains:WafDomains")
class WafDomains(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        domain: pulumi.Input[_builtins.str | None] = None,
        gateway_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a tse waf_domains

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        waf_domains = tencentcloud.tse.WafDomains("waf_domains",
            domain="tse.exmaple.com",
            gateway_id="gateway-ed63e957")
        ```

        ## Import

        tse waf_domains can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Tse/wafDomains:WafDomains waf_domains waf_domains_id
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] domain: The waf protected domain name.
        :param pulumi.Input[_builtins.str] gateway_id: Gateway ID.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: WafDomainsArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a tse waf_domains

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        waf_domains = tencentcloud.tse.WafDomains("waf_domains",
            domain="tse.exmaple.com",
            gateway_id="gateway-ed63e957")
        ```

        ## Import

        tse waf_domains can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Tse/wafDomains:WafDomains waf_domains waf_domains_id
        ```


        :param str resource_name: The name of the resource.
        :param WafDomainsArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            WafDomainsArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        domain: pulumi.Input[_builtins.str | None] = None,
        gateway_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = WafDomainsArgs.__new__(WafDomainsArgs)

            if domain is None and not opts.urn:
                raise TypeError("Missing required property 'domain'")
            __props__.__dict__["domain"] = domain
            if gateway_id is None and not opts.urn:
                raise TypeError("Missing required property 'gateway_id'")
            __props__.__dict__["gateway_id"] = gateway_id
        super().__init__(
            "tencentcloud:Tse/wafDomains:WafDomains", resource_name, __props__, opts
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        domain: pulumi.Input[_builtins.str | None] = None,
        gateway_id: pulumi.Input[_builtins.str | None] = None,
    ) -> "WafDomains":
        """
        Get an existing WafDomains resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] domain: The waf protected domain name.
        :param pulumi.Input[_builtins.str] gateway_id: Gateway ID.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _WafDomainsState.__new__(_WafDomainsState)

        __props__.__dict__["domain"] = domain
        __props__.__dict__["gateway_id"] = gateway_id
        return WafDomains(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter
    def domain(self) -> pulumi.Output[_builtins.str]:
        """
        The waf protected domain name.
        """
        return pulumi.get(self, "domain")

    @_builtins.property
    @pulumi.getter(name="gatewayId")
    def gateway_id(self) -> pulumi.Output[_builtins.str]:
        """
        Gateway ID.
        """
        return pulumi.get(self, "gateway_id")
