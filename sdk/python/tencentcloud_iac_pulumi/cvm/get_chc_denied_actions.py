# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs

__all__ = [
    "AwaitableGetChcDeniedActionsResult",
    "GetChcDeniedActionsResult",
    "get_chc_denied_actions",
    "get_chc_denied_actions_output",
]


@pulumi.output_type
class GetChcDeniedActionsResult:
    """
    A collection of values returned by getChcDeniedActions.
    """

    def __init__(
        __self__,
        chc_host_denied_action_sets=None,
        chc_ids=None,
        id=None,
        result_output_file=None,
    ):
        if chc_host_denied_action_sets and not isinstance(
            chc_host_denied_action_sets, list
        ):
            raise TypeError(
                "Expected argument 'chc_host_denied_action_sets' to be a list"
            )
        pulumi.set(__self__, "chc_host_denied_action_sets", chc_host_denied_action_sets)
        if chc_ids and not isinstance(chc_ids, list):
            raise TypeError("Expected argument 'chc_ids' to be a list")
        pulumi.set(__self__, "chc_ids", chc_ids)
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)
        if result_output_file and not isinstance(result_output_file, str):
            raise TypeError("Expected argument 'result_output_file' to be a str")
        pulumi.set(__self__, "result_output_file", result_output_file)

    @_builtins.property
    @pulumi.getter(name="chcHostDeniedActionSets")
    def chc_host_denied_action_sets(
        self,
    ) -> Sequence["outputs.GetChcDeniedActionsChcHostDeniedActionSetResult"]:
        """
        Actions not allowed for the CHC instance.
        """
        return pulumi.get(self, "chc_host_denied_action_sets")

    @_builtins.property
    @pulumi.getter(name="chcIds")
    def chc_ids(self) -> Sequence[_builtins.str]:
        return pulumi.get(self, "chc_ids")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The provider-assigned unique ID for this managed resource.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="resultOutputFile")
    def result_output_file(self) -> _builtins.str | None:
        return pulumi.get(self, "result_output_file")


class AwaitableGetChcDeniedActionsResult(GetChcDeniedActionsResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetChcDeniedActionsResult(
            chc_host_denied_action_sets=self.chc_host_denied_action_sets,
            chc_ids=self.chc_ids,
            id=self.id,
            result_output_file=self.result_output_file,
        )


def get_chc_denied_actions(
    chc_ids: Sequence[_builtins.str] | None = None,
    result_output_file: _builtins.str | None = None,
    opts: pulumi.InvokeOptions | None = None,
) -> AwaitableGetChcDeniedActionsResult:
    """
    Use this data source to query detailed information of cvm chc_denied_actions

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    chc_denied_actions = tencentcloud.cvm.get_chc_denied_actions(chc_ids=["chc-xxxxx"])
    ```


    :param Sequence[_builtins.str] chc_ids: CHC host IDs.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["chcIds"] = chc_ids
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke(
        "tencentcloud:Cvm/getChcDeniedActions:getChcDeniedActions",
        __args__,
        opts=opts,
        typ=GetChcDeniedActionsResult,
    ).value

    return AwaitableGetChcDeniedActionsResult(
        chc_host_denied_action_sets=pulumi.get(__ret__, "chc_host_denied_action_sets"),
        chc_ids=pulumi.get(__ret__, "chc_ids"),
        id=pulumi.get(__ret__, "id"),
        result_output_file=pulumi.get(__ret__, "result_output_file"),
    )


def get_chc_denied_actions_output(
    chc_ids: pulumi.Input[Sequence[_builtins.str] | None] = None,
    result_output_file: pulumi.Input[_builtins.str | None] = None,
    opts: pulumi.InvokeOptions | pulumi.InvokeOutputOptions | None = None,
) -> pulumi.Output[GetChcDeniedActionsResult]:
    """
    Use this data source to query detailed information of cvm chc_denied_actions

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    chc_denied_actions = tencentcloud.cvm.get_chc_denied_actions(chc_ids=["chc-xxxxx"])
    ```


    :param Sequence[_builtins.str] chc_ids: CHC host IDs.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["chcIds"] = chc_ids
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output(
        "tencentcloud:Cvm/getChcDeniedActions:getChcDeniedActions",
        __args__,
        opts=opts,
        typ=GetChcDeniedActionsResult,
    )
    return __ret__.apply(
        lambda __response__: GetChcDeniedActionsResult(
            chc_host_denied_action_sets=pulumi.get(
                __response__, "chc_host_denied_action_sets"
            ),
            chc_ids=pulumi.get(__response__, "chc_ids"),
            id=pulumi.get(__response__, "id"),
            result_output_file=pulumi.get(__response__, "result_output_file"),
        )
    )
