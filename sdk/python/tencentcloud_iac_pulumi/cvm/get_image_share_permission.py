# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs

__all__ = [
    "AwaitableGetImageSharePermissionResult",
    "GetImageSharePermissionResult",
    "get_image_share_permission",
    "get_image_share_permission_output",
]


@pulumi.output_type
class GetImageSharePermissionResult:
    """
    A collection of values returned by getImageSharePermission.
    """

    def __init__(
        __self__,
        id=None,
        image_id=None,
        result_output_file=None,
        share_permission_sets=None,
    ):
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)
        if image_id and not isinstance(image_id, str):
            raise TypeError("Expected argument 'image_id' to be a str")
        pulumi.set(__self__, "image_id", image_id)
        if result_output_file and not isinstance(result_output_file, str):
            raise TypeError("Expected argument 'result_output_file' to be a str")
        pulumi.set(__self__, "result_output_file", result_output_file)
        if share_permission_sets and not isinstance(share_permission_sets, list):
            raise TypeError("Expected argument 'share_permission_sets' to be a list")
        pulumi.set(__self__, "share_permission_sets", share_permission_sets)

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The provider-assigned unique ID for this managed resource.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="imageId")
    def image_id(self) -> _builtins.str:
        return pulumi.get(self, "image_id")

    @_builtins.property
    @pulumi.getter(name="resultOutputFile")
    def result_output_file(self) -> _builtins.str | None:
        return pulumi.get(self, "result_output_file")

    @_builtins.property
    @pulumi.getter(name="sharePermissionSets")
    def share_permission_sets(
        self,
    ) -> Sequence["outputs.GetImageSharePermissionSharePermissionSetResult"]:
        """
        Information on image sharing.
        """
        return pulumi.get(self, "share_permission_sets")


class AwaitableGetImageSharePermissionResult(GetImageSharePermissionResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetImageSharePermissionResult(
            id=self.id,
            image_id=self.image_id,
            result_output_file=self.result_output_file,
            share_permission_sets=self.share_permission_sets,
        )


def get_image_share_permission(
    image_id: _builtins.str | None = None,
    result_output_file: _builtins.str | None = None,
    opts: pulumi.InvokeOptions | None = None,
) -> AwaitableGetImageSharePermissionResult:
    """
    Use this data source to query detailed information of CVM image share permission

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    example = tencentcloud.cvm.get_image_share_permission(image_id="img-0elsru2u")
    ```


    :param _builtins.str image_id: The ID of the image to be shared.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["imageId"] = image_id
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke(
        "tencentcloud:Cvm/getImageSharePermission:getImageSharePermission",
        __args__,
        opts=opts,
        typ=GetImageSharePermissionResult,
    ).value

    return AwaitableGetImageSharePermissionResult(
        id=pulumi.get(__ret__, "id"),
        image_id=pulumi.get(__ret__, "image_id"),
        result_output_file=pulumi.get(__ret__, "result_output_file"),
        share_permission_sets=pulumi.get(__ret__, "share_permission_sets"),
    )


def get_image_share_permission_output(
    image_id: pulumi.Input[_builtins.str | None] = None,
    result_output_file: pulumi.Input[_builtins.str | None] = None,
    opts: pulumi.InvokeOptions | pulumi.InvokeOutputOptions | None = None,
) -> pulumi.Output[GetImageSharePermissionResult]:
    """
    Use this data source to query detailed information of CVM image share permission

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    example = tencentcloud.cvm.get_image_share_permission(image_id="img-0elsru2u")
    ```


    :param _builtins.str image_id: The ID of the image to be shared.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["imageId"] = image_id
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output(
        "tencentcloud:Cvm/getImageSharePermission:getImageSharePermission",
        __args__,
        opts=opts,
        typ=GetImageSharePermissionResult,
    )
    return __ret__.apply(
        lambda __response__: GetImageSharePermissionResult(
            id=pulumi.get(__response__, "id"),
            image_id=pulumi.get(__response__, "image_id"),
            result_output_file=pulumi.get(__response__, "result_output_file"),
            share_permission_sets=pulumi.get(__response__, "share_permission_sets"),
        )
    )
