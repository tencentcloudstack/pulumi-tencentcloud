# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence
from typing import Union

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = [
    "AwaitableGetListenersByTargetsResult",
    "GetListenersByTargetsResult",
    "get_listeners_by_targets",
    "get_listeners_by_targets_output",
]


@pulumi.output_type
class GetListenersByTargetsResult:
    """
    A collection of values returned by getListenersByTargets.
    """

    def __init__(
        __self__, backends=None, id=None, load_balancers=None, result_output_file=None
    ):
        if backends and not isinstance(backends, list):
            raise TypeError("Expected argument 'backends' to be a list")
        pulumi.set(__self__, "backends", backends)
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)
        if load_balancers and not isinstance(load_balancers, list):
            raise TypeError("Expected argument 'load_balancers' to be a list")
        pulumi.set(__self__, "load_balancers", load_balancers)
        if result_output_file and not isinstance(result_output_file, str):
            raise TypeError("Expected argument 'result_output_file' to be a str")
        pulumi.set(__self__, "result_output_file", result_output_file)

    @_builtins.property
    @pulumi.getter
    def backends(self) -> Sequence["outputs.GetListenersByTargetsBackendResult"]:
        return pulumi.get(self, "backends")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The provider-assigned unique ID for this managed resource.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="loadBalancers")
    def load_balancers(
        self,
    ) -> Sequence["outputs.GetListenersByTargetsLoadBalancerResult"]:
        """
        Detail of the CLB instance.
        """
        return pulumi.get(self, "load_balancers")

    @_builtins.property
    @pulumi.getter(name="resultOutputFile")
    def result_output_file(self) -> _builtins.str | None:
        return pulumi.get(self, "result_output_file")


class AwaitableGetListenersByTargetsResult(GetListenersByTargetsResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetListenersByTargetsResult(
            backends=self.backends,
            id=self.id,
            load_balancers=self.load_balancers,
            result_output_file=self.result_output_file,
        )


def get_listeners_by_targets(
    backends: (
        Sequence[
            Union[
                "GetListenersByTargetsBackendArgs",
                "GetListenersByTargetsBackendArgsDict",
            ]
        ]
        | None
    ) = None,
    result_output_file: _builtins.str | None = None,
    opts: pulumi.InvokeOptions | None = None,
) -> AwaitableGetListenersByTargetsResult:
    """
    Use this data source to query detailed information of clb listeners_by_targets

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    listeners_by_targets = tencentcloud.clb.get_listeners_by_targets(backends=[{
        "vpc_id": "vpc-4owdpnwr",
        "private_ip": "106.52.160.211",
    }])
    ```


    :param Sequence[Union['GetListenersByTargetsBackendArgs', 'GetListenersByTargetsBackendArgsDict']] backends: List of private network IPs to be queried.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["backends"] = backends
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke(
        "tencentcloud:Clb/getListenersByTargets:getListenersByTargets",
        __args__,
        opts=opts,
        typ=GetListenersByTargetsResult,
    ).value

    return AwaitableGetListenersByTargetsResult(
        backends=pulumi.get(__ret__, "backends"),
        id=pulumi.get(__ret__, "id"),
        load_balancers=pulumi.get(__ret__, "load_balancers"),
        result_output_file=pulumi.get(__ret__, "result_output_file"),
    )


def get_listeners_by_targets_output(
    backends: pulumi.Input[
        Sequence[
            Union[
                "GetListenersByTargetsBackendArgs",
                "GetListenersByTargetsBackendArgsDict",
            ]
        ]
        | None
    ] = None,
    result_output_file: pulumi.Input[_builtins.str | None] = None,
    opts: pulumi.InvokeOptions | pulumi.InvokeOutputOptions | None = None,
) -> pulumi.Output[GetListenersByTargetsResult]:
    """
    Use this data source to query detailed information of clb listeners_by_targets

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    listeners_by_targets = tencentcloud.clb.get_listeners_by_targets(backends=[{
        "vpc_id": "vpc-4owdpnwr",
        "private_ip": "106.52.160.211",
    }])
    ```


    :param Sequence[Union['GetListenersByTargetsBackendArgs', 'GetListenersByTargetsBackendArgsDict']] backends: List of private network IPs to be queried.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["backends"] = backends
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output(
        "tencentcloud:Clb/getListenersByTargets:getListenersByTargets",
        __args__,
        opts=opts,
        typ=GetListenersByTargetsResult,
    )
    return __ret__.apply(
        lambda __response__: GetListenersByTargetsResult(
            backends=pulumi.get(__response__, "backends"),
            id=pulumi.get(__response__, "id"),
            load_balancers=pulumi.get(__response__, "load_balancers"),
            result_output_file=pulumi.get(__response__, "result_output_file"),
        )
    )
