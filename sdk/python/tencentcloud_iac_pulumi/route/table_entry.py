# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["TableEntry", "TableEntryArgs"]


@pulumi.input_type
class TableEntryArgs:
    def __init__(
        __self__,
        *,
        destination_cidr_block: pulumi.Input[_builtins.str],
        next_hub: pulumi.Input[_builtins.str],
        next_type: pulumi.Input[_builtins.str],
        route_table_id: pulumi.Input[_builtins.str],
        description: pulumi.Input[_builtins.str | None] = None,
        disabled: pulumi.Input[_builtins.bool | None] = None
    ):
        """
        The set of arguments for constructing a TableEntry resource.

        :param pulumi.Input[_builtins.str] destination_cidr_block: Destination address block.
        :param pulumi.Input[_builtins.str] next_hub: ID of next-hop gateway. Note: when `next_type` is EIP, `next_hub` should be `0`.
        :param pulumi.Input[_builtins.str] next_type: Type of next-hop. Valid values: `CVM`, `VPN`, `DIRECTCONNECT`, `PEERCONNECTION`, `HAVIP`, `NAT`, `NORMAL_CVM`, `EIP`, `LOCAL_GATEWAY`, `INTRANAT`, `USER_CCN` and `GWLB_ENDPOINT`.
        :param pulumi.Input[_builtins.str] route_table_id: ID of routing table to which this entry belongs.
        :param pulumi.Input[_builtins.str] description: Description of the routing table entry.
        :param pulumi.Input[_builtins.bool] disabled: Whether the entry is disabled, default is `false`.
        """
        pulumi.set(__self__, "destination_cidr_block", destination_cidr_block)
        pulumi.set(__self__, "next_hub", next_hub)
        pulumi.set(__self__, "next_type", next_type)
        pulumi.set(__self__, "route_table_id", route_table_id)
        if description is not None:
            pulumi.set(__self__, "description", description)
        if disabled is not None:
            pulumi.set(__self__, "disabled", disabled)

    @_builtins.property
    @pulumi.getter(name="destinationCidrBlock")
    def destination_cidr_block(self) -> pulumi.Input[_builtins.str]:
        """
        Destination address block.
        """
        return pulumi.get(self, "destination_cidr_block")

    @destination_cidr_block.setter
    def destination_cidr_block(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "destination_cidr_block", value)

    @_builtins.property
    @pulumi.getter(name="nextHub")
    def next_hub(self) -> pulumi.Input[_builtins.str]:
        """
        ID of next-hop gateway. Note: when `next_type` is EIP, `next_hub` should be `0`.
        """
        return pulumi.get(self, "next_hub")

    @next_hub.setter
    def next_hub(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "next_hub", value)

    @_builtins.property
    @pulumi.getter(name="nextType")
    def next_type(self) -> pulumi.Input[_builtins.str]:
        """
        Type of next-hop. Valid values: `CVM`, `VPN`, `DIRECTCONNECT`, `PEERCONNECTION`, `HAVIP`, `NAT`, `NORMAL_CVM`, `EIP`, `LOCAL_GATEWAY`, `INTRANAT`, `USER_CCN` and `GWLB_ENDPOINT`.
        """
        return pulumi.get(self, "next_type")

    @next_type.setter
    def next_type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "next_type", value)

    @_builtins.property
    @pulumi.getter(name="routeTableId")
    def route_table_id(self) -> pulumi.Input[_builtins.str]:
        """
        ID of routing table to which this entry belongs.
        """
        return pulumi.get(self, "route_table_id")

    @route_table_id.setter
    def route_table_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "route_table_id", value)

    @_builtins.property
    @pulumi.getter
    def description(self) -> pulumi.Input[_builtins.str | None]:
        """
        Description of the routing table entry.
        """
        return pulumi.get(self, "description")

    @description.setter
    def description(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "description", value)

    @_builtins.property
    @pulumi.getter
    def disabled(self) -> pulumi.Input[_builtins.bool | None]:
        """
        Whether the entry is disabled, default is `false`.
        """
        return pulumi.get(self, "disabled")

    @disabled.setter
    def disabled(self, value: pulumi.Input[_builtins.bool | None]):
        pulumi.set(self, "disabled", value)


@pulumi.input_type
class _TableEntryState:
    def __init__(
        __self__,
        *,
        description: pulumi.Input[_builtins.str | None] = None,
        destination_cidr_block: pulumi.Input[_builtins.str | None] = None,
        disabled: pulumi.Input[_builtins.bool | None] = None,
        next_hub: pulumi.Input[_builtins.str | None] = None,
        next_type: pulumi.Input[_builtins.str | None] = None,
        route_entry_id: pulumi.Input[_builtins.int | None] = None,
        route_item_id: pulumi.Input[_builtins.str | None] = None,
        route_table_id: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering TableEntry resources.

        :param pulumi.Input[_builtins.str] description: Description of the routing table entry.
        :param pulumi.Input[_builtins.str] destination_cidr_block: Destination address block.
        :param pulumi.Input[_builtins.bool] disabled: Whether the entry is disabled, default is `false`.
        :param pulumi.Input[_builtins.str] next_hub: ID of next-hop gateway. Note: when `next_type` is EIP, `next_hub` should be `0`.
        :param pulumi.Input[_builtins.str] next_type: Type of next-hop. Valid values: `CVM`, `VPN`, `DIRECTCONNECT`, `PEERCONNECTION`, `HAVIP`, `NAT`, `NORMAL_CVM`, `EIP`, `LOCAL_GATEWAY`, `INTRANAT`, `USER_CCN` and `GWLB_ENDPOINT`.
        :param pulumi.Input[_builtins.int] route_entry_id: ID of route entry.
        :param pulumi.Input[_builtins.str] route_item_id: ID of route table entry.
        :param pulumi.Input[_builtins.str] route_table_id: ID of routing table to which this entry belongs.
        """
        if description is not None:
            pulumi.set(__self__, "description", description)
        if destination_cidr_block is not None:
            pulumi.set(__self__, "destination_cidr_block", destination_cidr_block)
        if disabled is not None:
            pulumi.set(__self__, "disabled", disabled)
        if next_hub is not None:
            pulumi.set(__self__, "next_hub", next_hub)
        if next_type is not None:
            pulumi.set(__self__, "next_type", next_type)
        if route_entry_id is not None:
            pulumi.set(__self__, "route_entry_id", route_entry_id)
        if route_item_id is not None:
            pulumi.set(__self__, "route_item_id", route_item_id)
        if route_table_id is not None:
            pulumi.set(__self__, "route_table_id", route_table_id)

    @_builtins.property
    @pulumi.getter
    def description(self) -> pulumi.Input[_builtins.str | None]:
        """
        Description of the routing table entry.
        """
        return pulumi.get(self, "description")

    @description.setter
    def description(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "description", value)

    @_builtins.property
    @pulumi.getter(name="destinationCidrBlock")
    def destination_cidr_block(self) -> pulumi.Input[_builtins.str | None]:
        """
        Destination address block.
        """
        return pulumi.get(self, "destination_cidr_block")

    @destination_cidr_block.setter
    def destination_cidr_block(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "destination_cidr_block", value)

    @_builtins.property
    @pulumi.getter
    def disabled(self) -> pulumi.Input[_builtins.bool | None]:
        """
        Whether the entry is disabled, default is `false`.
        """
        return pulumi.get(self, "disabled")

    @disabled.setter
    def disabled(self, value: pulumi.Input[_builtins.bool | None]):
        pulumi.set(self, "disabled", value)

    @_builtins.property
    @pulumi.getter(name="nextHub")
    def next_hub(self) -> pulumi.Input[_builtins.str | None]:
        """
        ID of next-hop gateway. Note: when `next_type` is EIP, `next_hub` should be `0`.
        """
        return pulumi.get(self, "next_hub")

    @next_hub.setter
    def next_hub(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "next_hub", value)

    @_builtins.property
    @pulumi.getter(name="nextType")
    def next_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Type of next-hop. Valid values: `CVM`, `VPN`, `DIRECTCONNECT`, `PEERCONNECTION`, `HAVIP`, `NAT`, `NORMAL_CVM`, `EIP`, `LOCAL_GATEWAY`, `INTRANAT`, `USER_CCN` and `GWLB_ENDPOINT`.
        """
        return pulumi.get(self, "next_type")

    @next_type.setter
    def next_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "next_type", value)

    @_builtins.property
    @pulumi.getter(name="routeEntryId")
    def route_entry_id(self) -> pulumi.Input[_builtins.int | None]:
        """
        ID of route entry.
        """
        return pulumi.get(self, "route_entry_id")

    @route_entry_id.setter
    def route_entry_id(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "route_entry_id", value)

    @_builtins.property
    @pulumi.getter(name="routeItemId")
    def route_item_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        ID of route table entry.
        """
        return pulumi.get(self, "route_item_id")

    @route_item_id.setter
    def route_item_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "route_item_id", value)

    @_builtins.property
    @pulumi.getter(name="routeTableId")
    def route_table_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        ID of routing table to which this entry belongs.
        """
        return pulumi.get(self, "route_table_id")

    @route_table_id.setter
    def route_table_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "route_table_id", value)


@pulumi.type_token("tencentcloud:Route/tableEntry:TableEntry")
class TableEntry(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        description: pulumi.Input[_builtins.str | None] = None,
        destination_cidr_block: pulumi.Input[_builtins.str | None] = None,
        disabled: pulumi.Input[_builtins.bool | None] = None,
        next_hub: pulumi.Input[_builtins.str | None] = None,
        next_type: pulumi.Input[_builtins.str | None] = None,
        route_table_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a Route table entry.

        > **NOTE:** When setting the route item switch, do not use it together with resource `Route.TableEntryConfig`.

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        config = pulumi.Config()
        availability_zone = config.get("availabilityZone")
        if availability_zone is None:
            availability_zone = "ap-guangzhou-4"
        # create vpc
        vpc = tencentcloud.vpc.Instance("vpc",
            name="vpc",
            cidr_block="10.0.0.0/16")
        # create route table
        example = tencentcloud.route.Table("example",
            vpc_id=vpc.id,
            name="tf-example")
        # create subnet
        subnet = tencentcloud.subnet.Instance("subnet",
            vpc_id=vpc.id,
            name="subnet",
            cidr_block="10.0.12.0/24",
            availability_zone=availability_zone,
            route_table_id=example.id)
        # create route table entry
        example_table_entry = tencentcloud.route.TableEntry("example",
            route_table_id=example.id,
            destination_cidr_block="10.12.12.0/24",
            next_type="EIP",
            next_hub="0",
            description="Terraform test.")
        pulumi.export("itemId", example_table_entry.route_item_id)
        ```

        ## Import

        Route table entry can be imported using the routeEntryId.routeTableId, e.g.

        ```sh
        $ pulumi import tencentcloud:Route/tableEntry:TableEntry example 3065857.rtb-b050fg94
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] description: Description of the routing table entry.
        :param pulumi.Input[_builtins.str] destination_cidr_block: Destination address block.
        :param pulumi.Input[_builtins.bool] disabled: Whether the entry is disabled, default is `false`.
        :param pulumi.Input[_builtins.str] next_hub: ID of next-hop gateway. Note: when `next_type` is EIP, `next_hub` should be `0`.
        :param pulumi.Input[_builtins.str] next_type: Type of next-hop. Valid values: `CVM`, `VPN`, `DIRECTCONNECT`, `PEERCONNECTION`, `HAVIP`, `NAT`, `NORMAL_CVM`, `EIP`, `LOCAL_GATEWAY`, `INTRANAT`, `USER_CCN` and `GWLB_ENDPOINT`.
        :param pulumi.Input[_builtins.str] route_table_id: ID of routing table to which this entry belongs.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: TableEntryArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a Route table entry.

        > **NOTE:** When setting the route item switch, do not use it together with resource `Route.TableEntryConfig`.

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        config = pulumi.Config()
        availability_zone = config.get("availabilityZone")
        if availability_zone is None:
            availability_zone = "ap-guangzhou-4"
        # create vpc
        vpc = tencentcloud.vpc.Instance("vpc",
            name="vpc",
            cidr_block="10.0.0.0/16")
        # create route table
        example = tencentcloud.route.Table("example",
            vpc_id=vpc.id,
            name="tf-example")
        # create subnet
        subnet = tencentcloud.subnet.Instance("subnet",
            vpc_id=vpc.id,
            name="subnet",
            cidr_block="10.0.12.0/24",
            availability_zone=availability_zone,
            route_table_id=example.id)
        # create route table entry
        example_table_entry = tencentcloud.route.TableEntry("example",
            route_table_id=example.id,
            destination_cidr_block="10.12.12.0/24",
            next_type="EIP",
            next_hub="0",
            description="Terraform test.")
        pulumi.export("itemId", example_table_entry.route_item_id)
        ```

        ## Import

        Route table entry can be imported using the routeEntryId.routeTableId, e.g.

        ```sh
        $ pulumi import tencentcloud:Route/tableEntry:TableEntry example 3065857.rtb-b050fg94
        ```


        :param str resource_name: The name of the resource.
        :param TableEntryArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            TableEntryArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        description: pulumi.Input[_builtins.str | None] = None,
        destination_cidr_block: pulumi.Input[_builtins.str | None] = None,
        disabled: pulumi.Input[_builtins.bool | None] = None,
        next_hub: pulumi.Input[_builtins.str | None] = None,
        next_type: pulumi.Input[_builtins.str | None] = None,
        route_table_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = TableEntryArgs.__new__(TableEntryArgs)

            __props__.__dict__["description"] = description
            if destination_cidr_block is None and not opts.urn:
                raise TypeError("Missing required property 'destination_cidr_block'")
            __props__.__dict__["destination_cidr_block"] = destination_cidr_block
            __props__.__dict__["disabled"] = disabled
            if next_hub is None and not opts.urn:
                raise TypeError("Missing required property 'next_hub'")
            __props__.__dict__["next_hub"] = next_hub
            if next_type is None and not opts.urn:
                raise TypeError("Missing required property 'next_type'")
            __props__.__dict__["next_type"] = next_type
            if route_table_id is None and not opts.urn:
                raise TypeError("Missing required property 'route_table_id'")
            __props__.__dict__["route_table_id"] = route_table_id
            __props__.__dict__["route_entry_id"] = None
            __props__.__dict__["route_item_id"] = None
        super().__init__(
            "tencentcloud:Route/tableEntry:TableEntry", resource_name, __props__, opts
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        description: pulumi.Input[_builtins.str | None] = None,
        destination_cidr_block: pulumi.Input[_builtins.str | None] = None,
        disabled: pulumi.Input[_builtins.bool | None] = None,
        next_hub: pulumi.Input[_builtins.str | None] = None,
        next_type: pulumi.Input[_builtins.str | None] = None,
        route_entry_id: pulumi.Input[_builtins.int | None] = None,
        route_item_id: pulumi.Input[_builtins.str | None] = None,
        route_table_id: pulumi.Input[_builtins.str | None] = None,
    ) -> "TableEntry":
        """
        Get an existing TableEntry resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] description: Description of the routing table entry.
        :param pulumi.Input[_builtins.str] destination_cidr_block: Destination address block.
        :param pulumi.Input[_builtins.bool] disabled: Whether the entry is disabled, default is `false`.
        :param pulumi.Input[_builtins.str] next_hub: ID of next-hop gateway. Note: when `next_type` is EIP, `next_hub` should be `0`.
        :param pulumi.Input[_builtins.str] next_type: Type of next-hop. Valid values: `CVM`, `VPN`, `DIRECTCONNECT`, `PEERCONNECTION`, `HAVIP`, `NAT`, `NORMAL_CVM`, `EIP`, `LOCAL_GATEWAY`, `INTRANAT`, `USER_CCN` and `GWLB_ENDPOINT`.
        :param pulumi.Input[_builtins.int] route_entry_id: ID of route entry.
        :param pulumi.Input[_builtins.str] route_item_id: ID of route table entry.
        :param pulumi.Input[_builtins.str] route_table_id: ID of routing table to which this entry belongs.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _TableEntryState.__new__(_TableEntryState)

        __props__.__dict__["description"] = description
        __props__.__dict__["destination_cidr_block"] = destination_cidr_block
        __props__.__dict__["disabled"] = disabled
        __props__.__dict__["next_hub"] = next_hub
        __props__.__dict__["next_type"] = next_type
        __props__.__dict__["route_entry_id"] = route_entry_id
        __props__.__dict__["route_item_id"] = route_item_id
        __props__.__dict__["route_table_id"] = route_table_id
        return TableEntry(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter
    def description(self) -> pulumi.Output[_builtins.str | None]:
        """
        Description of the routing table entry.
        """
        return pulumi.get(self, "description")

    @_builtins.property
    @pulumi.getter(name="destinationCidrBlock")
    def destination_cidr_block(self) -> pulumi.Output[_builtins.str]:
        """
        Destination address block.
        """
        return pulumi.get(self, "destination_cidr_block")

    @_builtins.property
    @pulumi.getter
    def disabled(self) -> pulumi.Output[_builtins.bool]:
        """
        Whether the entry is disabled, default is `false`.
        """
        return pulumi.get(self, "disabled")

    @_builtins.property
    @pulumi.getter(name="nextHub")
    def next_hub(self) -> pulumi.Output[_builtins.str]:
        """
        ID of next-hop gateway. Note: when `next_type` is EIP, `next_hub` should be `0`.
        """
        return pulumi.get(self, "next_hub")

    @_builtins.property
    @pulumi.getter(name="nextType")
    def next_type(self) -> pulumi.Output[_builtins.str]:
        """
        Type of next-hop. Valid values: `CVM`, `VPN`, `DIRECTCONNECT`, `PEERCONNECTION`, `HAVIP`, `NAT`, `NORMAL_CVM`, `EIP`, `LOCAL_GATEWAY`, `INTRANAT`, `USER_CCN` and `GWLB_ENDPOINT`.
        """
        return pulumi.get(self, "next_type")

    @_builtins.property
    @pulumi.getter(name="routeEntryId")
    def route_entry_id(self) -> pulumi.Output[_builtins.int]:
        """
        ID of route entry.
        """
        return pulumi.get(self, "route_entry_id")

    @_builtins.property
    @pulumi.getter(name="routeItemId")
    def route_item_id(self) -> pulumi.Output[_builtins.str]:
        """
        ID of route table entry.
        """
        return pulumi.get(self, "route_item_id")

    @_builtins.property
    @pulumi.getter(name="routeTableId")
    def route_table_id(self) -> pulumi.Output[_builtins.str]:
        """
        ID of routing table to which this entry belongs.
        """
        return pulumi.get(self, "route_table_id")
