# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["KeyPair", "KeyPairArgs"]


@pulumi.input_type
class KeyPairArgs:
    def __init__(
        __self__,
        *,
        key_name: pulumi.Input[_builtins.str],
        public_key: pulumi.Input[_builtins.str | None] = None
    ):
        """
        The set of arguments for constructing a KeyPair resource.

        :param pulumi.Input[_builtins.str] key_name: Key pair name, which can contain up to 25 digits, letters, and underscores.
        :param pulumi.Input[_builtins.str] public_key: Public key content of the key pair, which is in the OpenSSH RSA format.
        """
        pulumi.set(__self__, "key_name", key_name)
        if public_key is not None:
            pulumi.set(__self__, "public_key", public_key)

    @_builtins.property
    @pulumi.getter(name="keyName")
    def key_name(self) -> pulumi.Input[_builtins.str]:
        """
        Key pair name, which can contain up to 25 digits, letters, and underscores.
        """
        return pulumi.get(self, "key_name")

    @key_name.setter
    def key_name(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "key_name", value)

    @_builtins.property
    @pulumi.getter(name="publicKey")
    def public_key(self) -> pulumi.Input[_builtins.str | None]:
        """
        Public key content of the key pair, which is in the OpenSSH RSA format.
        """
        return pulumi.get(self, "public_key")

    @public_key.setter
    def public_key(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "public_key", value)


@pulumi.input_type
class _KeyPairState:
    def __init__(
        __self__,
        *,
        created_time: pulumi.Input[_builtins.str | None] = None,
        key_name: pulumi.Input[_builtins.str | None] = None,
        private_key: pulumi.Input[_builtins.str | None] = None,
        public_key: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering KeyPair resources.

        :param pulumi.Input[_builtins.str] created_time: Creation time. Expressed according to the ISO8601 standard, and using UTC time. Format: YYYY-MM-DDThh:mm:ssZ.
        :param pulumi.Input[_builtins.str] key_name: Key pair name, which can contain up to 25 digits, letters, and underscores.
        :param pulumi.Input[_builtins.str] private_key: Key to private key.
        :param pulumi.Input[_builtins.str] public_key: Public key content of the key pair, which is in the OpenSSH RSA format.
        """
        if created_time is not None:
            pulumi.set(__self__, "created_time", created_time)
        if key_name is not None:
            pulumi.set(__self__, "key_name", key_name)
        if private_key is not None:
            pulumi.set(__self__, "private_key", private_key)
        if public_key is not None:
            pulumi.set(__self__, "public_key", public_key)

    @_builtins.property
    @pulumi.getter(name="createdTime")
    def created_time(self) -> pulumi.Input[_builtins.str | None]:
        """
        Creation time. Expressed according to the ISO8601 standard, and using UTC time. Format: YYYY-MM-DDThh:mm:ssZ.
        """
        return pulumi.get(self, "created_time")

    @created_time.setter
    def created_time(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "created_time", value)

    @_builtins.property
    @pulumi.getter(name="keyName")
    def key_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Key pair name, which can contain up to 25 digits, letters, and underscores.
        """
        return pulumi.get(self, "key_name")

    @key_name.setter
    def key_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "key_name", value)

    @_builtins.property
    @pulumi.getter(name="privateKey")
    def private_key(self) -> pulumi.Input[_builtins.str | None]:
        """
        Key to private key.
        """
        return pulumi.get(self, "private_key")

    @private_key.setter
    def private_key(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "private_key", value)

    @_builtins.property
    @pulumi.getter(name="publicKey")
    def public_key(self) -> pulumi.Input[_builtins.str | None]:
        """
        Public key content of the key pair, which is in the OpenSSH RSA format.
        """
        return pulumi.get(self, "public_key")

    @public_key.setter
    def public_key(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "public_key", value)


@pulumi.type_token("tencentcloud:Lighthouse/keyPair:KeyPair")
class KeyPair(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        key_name: pulumi.Input[_builtins.str | None] = None,
        public_key: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a lighthouse key_pair

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        key_pair = tencentcloud.lighthouse.KeyPair("key_pair", key_name="key_name_test")
        pulumi.export("privateKey", key_pair.private_key)
        ```

        ## Import

        lighthouse key_pair can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Lighthouse/keyPair:KeyPair key_pair key_pair_id
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] key_name: Key pair name, which can contain up to 25 digits, letters, and underscores.
        :param pulumi.Input[_builtins.str] public_key: Public key content of the key pair, which is in the OpenSSH RSA format.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: KeyPairArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a lighthouse key_pair

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        key_pair = tencentcloud.lighthouse.KeyPair("key_pair", key_name="key_name_test")
        pulumi.export("privateKey", key_pair.private_key)
        ```

        ## Import

        lighthouse key_pair can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Lighthouse/keyPair:KeyPair key_pair key_pair_id
        ```


        :param str resource_name: The name of the resource.
        :param KeyPairArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            KeyPairArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        key_name: pulumi.Input[_builtins.str | None] = None,
        public_key: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = KeyPairArgs.__new__(KeyPairArgs)

            if key_name is None and not opts.urn:
                raise TypeError("Missing required property 'key_name'")
            __props__.__dict__["key_name"] = key_name
            __props__.__dict__["public_key"] = public_key
            __props__.__dict__["created_time"] = None
            __props__.__dict__["private_key"] = None
        super().__init__(
            "tencentcloud:Lighthouse/keyPair:KeyPair", resource_name, __props__, opts
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        created_time: pulumi.Input[_builtins.str | None] = None,
        key_name: pulumi.Input[_builtins.str | None] = None,
        private_key: pulumi.Input[_builtins.str | None] = None,
        public_key: pulumi.Input[_builtins.str | None] = None,
    ) -> "KeyPair":
        """
        Get an existing KeyPair resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] created_time: Creation time. Expressed according to the ISO8601 standard, and using UTC time. Format: YYYY-MM-DDThh:mm:ssZ.
        :param pulumi.Input[_builtins.str] key_name: Key pair name, which can contain up to 25 digits, letters, and underscores.
        :param pulumi.Input[_builtins.str] private_key: Key to private key.
        :param pulumi.Input[_builtins.str] public_key: Public key content of the key pair, which is in the OpenSSH RSA format.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _KeyPairState.__new__(_KeyPairState)

        __props__.__dict__["created_time"] = created_time
        __props__.__dict__["key_name"] = key_name
        __props__.__dict__["private_key"] = private_key
        __props__.__dict__["public_key"] = public_key
        return KeyPair(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="createdTime")
    def created_time(self) -> pulumi.Output[_builtins.str]:
        """
        Creation time. Expressed according to the ISO8601 standard, and using UTC time. Format: YYYY-MM-DDThh:mm:ssZ.
        """
        return pulumi.get(self, "created_time")

    @_builtins.property
    @pulumi.getter(name="keyName")
    def key_name(self) -> pulumi.Output[_builtins.str]:
        """
        Key pair name, which can contain up to 25 digits, letters, and underscores.
        """
        return pulumi.get(self, "key_name")

    @_builtins.property
    @pulumi.getter(name="privateKey")
    def private_key(self) -> pulumi.Output[_builtins.str]:
        """
        Key to private key.
        """
        return pulumi.get(self, "private_key")

    @_builtins.property
    @pulumi.getter(name="publicKey")
    def public_key(self) -> pulumi.Output[_builtins.str]:
        """
        Public key content of the key pair, which is in the OpenSSH RSA format.
        """
        return pulumi.get(self, "public_key")
