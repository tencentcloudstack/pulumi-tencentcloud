# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["SampleSnapshotTemplate", "SampleSnapshotTemplateArgs"]


@pulumi.input_type
class SampleSnapshotTemplateArgs:
    def __init__(
        __self__,
        *,
        sample_interval: pulumi.Input[_builtins.int],
        sample_type: pulumi.Input[_builtins.str],
        comment: pulumi.Input[_builtins.str | None] = None,
        fill_type: pulumi.Input[_builtins.str | None] = None,
        format: pulumi.Input[_builtins.str | None] = None,
        height: pulumi.Input[_builtins.int | None] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        resolution_adaptive: pulumi.Input[_builtins.str | None] = None,
        width: pulumi.Input[_builtins.int | None] = None
    ):
        """
        The set of arguments for constructing a SampleSnapshotTemplate resource.

        :param pulumi.Input[_builtins.int] sample_interval: Sampling interval.When SampleType is Percent, specify the percentage of the sampling interval.When SampleType is Time, specify the sampling interval time in seconds.
        :param pulumi.Input[_builtins.str] sample_type: Sampling snapshot type, optional value:Percent/Time.
        :param pulumi.Input[_builtins.str] comment: Template description information, length limit: 256 characters.
        :param pulumi.Input[_builtins.str] fill_type: Filling type, when the aspect ratio of the video stream configuration is inconsistent with the aspect ratio of the original video, the processing method for transcoding is filling. Optional filling type:stretch: Stretching, stretching each frame to fill the entire screen, which may cause the transcoded video to be squashed or stretched.black: Leave black, keep the video aspect ratio unchanged, and fill the rest of the edge with black.white: Leave blank, keep the aspect ratio of the video, and fill the rest of the edge with white.gauss: Gaussian blur, keep the aspect ratio of the video unchanged, and use Gaussian blur for the rest of the edge.Default value: black.
        :param pulumi.Input[_builtins.str] format: Image format, the value can be jpg, png, webp. Default is jpg.
        :param pulumi.Input[_builtins.int] height: The maximum value of the snapshot height (or short side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        :param pulumi.Input[_builtins.str] name: Sample snapshot template name, length limit: 64 characters.
        :param pulumi.Input[_builtins.str] resolution_adaptive: Adaptive resolution, optional value:open: At this time, Width represents the long side of the video, Height represents the short side of the video.close: At this point, Width represents the width of the video, and Height represents the height of the video.Default value: open.
        :param pulumi.Input[_builtins.int] width: The maximum value of the snapshot width (or long side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        """
        pulumi.set(__self__, "sample_interval", sample_interval)
        pulumi.set(__self__, "sample_type", sample_type)
        if comment is not None:
            pulumi.set(__self__, "comment", comment)
        if fill_type is not None:
            pulumi.set(__self__, "fill_type", fill_type)
        if format is not None:
            pulumi.set(__self__, "format", format)
        if height is not None:
            pulumi.set(__self__, "height", height)
        if name is not None:
            pulumi.set(__self__, "name", name)
        if resolution_adaptive is not None:
            pulumi.set(__self__, "resolution_adaptive", resolution_adaptive)
        if width is not None:
            pulumi.set(__self__, "width", width)

    @_builtins.property
    @pulumi.getter(name="sampleInterval")
    def sample_interval(self) -> pulumi.Input[_builtins.int]:
        """
        Sampling interval.When SampleType is Percent, specify the percentage of the sampling interval.When SampleType is Time, specify the sampling interval time in seconds.
        """
        return pulumi.get(self, "sample_interval")

    @sample_interval.setter
    def sample_interval(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "sample_interval", value)

    @_builtins.property
    @pulumi.getter(name="sampleType")
    def sample_type(self) -> pulumi.Input[_builtins.str]:
        """
        Sampling snapshot type, optional value:Percent/Time.
        """
        return pulumi.get(self, "sample_type")

    @sample_type.setter
    def sample_type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "sample_type", value)

    @_builtins.property
    @pulumi.getter
    def comment(self) -> pulumi.Input[_builtins.str | None]:
        """
        Template description information, length limit: 256 characters.
        """
        return pulumi.get(self, "comment")

    @comment.setter
    def comment(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "comment", value)

    @_builtins.property
    @pulumi.getter(name="fillType")
    def fill_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Filling type, when the aspect ratio of the video stream configuration is inconsistent with the aspect ratio of the original video, the processing method for transcoding is filling. Optional filling type:stretch: Stretching, stretching each frame to fill the entire screen, which may cause the transcoded video to be squashed or stretched.black: Leave black, keep the video aspect ratio unchanged, and fill the rest of the edge with black.white: Leave blank, keep the aspect ratio of the video, and fill the rest of the edge with white.gauss: Gaussian blur, keep the aspect ratio of the video unchanged, and use Gaussian blur for the rest of the edge.Default value: black.
        """
        return pulumi.get(self, "fill_type")

    @fill_type.setter
    def fill_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "fill_type", value)

    @_builtins.property
    @pulumi.getter
    def format(self) -> pulumi.Input[_builtins.str | None]:
        """
        Image format, the value can be jpg, png, webp. Default is jpg.
        """
        return pulumi.get(self, "format")

    @format.setter
    def format(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "format", value)

    @_builtins.property
    @pulumi.getter
    def height(self) -> pulumi.Input[_builtins.int | None]:
        """
        The maximum value of the snapshot height (or short side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        """
        return pulumi.get(self, "height")

    @height.setter
    def height(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "height", value)

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Sample snapshot template name, length limit: 64 characters.
        """
        return pulumi.get(self, "name")

    @name.setter
    def name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "name", value)

    @_builtins.property
    @pulumi.getter(name="resolutionAdaptive")
    def resolution_adaptive(self) -> pulumi.Input[_builtins.str | None]:
        """
        Adaptive resolution, optional value:open: At this time, Width represents the long side of the video, Height represents the short side of the video.close: At this point, Width represents the width of the video, and Height represents the height of the video.Default value: open.
        """
        return pulumi.get(self, "resolution_adaptive")

    @resolution_adaptive.setter
    def resolution_adaptive(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "resolution_adaptive", value)

    @_builtins.property
    @pulumi.getter
    def width(self) -> pulumi.Input[_builtins.int | None]:
        """
        The maximum value of the snapshot width (or long side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        """
        return pulumi.get(self, "width")

    @width.setter
    def width(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "width", value)


@pulumi.input_type
class _SampleSnapshotTemplateState:
    def __init__(
        __self__,
        *,
        comment: pulumi.Input[_builtins.str | None] = None,
        fill_type: pulumi.Input[_builtins.str | None] = None,
        format: pulumi.Input[_builtins.str | None] = None,
        height: pulumi.Input[_builtins.int | None] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        resolution_adaptive: pulumi.Input[_builtins.str | None] = None,
        sample_interval: pulumi.Input[_builtins.int | None] = None,
        sample_type: pulumi.Input[_builtins.str | None] = None,
        width: pulumi.Input[_builtins.int | None] = None
    ):
        """
        Input properties used for looking up and filtering SampleSnapshotTemplate resources.

        :param pulumi.Input[_builtins.str] comment: Template description information, length limit: 256 characters.
        :param pulumi.Input[_builtins.str] fill_type: Filling type, when the aspect ratio of the video stream configuration is inconsistent with the aspect ratio of the original video, the processing method for transcoding is filling. Optional filling type:stretch: Stretching, stretching each frame to fill the entire screen, which may cause the transcoded video to be squashed or stretched.black: Leave black, keep the video aspect ratio unchanged, and fill the rest of the edge with black.white: Leave blank, keep the aspect ratio of the video, and fill the rest of the edge with white.gauss: Gaussian blur, keep the aspect ratio of the video unchanged, and use Gaussian blur for the rest of the edge.Default value: black.
        :param pulumi.Input[_builtins.str] format: Image format, the value can be jpg, png, webp. Default is jpg.
        :param pulumi.Input[_builtins.int] height: The maximum value of the snapshot height (or short side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        :param pulumi.Input[_builtins.str] name: Sample snapshot template name, length limit: 64 characters.
        :param pulumi.Input[_builtins.str] resolution_adaptive: Adaptive resolution, optional value:open: At this time, Width represents the long side of the video, Height represents the short side of the video.close: At this point, Width represents the width of the video, and Height represents the height of the video.Default value: open.
        :param pulumi.Input[_builtins.int] sample_interval: Sampling interval.When SampleType is Percent, specify the percentage of the sampling interval.When SampleType is Time, specify the sampling interval time in seconds.
        :param pulumi.Input[_builtins.str] sample_type: Sampling snapshot type, optional value:Percent/Time.
        :param pulumi.Input[_builtins.int] width: The maximum value of the snapshot width (or long side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        """
        if comment is not None:
            pulumi.set(__self__, "comment", comment)
        if fill_type is not None:
            pulumi.set(__self__, "fill_type", fill_type)
        if format is not None:
            pulumi.set(__self__, "format", format)
        if height is not None:
            pulumi.set(__self__, "height", height)
        if name is not None:
            pulumi.set(__self__, "name", name)
        if resolution_adaptive is not None:
            pulumi.set(__self__, "resolution_adaptive", resolution_adaptive)
        if sample_interval is not None:
            pulumi.set(__self__, "sample_interval", sample_interval)
        if sample_type is not None:
            pulumi.set(__self__, "sample_type", sample_type)
        if width is not None:
            pulumi.set(__self__, "width", width)

    @_builtins.property
    @pulumi.getter
    def comment(self) -> pulumi.Input[_builtins.str | None]:
        """
        Template description information, length limit: 256 characters.
        """
        return pulumi.get(self, "comment")

    @comment.setter
    def comment(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "comment", value)

    @_builtins.property
    @pulumi.getter(name="fillType")
    def fill_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Filling type, when the aspect ratio of the video stream configuration is inconsistent with the aspect ratio of the original video, the processing method for transcoding is filling. Optional filling type:stretch: Stretching, stretching each frame to fill the entire screen, which may cause the transcoded video to be squashed or stretched.black: Leave black, keep the video aspect ratio unchanged, and fill the rest of the edge with black.white: Leave blank, keep the aspect ratio of the video, and fill the rest of the edge with white.gauss: Gaussian blur, keep the aspect ratio of the video unchanged, and use Gaussian blur for the rest of the edge.Default value: black.
        """
        return pulumi.get(self, "fill_type")

    @fill_type.setter
    def fill_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "fill_type", value)

    @_builtins.property
    @pulumi.getter
    def format(self) -> pulumi.Input[_builtins.str | None]:
        """
        Image format, the value can be jpg, png, webp. Default is jpg.
        """
        return pulumi.get(self, "format")

    @format.setter
    def format(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "format", value)

    @_builtins.property
    @pulumi.getter
    def height(self) -> pulumi.Input[_builtins.int | None]:
        """
        The maximum value of the snapshot height (or short side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        """
        return pulumi.get(self, "height")

    @height.setter
    def height(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "height", value)

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Sample snapshot template name, length limit: 64 characters.
        """
        return pulumi.get(self, "name")

    @name.setter
    def name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "name", value)

    @_builtins.property
    @pulumi.getter(name="resolutionAdaptive")
    def resolution_adaptive(self) -> pulumi.Input[_builtins.str | None]:
        """
        Adaptive resolution, optional value:open: At this time, Width represents the long side of the video, Height represents the short side of the video.close: At this point, Width represents the width of the video, and Height represents the height of the video.Default value: open.
        """
        return pulumi.get(self, "resolution_adaptive")

    @resolution_adaptive.setter
    def resolution_adaptive(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "resolution_adaptive", value)

    @_builtins.property
    @pulumi.getter(name="sampleInterval")
    def sample_interval(self) -> pulumi.Input[_builtins.int | None]:
        """
        Sampling interval.When SampleType is Percent, specify the percentage of the sampling interval.When SampleType is Time, specify the sampling interval time in seconds.
        """
        return pulumi.get(self, "sample_interval")

    @sample_interval.setter
    def sample_interval(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "sample_interval", value)

    @_builtins.property
    @pulumi.getter(name="sampleType")
    def sample_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Sampling snapshot type, optional value:Percent/Time.
        """
        return pulumi.get(self, "sample_type")

    @sample_type.setter
    def sample_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "sample_type", value)

    @_builtins.property
    @pulumi.getter
    def width(self) -> pulumi.Input[_builtins.int | None]:
        """
        The maximum value of the snapshot width (or long side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        """
        return pulumi.get(self, "width")

    @width.setter
    def width(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "width", value)


@pulumi.type_token("tencentcloud:Mps/sampleSnapshotTemplate:SampleSnapshotTemplate")
class SampleSnapshotTemplate(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        comment: pulumi.Input[_builtins.str | None] = None,
        fill_type: pulumi.Input[_builtins.str | None] = None,
        format: pulumi.Input[_builtins.str | None] = None,
        height: pulumi.Input[_builtins.int | None] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        resolution_adaptive: pulumi.Input[_builtins.str | None] = None,
        sample_interval: pulumi.Input[_builtins.int | None] = None,
        sample_type: pulumi.Input[_builtins.str | None] = None,
        width: pulumi.Input[_builtins.int | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a mps sample_snapshot_template

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        sample_snapshot_template = tencentcloud.mps.SampleSnapshotTemplate("sample_snapshot_template",
            fill_type="stretch",
            format="jpg",
            height=128,
            name="terraform-test-for",
            resolution_adaptive="open",
            sample_interval=10,
            sample_type="Percent",
            width=140)
        ```

        ## Import

        mps sample_snapshot_template can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Mps/sampleSnapshotTemplate:SampleSnapshotTemplate sample_snapshot_template sample_snapshot_template_id
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] comment: Template description information, length limit: 256 characters.
        :param pulumi.Input[_builtins.str] fill_type: Filling type, when the aspect ratio of the video stream configuration is inconsistent with the aspect ratio of the original video, the processing method for transcoding is filling. Optional filling type:stretch: Stretching, stretching each frame to fill the entire screen, which may cause the transcoded video to be squashed or stretched.black: Leave black, keep the video aspect ratio unchanged, and fill the rest of the edge with black.white: Leave blank, keep the aspect ratio of the video, and fill the rest of the edge with white.gauss: Gaussian blur, keep the aspect ratio of the video unchanged, and use Gaussian blur for the rest of the edge.Default value: black.
        :param pulumi.Input[_builtins.str] format: Image format, the value can be jpg, png, webp. Default is jpg.
        :param pulumi.Input[_builtins.int] height: The maximum value of the snapshot height (or short side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        :param pulumi.Input[_builtins.str] name: Sample snapshot template name, length limit: 64 characters.
        :param pulumi.Input[_builtins.str] resolution_adaptive: Adaptive resolution, optional value:open: At this time, Width represents the long side of the video, Height represents the short side of the video.close: At this point, Width represents the width of the video, and Height represents the height of the video.Default value: open.
        :param pulumi.Input[_builtins.int] sample_interval: Sampling interval.When SampleType is Percent, specify the percentage of the sampling interval.When SampleType is Time, specify the sampling interval time in seconds.
        :param pulumi.Input[_builtins.str] sample_type: Sampling snapshot type, optional value:Percent/Time.
        :param pulumi.Input[_builtins.int] width: The maximum value of the snapshot width (or long side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: SampleSnapshotTemplateArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a mps sample_snapshot_template

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        sample_snapshot_template = tencentcloud.mps.SampleSnapshotTemplate("sample_snapshot_template",
            fill_type="stretch",
            format="jpg",
            height=128,
            name="terraform-test-for",
            resolution_adaptive="open",
            sample_interval=10,
            sample_type="Percent",
            width=140)
        ```

        ## Import

        mps sample_snapshot_template can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Mps/sampleSnapshotTemplate:SampleSnapshotTemplate sample_snapshot_template sample_snapshot_template_id
        ```


        :param str resource_name: The name of the resource.
        :param SampleSnapshotTemplateArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            SampleSnapshotTemplateArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        comment: pulumi.Input[_builtins.str | None] = None,
        fill_type: pulumi.Input[_builtins.str | None] = None,
        format: pulumi.Input[_builtins.str | None] = None,
        height: pulumi.Input[_builtins.int | None] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        resolution_adaptive: pulumi.Input[_builtins.str | None] = None,
        sample_interval: pulumi.Input[_builtins.int | None] = None,
        sample_type: pulumi.Input[_builtins.str | None] = None,
        width: pulumi.Input[_builtins.int | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = SampleSnapshotTemplateArgs.__new__(SampleSnapshotTemplateArgs)

            __props__.__dict__["comment"] = comment
            __props__.__dict__["fill_type"] = fill_type
            __props__.__dict__["format"] = format
            __props__.__dict__["height"] = height
            __props__.__dict__["name"] = name
            __props__.__dict__["resolution_adaptive"] = resolution_adaptive
            if sample_interval is None and not opts.urn:
                raise TypeError("Missing required property 'sample_interval'")
            __props__.__dict__["sample_interval"] = sample_interval
            if sample_type is None and not opts.urn:
                raise TypeError("Missing required property 'sample_type'")
            __props__.__dict__["sample_type"] = sample_type
            __props__.__dict__["width"] = width
        super().__init__(
            "tencentcloud:Mps/sampleSnapshotTemplate:SampleSnapshotTemplate",
            resource_name,
            __props__,
            opts,
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        comment: pulumi.Input[_builtins.str | None] = None,
        fill_type: pulumi.Input[_builtins.str | None] = None,
        format: pulumi.Input[_builtins.str | None] = None,
        height: pulumi.Input[_builtins.int | None] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        resolution_adaptive: pulumi.Input[_builtins.str | None] = None,
        sample_interval: pulumi.Input[_builtins.int | None] = None,
        sample_type: pulumi.Input[_builtins.str | None] = None,
        width: pulumi.Input[_builtins.int | None] = None,
    ) -> "SampleSnapshotTemplate":
        """
        Get an existing SampleSnapshotTemplate resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] comment: Template description information, length limit: 256 characters.
        :param pulumi.Input[_builtins.str] fill_type: Filling type, when the aspect ratio of the video stream configuration is inconsistent with the aspect ratio of the original video, the processing method for transcoding is filling. Optional filling type:stretch: Stretching, stretching each frame to fill the entire screen, which may cause the transcoded video to be squashed or stretched.black: Leave black, keep the video aspect ratio unchanged, and fill the rest of the edge with black.white: Leave blank, keep the aspect ratio of the video, and fill the rest of the edge with white.gauss: Gaussian blur, keep the aspect ratio of the video unchanged, and use Gaussian blur for the rest of the edge.Default value: black.
        :param pulumi.Input[_builtins.str] format: Image format, the value can be jpg, png, webp. Default is jpg.
        :param pulumi.Input[_builtins.int] height: The maximum value of the snapshot height (or short side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        :param pulumi.Input[_builtins.str] name: Sample snapshot template name, length limit: 64 characters.
        :param pulumi.Input[_builtins.str] resolution_adaptive: Adaptive resolution, optional value:open: At this time, Width represents the long side of the video, Height represents the short side of the video.close: At this point, Width represents the width of the video, and Height represents the height of the video.Default value: open.
        :param pulumi.Input[_builtins.int] sample_interval: Sampling interval.When SampleType is Percent, specify the percentage of the sampling interval.When SampleType is Time, specify the sampling interval time in seconds.
        :param pulumi.Input[_builtins.str] sample_type: Sampling snapshot type, optional value:Percent/Time.
        :param pulumi.Input[_builtins.int] width: The maximum value of the snapshot width (or long side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _SampleSnapshotTemplateState.__new__(_SampleSnapshotTemplateState)

        __props__.__dict__["comment"] = comment
        __props__.__dict__["fill_type"] = fill_type
        __props__.__dict__["format"] = format
        __props__.__dict__["height"] = height
        __props__.__dict__["name"] = name
        __props__.__dict__["resolution_adaptive"] = resolution_adaptive
        __props__.__dict__["sample_interval"] = sample_interval
        __props__.__dict__["sample_type"] = sample_type
        __props__.__dict__["width"] = width
        return SampleSnapshotTemplate(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter
    def comment(self) -> pulumi.Output[_builtins.str | None]:
        """
        Template description information, length limit: 256 characters.
        """
        return pulumi.get(self, "comment")

    @_builtins.property
    @pulumi.getter(name="fillType")
    def fill_type(self) -> pulumi.Output[_builtins.str | None]:
        """
        Filling type, when the aspect ratio of the video stream configuration is inconsistent with the aspect ratio of the original video, the processing method for transcoding is filling. Optional filling type:stretch: Stretching, stretching each frame to fill the entire screen, which may cause the transcoded video to be squashed or stretched.black: Leave black, keep the video aspect ratio unchanged, and fill the rest of the edge with black.white: Leave blank, keep the aspect ratio of the video, and fill the rest of the edge with white.gauss: Gaussian blur, keep the aspect ratio of the video unchanged, and use Gaussian blur for the rest of the edge.Default value: black.
        """
        return pulumi.get(self, "fill_type")

    @_builtins.property
    @pulumi.getter
    def format(self) -> pulumi.Output[_builtins.str | None]:
        """
        Image format, the value can be jpg, png, webp. Default is jpg.
        """
        return pulumi.get(self, "format")

    @_builtins.property
    @pulumi.getter
    def height(self) -> pulumi.Output[_builtins.int | None]:
        """
        The maximum value of the snapshot height (or short side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        """
        return pulumi.get(self, "height")

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Output[_builtins.str]:
        """
        Sample snapshot template name, length limit: 64 characters.
        """
        return pulumi.get(self, "name")

    @_builtins.property
    @pulumi.getter(name="resolutionAdaptive")
    def resolution_adaptive(self) -> pulumi.Output[_builtins.str | None]:
        """
        Adaptive resolution, optional value:open: At this time, Width represents the long side of the video, Height represents the short side of the video.close: At this point, Width represents the width of the video, and Height represents the height of the video.Default value: open.
        """
        return pulumi.get(self, "resolution_adaptive")

    @_builtins.property
    @pulumi.getter(name="sampleInterval")
    def sample_interval(self) -> pulumi.Output[_builtins.int]:
        """
        Sampling interval.When SampleType is Percent, specify the percentage of the sampling interval.When SampleType is Time, specify the sampling interval time in seconds.
        """
        return pulumi.get(self, "sample_interval")

    @_builtins.property
    @pulumi.getter(name="sampleType")
    def sample_type(self) -> pulumi.Output[_builtins.str]:
        """
        Sampling snapshot type, optional value:Percent/Time.
        """
        return pulumi.get(self, "sample_type")

    @_builtins.property
    @pulumi.getter
    def width(self) -> pulumi.Output[_builtins.int | None]:
        """
        The maximum value of the snapshot width (or long side), value range: 0 and [128, 4096], unit: px.When Width and Height are both 0, the resolution is the same.When Width is 0 and Height is not 0, Width is scaled proportionally.When Width is not 0 and Height is 0, Height is scaled proportionally.When both Width and Height are not 0, the resolution is specified by the user.Default value: 0.
        """
        return pulumi.get(self, "width")
