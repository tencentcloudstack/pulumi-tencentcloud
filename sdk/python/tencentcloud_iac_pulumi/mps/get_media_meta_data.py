# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence
from typing import Union

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = [
    "AwaitableGetMediaMetaDataResult",
    "GetMediaMetaDataResult",
    "get_media_meta_data",
    "get_media_meta_data_output",
]


@pulumi.output_type
class GetMediaMetaDataResult:
    """
    A collection of values returned by getMediaMetaData.
    """

    def __init__(
        __self__, id=None, input_info=None, meta_datas=None, result_output_file=None
    ):
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)
        if input_info and not isinstance(input_info, dict):
            raise TypeError("Expected argument 'input_info' to be a dict")
        pulumi.set(__self__, "input_info", input_info)
        if meta_datas and not isinstance(meta_datas, list):
            raise TypeError("Expected argument 'meta_datas' to be a list")
        pulumi.set(__self__, "meta_datas", meta_datas)
        if result_output_file and not isinstance(result_output_file, str):
            raise TypeError("Expected argument 'result_output_file' to be a str")
        pulumi.set(__self__, "result_output_file", result_output_file)

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The provider-assigned unique ID for this managed resource.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="inputInfo")
    def input_info(self) -> "outputs.GetMediaMetaDataInputInfoResult":
        return pulumi.get(self, "input_info")

    @_builtins.property
    @pulumi.getter(name="metaDatas")
    def meta_datas(self) -> Sequence["outputs.GetMediaMetaDataMetaDataResult"]:
        """
        Media metadata.
        """
        return pulumi.get(self, "meta_datas")

    @_builtins.property
    @pulumi.getter(name="resultOutputFile")
    def result_output_file(self) -> _builtins.str | None:
        return pulumi.get(self, "result_output_file")


class AwaitableGetMediaMetaDataResult(GetMediaMetaDataResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetMediaMetaDataResult(
            id=self.id,
            input_info=self.input_info,
            meta_datas=self.meta_datas,
            result_output_file=self.result_output_file,
        )


def get_media_meta_data(
    input_info: (
        Union["GetMediaMetaDataInputInfoArgs", "GetMediaMetaDataInputInfoArgsDict"]
        | None
    ) = None,
    result_output_file: _builtins.str | None = None,
    opts: pulumi.InvokeOptions | None = None,
) -> AwaitableGetMediaMetaDataResult:
    """
    Use this data source to query detailed information of mps media_meta_data

    ## Example Usage

    ### Query the mps media meta data through COS

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    object = tencentcloud.cos.get_bucket_object(bucket=f"keep-bucket-{app_id}",
        key="/mps-test/test.mov")
    metadata = tencentcloud.mps.get_media_meta_data(input_info={
        "type": "COS",
        "cos_input_info": {
            "bucket": object.bucket,
            "region": "%s",
            "object": object.key,
        },
    })
    ```


    :param Union['GetMediaMetaDataInputInfoArgs', 'GetMediaMetaDataInputInfoArgsDict'] input_info: Input information of file for metadata getting.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["inputInfo"] = input_info
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke(
        "tencentcloud:Mps/getMediaMetaData:getMediaMetaData",
        __args__,
        opts=opts,
        typ=GetMediaMetaDataResult,
    ).value

    return AwaitableGetMediaMetaDataResult(
        id=pulumi.get(__ret__, "id"),
        input_info=pulumi.get(__ret__, "input_info"),
        meta_datas=pulumi.get(__ret__, "meta_datas"),
        result_output_file=pulumi.get(__ret__, "result_output_file"),
    )


def get_media_meta_data_output(
    input_info: pulumi.Input[
        Union["GetMediaMetaDataInputInfoArgs", "GetMediaMetaDataInputInfoArgsDict"]
        | None
    ] = None,
    result_output_file: pulumi.Input[_builtins.str | None] = None,
    opts: pulumi.InvokeOptions | pulumi.InvokeOutputOptions | None = None,
) -> pulumi.Output[GetMediaMetaDataResult]:
    """
    Use this data source to query detailed information of mps media_meta_data

    ## Example Usage

    ### Query the mps media meta data through COS

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    object = tencentcloud.cos.get_bucket_object(bucket=f"keep-bucket-{app_id}",
        key="/mps-test/test.mov")
    metadata = tencentcloud.mps.get_media_meta_data(input_info={
        "type": "COS",
        "cos_input_info": {
            "bucket": object.bucket,
            "region": "%s",
            "object": object.key,
        },
    })
    ```


    :param Union['GetMediaMetaDataInputInfoArgs', 'GetMediaMetaDataInputInfoArgsDict'] input_info: Input information of file for metadata getting.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["inputInfo"] = input_info
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output(
        "tencentcloud:Mps/getMediaMetaData:getMediaMetaData",
        __args__,
        opts=opts,
        typ=GetMediaMetaDataResult,
    )
    return __ret__.apply(
        lambda __response__: GetMediaMetaDataResult(
            id=pulumi.get(__response__, "id"),
            input_info=pulumi.get(__response__, "input_info"),
            meta_datas=pulumi.get(__response__, "meta_datas"),
            result_output_file=pulumi.get(__response__, "result_output_file"),
        )
    )
