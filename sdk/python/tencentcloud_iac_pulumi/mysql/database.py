# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["Database", "DatabaseArgs"]


@pulumi.input_type
class DatabaseArgs:
    def __init__(
        __self__,
        *,
        character_set_name: pulumi.Input[_builtins.str],
        db_name: pulumi.Input[_builtins.str],
        instance_id: pulumi.Input[_builtins.str]
    ):
        """
        The set of arguments for constructing a Database resource.

        :param pulumi.Input[_builtins.str] character_set_name: Character set. Valid values:  `utf8`, `gbk`, `latin1`, `utf8mb4`.
        :param pulumi.Input[_builtins.str] db_name: Name of Database.
        :param pulumi.Input[_builtins.str] instance_id: Instance ID in the format of `cdb-c1nl9rpv`,  which is the same as the one displayed in the TencentDB console.
        """
        pulumi.set(__self__, "character_set_name", character_set_name)
        pulumi.set(__self__, "db_name", db_name)
        pulumi.set(__self__, "instance_id", instance_id)

    @_builtins.property
    @pulumi.getter(name="characterSetName")
    def character_set_name(self) -> pulumi.Input[_builtins.str]:
        """
        Character set. Valid values:  `utf8`, `gbk`, `latin1`, `utf8mb4`.
        """
        return pulumi.get(self, "character_set_name")

    @character_set_name.setter
    def character_set_name(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "character_set_name", value)

    @_builtins.property
    @pulumi.getter(name="dbName")
    def db_name(self) -> pulumi.Input[_builtins.str]:
        """
        Name of Database.
        """
        return pulumi.get(self, "db_name")

    @db_name.setter
    def db_name(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "db_name", value)

    @_builtins.property
    @pulumi.getter(name="instanceId")
    def instance_id(self) -> pulumi.Input[_builtins.str]:
        """
        Instance ID in the format of `cdb-c1nl9rpv`,  which is the same as the one displayed in the TencentDB console.
        """
        return pulumi.get(self, "instance_id")

    @instance_id.setter
    def instance_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "instance_id", value)


@pulumi.input_type
class _DatabaseState:
    def __init__(
        __self__,
        *,
        character_set_name: pulumi.Input[_builtins.str | None] = None,
        db_name: pulumi.Input[_builtins.str | None] = None,
        instance_id: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering Database resources.

        :param pulumi.Input[_builtins.str] character_set_name: Character set. Valid values:  `utf8`, `gbk`, `latin1`, `utf8mb4`.
        :param pulumi.Input[_builtins.str] db_name: Name of Database.
        :param pulumi.Input[_builtins.str] instance_id: Instance ID in the format of `cdb-c1nl9rpv`,  which is the same as the one displayed in the TencentDB console.
        """
        if character_set_name is not None:
            pulumi.set(__self__, "character_set_name", character_set_name)
        if db_name is not None:
            pulumi.set(__self__, "db_name", db_name)
        if instance_id is not None:
            pulumi.set(__self__, "instance_id", instance_id)

    @_builtins.property
    @pulumi.getter(name="characterSetName")
    def character_set_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Character set. Valid values:  `utf8`, `gbk`, `latin1`, `utf8mb4`.
        """
        return pulumi.get(self, "character_set_name")

    @character_set_name.setter
    def character_set_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "character_set_name", value)

    @_builtins.property
    @pulumi.getter(name="dbName")
    def db_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Name of Database.
        """
        return pulumi.get(self, "db_name")

    @db_name.setter
    def db_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "db_name", value)

    @_builtins.property
    @pulumi.getter(name="instanceId")
    def instance_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Instance ID in the format of `cdb-c1nl9rpv`,  which is the same as the one displayed in the TencentDB console.
        """
        return pulumi.get(self, "instance_id")

    @instance_id.setter
    def instance_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "instance_id", value)


@pulumi.type_token("tencentcloud:Mysql/database:Database")
class Database(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        character_set_name: pulumi.Input[_builtins.str | None] = None,
        db_name: pulumi.Input[_builtins.str | None] = None,
        instance_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a mysql database

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        database = tencentcloud.mysql.Database("database",
            instance_id="cdb-i9xfdf7z",
            db_name="for_tf_test",
            character_set_name="utf8")
        ```

        ## Import

        mysql database can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Mysql/database:Database database instanceId#dbName
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] character_set_name: Character set. Valid values:  `utf8`, `gbk`, `latin1`, `utf8mb4`.
        :param pulumi.Input[_builtins.str] db_name: Name of Database.
        :param pulumi.Input[_builtins.str] instance_id: Instance ID in the format of `cdb-c1nl9rpv`,  which is the same as the one displayed in the TencentDB console.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: DatabaseArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a mysql database

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        database = tencentcloud.mysql.Database("database",
            instance_id="cdb-i9xfdf7z",
            db_name="for_tf_test",
            character_set_name="utf8")
        ```

        ## Import

        mysql database can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Mysql/database:Database database instanceId#dbName
        ```


        :param str resource_name: The name of the resource.
        :param DatabaseArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            DatabaseArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        character_set_name: pulumi.Input[_builtins.str | None] = None,
        db_name: pulumi.Input[_builtins.str | None] = None,
        instance_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = DatabaseArgs.__new__(DatabaseArgs)

            if character_set_name is None and not opts.urn:
                raise TypeError("Missing required property 'character_set_name'")
            __props__.__dict__["character_set_name"] = character_set_name
            if db_name is None and not opts.urn:
                raise TypeError("Missing required property 'db_name'")
            __props__.__dict__["db_name"] = db_name
            if instance_id is None and not opts.urn:
                raise TypeError("Missing required property 'instance_id'")
            __props__.__dict__["instance_id"] = instance_id
        super().__init__(
            "tencentcloud:Mysql/database:Database", resource_name, __props__, opts
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        character_set_name: pulumi.Input[_builtins.str | None] = None,
        db_name: pulumi.Input[_builtins.str | None] = None,
        instance_id: pulumi.Input[_builtins.str | None] = None,
    ) -> "Database":
        """
        Get an existing Database resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] character_set_name: Character set. Valid values:  `utf8`, `gbk`, `latin1`, `utf8mb4`.
        :param pulumi.Input[_builtins.str] db_name: Name of Database.
        :param pulumi.Input[_builtins.str] instance_id: Instance ID in the format of `cdb-c1nl9rpv`,  which is the same as the one displayed in the TencentDB console.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _DatabaseState.__new__(_DatabaseState)

        __props__.__dict__["character_set_name"] = character_set_name
        __props__.__dict__["db_name"] = db_name
        __props__.__dict__["instance_id"] = instance_id
        return Database(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="characterSetName")
    def character_set_name(self) -> pulumi.Output[_builtins.str]:
        """
        Character set. Valid values:  `utf8`, `gbk`, `latin1`, `utf8mb4`.
        """
        return pulumi.get(self, "character_set_name")

    @_builtins.property
    @pulumi.getter(name="dbName")
    def db_name(self) -> pulumi.Output[_builtins.str]:
        """
        Name of Database.
        """
        return pulumi.get(self, "db_name")

    @_builtins.property
    @pulumi.getter(name="instanceId")
    def instance_id(self) -> pulumi.Output[_builtins.str]:
        """
        Instance ID in the format of `cdb-c1nl9rpv`,  which is the same as the one displayed in the TencentDB console.
        """
        return pulumi.get(self, "instance_id")
