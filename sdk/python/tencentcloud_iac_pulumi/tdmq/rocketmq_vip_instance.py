# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence
from typing import Optional, Union, overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ["RocketmqVipInstance", "RocketmqVipInstanceArgs"]


@pulumi.input_type
class RocketmqVipInstanceArgs:
    def __init__(
        __self__,
        *,
        node_count: pulumi.Input[_builtins.int],
        spec: pulumi.Input[_builtins.str],
        storage_size: pulumi.Input[_builtins.int],
        time_span: pulumi.Input[_builtins.int],
        vpc_info: pulumi.Input["RocketmqVipInstanceVpcInfoArgs"],
        zone_ids: pulumi.Input[Sequence[pulumi.Input[_builtins.str]]],
        ip_rules: pulumi.Input[
            Sequence[pulumi.Input["RocketmqVipInstanceIpRuleArgs"]] | None
        ] = None,
        name: pulumi.Input[_builtins.str | None] = None
    ):
        """
        The set of arguments for constructing a RocketmqVipInstance resource.

        :param pulumi.Input[_builtins.int] node_count: Number of nodes, minimum 2, maximum 20.
        :param pulumi.Input[_builtins.str] spec: Instance specification: Universal type, rocket-vip-basic-0, Basic type: `rocket-vip-basic-1`, Standard type: `rocket-vip-basic-2`, Advanced Type I: `rocket-vip-basic-3`, Advanced Type II: `rocket-vip-basic-4`.
        :param pulumi.Input[_builtins.int] storage_size: Single node storage space, in GB, minimum 200GB.
        :param pulumi.Input[_builtins.int] time_span: Purchase period, in months.
        :param pulumi.Input['RocketmqVipInstanceVpcInfoArgs'] vpc_info: VPC information.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] zone_ids: The Zone ID list for node deployment, such as Guangzhou Zone 1, is 100001. For details, please refer to the official website of Tencent Cloud.
        :param pulumi.Input[Sequence[pulumi.Input['RocketmqVipInstanceIpRuleArgs']]] ip_rules: Public IP access control rules.
        :param pulumi.Input[_builtins.str] name: Instance name.
        """
        pulumi.set(__self__, "node_count", node_count)
        pulumi.set(__self__, "spec", spec)
        pulumi.set(__self__, "storage_size", storage_size)
        pulumi.set(__self__, "time_span", time_span)
        pulumi.set(__self__, "vpc_info", vpc_info)
        pulumi.set(__self__, "zone_ids", zone_ids)
        if ip_rules is not None:
            pulumi.set(__self__, "ip_rules", ip_rules)
        if name is not None:
            pulumi.set(__self__, "name", name)

    @_builtins.property
    @pulumi.getter(name="nodeCount")
    def node_count(self) -> pulumi.Input[_builtins.int]:
        """
        Number of nodes, minimum 2, maximum 20.
        """
        return pulumi.get(self, "node_count")

    @node_count.setter
    def node_count(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "node_count", value)

    @_builtins.property
    @pulumi.getter
    def spec(self) -> pulumi.Input[_builtins.str]:
        """
        Instance specification: Universal type, rocket-vip-basic-0, Basic type: `rocket-vip-basic-1`, Standard type: `rocket-vip-basic-2`, Advanced Type I: `rocket-vip-basic-3`, Advanced Type II: `rocket-vip-basic-4`.
        """
        return pulumi.get(self, "spec")

    @spec.setter
    def spec(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "spec", value)

    @_builtins.property
    @pulumi.getter(name="storageSize")
    def storage_size(self) -> pulumi.Input[_builtins.int]:
        """
        Single node storage space, in GB, minimum 200GB.
        """
        return pulumi.get(self, "storage_size")

    @storage_size.setter
    def storage_size(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "storage_size", value)

    @_builtins.property
    @pulumi.getter(name="timeSpan")
    def time_span(self) -> pulumi.Input[_builtins.int]:
        """
        Purchase period, in months.
        """
        return pulumi.get(self, "time_span")

    @time_span.setter
    def time_span(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "time_span", value)

    @_builtins.property
    @pulumi.getter(name="vpcInfo")
    def vpc_info(self) -> pulumi.Input["RocketmqVipInstanceVpcInfoArgs"]:
        """
        VPC information.
        """
        return pulumi.get(self, "vpc_info")

    @vpc_info.setter
    def vpc_info(self, value: pulumi.Input["RocketmqVipInstanceVpcInfoArgs"]):
        pulumi.set(self, "vpc_info", value)

    @_builtins.property
    @pulumi.getter(name="zoneIds")
    def zone_ids(self) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]]]:
        """
        The Zone ID list for node deployment, such as Guangzhou Zone 1, is 100001. For details, please refer to the official website of Tencent Cloud.
        """
        return pulumi.get(self, "zone_ids")

    @zone_ids.setter
    def zone_ids(self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]]]):
        pulumi.set(self, "zone_ids", value)

    @_builtins.property
    @pulumi.getter(name="ipRules")
    def ip_rules(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input["RocketmqVipInstanceIpRuleArgs"]] | None]:
        """
        Public IP access control rules.
        """
        return pulumi.get(self, "ip_rules")

    @ip_rules.setter
    def ip_rules(
        self,
        value: pulumi.Input[
            Sequence[pulumi.Input["RocketmqVipInstanceIpRuleArgs"]] | None
        ],
    ):
        pulumi.set(self, "ip_rules", value)

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Instance name.
        """
        return pulumi.get(self, "name")

    @name.setter
    def name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "name", value)


@pulumi.input_type
class _RocketmqVipInstanceState:
    def __init__(
        __self__,
        *,
        ip_rules: pulumi.Input[
            Sequence[pulumi.Input["RocketmqVipInstanceIpRuleArgs"]] | None
        ] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        node_count: pulumi.Input[_builtins.int | None] = None,
        spec: pulumi.Input[_builtins.str | None] = None,
        storage_size: pulumi.Input[_builtins.int | None] = None,
        time_span: pulumi.Input[_builtins.int | None] = None,
        vpc_info: pulumi.Input[Optional["RocketmqVipInstanceVpcInfoArgs"]] = None,
        zone_ids: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None
    ):
        """
        Input properties used for looking up and filtering RocketmqVipInstance resources.

        :param pulumi.Input[Sequence[pulumi.Input['RocketmqVipInstanceIpRuleArgs']]] ip_rules: Public IP access control rules.
        :param pulumi.Input[_builtins.str] name: Instance name.
        :param pulumi.Input[_builtins.int] node_count: Number of nodes, minimum 2, maximum 20.
        :param pulumi.Input[_builtins.str] spec: Instance specification: Universal type, rocket-vip-basic-0, Basic type: `rocket-vip-basic-1`, Standard type: `rocket-vip-basic-2`, Advanced Type I: `rocket-vip-basic-3`, Advanced Type II: `rocket-vip-basic-4`.
        :param pulumi.Input[_builtins.int] storage_size: Single node storage space, in GB, minimum 200GB.
        :param pulumi.Input[_builtins.int] time_span: Purchase period, in months.
        :param pulumi.Input['RocketmqVipInstanceVpcInfoArgs'] vpc_info: VPC information.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] zone_ids: The Zone ID list for node deployment, such as Guangzhou Zone 1, is 100001. For details, please refer to the official website of Tencent Cloud.
        """
        if ip_rules is not None:
            pulumi.set(__self__, "ip_rules", ip_rules)
        if name is not None:
            pulumi.set(__self__, "name", name)
        if node_count is not None:
            pulumi.set(__self__, "node_count", node_count)
        if spec is not None:
            pulumi.set(__self__, "spec", spec)
        if storage_size is not None:
            pulumi.set(__self__, "storage_size", storage_size)
        if time_span is not None:
            pulumi.set(__self__, "time_span", time_span)
        if vpc_info is not None:
            pulumi.set(__self__, "vpc_info", vpc_info)
        if zone_ids is not None:
            pulumi.set(__self__, "zone_ids", zone_ids)

    @_builtins.property
    @pulumi.getter(name="ipRules")
    def ip_rules(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input["RocketmqVipInstanceIpRuleArgs"]] | None]:
        """
        Public IP access control rules.
        """
        return pulumi.get(self, "ip_rules")

    @ip_rules.setter
    def ip_rules(
        self,
        value: pulumi.Input[
            Sequence[pulumi.Input["RocketmqVipInstanceIpRuleArgs"]] | None
        ],
    ):
        pulumi.set(self, "ip_rules", value)

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Instance name.
        """
        return pulumi.get(self, "name")

    @name.setter
    def name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "name", value)

    @_builtins.property
    @pulumi.getter(name="nodeCount")
    def node_count(self) -> pulumi.Input[_builtins.int | None]:
        """
        Number of nodes, minimum 2, maximum 20.
        """
        return pulumi.get(self, "node_count")

    @node_count.setter
    def node_count(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "node_count", value)

    @_builtins.property
    @pulumi.getter
    def spec(self) -> pulumi.Input[_builtins.str | None]:
        """
        Instance specification: Universal type, rocket-vip-basic-0, Basic type: `rocket-vip-basic-1`, Standard type: `rocket-vip-basic-2`, Advanced Type I: `rocket-vip-basic-3`, Advanced Type II: `rocket-vip-basic-4`.
        """
        return pulumi.get(self, "spec")

    @spec.setter
    def spec(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "spec", value)

    @_builtins.property
    @pulumi.getter(name="storageSize")
    def storage_size(self) -> pulumi.Input[_builtins.int | None]:
        """
        Single node storage space, in GB, minimum 200GB.
        """
        return pulumi.get(self, "storage_size")

    @storage_size.setter
    def storage_size(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "storage_size", value)

    @_builtins.property
    @pulumi.getter(name="timeSpan")
    def time_span(self) -> pulumi.Input[_builtins.int | None]:
        """
        Purchase period, in months.
        """
        return pulumi.get(self, "time_span")

    @time_span.setter
    def time_span(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "time_span", value)

    @_builtins.property
    @pulumi.getter(name="vpcInfo")
    def vpc_info(self) -> pulumi.Input[Optional["RocketmqVipInstanceVpcInfoArgs"]]:
        """
        VPC information.
        """
        return pulumi.get(self, "vpc_info")

    @vpc_info.setter
    def vpc_info(self, value: pulumi.Input[Optional["RocketmqVipInstanceVpcInfoArgs"]]):
        pulumi.set(self, "vpc_info", value)

    @_builtins.property
    @pulumi.getter(name="zoneIds")
    def zone_ids(self) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        The Zone ID list for node deployment, such as Guangzhou Zone 1, is 100001. For details, please refer to the official website of Tencent Cloud.
        """
        return pulumi.get(self, "zone_ids")

    @zone_ids.setter
    def zone_ids(
        self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "zone_ids", value)


@pulumi.type_token("tencentcloud:Tdmq/rocketmqVipInstance:RocketmqVipInstance")
class RocketmqVipInstance(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        ip_rules: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union[
                        "RocketmqVipInstanceIpRuleArgs",
                        "RocketmqVipInstanceIpRuleArgsDict",
                    ]
                ]
            ]
            | None
        ] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        node_count: pulumi.Input[_builtins.int | None] = None,
        spec: pulumi.Input[_builtins.str | None] = None,
        storage_size: pulumi.Input[_builtins.int | None] = None,
        time_span: pulumi.Input[_builtins.int | None] = None,
        vpc_info: pulumi.Input[
            Union[
                "RocketmqVipInstanceVpcInfoArgs", "RocketmqVipInstanceVpcInfoArgsDict"
            ]
            | None
        ] = None,
        zone_ids: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a tdmq rocketmq_vip_instance

        > **NOTE:** The instance cannot be downgraded, Include parameters `node_count`, `spec`, `storage_size`.
        **NOTE:** If `spec` is `rocket-vip-basic-2`, configuration changes are not supported.


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[Sequence[pulumi.Input[Union['RocketmqVipInstanceIpRuleArgs', 'RocketmqVipInstanceIpRuleArgsDict']]]] ip_rules: Public IP access control rules.
        :param pulumi.Input[_builtins.str] name: Instance name.
        :param pulumi.Input[_builtins.int] node_count: Number of nodes, minimum 2, maximum 20.
        :param pulumi.Input[_builtins.str] spec: Instance specification: Universal type, rocket-vip-basic-0, Basic type: `rocket-vip-basic-1`, Standard type: `rocket-vip-basic-2`, Advanced Type I: `rocket-vip-basic-3`, Advanced Type II: `rocket-vip-basic-4`.
        :param pulumi.Input[_builtins.int] storage_size: Single node storage space, in GB, minimum 200GB.
        :param pulumi.Input[_builtins.int] time_span: Purchase period, in months.
        :param pulumi.Input[Union['RocketmqVipInstanceVpcInfoArgs', 'RocketmqVipInstanceVpcInfoArgsDict']] vpc_info: VPC information.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] zone_ids: The Zone ID list for node deployment, such as Guangzhou Zone 1, is 100001. For details, please refer to the official website of Tencent Cloud.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: RocketmqVipInstanceArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a tdmq rocketmq_vip_instance

        > **NOTE:** The instance cannot be downgraded, Include parameters `node_count`, `spec`, `storage_size`.
        **NOTE:** If `spec` is `rocket-vip-basic-2`, configuration changes are not supported.


        :param str resource_name: The name of the resource.
        :param RocketmqVipInstanceArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            RocketmqVipInstanceArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        ip_rules: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union[
                        "RocketmqVipInstanceIpRuleArgs",
                        "RocketmqVipInstanceIpRuleArgsDict",
                    ]
                ]
            ]
            | None
        ] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        node_count: pulumi.Input[_builtins.int | None] = None,
        spec: pulumi.Input[_builtins.str | None] = None,
        storage_size: pulumi.Input[_builtins.int | None] = None,
        time_span: pulumi.Input[_builtins.int | None] = None,
        vpc_info: pulumi.Input[
            Union[
                "RocketmqVipInstanceVpcInfoArgs", "RocketmqVipInstanceVpcInfoArgsDict"
            ]
            | None
        ] = None,
        zone_ids: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = RocketmqVipInstanceArgs.__new__(RocketmqVipInstanceArgs)

            __props__.__dict__["ip_rules"] = ip_rules
            __props__.__dict__["name"] = name
            if node_count is None and not opts.urn:
                raise TypeError("Missing required property 'node_count'")
            __props__.__dict__["node_count"] = node_count
            if spec is None and not opts.urn:
                raise TypeError("Missing required property 'spec'")
            __props__.__dict__["spec"] = spec
            if storage_size is None and not opts.urn:
                raise TypeError("Missing required property 'storage_size'")
            __props__.__dict__["storage_size"] = storage_size
            if time_span is None and not opts.urn:
                raise TypeError("Missing required property 'time_span'")
            __props__.__dict__["time_span"] = time_span
            if vpc_info is None and not opts.urn:
                raise TypeError("Missing required property 'vpc_info'")
            __props__.__dict__["vpc_info"] = vpc_info
            if zone_ids is None and not opts.urn:
                raise TypeError("Missing required property 'zone_ids'")
            __props__.__dict__["zone_ids"] = zone_ids
        super().__init__(
            "tencentcloud:Tdmq/rocketmqVipInstance:RocketmqVipInstance",
            resource_name,
            __props__,
            opts,
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        ip_rules: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union[
                        "RocketmqVipInstanceIpRuleArgs",
                        "RocketmqVipInstanceIpRuleArgsDict",
                    ]
                ]
            ]
            | None
        ] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        node_count: pulumi.Input[_builtins.int | None] = None,
        spec: pulumi.Input[_builtins.str | None] = None,
        storage_size: pulumi.Input[_builtins.int | None] = None,
        time_span: pulumi.Input[_builtins.int | None] = None,
        vpc_info: pulumi.Input[
            Union[
                "RocketmqVipInstanceVpcInfoArgs", "RocketmqVipInstanceVpcInfoArgsDict"
            ]
            | None
        ] = None,
        zone_ids: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
    ) -> "RocketmqVipInstance":
        """
        Get an existing RocketmqVipInstance resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[Sequence[pulumi.Input[Union['RocketmqVipInstanceIpRuleArgs', 'RocketmqVipInstanceIpRuleArgsDict']]]] ip_rules: Public IP access control rules.
        :param pulumi.Input[_builtins.str] name: Instance name.
        :param pulumi.Input[_builtins.int] node_count: Number of nodes, minimum 2, maximum 20.
        :param pulumi.Input[_builtins.str] spec: Instance specification: Universal type, rocket-vip-basic-0, Basic type: `rocket-vip-basic-1`, Standard type: `rocket-vip-basic-2`, Advanced Type I: `rocket-vip-basic-3`, Advanced Type II: `rocket-vip-basic-4`.
        :param pulumi.Input[_builtins.int] storage_size: Single node storage space, in GB, minimum 200GB.
        :param pulumi.Input[_builtins.int] time_span: Purchase period, in months.
        :param pulumi.Input[Union['RocketmqVipInstanceVpcInfoArgs', 'RocketmqVipInstanceVpcInfoArgsDict']] vpc_info: VPC information.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] zone_ids: The Zone ID list for node deployment, such as Guangzhou Zone 1, is 100001. For details, please refer to the official website of Tencent Cloud.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _RocketmqVipInstanceState.__new__(_RocketmqVipInstanceState)

        __props__.__dict__["ip_rules"] = ip_rules
        __props__.__dict__["name"] = name
        __props__.__dict__["node_count"] = node_count
        __props__.__dict__["spec"] = spec
        __props__.__dict__["storage_size"] = storage_size
        __props__.__dict__["time_span"] = time_span
        __props__.__dict__["vpc_info"] = vpc_info
        __props__.__dict__["zone_ids"] = zone_ids
        return RocketmqVipInstance(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="ipRules")
    def ip_rules(
        self,
    ) -> pulumi.Output[Sequence["outputs.RocketmqVipInstanceIpRule"] | None]:
        """
        Public IP access control rules.
        """
        return pulumi.get(self, "ip_rules")

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Output[_builtins.str]:
        """
        Instance name.
        """
        return pulumi.get(self, "name")

    @_builtins.property
    @pulumi.getter(name="nodeCount")
    def node_count(self) -> pulumi.Output[_builtins.int]:
        """
        Number of nodes, minimum 2, maximum 20.
        """
        return pulumi.get(self, "node_count")

    @_builtins.property
    @pulumi.getter
    def spec(self) -> pulumi.Output[_builtins.str]:
        """
        Instance specification: Universal type, rocket-vip-basic-0, Basic type: `rocket-vip-basic-1`, Standard type: `rocket-vip-basic-2`, Advanced Type I: `rocket-vip-basic-3`, Advanced Type II: `rocket-vip-basic-4`.
        """
        return pulumi.get(self, "spec")

    @_builtins.property
    @pulumi.getter(name="storageSize")
    def storage_size(self) -> pulumi.Output[_builtins.int]:
        """
        Single node storage space, in GB, minimum 200GB.
        """
        return pulumi.get(self, "storage_size")

    @_builtins.property
    @pulumi.getter(name="timeSpan")
    def time_span(self) -> pulumi.Output[_builtins.int]:
        """
        Purchase period, in months.
        """
        return pulumi.get(self, "time_span")

    @_builtins.property
    @pulumi.getter(name="vpcInfo")
    def vpc_info(self) -> pulumi.Output["outputs.RocketmqVipInstanceVpcInfo"]:
        """
        VPC information.
        """
        return pulumi.get(self, "vpc_info")

    @_builtins.property
    @pulumi.getter(name="zoneIds")
    def zone_ids(self) -> pulumi.Output[Sequence[_builtins.str]]:
        """
        The Zone ID list for node deployment, such as Guangzhou Zone 1, is 100001. For details, please refer to the official website of Tencent Cloud.
        """
        return pulumi.get(self, "zone_ids")
