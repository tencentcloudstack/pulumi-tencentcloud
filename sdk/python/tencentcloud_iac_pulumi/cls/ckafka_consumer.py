# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import Optional, Union, overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ["CkafkaConsumer", "CkafkaConsumerArgs"]


@pulumi.input_type
class CkafkaConsumerArgs:
    def __init__(
        __self__,
        *,
        topic_id: pulumi.Input[_builtins.str],
        ckafka: pulumi.Input[Optional["CkafkaConsumerCkafkaArgs"]] = None,
        compression: pulumi.Input[_builtins.int | None] = None,
        content: pulumi.Input[Optional["CkafkaConsumerContentArgs"]] = None,
        need_content: pulumi.Input[_builtins.bool | None] = None
    ):
        """
        The set of arguments for constructing a CkafkaConsumer resource.

        :param pulumi.Input[_builtins.str] topic_id: topic id.
        :param pulumi.Input['CkafkaConsumerCkafkaArgs'] ckafka: ckafka info.
        :param pulumi.Input[_builtins.int] compression: compression method. 0 for NONE, 2 for SNAPPY, 3 for LZ4.
        :param pulumi.Input['CkafkaConsumerContentArgs'] content: metadata information.
        :param pulumi.Input[_builtins.bool] need_content: whether to deliver the metadata information of the log.
        """
        pulumi.set(__self__, "topic_id", topic_id)
        if ckafka is not None:
            pulumi.set(__self__, "ckafka", ckafka)
        if compression is not None:
            pulumi.set(__self__, "compression", compression)
        if content is not None:
            pulumi.set(__self__, "content", content)
        if need_content is not None:
            pulumi.set(__self__, "need_content", need_content)

    @_builtins.property
    @pulumi.getter(name="topicId")
    def topic_id(self) -> pulumi.Input[_builtins.str]:
        """
        topic id.
        """
        return pulumi.get(self, "topic_id")

    @topic_id.setter
    def topic_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "topic_id", value)

    @_builtins.property
    @pulumi.getter
    def ckafka(self) -> pulumi.Input[Optional["CkafkaConsumerCkafkaArgs"]]:
        """
        ckafka info.
        """
        return pulumi.get(self, "ckafka")

    @ckafka.setter
    def ckafka(self, value: pulumi.Input[Optional["CkafkaConsumerCkafkaArgs"]]):
        pulumi.set(self, "ckafka", value)

    @_builtins.property
    @pulumi.getter
    def compression(self) -> pulumi.Input[_builtins.int | None]:
        """
        compression method. 0 for NONE, 2 for SNAPPY, 3 for LZ4.
        """
        return pulumi.get(self, "compression")

    @compression.setter
    def compression(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "compression", value)

    @_builtins.property
    @pulumi.getter
    def content(self) -> pulumi.Input[Optional["CkafkaConsumerContentArgs"]]:
        """
        metadata information.
        """
        return pulumi.get(self, "content")

    @content.setter
    def content(self, value: pulumi.Input[Optional["CkafkaConsumerContentArgs"]]):
        pulumi.set(self, "content", value)

    @_builtins.property
    @pulumi.getter(name="needContent")
    def need_content(self) -> pulumi.Input[_builtins.bool | None]:
        """
        whether to deliver the metadata information of the log.
        """
        return pulumi.get(self, "need_content")

    @need_content.setter
    def need_content(self, value: pulumi.Input[_builtins.bool | None]):
        pulumi.set(self, "need_content", value)


@pulumi.input_type
class _CkafkaConsumerState:
    def __init__(
        __self__,
        *,
        ckafka: pulumi.Input[Optional["CkafkaConsumerCkafkaArgs"]] = None,
        compression: pulumi.Input[_builtins.int | None] = None,
        content: pulumi.Input[Optional["CkafkaConsumerContentArgs"]] = None,
        need_content: pulumi.Input[_builtins.bool | None] = None,
        topic_id: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering CkafkaConsumer resources.

        :param pulumi.Input['CkafkaConsumerCkafkaArgs'] ckafka: ckafka info.
        :param pulumi.Input[_builtins.int] compression: compression method. 0 for NONE, 2 for SNAPPY, 3 for LZ4.
        :param pulumi.Input['CkafkaConsumerContentArgs'] content: metadata information.
        :param pulumi.Input[_builtins.bool] need_content: whether to deliver the metadata information of the log.
        :param pulumi.Input[_builtins.str] topic_id: topic id.
        """
        if ckafka is not None:
            pulumi.set(__self__, "ckafka", ckafka)
        if compression is not None:
            pulumi.set(__self__, "compression", compression)
        if content is not None:
            pulumi.set(__self__, "content", content)
        if need_content is not None:
            pulumi.set(__self__, "need_content", need_content)
        if topic_id is not None:
            pulumi.set(__self__, "topic_id", topic_id)

    @_builtins.property
    @pulumi.getter
    def ckafka(self) -> pulumi.Input[Optional["CkafkaConsumerCkafkaArgs"]]:
        """
        ckafka info.
        """
        return pulumi.get(self, "ckafka")

    @ckafka.setter
    def ckafka(self, value: pulumi.Input[Optional["CkafkaConsumerCkafkaArgs"]]):
        pulumi.set(self, "ckafka", value)

    @_builtins.property
    @pulumi.getter
    def compression(self) -> pulumi.Input[_builtins.int | None]:
        """
        compression method. 0 for NONE, 2 for SNAPPY, 3 for LZ4.
        """
        return pulumi.get(self, "compression")

    @compression.setter
    def compression(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "compression", value)

    @_builtins.property
    @pulumi.getter
    def content(self) -> pulumi.Input[Optional["CkafkaConsumerContentArgs"]]:
        """
        metadata information.
        """
        return pulumi.get(self, "content")

    @content.setter
    def content(self, value: pulumi.Input[Optional["CkafkaConsumerContentArgs"]]):
        pulumi.set(self, "content", value)

    @_builtins.property
    @pulumi.getter(name="needContent")
    def need_content(self) -> pulumi.Input[_builtins.bool | None]:
        """
        whether to deliver the metadata information of the log.
        """
        return pulumi.get(self, "need_content")

    @need_content.setter
    def need_content(self, value: pulumi.Input[_builtins.bool | None]):
        pulumi.set(self, "need_content", value)

    @_builtins.property
    @pulumi.getter(name="topicId")
    def topic_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        topic id.
        """
        return pulumi.get(self, "topic_id")

    @topic_id.setter
    def topic_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "topic_id", value)


@pulumi.type_token("tencentcloud:Cls/ckafkaConsumer:CkafkaConsumer")
class CkafkaConsumer(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        ckafka: pulumi.Input[
            Union["CkafkaConsumerCkafkaArgs", "CkafkaConsumerCkafkaArgsDict"] | None
        ] = None,
        compression: pulumi.Input[_builtins.int | None] = None,
        content: pulumi.Input[
            Union["CkafkaConsumerContentArgs", "CkafkaConsumerContentArgsDict"] | None
        ] = None,
        need_content: pulumi.Input[_builtins.bool | None] = None,
        topic_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a CLS ckafka consumer

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        example = tencentcloud.cls.CkafkaConsumer("example",
            compression=0,
            need_content=True,
            topic_id="7e34a3a7-635e-4da8-9005-88106c1fde69",
            ckafka={
                "instance_id": "ckafka-qzoeaqx8",
                "instance_name": "ckafka-instance",
                "topic_id": "topic-c6tm4kpm",
                "topic_name": "name",
                "vip": "172.16.112.23",
                "vport": "9092",
            },
            content={
                "enable_tag": True,
                "meta_fields": [
                    "__FILENAME__",
                    "__HOSTNAME__",
                    "__PKGID__",
                    "__SOURCE__",
                    "__TIMESTAMP__",
                ],
                "tag_json_not_tiled": True,
                "timestamp_accuracy": 2,
                "json_type": 1,
            })
        ```

        ## Import

        cls ckafka_consumer can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Cls/ckafkaConsumer:CkafkaConsumer example 7e34a3a7-635e-4da8-9005-88106c1fde69
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[Union['CkafkaConsumerCkafkaArgs', 'CkafkaConsumerCkafkaArgsDict']] ckafka: ckafka info.
        :param pulumi.Input[_builtins.int] compression: compression method. 0 for NONE, 2 for SNAPPY, 3 for LZ4.
        :param pulumi.Input[Union['CkafkaConsumerContentArgs', 'CkafkaConsumerContentArgsDict']] content: metadata information.
        :param pulumi.Input[_builtins.bool] need_content: whether to deliver the metadata information of the log.
        :param pulumi.Input[_builtins.str] topic_id: topic id.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: CkafkaConsumerArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a CLS ckafka consumer

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        example = tencentcloud.cls.CkafkaConsumer("example",
            compression=0,
            need_content=True,
            topic_id="7e34a3a7-635e-4da8-9005-88106c1fde69",
            ckafka={
                "instance_id": "ckafka-qzoeaqx8",
                "instance_name": "ckafka-instance",
                "topic_id": "topic-c6tm4kpm",
                "topic_name": "name",
                "vip": "172.16.112.23",
                "vport": "9092",
            },
            content={
                "enable_tag": True,
                "meta_fields": [
                    "__FILENAME__",
                    "__HOSTNAME__",
                    "__PKGID__",
                    "__SOURCE__",
                    "__TIMESTAMP__",
                ],
                "tag_json_not_tiled": True,
                "timestamp_accuracy": 2,
                "json_type": 1,
            })
        ```

        ## Import

        cls ckafka_consumer can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Cls/ckafkaConsumer:CkafkaConsumer example 7e34a3a7-635e-4da8-9005-88106c1fde69
        ```


        :param str resource_name: The name of the resource.
        :param CkafkaConsumerArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            CkafkaConsumerArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        ckafka: pulumi.Input[
            Union["CkafkaConsumerCkafkaArgs", "CkafkaConsumerCkafkaArgsDict"] | None
        ] = None,
        compression: pulumi.Input[_builtins.int | None] = None,
        content: pulumi.Input[
            Union["CkafkaConsumerContentArgs", "CkafkaConsumerContentArgsDict"] | None
        ] = None,
        need_content: pulumi.Input[_builtins.bool | None] = None,
        topic_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = CkafkaConsumerArgs.__new__(CkafkaConsumerArgs)

            __props__.__dict__["ckafka"] = ckafka
            __props__.__dict__["compression"] = compression
            __props__.__dict__["content"] = content
            __props__.__dict__["need_content"] = need_content
            if topic_id is None and not opts.urn:
                raise TypeError("Missing required property 'topic_id'")
            __props__.__dict__["topic_id"] = topic_id
        super().__init__(
            "tencentcloud:Cls/ckafkaConsumer:CkafkaConsumer",
            resource_name,
            __props__,
            opts,
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        ckafka: pulumi.Input[
            Union["CkafkaConsumerCkafkaArgs", "CkafkaConsumerCkafkaArgsDict"] | None
        ] = None,
        compression: pulumi.Input[_builtins.int | None] = None,
        content: pulumi.Input[
            Union["CkafkaConsumerContentArgs", "CkafkaConsumerContentArgsDict"] | None
        ] = None,
        need_content: pulumi.Input[_builtins.bool | None] = None,
        topic_id: pulumi.Input[_builtins.str | None] = None,
    ) -> "CkafkaConsumer":
        """
        Get an existing CkafkaConsumer resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[Union['CkafkaConsumerCkafkaArgs', 'CkafkaConsumerCkafkaArgsDict']] ckafka: ckafka info.
        :param pulumi.Input[_builtins.int] compression: compression method. 0 for NONE, 2 for SNAPPY, 3 for LZ4.
        :param pulumi.Input[Union['CkafkaConsumerContentArgs', 'CkafkaConsumerContentArgsDict']] content: metadata information.
        :param pulumi.Input[_builtins.bool] need_content: whether to deliver the metadata information of the log.
        :param pulumi.Input[_builtins.str] topic_id: topic id.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _CkafkaConsumerState.__new__(_CkafkaConsumerState)

        __props__.__dict__["ckafka"] = ckafka
        __props__.__dict__["compression"] = compression
        __props__.__dict__["content"] = content
        __props__.__dict__["need_content"] = need_content
        __props__.__dict__["topic_id"] = topic_id
        return CkafkaConsumer(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter
    def ckafka(self) -> pulumi.Output[Optional["outputs.CkafkaConsumerCkafka"]]:
        """
        ckafka info.
        """
        return pulumi.get(self, "ckafka")

    @_builtins.property
    @pulumi.getter
    def compression(self) -> pulumi.Output[_builtins.int | None]:
        """
        compression method. 0 for NONE, 2 for SNAPPY, 3 for LZ4.
        """
        return pulumi.get(self, "compression")

    @_builtins.property
    @pulumi.getter
    def content(self) -> pulumi.Output[Optional["outputs.CkafkaConsumerContent"]]:
        """
        metadata information.
        """
        return pulumi.get(self, "content")

    @_builtins.property
    @pulumi.getter(name="needContent")
    def need_content(self) -> pulumi.Output[_builtins.bool | None]:
        """
        whether to deliver the metadata information of the log.
        """
        return pulumi.get(self, "need_content")

    @_builtins.property
    @pulumi.getter(name="topicId")
    def topic_id(self) -> pulumi.Output[_builtins.str]:
        """
        topic id.
        """
        return pulumi.get(self, "topic_id")
