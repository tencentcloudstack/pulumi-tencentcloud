# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence
from typing import Union

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = [
    "AwaitableGetInstancesResult",
    "GetInstancesResult",
    "get_instances",
    "get_instances_output",
]


@pulumi.output_type
class GetInstancesResult:
    """
    A collection of values returned by getInstances.
    """

    def __init__(
        __self__,
        filters=None,
        id=None,
        instance_ids=None,
        instance_lists=None,
        result_output_file=None,
    ):
        if filters and not isinstance(filters, list):
            raise TypeError("Expected argument 'filters' to be a list")
        pulumi.set(__self__, "filters", filters)
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)
        if instance_ids and not isinstance(instance_ids, list):
            raise TypeError("Expected argument 'instance_ids' to be a list")
        pulumi.set(__self__, "instance_ids", instance_ids)
        if instance_lists and not isinstance(instance_lists, list):
            raise TypeError("Expected argument 'instance_lists' to be a list")
        pulumi.set(__self__, "instance_lists", instance_lists)
        if result_output_file and not isinstance(result_output_file, str):
            raise TypeError("Expected argument 'result_output_file' to be a str")
        pulumi.set(__self__, "result_output_file", result_output_file)

    @_builtins.property
    @pulumi.getter
    def filters(self) -> Sequence["outputs.GetInstancesFilterResult"] | None:
        return pulumi.get(self, "filters")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The provider-assigned unique ID for this managed resource.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="instanceIds")
    def instance_ids(self) -> Sequence[_builtins.str] | None:
        return pulumi.get(self, "instance_ids")

    @_builtins.property
    @pulumi.getter(name="instanceLists")
    def instance_lists(self) -> Sequence["outputs.GetInstancesInstanceListResult"]:
        """
        List of instance details.
        """
        return pulumi.get(self, "instance_lists")

    @_builtins.property
    @pulumi.getter(name="resultOutputFile")
    def result_output_file(self) -> _builtins.str | None:
        return pulumi.get(self, "result_output_file")


class AwaitableGetInstancesResult(GetInstancesResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetInstancesResult(
            filters=self.filters,
            id=self.id,
            instance_ids=self.instance_ids,
            instance_lists=self.instance_lists,
            result_output_file=self.result_output_file,
        )


def get_instances(
    filters: (
        Sequence[Union["GetInstancesFilterArgs", "GetInstancesFilterArgsDict"]] | None
    ) = None,
    instance_ids: Sequence[_builtins.str] | None = None,
    result_output_file: _builtins.str | None = None,
    opts: pulumi.InvokeOptions | None = None,
) -> AwaitableGetInstancesResult:
    """
    Use this data source to query detailed information of as instances

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud
    import tencentcloud_iac_pulumi as tencentcloud

    scaling_group = tencentcloud.as_.ScalingGroup("scaling_group",
        scaling_group_name="tf-as-group-ds-ins-basic",
        configuration_id="your_launch_configuration_id",
        max_size=1,
        min_size=1,
        vpc_id="your_vpc_id",
        subnet_ids=["your_subnet_id"],
        tags={
            "test": "test",
        })
    instances = tencentcloud.as_.get_instances_output(filters=[{
        "name": "auto-scaling-group-id",
        "values": [scaling_group.id],
    }])
    ```


    :param Sequence[Union['GetInstancesFilterArgs', 'GetInstancesFilterArgsDict']] filters: Filter conditions. If there are multiple Filters, the relationship between Filters is a logical AND (AND) relationship. If there are multiple Values in the same Filter, the relationship between Values under the same Filter is a logical OR (OR) relationship.
    :param Sequence[_builtins.str] instance_ids: Instance ID of the cloud server (CVM) to be queried. The limit is 100 per request.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["filters"] = filters
    __args__["instanceIds"] = instance_ids
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke(
        "tencentcloud:As/getInstances:getInstances",
        __args__,
        opts=opts,
        typ=GetInstancesResult,
    ).value

    return AwaitableGetInstancesResult(
        filters=pulumi.get(__ret__, "filters"),
        id=pulumi.get(__ret__, "id"),
        instance_ids=pulumi.get(__ret__, "instance_ids"),
        instance_lists=pulumi.get(__ret__, "instance_lists"),
        result_output_file=pulumi.get(__ret__, "result_output_file"),
    )


def get_instances_output(
    filters: pulumi.Input[
        Sequence[Union["GetInstancesFilterArgs", "GetInstancesFilterArgsDict"]] | None
    ] = None,
    instance_ids: pulumi.Input[Sequence[_builtins.str] | None] = None,
    result_output_file: pulumi.Input[_builtins.str | None] = None,
    opts: pulumi.InvokeOptions | pulumi.InvokeOutputOptions | None = None,
) -> pulumi.Output[GetInstancesResult]:
    """
    Use this data source to query detailed information of as instances

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud
    import tencentcloud_iac_pulumi as tencentcloud

    scaling_group = tencentcloud.as_.ScalingGroup("scaling_group",
        scaling_group_name="tf-as-group-ds-ins-basic",
        configuration_id="your_launch_configuration_id",
        max_size=1,
        min_size=1,
        vpc_id="your_vpc_id",
        subnet_ids=["your_subnet_id"],
        tags={
            "test": "test",
        })
    instances = tencentcloud.as_.get_instances_output(filters=[{
        "name": "auto-scaling-group-id",
        "values": [scaling_group.id],
    }])
    ```


    :param Sequence[Union['GetInstancesFilterArgs', 'GetInstancesFilterArgsDict']] filters: Filter conditions. If there are multiple Filters, the relationship between Filters is a logical AND (AND) relationship. If there are multiple Values in the same Filter, the relationship between Values under the same Filter is a logical OR (OR) relationship.
    :param Sequence[_builtins.str] instance_ids: Instance ID of the cloud server (CVM) to be queried. The limit is 100 per request.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["filters"] = filters
    __args__["instanceIds"] = instance_ids
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output(
        "tencentcloud:As/getInstances:getInstances",
        __args__,
        opts=opts,
        typ=GetInstancesResult,
    )
    return __ret__.apply(
        lambda __response__: GetInstancesResult(
            filters=pulumi.get(__response__, "filters"),
            id=pulumi.get(__response__, "id"),
            instance_ids=pulumi.get(__response__, "instance_ids"),
            instance_lists=pulumi.get(__response__, "instance_lists"),
            result_output_file=pulumi.get(__response__, "result_output_file"),
        )
    )
