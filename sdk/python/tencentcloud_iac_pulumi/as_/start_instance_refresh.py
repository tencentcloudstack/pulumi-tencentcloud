# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import Optional, Union, overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ["StartInstanceRefresh", "StartInstanceRefreshArgs"]


@pulumi.input_type
class StartInstanceRefreshArgs:
    def __init__(
        __self__,
        *,
        auto_scaling_group_id: pulumi.Input[_builtins.str],
        refresh_settings: pulumi.Input["StartInstanceRefreshRefreshSettingsArgs"],
        refresh_mode: pulumi.Input[_builtins.str | None] = None
    ):
        """
        The set of arguments for constructing a StartInstanceRefresh resource.

        :param pulumi.Input[_builtins.str] auto_scaling_group_id: Scaling group ID.
        :param pulumi.Input['StartInstanceRefreshRefreshSettingsArgs'] refresh_settings: Refresh settings.
        :param pulumi.Input[_builtins.str] refresh_mode: Refresh mode. Value range: ROLLING_UPDATE_RESET: Reinstall the system for rolling update; ROLLING_UPDATE_REPLACE: Create a new instance for rolling update. This mode does not support the rollback interface yet.
        """
        pulumi.set(__self__, "auto_scaling_group_id", auto_scaling_group_id)
        pulumi.set(__self__, "refresh_settings", refresh_settings)
        if refresh_mode is not None:
            pulumi.set(__self__, "refresh_mode", refresh_mode)

    @_builtins.property
    @pulumi.getter(name="autoScalingGroupId")
    def auto_scaling_group_id(self) -> pulumi.Input[_builtins.str]:
        """
        Scaling group ID.
        """
        return pulumi.get(self, "auto_scaling_group_id")

    @auto_scaling_group_id.setter
    def auto_scaling_group_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "auto_scaling_group_id", value)

    @_builtins.property
    @pulumi.getter(name="refreshSettings")
    def refresh_settings(
        self,
    ) -> pulumi.Input["StartInstanceRefreshRefreshSettingsArgs"]:
        """
        Refresh settings.
        """
        return pulumi.get(self, "refresh_settings")

    @refresh_settings.setter
    def refresh_settings(
        self, value: pulumi.Input["StartInstanceRefreshRefreshSettingsArgs"]
    ):
        pulumi.set(self, "refresh_settings", value)

    @_builtins.property
    @pulumi.getter(name="refreshMode")
    def refresh_mode(self) -> pulumi.Input[_builtins.str | None]:
        """
        Refresh mode. Value range: ROLLING_UPDATE_RESET: Reinstall the system for rolling update; ROLLING_UPDATE_REPLACE: Create a new instance for rolling update. This mode does not support the rollback interface yet.
        """
        return pulumi.get(self, "refresh_mode")

    @refresh_mode.setter
    def refresh_mode(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "refresh_mode", value)


@pulumi.input_type
class _StartInstanceRefreshState:
    def __init__(
        __self__,
        *,
        auto_scaling_group_id: pulumi.Input[_builtins.str | None] = None,
        refresh_mode: pulumi.Input[_builtins.str | None] = None,
        refresh_settings: pulumi.Input[
            Optional["StartInstanceRefreshRefreshSettingsArgs"]
        ] = None
    ):
        """
        Input properties used for looking up and filtering StartInstanceRefresh resources.

        :param pulumi.Input[_builtins.str] auto_scaling_group_id: Scaling group ID.
        :param pulumi.Input[_builtins.str] refresh_mode: Refresh mode. Value range: ROLLING_UPDATE_RESET: Reinstall the system for rolling update; ROLLING_UPDATE_REPLACE: Create a new instance for rolling update. This mode does not support the rollback interface yet.
        :param pulumi.Input['StartInstanceRefreshRefreshSettingsArgs'] refresh_settings: Refresh settings.
        """
        if auto_scaling_group_id is not None:
            pulumi.set(__self__, "auto_scaling_group_id", auto_scaling_group_id)
        if refresh_mode is not None:
            pulumi.set(__self__, "refresh_mode", refresh_mode)
        if refresh_settings is not None:
            pulumi.set(__self__, "refresh_settings", refresh_settings)

    @_builtins.property
    @pulumi.getter(name="autoScalingGroupId")
    def auto_scaling_group_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Scaling group ID.
        """
        return pulumi.get(self, "auto_scaling_group_id")

    @auto_scaling_group_id.setter
    def auto_scaling_group_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "auto_scaling_group_id", value)

    @_builtins.property
    @pulumi.getter(name="refreshMode")
    def refresh_mode(self) -> pulumi.Input[_builtins.str | None]:
        """
        Refresh mode. Value range: ROLLING_UPDATE_RESET: Reinstall the system for rolling update; ROLLING_UPDATE_REPLACE: Create a new instance for rolling update. This mode does not support the rollback interface yet.
        """
        return pulumi.get(self, "refresh_mode")

    @refresh_mode.setter
    def refresh_mode(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "refresh_mode", value)

    @_builtins.property
    @pulumi.getter(name="refreshSettings")
    def refresh_settings(
        self,
    ) -> pulumi.Input[Optional["StartInstanceRefreshRefreshSettingsArgs"]]:
        """
        Refresh settings.
        """
        return pulumi.get(self, "refresh_settings")

    @refresh_settings.setter
    def refresh_settings(
        self, value: pulumi.Input[Optional["StartInstanceRefreshRefreshSettingsArgs"]]
    ):
        pulumi.set(self, "refresh_settings", value)


@pulumi.type_token("tencentcloud:As/startInstanceRefresh:StartInstanceRefresh")
class StartInstanceRefresh(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        auto_scaling_group_id: pulumi.Input[_builtins.str | None] = None,
        refresh_mode: pulumi.Input[_builtins.str | None] = None,
        refresh_settings: pulumi.Input[
            Union[
                "StartInstanceRefreshRefreshSettingsArgs",
                "StartInstanceRefreshRefreshSettingsArgsDict",
            ]
            | None
        ] = None,
        __props__=None,
    ):
        """
        Provides a resource to create as instance refresh

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        example = tencentcloud.as_.StartInstanceRefresh("example",
            auto_scaling_group_id="asg-8n7fdm28",
            refresh_mode="ROLLING_UPDATE_RESET",
            refresh_settings={
                "check_instance_target_health": False,
                "rolling_update_settings": {
                    "batch_number": 1,
                    "batch_pause": "AUTOMATIC",
                    "max_surge": 1,
                    "fail_process": "AUTO_PAUSE",
                },
                "check_instance_target_health_timeout": 1800,
            })
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] auto_scaling_group_id: Scaling group ID.
        :param pulumi.Input[_builtins.str] refresh_mode: Refresh mode. Value range: ROLLING_UPDATE_RESET: Reinstall the system for rolling update; ROLLING_UPDATE_REPLACE: Create a new instance for rolling update. This mode does not support the rollback interface yet.
        :param pulumi.Input[Union['StartInstanceRefreshRefreshSettingsArgs', 'StartInstanceRefreshRefreshSettingsArgsDict']] refresh_settings: Refresh settings.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: StartInstanceRefreshArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create as instance refresh

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        example = tencentcloud.as_.StartInstanceRefresh("example",
            auto_scaling_group_id="asg-8n7fdm28",
            refresh_mode="ROLLING_UPDATE_RESET",
            refresh_settings={
                "check_instance_target_health": False,
                "rolling_update_settings": {
                    "batch_number": 1,
                    "batch_pause": "AUTOMATIC",
                    "max_surge": 1,
                    "fail_process": "AUTO_PAUSE",
                },
                "check_instance_target_health_timeout": 1800,
            })
        ```


        :param str resource_name: The name of the resource.
        :param StartInstanceRefreshArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            StartInstanceRefreshArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        auto_scaling_group_id: pulumi.Input[_builtins.str | None] = None,
        refresh_mode: pulumi.Input[_builtins.str | None] = None,
        refresh_settings: pulumi.Input[
            Union[
                "StartInstanceRefreshRefreshSettingsArgs",
                "StartInstanceRefreshRefreshSettingsArgsDict",
            ]
            | None
        ] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = StartInstanceRefreshArgs.__new__(StartInstanceRefreshArgs)

            if auto_scaling_group_id is None and not opts.urn:
                raise TypeError("Missing required property 'auto_scaling_group_id'")
            __props__.__dict__["auto_scaling_group_id"] = auto_scaling_group_id
            __props__.__dict__["refresh_mode"] = refresh_mode
            if refresh_settings is None and not opts.urn:
                raise TypeError("Missing required property 'refresh_settings'")
            __props__.__dict__["refresh_settings"] = refresh_settings
        super().__init__(
            "tencentcloud:As/startInstanceRefresh:StartInstanceRefresh",
            resource_name,
            __props__,
            opts,
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        auto_scaling_group_id: pulumi.Input[_builtins.str | None] = None,
        refresh_mode: pulumi.Input[_builtins.str | None] = None,
        refresh_settings: pulumi.Input[
            Union[
                "StartInstanceRefreshRefreshSettingsArgs",
                "StartInstanceRefreshRefreshSettingsArgsDict",
            ]
            | None
        ] = None,
    ) -> "StartInstanceRefresh":
        """
        Get an existing StartInstanceRefresh resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] auto_scaling_group_id: Scaling group ID.
        :param pulumi.Input[_builtins.str] refresh_mode: Refresh mode. Value range: ROLLING_UPDATE_RESET: Reinstall the system for rolling update; ROLLING_UPDATE_REPLACE: Create a new instance for rolling update. This mode does not support the rollback interface yet.
        :param pulumi.Input[Union['StartInstanceRefreshRefreshSettingsArgs', 'StartInstanceRefreshRefreshSettingsArgsDict']] refresh_settings: Refresh settings.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _StartInstanceRefreshState.__new__(_StartInstanceRefreshState)

        __props__.__dict__["auto_scaling_group_id"] = auto_scaling_group_id
        __props__.__dict__["refresh_mode"] = refresh_mode
        __props__.__dict__["refresh_settings"] = refresh_settings
        return StartInstanceRefresh(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="autoScalingGroupId")
    def auto_scaling_group_id(self) -> pulumi.Output[_builtins.str]:
        """
        Scaling group ID.
        """
        return pulumi.get(self, "auto_scaling_group_id")

    @_builtins.property
    @pulumi.getter(name="refreshMode")
    def refresh_mode(self) -> pulumi.Output[_builtins.str | None]:
        """
        Refresh mode. Value range: ROLLING_UPDATE_RESET: Reinstall the system for rolling update; ROLLING_UPDATE_REPLACE: Create a new instance for rolling update. This mode does not support the rollback interface yet.
        """
        return pulumi.get(self, "refresh_mode")

    @_builtins.property
    @pulumi.getter(name="refreshSettings")
    def refresh_settings(
        self,
    ) -> pulumi.Output["outputs.StartInstanceRefreshRefreshSettings"]:
        """
        Refresh settings.
        """
        return pulumi.get(self, "refresh_settings")
