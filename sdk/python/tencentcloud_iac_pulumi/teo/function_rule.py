# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence
from typing import Union, overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ["FunctionRule", "FunctionRuleArgs"]


@pulumi.input_type
class FunctionRuleArgs:
    def __init__(
        __self__,
        *,
        function_id: pulumi.Input[_builtins.str],
        function_rule_conditions: pulumi.Input[
            Sequence[pulumi.Input["FunctionRuleFunctionRuleConditionArgs"]]
        ],
        zone_id: pulumi.Input[_builtins.str],
        remark: pulumi.Input[_builtins.str | None] = None,
        trigger_type: pulumi.Input[_builtins.str | None] = None
    ):
        """
        The set of arguments for constructing a FunctionRule resource.

        :param pulumi.Input[_builtins.str] function_id: ID of the Function.
        :param pulumi.Input[Sequence[pulumi.Input['FunctionRuleFunctionRuleConditionArgs']]] function_rule_conditions: The list of rule conditions, where the conditions are connected by an "OR" relationship.
        :param pulumi.Input[_builtins.str] zone_id: ID of the site.
        :param pulumi.Input[_builtins.str] remark: Rule description, maximum support of 60 characters.
        :param pulumi.Input[_builtins.str] trigger_type: Function selection configuration type. Valid values: `direct`, `weight`, `region`. Defaults to `direct` when not specified.
        """
        pulumi.set(__self__, "function_id", function_id)
        pulumi.set(__self__, "function_rule_conditions", function_rule_conditions)
        pulumi.set(__self__, "zone_id", zone_id)
        if remark is not None:
            pulumi.set(__self__, "remark", remark)
        if trigger_type is not None:
            pulumi.set(__self__, "trigger_type", trigger_type)

    @_builtins.property
    @pulumi.getter(name="functionId")
    def function_id(self) -> pulumi.Input[_builtins.str]:
        """
        ID of the Function.
        """
        return pulumi.get(self, "function_id")

    @function_id.setter
    def function_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "function_id", value)

    @_builtins.property
    @pulumi.getter(name="functionRuleConditions")
    def function_rule_conditions(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input["FunctionRuleFunctionRuleConditionArgs"]]]:
        """
        The list of rule conditions, where the conditions are connected by an "OR" relationship.
        """
        return pulumi.get(self, "function_rule_conditions")

    @function_rule_conditions.setter
    def function_rule_conditions(
        self,
        value: pulumi.Input[
            Sequence[pulumi.Input["FunctionRuleFunctionRuleConditionArgs"]]
        ],
    ):
        pulumi.set(self, "function_rule_conditions", value)

    @_builtins.property
    @pulumi.getter(name="zoneId")
    def zone_id(self) -> pulumi.Input[_builtins.str]:
        """
        ID of the site.
        """
        return pulumi.get(self, "zone_id")

    @zone_id.setter
    def zone_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "zone_id", value)

    @_builtins.property
    @pulumi.getter
    def remark(self) -> pulumi.Input[_builtins.str | None]:
        """
        Rule description, maximum support of 60 characters.
        """
        return pulumi.get(self, "remark")

    @remark.setter
    def remark(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "remark", value)

    @_builtins.property
    @pulumi.getter(name="triggerType")
    def trigger_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Function selection configuration type. Valid values: `direct`, `weight`, `region`. Defaults to `direct` when not specified.
        """
        return pulumi.get(self, "trigger_type")

    @trigger_type.setter
    def trigger_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "trigger_type", value)


@pulumi.input_type
class _FunctionRuleState:
    def __init__(
        __self__,
        *,
        function_id: pulumi.Input[_builtins.str | None] = None,
        function_name: pulumi.Input[_builtins.str | None] = None,
        function_rule_conditions: pulumi.Input[
            Sequence[pulumi.Input["FunctionRuleFunctionRuleConditionArgs"]] | None
        ] = None,
        priority: pulumi.Input[_builtins.int | None] = None,
        remark: pulumi.Input[_builtins.str | None] = None,
        rule_id: pulumi.Input[_builtins.str | None] = None,
        trigger_type: pulumi.Input[_builtins.str | None] = None,
        zone_id: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering FunctionRule resources.

        :param pulumi.Input[_builtins.str] function_id: ID of the Function.
        :param pulumi.Input[_builtins.str] function_name: The name of the function.
        :param pulumi.Input[Sequence[pulumi.Input['FunctionRuleFunctionRuleConditionArgs']]] function_rule_conditions: The list of rule conditions, where the conditions are connected by an "OR" relationship.
        :param pulumi.Input[_builtins.int] priority: The priority of the function trigger rule. A higher numerical value indicates a higher priority.
        :param pulumi.Input[_builtins.str] remark: Rule description, maximum support of 60 characters.
        :param pulumi.Input[_builtins.str] rule_id: ID of the Function Rule.
        :param pulumi.Input[_builtins.str] trigger_type: Function selection configuration type. Valid values: `direct`, `weight`, `region`. Defaults to `direct` when not specified.
        :param pulumi.Input[_builtins.str] zone_id: ID of the site.
        """
        if function_id is not None:
            pulumi.set(__self__, "function_id", function_id)
        if function_name is not None:
            pulumi.set(__self__, "function_name", function_name)
        if function_rule_conditions is not None:
            pulumi.set(__self__, "function_rule_conditions", function_rule_conditions)
        if priority is not None:
            pulumi.set(__self__, "priority", priority)
        if remark is not None:
            pulumi.set(__self__, "remark", remark)
        if rule_id is not None:
            pulumi.set(__self__, "rule_id", rule_id)
        if trigger_type is not None:
            pulumi.set(__self__, "trigger_type", trigger_type)
        if zone_id is not None:
            pulumi.set(__self__, "zone_id", zone_id)

    @_builtins.property
    @pulumi.getter(name="functionId")
    def function_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        ID of the Function.
        """
        return pulumi.get(self, "function_id")

    @function_id.setter
    def function_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "function_id", value)

    @_builtins.property
    @pulumi.getter(name="functionName")
    def function_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        The name of the function.
        """
        return pulumi.get(self, "function_name")

    @function_name.setter
    def function_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "function_name", value)

    @_builtins.property
    @pulumi.getter(name="functionRuleConditions")
    def function_rule_conditions(
        self,
    ) -> pulumi.Input[
        Sequence[pulumi.Input["FunctionRuleFunctionRuleConditionArgs"]] | None
    ]:
        """
        The list of rule conditions, where the conditions are connected by an "OR" relationship.
        """
        return pulumi.get(self, "function_rule_conditions")

    @function_rule_conditions.setter
    def function_rule_conditions(
        self,
        value: pulumi.Input[
            Sequence[pulumi.Input["FunctionRuleFunctionRuleConditionArgs"]] | None
        ],
    ):
        pulumi.set(self, "function_rule_conditions", value)

    @_builtins.property
    @pulumi.getter
    def priority(self) -> pulumi.Input[_builtins.int | None]:
        """
        The priority of the function trigger rule. A higher numerical value indicates a higher priority.
        """
        return pulumi.get(self, "priority")

    @priority.setter
    def priority(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "priority", value)

    @_builtins.property
    @pulumi.getter
    def remark(self) -> pulumi.Input[_builtins.str | None]:
        """
        Rule description, maximum support of 60 characters.
        """
        return pulumi.get(self, "remark")

    @remark.setter
    def remark(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "remark", value)

    @_builtins.property
    @pulumi.getter(name="ruleId")
    def rule_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        ID of the Function Rule.
        """
        return pulumi.get(self, "rule_id")

    @rule_id.setter
    def rule_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "rule_id", value)

    @_builtins.property
    @pulumi.getter(name="triggerType")
    def trigger_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Function selection configuration type. Valid values: `direct`, `weight`, `region`. Defaults to `direct` when not specified.
        """
        return pulumi.get(self, "trigger_type")

    @trigger_type.setter
    def trigger_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "trigger_type", value)

    @_builtins.property
    @pulumi.getter(name="zoneId")
    def zone_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        ID of the site.
        """
        return pulumi.get(self, "zone_id")

    @zone_id.setter
    def zone_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "zone_id", value)


@pulumi.type_token("tencentcloud:Teo/functionRule:FunctionRule")
class FunctionRule(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        function_id: pulumi.Input[_builtins.str | None] = None,
        function_rule_conditions: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union[
                        "FunctionRuleFunctionRuleConditionArgs",
                        "FunctionRuleFunctionRuleConditionArgsDict",
                    ]
                ]
            ]
            | None
        ] = None,
        remark: pulumi.Input[_builtins.str | None] = None,
        trigger_type: pulumi.Input[_builtins.str | None] = None,
        zone_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a TEO function rule

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        example = tencentcloud.teo.FunctionRule("example",
            function_id="ef-m01xn26e",
            remark="remark.",
            trigger_type="direct",
            zone_id="zone-3fkff38fyw8s",
            function_rule_conditions=[{
                "rule_conditions": [
                    {
                        "ignore_case": False,
                        "operator": "equal",
                        "target": "host",
                        "values": ["test.makn.cn"],
                    },
                    {
                        "ignore_case": False,
                        "operator": "equal",
                        "target": "url",
                        "values": ["/path"],
                    },
                ],
            }])
        ```

        ## Import

        teo teo_function_rule can be imported using the zoneId#functionId#ruleId, e.g.

        ```sh
        $ pulumi import tencentcloud:Teo/functionRule:FunctionRule example zone-3fkff38fyw8s#ef-m01xn26e#rule-yuvufj6h
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] function_id: ID of the Function.
        :param pulumi.Input[Sequence[pulumi.Input[Union['FunctionRuleFunctionRuleConditionArgs', 'FunctionRuleFunctionRuleConditionArgsDict']]]] function_rule_conditions: The list of rule conditions, where the conditions are connected by an "OR" relationship.
        :param pulumi.Input[_builtins.str] remark: Rule description, maximum support of 60 characters.
        :param pulumi.Input[_builtins.str] trigger_type: Function selection configuration type. Valid values: `direct`, `weight`, `region`. Defaults to `direct` when not specified.
        :param pulumi.Input[_builtins.str] zone_id: ID of the site.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: FunctionRuleArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a TEO function rule

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        example = tencentcloud.teo.FunctionRule("example",
            function_id="ef-m01xn26e",
            remark="remark.",
            trigger_type="direct",
            zone_id="zone-3fkff38fyw8s",
            function_rule_conditions=[{
                "rule_conditions": [
                    {
                        "ignore_case": False,
                        "operator": "equal",
                        "target": "host",
                        "values": ["test.makn.cn"],
                    },
                    {
                        "ignore_case": False,
                        "operator": "equal",
                        "target": "url",
                        "values": ["/path"],
                    },
                ],
            }])
        ```

        ## Import

        teo teo_function_rule can be imported using the zoneId#functionId#ruleId, e.g.

        ```sh
        $ pulumi import tencentcloud:Teo/functionRule:FunctionRule example zone-3fkff38fyw8s#ef-m01xn26e#rule-yuvufj6h
        ```


        :param str resource_name: The name of the resource.
        :param FunctionRuleArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            FunctionRuleArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        function_id: pulumi.Input[_builtins.str | None] = None,
        function_rule_conditions: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union[
                        "FunctionRuleFunctionRuleConditionArgs",
                        "FunctionRuleFunctionRuleConditionArgsDict",
                    ]
                ]
            ]
            | None
        ] = None,
        remark: pulumi.Input[_builtins.str | None] = None,
        trigger_type: pulumi.Input[_builtins.str | None] = None,
        zone_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = FunctionRuleArgs.__new__(FunctionRuleArgs)

            if function_id is None and not opts.urn:
                raise TypeError("Missing required property 'function_id'")
            __props__.__dict__["function_id"] = function_id
            if function_rule_conditions is None and not opts.urn:
                raise TypeError("Missing required property 'function_rule_conditions'")
            __props__.__dict__["function_rule_conditions"] = function_rule_conditions
            __props__.__dict__["remark"] = remark
            __props__.__dict__["trigger_type"] = trigger_type
            if zone_id is None and not opts.urn:
                raise TypeError("Missing required property 'zone_id'")
            __props__.__dict__["zone_id"] = zone_id
            __props__.__dict__["function_name"] = None
            __props__.__dict__["priority"] = None
            __props__.__dict__["rule_id"] = None
        super().__init__(
            "tencentcloud:Teo/functionRule:FunctionRule", resource_name, __props__, opts
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        function_id: pulumi.Input[_builtins.str | None] = None,
        function_name: pulumi.Input[_builtins.str | None] = None,
        function_rule_conditions: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union[
                        "FunctionRuleFunctionRuleConditionArgs",
                        "FunctionRuleFunctionRuleConditionArgsDict",
                    ]
                ]
            ]
            | None
        ] = None,
        priority: pulumi.Input[_builtins.int | None] = None,
        remark: pulumi.Input[_builtins.str | None] = None,
        rule_id: pulumi.Input[_builtins.str | None] = None,
        trigger_type: pulumi.Input[_builtins.str | None] = None,
        zone_id: pulumi.Input[_builtins.str | None] = None,
    ) -> "FunctionRule":
        """
        Get an existing FunctionRule resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] function_id: ID of the Function.
        :param pulumi.Input[_builtins.str] function_name: The name of the function.
        :param pulumi.Input[Sequence[pulumi.Input[Union['FunctionRuleFunctionRuleConditionArgs', 'FunctionRuleFunctionRuleConditionArgsDict']]]] function_rule_conditions: The list of rule conditions, where the conditions are connected by an "OR" relationship.
        :param pulumi.Input[_builtins.int] priority: The priority of the function trigger rule. A higher numerical value indicates a higher priority.
        :param pulumi.Input[_builtins.str] remark: Rule description, maximum support of 60 characters.
        :param pulumi.Input[_builtins.str] rule_id: ID of the Function Rule.
        :param pulumi.Input[_builtins.str] trigger_type: Function selection configuration type. Valid values: `direct`, `weight`, `region`. Defaults to `direct` when not specified.
        :param pulumi.Input[_builtins.str] zone_id: ID of the site.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _FunctionRuleState.__new__(_FunctionRuleState)

        __props__.__dict__["function_id"] = function_id
        __props__.__dict__["function_name"] = function_name
        __props__.__dict__["function_rule_conditions"] = function_rule_conditions
        __props__.__dict__["priority"] = priority
        __props__.__dict__["remark"] = remark
        __props__.__dict__["rule_id"] = rule_id
        __props__.__dict__["trigger_type"] = trigger_type
        __props__.__dict__["zone_id"] = zone_id
        return FunctionRule(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="functionId")
    def function_id(self) -> pulumi.Output[_builtins.str]:
        """
        ID of the Function.
        """
        return pulumi.get(self, "function_id")

    @_builtins.property
    @pulumi.getter(name="functionName")
    def function_name(self) -> pulumi.Output[_builtins.str]:
        """
        The name of the function.
        """
        return pulumi.get(self, "function_name")

    @_builtins.property
    @pulumi.getter(name="functionRuleConditions")
    def function_rule_conditions(
        self,
    ) -> pulumi.Output[Sequence["outputs.FunctionRuleFunctionRuleCondition"]]:
        """
        The list of rule conditions, where the conditions are connected by an "OR" relationship.
        """
        return pulumi.get(self, "function_rule_conditions")

    @_builtins.property
    @pulumi.getter
    def priority(self) -> pulumi.Output[_builtins.int]:
        """
        The priority of the function trigger rule. A higher numerical value indicates a higher priority.
        """
        return pulumi.get(self, "priority")

    @_builtins.property
    @pulumi.getter
    def remark(self) -> pulumi.Output[_builtins.str | None]:
        """
        Rule description, maximum support of 60 characters.
        """
        return pulumi.get(self, "remark")

    @_builtins.property
    @pulumi.getter(name="ruleId")
    def rule_id(self) -> pulumi.Output[_builtins.str]:
        """
        ID of the Function Rule.
        """
        return pulumi.get(self, "rule_id")

    @_builtins.property
    @pulumi.getter(name="triggerType")
    def trigger_type(self) -> pulumi.Output[_builtins.str]:
        """
        Function selection configuration type. Valid values: `direct`, `weight`, `region`. Defaults to `direct` when not specified.
        """
        return pulumi.get(self, "trigger_type")

    @_builtins.property
    @pulumi.getter(name="zoneId")
    def zone_id(self) -> pulumi.Output[_builtins.str]:
        """
        ID of the site.
        """
        return pulumi.get(self, "zone_id")
