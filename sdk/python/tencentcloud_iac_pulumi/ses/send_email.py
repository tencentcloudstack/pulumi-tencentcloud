# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence
from typing import Optional, Union, overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ["SendEmail", "SendEmailArgs"]


@pulumi.input_type
class SendEmailArgs:
    def __init__(
        __self__,
        *,
        destinations: pulumi.Input[Sequence[pulumi.Input[_builtins.str]]],
        from_email_address: pulumi.Input[_builtins.str],
        subject: pulumi.Input[_builtins.str],
        attachments: pulumi.Input[
            Sequence[pulumi.Input["SendEmailAttachmentArgs"]] | None
        ] = None,
        bccs: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        ccs: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        reply_to_addresses: pulumi.Input[_builtins.str | None] = None,
        template: pulumi.Input[Optional["SendEmailTemplateArgs"]] = None,
        trigger_type: pulumi.Input[_builtins.int | None] = None,
        unsubscribe: pulumi.Input[_builtins.str | None] = None
    ):
        """
        The set of arguments for constructing a SendEmail resource.

        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] destinations: Recipient email addresses. You can send an email to up to 50 recipients at a time. Note: the email content will display all recipient addresses. To send one-to-one emails to several recipients, please call the API multiple times to send the emails.
        :param pulumi.Input[_builtins.str] from_email_address: Sender address. Enter a sender address, for example, noreply@mail.qcloud.com.To display the sender name, enter the address in the following format:Sender.
        :param pulumi.Input[_builtins.str] subject: Email subject.
        :param pulumi.Input[Sequence[pulumi.Input['SendEmailAttachmentArgs']]] attachments: Parameters for the attachments to be sent. The TencentCloud API supports a request packet of up to 8 MB in size,and the size of the attachment content will increase by 1.5 times after Base64 encoding. Therefore,you need to keep the total size of all attachments below 4 MB. If the entire request exceeds 8 MB,the API will return an error.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] bccs: The email address of the cc recipient can support up to 20 cc recipients.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] ccs: Cc recipient email address, up to 20 people can be copied.
        :param pulumi.Input[_builtins.str] reply_to_addresses: Reply-to address. You can enter a valid personal email address that can receive emails. If this parameter is left empty, reply emails will fail to be sent.
        :param pulumi.Input['SendEmailTemplateArgs'] template: Template parameters for template-based sending. As Simple has been disused, Template is required.
        :param pulumi.Input[_builtins.int] trigger_type: Email triggering type. 0 (default): non-trigger-based, suitable for marketing emails and non-immediate emails;1: trigger-based, suitable for immediate emails such as emails containing verification codes.If the size of an email exceeds a specified value,the system will automatically choose the non-trigger-based type.
        :param pulumi.Input[_builtins.str] unsubscribe: Unsubscribe link option.  0: Do not add unsubscribe link; 1: English 2: Simplified Chinese;  3: Traditional Chinese; 4: Spanish; 5: French;  6: German; 7: Japanese; 8: Korean;  9: Arabic; 10: Thai.
        """
        pulumi.set(__self__, "destinations", destinations)
        pulumi.set(__self__, "from_email_address", from_email_address)
        pulumi.set(__self__, "subject", subject)
        if attachments is not None:
            pulumi.set(__self__, "attachments", attachments)
        if bccs is not None:
            pulumi.set(__self__, "bccs", bccs)
        if ccs is not None:
            pulumi.set(__self__, "ccs", ccs)
        if reply_to_addresses is not None:
            pulumi.set(__self__, "reply_to_addresses", reply_to_addresses)
        if template is not None:
            pulumi.set(__self__, "template", template)
        if trigger_type is not None:
            pulumi.set(__self__, "trigger_type", trigger_type)
        if unsubscribe is not None:
            pulumi.set(__self__, "unsubscribe", unsubscribe)

    @_builtins.property
    @pulumi.getter
    def destinations(self) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]]]:
        """
        Recipient email addresses. You can send an email to up to 50 recipients at a time. Note: the email content will display all recipient addresses. To send one-to-one emails to several recipients, please call the API multiple times to send the emails.
        """
        return pulumi.get(self, "destinations")

    @destinations.setter
    def destinations(self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]]]):
        pulumi.set(self, "destinations", value)

    @_builtins.property
    @pulumi.getter(name="fromEmailAddress")
    def from_email_address(self) -> pulumi.Input[_builtins.str]:
        """
        Sender address. Enter a sender address, for example, noreply@mail.qcloud.com.To display the sender name, enter the address in the following format:Sender.
        """
        return pulumi.get(self, "from_email_address")

    @from_email_address.setter
    def from_email_address(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "from_email_address", value)

    @_builtins.property
    @pulumi.getter
    def subject(self) -> pulumi.Input[_builtins.str]:
        """
        Email subject.
        """
        return pulumi.get(self, "subject")

    @subject.setter
    def subject(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "subject", value)

    @_builtins.property
    @pulumi.getter
    def attachments(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input["SendEmailAttachmentArgs"]] | None]:
        """
        Parameters for the attachments to be sent. The TencentCloud API supports a request packet of up to 8 MB in size,and the size of the attachment content will increase by 1.5 times after Base64 encoding. Therefore,you need to keep the total size of all attachments below 4 MB. If the entire request exceeds 8 MB,the API will return an error.
        """
        return pulumi.get(self, "attachments")

    @attachments.setter
    def attachments(
        self,
        value: pulumi.Input[Sequence[pulumi.Input["SendEmailAttachmentArgs"]] | None],
    ):
        pulumi.set(self, "attachments", value)

    @_builtins.property
    @pulumi.getter
    def bccs(self) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        The email address of the cc recipient can support up to 20 cc recipients.
        """
        return pulumi.get(self, "bccs")

    @bccs.setter
    def bccs(self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]):
        pulumi.set(self, "bccs", value)

    @_builtins.property
    @pulumi.getter
    def ccs(self) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        Cc recipient email address, up to 20 people can be copied.
        """
        return pulumi.get(self, "ccs")

    @ccs.setter
    def ccs(self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]):
        pulumi.set(self, "ccs", value)

    @_builtins.property
    @pulumi.getter(name="replyToAddresses")
    def reply_to_addresses(self) -> pulumi.Input[_builtins.str | None]:
        """
        Reply-to address. You can enter a valid personal email address that can receive emails. If this parameter is left empty, reply emails will fail to be sent.
        """
        return pulumi.get(self, "reply_to_addresses")

    @reply_to_addresses.setter
    def reply_to_addresses(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "reply_to_addresses", value)

    @_builtins.property
    @pulumi.getter
    def template(self) -> pulumi.Input[Optional["SendEmailTemplateArgs"]]:
        """
        Template parameters for template-based sending. As Simple has been disused, Template is required.
        """
        return pulumi.get(self, "template")

    @template.setter
    def template(self, value: pulumi.Input[Optional["SendEmailTemplateArgs"]]):
        pulumi.set(self, "template", value)

    @_builtins.property
    @pulumi.getter(name="triggerType")
    def trigger_type(self) -> pulumi.Input[_builtins.int | None]:
        """
        Email triggering type. 0 (default): non-trigger-based, suitable for marketing emails and non-immediate emails;1: trigger-based, suitable for immediate emails such as emails containing verification codes.If the size of an email exceeds a specified value,the system will automatically choose the non-trigger-based type.
        """
        return pulumi.get(self, "trigger_type")

    @trigger_type.setter
    def trigger_type(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "trigger_type", value)

    @_builtins.property
    @pulumi.getter
    def unsubscribe(self) -> pulumi.Input[_builtins.str | None]:
        """
        Unsubscribe link option.  0: Do not add unsubscribe link; 1: English 2: Simplified Chinese;  3: Traditional Chinese; 4: Spanish; 5: French;  6: German; 7: Japanese; 8: Korean;  9: Arabic; 10: Thai.
        """
        return pulumi.get(self, "unsubscribe")

    @unsubscribe.setter
    def unsubscribe(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "unsubscribe", value)


@pulumi.input_type
class _SendEmailState:
    def __init__(
        __self__,
        *,
        attachments: pulumi.Input[
            Sequence[pulumi.Input["SendEmailAttachmentArgs"]] | None
        ] = None,
        bccs: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        ccs: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        destinations: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        from_email_address: pulumi.Input[_builtins.str | None] = None,
        reply_to_addresses: pulumi.Input[_builtins.str | None] = None,
        subject: pulumi.Input[_builtins.str | None] = None,
        template: pulumi.Input[Optional["SendEmailTemplateArgs"]] = None,
        trigger_type: pulumi.Input[_builtins.int | None] = None,
        unsubscribe: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering SendEmail resources.

        :param pulumi.Input[Sequence[pulumi.Input['SendEmailAttachmentArgs']]] attachments: Parameters for the attachments to be sent. The TencentCloud API supports a request packet of up to 8 MB in size,and the size of the attachment content will increase by 1.5 times after Base64 encoding. Therefore,you need to keep the total size of all attachments below 4 MB. If the entire request exceeds 8 MB,the API will return an error.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] bccs: The email address of the cc recipient can support up to 20 cc recipients.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] ccs: Cc recipient email address, up to 20 people can be copied.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] destinations: Recipient email addresses. You can send an email to up to 50 recipients at a time. Note: the email content will display all recipient addresses. To send one-to-one emails to several recipients, please call the API multiple times to send the emails.
        :param pulumi.Input[_builtins.str] from_email_address: Sender address. Enter a sender address, for example, noreply@mail.qcloud.com.To display the sender name, enter the address in the following format:Sender.
        :param pulumi.Input[_builtins.str] reply_to_addresses: Reply-to address. You can enter a valid personal email address that can receive emails. If this parameter is left empty, reply emails will fail to be sent.
        :param pulumi.Input[_builtins.str] subject: Email subject.
        :param pulumi.Input['SendEmailTemplateArgs'] template: Template parameters for template-based sending. As Simple has been disused, Template is required.
        :param pulumi.Input[_builtins.int] trigger_type: Email triggering type. 0 (default): non-trigger-based, suitable for marketing emails and non-immediate emails;1: trigger-based, suitable for immediate emails such as emails containing verification codes.If the size of an email exceeds a specified value,the system will automatically choose the non-trigger-based type.
        :param pulumi.Input[_builtins.str] unsubscribe: Unsubscribe link option.  0: Do not add unsubscribe link; 1: English 2: Simplified Chinese;  3: Traditional Chinese; 4: Spanish; 5: French;  6: German; 7: Japanese; 8: Korean;  9: Arabic; 10: Thai.
        """
        if attachments is not None:
            pulumi.set(__self__, "attachments", attachments)
        if bccs is not None:
            pulumi.set(__self__, "bccs", bccs)
        if ccs is not None:
            pulumi.set(__self__, "ccs", ccs)
        if destinations is not None:
            pulumi.set(__self__, "destinations", destinations)
        if from_email_address is not None:
            pulumi.set(__self__, "from_email_address", from_email_address)
        if reply_to_addresses is not None:
            pulumi.set(__self__, "reply_to_addresses", reply_to_addresses)
        if subject is not None:
            pulumi.set(__self__, "subject", subject)
        if template is not None:
            pulumi.set(__self__, "template", template)
        if trigger_type is not None:
            pulumi.set(__self__, "trigger_type", trigger_type)
        if unsubscribe is not None:
            pulumi.set(__self__, "unsubscribe", unsubscribe)

    @_builtins.property
    @pulumi.getter
    def attachments(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input["SendEmailAttachmentArgs"]] | None]:
        """
        Parameters for the attachments to be sent. The TencentCloud API supports a request packet of up to 8 MB in size,and the size of the attachment content will increase by 1.5 times after Base64 encoding. Therefore,you need to keep the total size of all attachments below 4 MB. If the entire request exceeds 8 MB,the API will return an error.
        """
        return pulumi.get(self, "attachments")

    @attachments.setter
    def attachments(
        self,
        value: pulumi.Input[Sequence[pulumi.Input["SendEmailAttachmentArgs"]] | None],
    ):
        pulumi.set(self, "attachments", value)

    @_builtins.property
    @pulumi.getter
    def bccs(self) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        The email address of the cc recipient can support up to 20 cc recipients.
        """
        return pulumi.get(self, "bccs")

    @bccs.setter
    def bccs(self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]):
        pulumi.set(self, "bccs", value)

    @_builtins.property
    @pulumi.getter
    def ccs(self) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        Cc recipient email address, up to 20 people can be copied.
        """
        return pulumi.get(self, "ccs")

    @ccs.setter
    def ccs(self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]):
        pulumi.set(self, "ccs", value)

    @_builtins.property
    @pulumi.getter
    def destinations(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        Recipient email addresses. You can send an email to up to 50 recipients at a time. Note: the email content will display all recipient addresses. To send one-to-one emails to several recipients, please call the API multiple times to send the emails.
        """
        return pulumi.get(self, "destinations")

    @destinations.setter
    def destinations(
        self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "destinations", value)

    @_builtins.property
    @pulumi.getter(name="fromEmailAddress")
    def from_email_address(self) -> pulumi.Input[_builtins.str | None]:
        """
        Sender address. Enter a sender address, for example, noreply@mail.qcloud.com.To display the sender name, enter the address in the following format:Sender.
        """
        return pulumi.get(self, "from_email_address")

    @from_email_address.setter
    def from_email_address(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "from_email_address", value)

    @_builtins.property
    @pulumi.getter(name="replyToAddresses")
    def reply_to_addresses(self) -> pulumi.Input[_builtins.str | None]:
        """
        Reply-to address. You can enter a valid personal email address that can receive emails. If this parameter is left empty, reply emails will fail to be sent.
        """
        return pulumi.get(self, "reply_to_addresses")

    @reply_to_addresses.setter
    def reply_to_addresses(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "reply_to_addresses", value)

    @_builtins.property
    @pulumi.getter
    def subject(self) -> pulumi.Input[_builtins.str | None]:
        """
        Email subject.
        """
        return pulumi.get(self, "subject")

    @subject.setter
    def subject(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "subject", value)

    @_builtins.property
    @pulumi.getter
    def template(self) -> pulumi.Input[Optional["SendEmailTemplateArgs"]]:
        """
        Template parameters for template-based sending. As Simple has been disused, Template is required.
        """
        return pulumi.get(self, "template")

    @template.setter
    def template(self, value: pulumi.Input[Optional["SendEmailTemplateArgs"]]):
        pulumi.set(self, "template", value)

    @_builtins.property
    @pulumi.getter(name="triggerType")
    def trigger_type(self) -> pulumi.Input[_builtins.int | None]:
        """
        Email triggering type. 0 (default): non-trigger-based, suitable for marketing emails and non-immediate emails;1: trigger-based, suitable for immediate emails such as emails containing verification codes.If the size of an email exceeds a specified value,the system will automatically choose the non-trigger-based type.
        """
        return pulumi.get(self, "trigger_type")

    @trigger_type.setter
    def trigger_type(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "trigger_type", value)

    @_builtins.property
    @pulumi.getter
    def unsubscribe(self) -> pulumi.Input[_builtins.str | None]:
        """
        Unsubscribe link option.  0: Do not add unsubscribe link; 1: English 2: Simplified Chinese;  3: Traditional Chinese; 4: Spanish; 5: French;  6: German; 7: Japanese; 8: Korean;  9: Arabic; 10: Thai.
        """
        return pulumi.get(self, "unsubscribe")

    @unsubscribe.setter
    def unsubscribe(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "unsubscribe", value)


@pulumi.type_token("tencentcloud:Ses/sendEmail:SendEmail")
class SendEmail(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        attachments: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union["SendEmailAttachmentArgs", "SendEmailAttachmentArgsDict"]
                ]
            ]
            | None
        ] = None,
        bccs: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        ccs: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        destinations: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        from_email_address: pulumi.Input[_builtins.str | None] = None,
        reply_to_addresses: pulumi.Input[_builtins.str | None] = None,
        subject: pulumi.Input[_builtins.str | None] = None,
        template: pulumi.Input[
            Union["SendEmailTemplateArgs", "SendEmailTemplateArgsDict"] | None
        ] = None,
        trigger_type: pulumi.Input[_builtins.int | None] = None,
        unsubscribe: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a ses send_email

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        send_email = tencentcloud.ses.SendEmail("send_email",
            from_email_address="aaa@iac-tf.cloud",
            destinations=["1055482519@qq.com"],
            subject="test subject",
            reply_to_addresses="aaa@iac-tf.cloud",
            template={
                "template_id": 99629,
                "template_data": "{\\"name\\":\\"xxx\\",\\"age\\":\\"xx\\"}",
            },
            unsubscribe="1",
            trigger_type=1)
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[Sequence[pulumi.Input[Union['SendEmailAttachmentArgs', 'SendEmailAttachmentArgsDict']]]] attachments: Parameters for the attachments to be sent. The TencentCloud API supports a request packet of up to 8 MB in size,and the size of the attachment content will increase by 1.5 times after Base64 encoding. Therefore,you need to keep the total size of all attachments below 4 MB. If the entire request exceeds 8 MB,the API will return an error.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] bccs: The email address of the cc recipient can support up to 20 cc recipients.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] ccs: Cc recipient email address, up to 20 people can be copied.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] destinations: Recipient email addresses. You can send an email to up to 50 recipients at a time. Note: the email content will display all recipient addresses. To send one-to-one emails to several recipients, please call the API multiple times to send the emails.
        :param pulumi.Input[_builtins.str] from_email_address: Sender address. Enter a sender address, for example, noreply@mail.qcloud.com.To display the sender name, enter the address in the following format:Sender.
        :param pulumi.Input[_builtins.str] reply_to_addresses: Reply-to address. You can enter a valid personal email address that can receive emails. If this parameter is left empty, reply emails will fail to be sent.
        :param pulumi.Input[_builtins.str] subject: Email subject.
        :param pulumi.Input[Union['SendEmailTemplateArgs', 'SendEmailTemplateArgsDict']] template: Template parameters for template-based sending. As Simple has been disused, Template is required.
        :param pulumi.Input[_builtins.int] trigger_type: Email triggering type. 0 (default): non-trigger-based, suitable for marketing emails and non-immediate emails;1: trigger-based, suitable for immediate emails such as emails containing verification codes.If the size of an email exceeds a specified value,the system will automatically choose the non-trigger-based type.
        :param pulumi.Input[_builtins.str] unsubscribe: Unsubscribe link option.  0: Do not add unsubscribe link; 1: English 2: Simplified Chinese;  3: Traditional Chinese; 4: Spanish; 5: French;  6: German; 7: Japanese; 8: Korean;  9: Arabic; 10: Thai.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: SendEmailArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a ses send_email

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        send_email = tencentcloud.ses.SendEmail("send_email",
            from_email_address="aaa@iac-tf.cloud",
            destinations=["1055482519@qq.com"],
            subject="test subject",
            reply_to_addresses="aaa@iac-tf.cloud",
            template={
                "template_id": 99629,
                "template_data": "{\\"name\\":\\"xxx\\",\\"age\\":\\"xx\\"}",
            },
            unsubscribe="1",
            trigger_type=1)
        ```


        :param str resource_name: The name of the resource.
        :param SendEmailArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            SendEmailArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        attachments: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union["SendEmailAttachmentArgs", "SendEmailAttachmentArgsDict"]
                ]
            ]
            | None
        ] = None,
        bccs: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        ccs: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        destinations: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        from_email_address: pulumi.Input[_builtins.str | None] = None,
        reply_to_addresses: pulumi.Input[_builtins.str | None] = None,
        subject: pulumi.Input[_builtins.str | None] = None,
        template: pulumi.Input[
            Union["SendEmailTemplateArgs", "SendEmailTemplateArgsDict"] | None
        ] = None,
        trigger_type: pulumi.Input[_builtins.int | None] = None,
        unsubscribe: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = SendEmailArgs.__new__(SendEmailArgs)

            __props__.__dict__["attachments"] = attachments
            __props__.__dict__["bccs"] = bccs
            __props__.__dict__["ccs"] = ccs
            if destinations is None and not opts.urn:
                raise TypeError("Missing required property 'destinations'")
            __props__.__dict__["destinations"] = destinations
            if from_email_address is None and not opts.urn:
                raise TypeError("Missing required property 'from_email_address'")
            __props__.__dict__["from_email_address"] = from_email_address
            __props__.__dict__["reply_to_addresses"] = reply_to_addresses
            if subject is None and not opts.urn:
                raise TypeError("Missing required property 'subject'")
            __props__.__dict__["subject"] = subject
            __props__.__dict__["template"] = template
            __props__.__dict__["trigger_type"] = trigger_type
            __props__.__dict__["unsubscribe"] = unsubscribe
        super().__init__(
            "tencentcloud:Ses/sendEmail:SendEmail", resource_name, __props__, opts
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        attachments: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union["SendEmailAttachmentArgs", "SendEmailAttachmentArgsDict"]
                ]
            ]
            | None
        ] = None,
        bccs: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        ccs: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        destinations: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None,
        from_email_address: pulumi.Input[_builtins.str | None] = None,
        reply_to_addresses: pulumi.Input[_builtins.str | None] = None,
        subject: pulumi.Input[_builtins.str | None] = None,
        template: pulumi.Input[
            Union["SendEmailTemplateArgs", "SendEmailTemplateArgsDict"] | None
        ] = None,
        trigger_type: pulumi.Input[_builtins.int | None] = None,
        unsubscribe: pulumi.Input[_builtins.str | None] = None,
    ) -> "SendEmail":
        """
        Get an existing SendEmail resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[Sequence[pulumi.Input[Union['SendEmailAttachmentArgs', 'SendEmailAttachmentArgsDict']]]] attachments: Parameters for the attachments to be sent. The TencentCloud API supports a request packet of up to 8 MB in size,and the size of the attachment content will increase by 1.5 times after Base64 encoding. Therefore,you need to keep the total size of all attachments below 4 MB. If the entire request exceeds 8 MB,the API will return an error.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] bccs: The email address of the cc recipient can support up to 20 cc recipients.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] ccs: Cc recipient email address, up to 20 people can be copied.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] destinations: Recipient email addresses. You can send an email to up to 50 recipients at a time. Note: the email content will display all recipient addresses. To send one-to-one emails to several recipients, please call the API multiple times to send the emails.
        :param pulumi.Input[_builtins.str] from_email_address: Sender address. Enter a sender address, for example, noreply@mail.qcloud.com.To display the sender name, enter the address in the following format:Sender.
        :param pulumi.Input[_builtins.str] reply_to_addresses: Reply-to address. You can enter a valid personal email address that can receive emails. If this parameter is left empty, reply emails will fail to be sent.
        :param pulumi.Input[_builtins.str] subject: Email subject.
        :param pulumi.Input[Union['SendEmailTemplateArgs', 'SendEmailTemplateArgsDict']] template: Template parameters for template-based sending. As Simple has been disused, Template is required.
        :param pulumi.Input[_builtins.int] trigger_type: Email triggering type. 0 (default): non-trigger-based, suitable for marketing emails and non-immediate emails;1: trigger-based, suitable for immediate emails such as emails containing verification codes.If the size of an email exceeds a specified value,the system will automatically choose the non-trigger-based type.
        :param pulumi.Input[_builtins.str] unsubscribe: Unsubscribe link option.  0: Do not add unsubscribe link; 1: English 2: Simplified Chinese;  3: Traditional Chinese; 4: Spanish; 5: French;  6: German; 7: Japanese; 8: Korean;  9: Arabic; 10: Thai.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _SendEmailState.__new__(_SendEmailState)

        __props__.__dict__["attachments"] = attachments
        __props__.__dict__["bccs"] = bccs
        __props__.__dict__["ccs"] = ccs
        __props__.__dict__["destinations"] = destinations
        __props__.__dict__["from_email_address"] = from_email_address
        __props__.__dict__["reply_to_addresses"] = reply_to_addresses
        __props__.__dict__["subject"] = subject
        __props__.__dict__["template"] = template
        __props__.__dict__["trigger_type"] = trigger_type
        __props__.__dict__["unsubscribe"] = unsubscribe
        return SendEmail(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter
    def attachments(
        self,
    ) -> pulumi.Output[Sequence["outputs.SendEmailAttachment"] | None]:
        """
        Parameters for the attachments to be sent. The TencentCloud API supports a request packet of up to 8 MB in size,and the size of the attachment content will increase by 1.5 times after Base64 encoding. Therefore,you need to keep the total size of all attachments below 4 MB. If the entire request exceeds 8 MB,the API will return an error.
        """
        return pulumi.get(self, "attachments")

    @_builtins.property
    @pulumi.getter
    def bccs(self) -> pulumi.Output[Sequence[_builtins.str] | None]:
        """
        The email address of the cc recipient can support up to 20 cc recipients.
        """
        return pulumi.get(self, "bccs")

    @_builtins.property
    @pulumi.getter
    def ccs(self) -> pulumi.Output[Sequence[_builtins.str] | None]:
        """
        Cc recipient email address, up to 20 people can be copied.
        """
        return pulumi.get(self, "ccs")

    @_builtins.property
    @pulumi.getter
    def destinations(self) -> pulumi.Output[Sequence[_builtins.str]]:
        """
        Recipient email addresses. You can send an email to up to 50 recipients at a time. Note: the email content will display all recipient addresses. To send one-to-one emails to several recipients, please call the API multiple times to send the emails.
        """
        return pulumi.get(self, "destinations")

    @_builtins.property
    @pulumi.getter(name="fromEmailAddress")
    def from_email_address(self) -> pulumi.Output[_builtins.str]:
        """
        Sender address. Enter a sender address, for example, noreply@mail.qcloud.com.To display the sender name, enter the address in the following format:Sender.
        """
        return pulumi.get(self, "from_email_address")

    @_builtins.property
    @pulumi.getter(name="replyToAddresses")
    def reply_to_addresses(self) -> pulumi.Output[_builtins.str | None]:
        """
        Reply-to address. You can enter a valid personal email address that can receive emails. If this parameter is left empty, reply emails will fail to be sent.
        """
        return pulumi.get(self, "reply_to_addresses")

    @_builtins.property
    @pulumi.getter
    def subject(self) -> pulumi.Output[_builtins.str]:
        """
        Email subject.
        """
        return pulumi.get(self, "subject")

    @_builtins.property
    @pulumi.getter
    def template(self) -> pulumi.Output[Optional["outputs.SendEmailTemplate"]]:
        """
        Template parameters for template-based sending. As Simple has been disused, Template is required.
        """
        return pulumi.get(self, "template")

    @_builtins.property
    @pulumi.getter(name="triggerType")
    def trigger_type(self) -> pulumi.Output[_builtins.int | None]:
        """
        Email triggering type. 0 (default): non-trigger-based, suitable for marketing emails and non-immediate emails;1: trigger-based, suitable for immediate emails such as emails containing verification codes.If the size of an email exceeds a specified value,the system will automatically choose the non-trigger-based type.
        """
        return pulumi.get(self, "trigger_type")

    @_builtins.property
    @pulumi.getter
    def unsubscribe(self) -> pulumi.Output[_builtins.str | None]:
        """
        Unsubscribe link option.  0: Do not add unsubscribe link; 1: English 2: Simplified Chinese;  3: Traditional Chinese; 4: Spanish; 5: French;  6: German; 7: Japanese; 8: Korean;  9: Arabic; 10: Thai.
        """
        return pulumi.get(self, "unsubscribe")
