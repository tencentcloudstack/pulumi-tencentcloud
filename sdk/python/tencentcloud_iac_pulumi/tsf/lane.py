# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence
from typing import Union, overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ["Lane", "LaneArgs"]


@pulumi.input_type
class LaneArgs:
    def __init__(
        __self__,
        *,
        lane_group_lists: pulumi.Input[Sequence[pulumi.Input["LaneLaneGroupListArgs"]]],
        lane_name: pulumi.Input[_builtins.str],
        remark: pulumi.Input[_builtins.str],
        program_id_lists: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None
    ):
        """
        The set of arguments for constructing a Lane resource.

        :param pulumi.Input[Sequence[pulumi.Input['LaneLaneGroupListArgs']]] lane_group_lists: Swimlane Deployment Group Information.
        :param pulumi.Input[_builtins.str] lane_name: Lane name.
        :param pulumi.Input[_builtins.str] remark: Lane Remarks.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] program_id_lists: Program id list.
        """
        pulumi.set(__self__, "lane_group_lists", lane_group_lists)
        pulumi.set(__self__, "lane_name", lane_name)
        pulumi.set(__self__, "remark", remark)
        if program_id_lists is not None:
            pulumi.set(__self__, "program_id_lists", program_id_lists)

    @_builtins.property
    @pulumi.getter(name="laneGroupLists")
    def lane_group_lists(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input["LaneLaneGroupListArgs"]]]:
        """
        Swimlane Deployment Group Information.
        """
        return pulumi.get(self, "lane_group_lists")

    @lane_group_lists.setter
    def lane_group_lists(
        self, value: pulumi.Input[Sequence[pulumi.Input["LaneLaneGroupListArgs"]]]
    ):
        pulumi.set(self, "lane_group_lists", value)

    @_builtins.property
    @pulumi.getter(name="laneName")
    def lane_name(self) -> pulumi.Input[_builtins.str]:
        """
        Lane name.
        """
        return pulumi.get(self, "lane_name")

    @lane_name.setter
    def lane_name(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "lane_name", value)

    @_builtins.property
    @pulumi.getter
    def remark(self) -> pulumi.Input[_builtins.str]:
        """
        Lane Remarks.
        """
        return pulumi.get(self, "remark")

    @remark.setter
    def remark(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "remark", value)

    @_builtins.property
    @pulumi.getter(name="programIdLists")
    def program_id_lists(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        Program id list.
        """
        return pulumi.get(self, "program_id_lists")

    @program_id_lists.setter
    def program_id_lists(
        self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "program_id_lists", value)


@pulumi.input_type
class _LaneState:
    def __init__(
        __self__,
        *,
        create_time: pulumi.Input[_builtins.int | None] = None,
        entrance: pulumi.Input[_builtins.bool | None] = None,
        lane_group_lists: pulumi.Input[
            Sequence[pulumi.Input["LaneLaneGroupListArgs"]] | None
        ] = None,
        lane_id: pulumi.Input[_builtins.str | None] = None,
        lane_name: pulumi.Input[_builtins.str | None] = None,
        namespace_id_lists: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None,
        program_id_lists: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None,
        remark: pulumi.Input[_builtins.str | None] = None,
        update_time: pulumi.Input[_builtins.int | None] = None
    ):
        """
        Input properties used for looking up and filtering Lane resources.

        :param pulumi.Input[_builtins.int] create_time: creation time.
        :param pulumi.Input[_builtins.bool] entrance: Whether to enter the application.
        :param pulumi.Input[Sequence[pulumi.Input['LaneLaneGroupListArgs']]] lane_group_lists: Swimlane Deployment Group Information.
        :param pulumi.Input[_builtins.str] lane_id: Lane id.
        :param pulumi.Input[_builtins.str] lane_name: Lane name.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] namespace_id_lists: A list of namespaces to which the swimlane has associated deployment groups.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] program_id_lists: Program id list.
        :param pulumi.Input[_builtins.str] remark: Lane Remarks.
        :param pulumi.Input[_builtins.int] update_time: update time.
        """
        if create_time is not None:
            pulumi.set(__self__, "create_time", create_time)
        if entrance is not None:
            pulumi.set(__self__, "entrance", entrance)
        if lane_group_lists is not None:
            pulumi.set(__self__, "lane_group_lists", lane_group_lists)
        if lane_id is not None:
            pulumi.set(__self__, "lane_id", lane_id)
        if lane_name is not None:
            pulumi.set(__self__, "lane_name", lane_name)
        if namespace_id_lists is not None:
            pulumi.set(__self__, "namespace_id_lists", namespace_id_lists)
        if program_id_lists is not None:
            pulumi.set(__self__, "program_id_lists", program_id_lists)
        if remark is not None:
            pulumi.set(__self__, "remark", remark)
        if update_time is not None:
            pulumi.set(__self__, "update_time", update_time)

    @_builtins.property
    @pulumi.getter(name="createTime")
    def create_time(self) -> pulumi.Input[_builtins.int | None]:
        """
        creation time.
        """
        return pulumi.get(self, "create_time")

    @create_time.setter
    def create_time(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "create_time", value)

    @_builtins.property
    @pulumi.getter
    def entrance(self) -> pulumi.Input[_builtins.bool | None]:
        """
        Whether to enter the application.
        """
        return pulumi.get(self, "entrance")

    @entrance.setter
    def entrance(self, value: pulumi.Input[_builtins.bool | None]):
        pulumi.set(self, "entrance", value)

    @_builtins.property
    @pulumi.getter(name="laneGroupLists")
    def lane_group_lists(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input["LaneLaneGroupListArgs"]] | None]:
        """
        Swimlane Deployment Group Information.
        """
        return pulumi.get(self, "lane_group_lists")

    @lane_group_lists.setter
    def lane_group_lists(
        self,
        value: pulumi.Input[Sequence[pulumi.Input["LaneLaneGroupListArgs"]] | None],
    ):
        pulumi.set(self, "lane_group_lists", value)

    @_builtins.property
    @pulumi.getter(name="laneId")
    def lane_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Lane id.
        """
        return pulumi.get(self, "lane_id")

    @lane_id.setter
    def lane_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "lane_id", value)

    @_builtins.property
    @pulumi.getter(name="laneName")
    def lane_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Lane name.
        """
        return pulumi.get(self, "lane_name")

    @lane_name.setter
    def lane_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "lane_name", value)

    @_builtins.property
    @pulumi.getter(name="namespaceIdLists")
    def namespace_id_lists(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        A list of namespaces to which the swimlane has associated deployment groups.
        """
        return pulumi.get(self, "namespace_id_lists")

    @namespace_id_lists.setter
    def namespace_id_lists(
        self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "namespace_id_lists", value)

    @_builtins.property
    @pulumi.getter(name="programIdLists")
    def program_id_lists(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        Program id list.
        """
        return pulumi.get(self, "program_id_lists")

    @program_id_lists.setter
    def program_id_lists(
        self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "program_id_lists", value)

    @_builtins.property
    @pulumi.getter
    def remark(self) -> pulumi.Input[_builtins.str | None]:
        """
        Lane Remarks.
        """
        return pulumi.get(self, "remark")

    @remark.setter
    def remark(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "remark", value)

    @_builtins.property
    @pulumi.getter(name="updateTime")
    def update_time(self) -> pulumi.Input[_builtins.int | None]:
        """
        update time.
        """
        return pulumi.get(self, "update_time")

    @update_time.setter
    def update_time(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "update_time", value)


@pulumi.type_token("tencentcloud:Tsf/lane:Lane")
class Lane(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        lane_group_lists: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union["LaneLaneGroupListArgs", "LaneLaneGroupListArgsDict"]
                ]
            ]
            | None
        ] = None,
        lane_name: pulumi.Input[_builtins.str | None] = None,
        program_id_lists: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None,
        remark: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a tsf lane

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        lane = tencentcloud.tsf.Lane("lane",
            lane_name="lane-name-1",
            remark="lane desc1",
            lane_group_lists=[{
                "group_id": "group-yn7j5l8a",
                "entrance": True,
            }])
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[Sequence[pulumi.Input[Union['LaneLaneGroupListArgs', 'LaneLaneGroupListArgsDict']]]] lane_group_lists: Swimlane Deployment Group Information.
        :param pulumi.Input[_builtins.str] lane_name: Lane name.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] program_id_lists: Program id list.
        :param pulumi.Input[_builtins.str] remark: Lane Remarks.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: LaneArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a tsf lane

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        lane = tencentcloud.tsf.Lane("lane",
            lane_name="lane-name-1",
            remark="lane desc1",
            lane_group_lists=[{
                "group_id": "group-yn7j5l8a",
                "entrance": True,
            }])
        ```


        :param str resource_name: The name of the resource.
        :param LaneArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            LaneArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        lane_group_lists: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union["LaneLaneGroupListArgs", "LaneLaneGroupListArgsDict"]
                ]
            ]
            | None
        ] = None,
        lane_name: pulumi.Input[_builtins.str | None] = None,
        program_id_lists: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None,
        remark: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = LaneArgs.__new__(LaneArgs)

            if lane_group_lists is None and not opts.urn:
                raise TypeError("Missing required property 'lane_group_lists'")
            __props__.__dict__["lane_group_lists"] = lane_group_lists
            if lane_name is None and not opts.urn:
                raise TypeError("Missing required property 'lane_name'")
            __props__.__dict__["lane_name"] = lane_name
            __props__.__dict__["program_id_lists"] = program_id_lists
            if remark is None and not opts.urn:
                raise TypeError("Missing required property 'remark'")
            __props__.__dict__["remark"] = remark
            __props__.__dict__["create_time"] = None
            __props__.__dict__["entrance"] = None
            __props__.__dict__["lane_id"] = None
            __props__.__dict__["namespace_id_lists"] = None
            __props__.__dict__["update_time"] = None
        super().__init__("tencentcloud:Tsf/lane:Lane", resource_name, __props__, opts)

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        create_time: pulumi.Input[_builtins.int | None] = None,
        entrance: pulumi.Input[_builtins.bool | None] = None,
        lane_group_lists: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union["LaneLaneGroupListArgs", "LaneLaneGroupListArgsDict"]
                ]
            ]
            | None
        ] = None,
        lane_id: pulumi.Input[_builtins.str | None] = None,
        lane_name: pulumi.Input[_builtins.str | None] = None,
        namespace_id_lists: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None,
        program_id_lists: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None,
        remark: pulumi.Input[_builtins.str | None] = None,
        update_time: pulumi.Input[_builtins.int | None] = None,
    ) -> "Lane":
        """
        Get an existing Lane resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.int] create_time: creation time.
        :param pulumi.Input[_builtins.bool] entrance: Whether to enter the application.
        :param pulumi.Input[Sequence[pulumi.Input[Union['LaneLaneGroupListArgs', 'LaneLaneGroupListArgsDict']]]] lane_group_lists: Swimlane Deployment Group Information.
        :param pulumi.Input[_builtins.str] lane_id: Lane id.
        :param pulumi.Input[_builtins.str] lane_name: Lane name.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] namespace_id_lists: A list of namespaces to which the swimlane has associated deployment groups.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] program_id_lists: Program id list.
        :param pulumi.Input[_builtins.str] remark: Lane Remarks.
        :param pulumi.Input[_builtins.int] update_time: update time.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _LaneState.__new__(_LaneState)

        __props__.__dict__["create_time"] = create_time
        __props__.__dict__["entrance"] = entrance
        __props__.__dict__["lane_group_lists"] = lane_group_lists
        __props__.__dict__["lane_id"] = lane_id
        __props__.__dict__["lane_name"] = lane_name
        __props__.__dict__["namespace_id_lists"] = namespace_id_lists
        __props__.__dict__["program_id_lists"] = program_id_lists
        __props__.__dict__["remark"] = remark
        __props__.__dict__["update_time"] = update_time
        return Lane(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="createTime")
    def create_time(self) -> pulumi.Output[_builtins.int]:
        """
        creation time.
        """
        return pulumi.get(self, "create_time")

    @_builtins.property
    @pulumi.getter
    def entrance(self) -> pulumi.Output[_builtins.bool]:
        """
        Whether to enter the application.
        """
        return pulumi.get(self, "entrance")

    @_builtins.property
    @pulumi.getter(name="laneGroupLists")
    def lane_group_lists(self) -> pulumi.Output[Sequence["outputs.LaneLaneGroupList"]]:
        """
        Swimlane Deployment Group Information.
        """
        return pulumi.get(self, "lane_group_lists")

    @_builtins.property
    @pulumi.getter(name="laneId")
    def lane_id(self) -> pulumi.Output[_builtins.str]:
        """
        Lane id.
        """
        return pulumi.get(self, "lane_id")

    @_builtins.property
    @pulumi.getter(name="laneName")
    def lane_name(self) -> pulumi.Output[_builtins.str]:
        """
        Lane name.
        """
        return pulumi.get(self, "lane_name")

    @_builtins.property
    @pulumi.getter(name="namespaceIdLists")
    def namespace_id_lists(self) -> pulumi.Output[Sequence[_builtins.str]]:
        """
        A list of namespaces to which the swimlane has associated deployment groups.
        """
        return pulumi.get(self, "namespace_id_lists")

    @_builtins.property
    @pulumi.getter(name="programIdLists")
    def program_id_lists(self) -> pulumi.Output[Sequence[_builtins.str] | None]:
        """
        Program id list.
        """
        return pulumi.get(self, "program_id_lists")

    @_builtins.property
    @pulumi.getter
    def remark(self) -> pulumi.Output[_builtins.str]:
        """
        Lane Remarks.
        """
        return pulumi.get(self, "remark")

    @_builtins.property
    @pulumi.getter(name="updateTime")
    def update_time(self) -> pulumi.Output[_builtins.int]:
        """
        update time.
        """
        return pulumi.get(self, "update_time")
