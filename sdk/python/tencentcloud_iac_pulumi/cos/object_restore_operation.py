# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["ObjectRestoreOperation", "ObjectRestoreOperationArgs"]


@pulumi.input_type
class ObjectRestoreOperationArgs:
    def __init__(
        __self__,
        *,
        bucket: pulumi.Input[_builtins.str],
        days: pulumi.Input[_builtins.int],
        key: pulumi.Input[_builtins.str],
        tier: pulumi.Input[_builtins.str]
    ):
        """
        The set of arguments for constructing a ObjectRestoreOperation resource.

        :param pulumi.Input[_builtins.str] bucket: Bucket.
        :param pulumi.Input[_builtins.int] days: Specifies the valid duration of the restored temporary copy in days.
        :param pulumi.Input[_builtins.str] key: Object key.
        :param pulumi.Input[_builtins.str] tier: when restoring, Tier can be specified as the supported recovery model.
               There are three recovery models for recovering archived storage type data, which are:
               - Expedited: quick retrieval mode, and the recovery task can be completed in 1-5 minutes.
               - Standard: standard retrieval mode. Recovery task is completed within 3-5 hours.
               - Bulk: batch retrieval mode, and the recovery task is completed within 5-12 hours.
                 For deep recovery archive storage type data, there are two recovery models, which are:
               - Standard: standard retrieval mode, recovery time is 12-24 hours.
               - Bulk: batch retrieval mode, recovery time is 24-48 hours.
        """
        pulumi.set(__self__, "bucket", bucket)
        pulumi.set(__self__, "days", days)
        pulumi.set(__self__, "key", key)
        pulumi.set(__self__, "tier", tier)

    @_builtins.property
    @pulumi.getter
    def bucket(self) -> pulumi.Input[_builtins.str]:
        """
        Bucket.
        """
        return pulumi.get(self, "bucket")

    @bucket.setter
    def bucket(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "bucket", value)

    @_builtins.property
    @pulumi.getter
    def days(self) -> pulumi.Input[_builtins.int]:
        """
        Specifies the valid duration of the restored temporary copy in days.
        """
        return pulumi.get(self, "days")

    @days.setter
    def days(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "days", value)

    @_builtins.property
    @pulumi.getter
    def key(self) -> pulumi.Input[_builtins.str]:
        """
        Object key.
        """
        return pulumi.get(self, "key")

    @key.setter
    def key(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "key", value)

    @_builtins.property
    @pulumi.getter
    def tier(self) -> pulumi.Input[_builtins.str]:
        """
        when restoring, Tier can be specified as the supported recovery model.
        There are three recovery models for recovering archived storage type data, which are:
        - Expedited: quick retrieval mode, and the recovery task can be completed in 1-5 minutes.
        - Standard: standard retrieval mode. Recovery task is completed within 3-5 hours.
        - Bulk: batch retrieval mode, and the recovery task is completed within 5-12 hours.
          For deep recovery archive storage type data, there are two recovery models, which are:
        - Standard: standard retrieval mode, recovery time is 12-24 hours.
        - Bulk: batch retrieval mode, recovery time is 24-48 hours.
        """
        return pulumi.get(self, "tier")

    @tier.setter
    def tier(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "tier", value)


@pulumi.input_type
class _ObjectRestoreOperationState:
    def __init__(
        __self__,
        *,
        bucket: pulumi.Input[_builtins.str | None] = None,
        days: pulumi.Input[_builtins.int | None] = None,
        key: pulumi.Input[_builtins.str | None] = None,
        tier: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering ObjectRestoreOperation resources.

        :param pulumi.Input[_builtins.str] bucket: Bucket.
        :param pulumi.Input[_builtins.int] days: Specifies the valid duration of the restored temporary copy in days.
        :param pulumi.Input[_builtins.str] key: Object key.
        :param pulumi.Input[_builtins.str] tier: when restoring, Tier can be specified as the supported recovery model.
               There are three recovery models for recovering archived storage type data, which are:
               - Expedited: quick retrieval mode, and the recovery task can be completed in 1-5 minutes.
               - Standard: standard retrieval mode. Recovery task is completed within 3-5 hours.
               - Bulk: batch retrieval mode, and the recovery task is completed within 5-12 hours.
                 For deep recovery archive storage type data, there are two recovery models, which are:
               - Standard: standard retrieval mode, recovery time is 12-24 hours.
               - Bulk: batch retrieval mode, recovery time is 24-48 hours.
        """
        if bucket is not None:
            pulumi.set(__self__, "bucket", bucket)
        if days is not None:
            pulumi.set(__self__, "days", days)
        if key is not None:
            pulumi.set(__self__, "key", key)
        if tier is not None:
            pulumi.set(__self__, "tier", tier)

    @_builtins.property
    @pulumi.getter
    def bucket(self) -> pulumi.Input[_builtins.str | None]:
        """
        Bucket.
        """
        return pulumi.get(self, "bucket")

    @bucket.setter
    def bucket(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "bucket", value)

    @_builtins.property
    @pulumi.getter
    def days(self) -> pulumi.Input[_builtins.int | None]:
        """
        Specifies the valid duration of the restored temporary copy in days.
        """
        return pulumi.get(self, "days")

    @days.setter
    def days(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "days", value)

    @_builtins.property
    @pulumi.getter
    def key(self) -> pulumi.Input[_builtins.str | None]:
        """
        Object key.
        """
        return pulumi.get(self, "key")

    @key.setter
    def key(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "key", value)

    @_builtins.property
    @pulumi.getter
    def tier(self) -> pulumi.Input[_builtins.str | None]:
        """
        when restoring, Tier can be specified as the supported recovery model.
        There are three recovery models for recovering archived storage type data, which are:
        - Expedited: quick retrieval mode, and the recovery task can be completed in 1-5 minutes.
        - Standard: standard retrieval mode. Recovery task is completed within 3-5 hours.
        - Bulk: batch retrieval mode, and the recovery task is completed within 5-12 hours.
          For deep recovery archive storage type data, there are two recovery models, which are:
        - Standard: standard retrieval mode, recovery time is 12-24 hours.
        - Bulk: batch retrieval mode, recovery time is 24-48 hours.
        """
        return pulumi.get(self, "tier")

    @tier.setter
    def tier(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "tier", value)


@pulumi.type_token("tencentcloud:Cos/objectRestoreOperation:ObjectRestoreOperation")
class ObjectRestoreOperation(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        bucket: pulumi.Input[_builtins.str | None] = None,
        days: pulumi.Input[_builtins.int | None] = None,
        key: pulumi.Input[_builtins.str | None] = None,
        tier: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to restore object

        > **NOTE:** The current resource does not support cdc.

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        object_restore = tencentcloud.cos.ObjectRestoreOperation("object_restore",
            bucket="keep-test-1308919341",
            key="test-restore.txt",
            tier="Expedited",
            days=2)
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] bucket: Bucket.
        :param pulumi.Input[_builtins.int] days: Specifies the valid duration of the restored temporary copy in days.
        :param pulumi.Input[_builtins.str] key: Object key.
        :param pulumi.Input[_builtins.str] tier: when restoring, Tier can be specified as the supported recovery model.
               There are three recovery models for recovering archived storage type data, which are:
               - Expedited: quick retrieval mode, and the recovery task can be completed in 1-5 minutes.
               - Standard: standard retrieval mode. Recovery task is completed within 3-5 hours.
               - Bulk: batch retrieval mode, and the recovery task is completed within 5-12 hours.
                 For deep recovery archive storage type data, there are two recovery models, which are:
               - Standard: standard retrieval mode, recovery time is 12-24 hours.
               - Bulk: batch retrieval mode, recovery time is 24-48 hours.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: ObjectRestoreOperationArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to restore object

        > **NOTE:** The current resource does not support cdc.

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        object_restore = tencentcloud.cos.ObjectRestoreOperation("object_restore",
            bucket="keep-test-1308919341",
            key="test-restore.txt",
            tier="Expedited",
            days=2)
        ```


        :param str resource_name: The name of the resource.
        :param ObjectRestoreOperationArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            ObjectRestoreOperationArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        bucket: pulumi.Input[_builtins.str | None] = None,
        days: pulumi.Input[_builtins.int | None] = None,
        key: pulumi.Input[_builtins.str | None] = None,
        tier: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = ObjectRestoreOperationArgs.__new__(ObjectRestoreOperationArgs)

            if bucket is None and not opts.urn:
                raise TypeError("Missing required property 'bucket'")
            __props__.__dict__["bucket"] = bucket
            if days is None and not opts.urn:
                raise TypeError("Missing required property 'days'")
            __props__.__dict__["days"] = days
            if key is None and not opts.urn:
                raise TypeError("Missing required property 'key'")
            __props__.__dict__["key"] = key
            if tier is None and not opts.urn:
                raise TypeError("Missing required property 'tier'")
            __props__.__dict__["tier"] = tier
        super().__init__(
            "tencentcloud:Cos/objectRestoreOperation:ObjectRestoreOperation",
            resource_name,
            __props__,
            opts,
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        bucket: pulumi.Input[_builtins.str | None] = None,
        days: pulumi.Input[_builtins.int | None] = None,
        key: pulumi.Input[_builtins.str | None] = None,
        tier: pulumi.Input[_builtins.str | None] = None,
    ) -> "ObjectRestoreOperation":
        """
        Get an existing ObjectRestoreOperation resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] bucket: Bucket.
        :param pulumi.Input[_builtins.int] days: Specifies the valid duration of the restored temporary copy in days.
        :param pulumi.Input[_builtins.str] key: Object key.
        :param pulumi.Input[_builtins.str] tier: when restoring, Tier can be specified as the supported recovery model.
               There are three recovery models for recovering archived storage type data, which are:
               - Expedited: quick retrieval mode, and the recovery task can be completed in 1-5 minutes.
               - Standard: standard retrieval mode. Recovery task is completed within 3-5 hours.
               - Bulk: batch retrieval mode, and the recovery task is completed within 5-12 hours.
                 For deep recovery archive storage type data, there are two recovery models, which are:
               - Standard: standard retrieval mode, recovery time is 12-24 hours.
               - Bulk: batch retrieval mode, recovery time is 24-48 hours.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _ObjectRestoreOperationState.__new__(_ObjectRestoreOperationState)

        __props__.__dict__["bucket"] = bucket
        __props__.__dict__["days"] = days
        __props__.__dict__["key"] = key
        __props__.__dict__["tier"] = tier
        return ObjectRestoreOperation(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter
    def bucket(self) -> pulumi.Output[_builtins.str]:
        """
        Bucket.
        """
        return pulumi.get(self, "bucket")

    @_builtins.property
    @pulumi.getter
    def days(self) -> pulumi.Output[_builtins.int]:
        """
        Specifies the valid duration of the restored temporary copy in days.
        """
        return pulumi.get(self, "days")

    @_builtins.property
    @pulumi.getter
    def key(self) -> pulumi.Output[_builtins.str]:
        """
        Object key.
        """
        return pulumi.get(self, "key")

    @_builtins.property
    @pulumi.getter
    def tier(self) -> pulumi.Output[_builtins.str]:
        """
        when restoring, Tier can be specified as the supported recovery model.
        There are three recovery models for recovering archived storage type data, which are:
        - Expedited: quick retrieval mode, and the recovery task can be completed in 1-5 minutes.
        - Standard: standard retrieval mode. Recovery task is completed within 3-5 hours.
        - Bulk: batch retrieval mode, and the recovery task is completed within 5-12 hours.
          For deep recovery archive storage type data, there are two recovery models, which are:
        - Standard: standard retrieval mode, recovery time is 12-24 hours.
        - Bulk: batch retrieval mode, recovery time is 24-48 hours.
        """
        return pulumi.get(self, "tier")
