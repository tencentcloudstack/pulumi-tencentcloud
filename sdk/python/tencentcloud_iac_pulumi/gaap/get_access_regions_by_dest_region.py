# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs

__all__ = [
    "AwaitableGetAccessRegionsByDestRegionResult",
    "GetAccessRegionsByDestRegionResult",
    "get_access_regions_by_dest_region",
    "get_access_regions_by_dest_region_output",
]


@pulumi.output_type
class GetAccessRegionsByDestRegionResult:
    """
    A collection of values returned by getAccessRegionsByDestRegion.
    """

    def __init__(
        __self__,
        access_region_sets=None,
        dest_region=None,
        id=None,
        ip_address_version=None,
        package_type=None,
        result_output_file=None,
    ):
        if access_region_sets and not isinstance(access_region_sets, list):
            raise TypeError("Expected argument 'access_region_sets' to be a list")
        pulumi.set(__self__, "access_region_sets", access_region_sets)
        if dest_region and not isinstance(dest_region, str):
            raise TypeError("Expected argument 'dest_region' to be a str")
        pulumi.set(__self__, "dest_region", dest_region)
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)
        if ip_address_version and not isinstance(ip_address_version, str):
            raise TypeError("Expected argument 'ip_address_version' to be a str")
        pulumi.set(__self__, "ip_address_version", ip_address_version)
        if package_type and not isinstance(package_type, str):
            raise TypeError("Expected argument 'package_type' to be a str")
        pulumi.set(__self__, "package_type", package_type)
        if result_output_file and not isinstance(result_output_file, str):
            raise TypeError("Expected argument 'result_output_file' to be a str")
        pulumi.set(__self__, "result_output_file", result_output_file)

    @_builtins.property
    @pulumi.getter(name="accessRegionSets")
    def access_region_sets(
        self,
    ) -> Sequence["outputs.GetAccessRegionsByDestRegionAccessRegionSetResult"]:
        """
        List of available acceleration zone information.
        """
        return pulumi.get(self, "access_region_sets")

    @_builtins.property
    @pulumi.getter(name="destRegion")
    def dest_region(self) -> _builtins.str:
        return pulumi.get(self, "dest_region")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The provider-assigned unique ID for this managed resource.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="ipAddressVersion")
    def ip_address_version(self) -> _builtins.str | None:
        return pulumi.get(self, "ip_address_version")

    @_builtins.property
    @pulumi.getter(name="packageType")
    def package_type(self) -> _builtins.str | None:
        return pulumi.get(self, "package_type")

    @_builtins.property
    @pulumi.getter(name="resultOutputFile")
    def result_output_file(self) -> _builtins.str | None:
        return pulumi.get(self, "result_output_file")


class AwaitableGetAccessRegionsByDestRegionResult(GetAccessRegionsByDestRegionResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetAccessRegionsByDestRegionResult(
            access_region_sets=self.access_region_sets,
            dest_region=self.dest_region,
            id=self.id,
            ip_address_version=self.ip_address_version,
            package_type=self.package_type,
            result_output_file=self.result_output_file,
        )


def get_access_regions_by_dest_region(
    dest_region: _builtins.str | None = None,
    ip_address_version: _builtins.str | None = None,
    package_type: _builtins.str | None = None,
    result_output_file: _builtins.str | None = None,
    opts: pulumi.InvokeOptions | None = None,
) -> AwaitableGetAccessRegionsByDestRegionResult:
    """
    Use this data source to query detailed information of gaap access regions by dest region

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    access_regions_by_dest_region = tencentcloud.gaap.get_access_regions_by_dest_region(dest_region="SouthChina")
    ```


    :param _builtins.str dest_region: Origin region.
    :param _builtins.str ip_address_version: IP version, can be taken as IPv4 or IPv6, with a default value of IPv4.
    :param _builtins.str package_type: Channel package type, where Thunder represents a standard proxy group, Accelerator represents a game accelerator proxy, and CrossBorder represents a cross-border proxy.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["destRegion"] = dest_region
    __args__["ipAddressVersion"] = ip_address_version
    __args__["packageType"] = package_type
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke(
        "tencentcloud:Gaap/getAccessRegionsByDestRegion:getAccessRegionsByDestRegion",
        __args__,
        opts=opts,
        typ=GetAccessRegionsByDestRegionResult,
    ).value

    return AwaitableGetAccessRegionsByDestRegionResult(
        access_region_sets=pulumi.get(__ret__, "access_region_sets"),
        dest_region=pulumi.get(__ret__, "dest_region"),
        id=pulumi.get(__ret__, "id"),
        ip_address_version=pulumi.get(__ret__, "ip_address_version"),
        package_type=pulumi.get(__ret__, "package_type"),
        result_output_file=pulumi.get(__ret__, "result_output_file"),
    )


def get_access_regions_by_dest_region_output(
    dest_region: pulumi.Input[_builtins.str | None] = None,
    ip_address_version: pulumi.Input[_builtins.str | None] = None,
    package_type: pulumi.Input[_builtins.str | None] = None,
    result_output_file: pulumi.Input[_builtins.str | None] = None,
    opts: pulumi.InvokeOptions | pulumi.InvokeOutputOptions | None = None,
) -> pulumi.Output[GetAccessRegionsByDestRegionResult]:
    """
    Use this data source to query detailed information of gaap access regions by dest region

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    access_regions_by_dest_region = tencentcloud.gaap.get_access_regions_by_dest_region(dest_region="SouthChina")
    ```


    :param _builtins.str dest_region: Origin region.
    :param _builtins.str ip_address_version: IP version, can be taken as IPv4 or IPv6, with a default value of IPv4.
    :param _builtins.str package_type: Channel package type, where Thunder represents a standard proxy group, Accelerator represents a game accelerator proxy, and CrossBorder represents a cross-border proxy.
    :param _builtins.str result_output_file: Used to save results.
    """
    __args__ = dict()
    __args__["destRegion"] = dest_region
    __args__["ipAddressVersion"] = ip_address_version
    __args__["packageType"] = package_type
    __args__["resultOutputFile"] = result_output_file
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output(
        "tencentcloud:Gaap/getAccessRegionsByDestRegion:getAccessRegionsByDestRegion",
        __args__,
        opts=opts,
        typ=GetAccessRegionsByDestRegionResult,
    )
    return __ret__.apply(
        lambda __response__: GetAccessRegionsByDestRegionResult(
            access_region_sets=pulumi.get(__response__, "access_region_sets"),
            dest_region=pulumi.get(__response__, "dest_region"),
            id=pulumi.get(__response__, "id"),
            ip_address_version=pulumi.get(__response__, "ip_address_version"),
            package_type=pulumi.get(__response__, "package_type"),
            result_output_file=pulumi.get(__response__, "result_output_file"),
        )
    )
