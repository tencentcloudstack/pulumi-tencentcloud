# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["MountPoint", "MountPointArgs"]


@pulumi.input_type
class MountPointArgs:
    def __init__(
        __self__,
        *,
        file_system_id: pulumi.Input[_builtins.str],
        mount_point_name: pulumi.Input[_builtins.str],
        mount_point_status: pulumi.Input[_builtins.int]
    ):
        """
        The set of arguments for constructing a MountPoint resource.

        :param pulumi.Input[_builtins.str] file_system_id: file system id you want to mount.
        :param pulumi.Input[_builtins.str] mount_point_name: mount point name.
        :param pulumi.Input[_builtins.int] mount_point_status: mount status 1:open, 2:close.
        """
        pulumi.set(__self__, "file_system_id", file_system_id)
        pulumi.set(__self__, "mount_point_name", mount_point_name)
        pulumi.set(__self__, "mount_point_status", mount_point_status)

    @_builtins.property
    @pulumi.getter(name="fileSystemId")
    def file_system_id(self) -> pulumi.Input[_builtins.str]:
        """
        file system id you want to mount.
        """
        return pulumi.get(self, "file_system_id")

    @file_system_id.setter
    def file_system_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "file_system_id", value)

    @_builtins.property
    @pulumi.getter(name="mountPointName")
    def mount_point_name(self) -> pulumi.Input[_builtins.str]:
        """
        mount point name.
        """
        return pulumi.get(self, "mount_point_name")

    @mount_point_name.setter
    def mount_point_name(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "mount_point_name", value)

    @_builtins.property
    @pulumi.getter(name="mountPointStatus")
    def mount_point_status(self) -> pulumi.Input[_builtins.int]:
        """
        mount status 1:open, 2:close.
        """
        return pulumi.get(self, "mount_point_status")

    @mount_point_status.setter
    def mount_point_status(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "mount_point_status", value)


@pulumi.input_type
class _MountPointState:
    def __init__(
        __self__,
        *,
        file_system_id: pulumi.Input[_builtins.str | None] = None,
        mount_point_name: pulumi.Input[_builtins.str | None] = None,
        mount_point_status: pulumi.Input[_builtins.int | None] = None
    ):
        """
        Input properties used for looking up and filtering MountPoint resources.

        :param pulumi.Input[_builtins.str] file_system_id: file system id you want to mount.
        :param pulumi.Input[_builtins.str] mount_point_name: mount point name.
        :param pulumi.Input[_builtins.int] mount_point_status: mount status 1:open, 2:close.
        """
        if file_system_id is not None:
            pulumi.set(__self__, "file_system_id", file_system_id)
        if mount_point_name is not None:
            pulumi.set(__self__, "mount_point_name", mount_point_name)
        if mount_point_status is not None:
            pulumi.set(__self__, "mount_point_status", mount_point_status)

    @_builtins.property
    @pulumi.getter(name="fileSystemId")
    def file_system_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        file system id you want to mount.
        """
        return pulumi.get(self, "file_system_id")

    @file_system_id.setter
    def file_system_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "file_system_id", value)

    @_builtins.property
    @pulumi.getter(name="mountPointName")
    def mount_point_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        mount point name.
        """
        return pulumi.get(self, "mount_point_name")

    @mount_point_name.setter
    def mount_point_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "mount_point_name", value)

    @_builtins.property
    @pulumi.getter(name="mountPointStatus")
    def mount_point_status(self) -> pulumi.Input[_builtins.int | None]:
        """
        mount status 1:open, 2:close.
        """
        return pulumi.get(self, "mount_point_status")

    @mount_point_status.setter
    def mount_point_status(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "mount_point_status", value)


@pulumi.type_token("tencentcloud:Chdfs/mountPoint:MountPoint")
class MountPoint(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        file_system_id: pulumi.Input[_builtins.str | None] = None,
        mount_point_name: pulumi.Input[_builtins.str | None] = None,
        mount_point_status: pulumi.Input[_builtins.int | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a chdfs mount_point

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        mount_point = tencentcloud.chdfs.MountPoint("mount_point",
            file_system_id="f14mpfy5lh4e",
            mount_point_name="terraform-test",
            mount_point_status=1)
        ```

        ## Import

        chdfs mount_point can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Chdfs/mountPoint:MountPoint mount_point mount_point_id
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] file_system_id: file system id you want to mount.
        :param pulumi.Input[_builtins.str] mount_point_name: mount point name.
        :param pulumi.Input[_builtins.int] mount_point_status: mount status 1:open, 2:close.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: MountPointArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a chdfs mount_point

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        mount_point = tencentcloud.chdfs.MountPoint("mount_point",
            file_system_id="f14mpfy5lh4e",
            mount_point_name="terraform-test",
            mount_point_status=1)
        ```

        ## Import

        chdfs mount_point can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Chdfs/mountPoint:MountPoint mount_point mount_point_id
        ```


        :param str resource_name: The name of the resource.
        :param MountPointArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            MountPointArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        file_system_id: pulumi.Input[_builtins.str | None] = None,
        mount_point_name: pulumi.Input[_builtins.str | None] = None,
        mount_point_status: pulumi.Input[_builtins.int | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = MountPointArgs.__new__(MountPointArgs)

            if file_system_id is None and not opts.urn:
                raise TypeError("Missing required property 'file_system_id'")
            __props__.__dict__["file_system_id"] = file_system_id
            if mount_point_name is None and not opts.urn:
                raise TypeError("Missing required property 'mount_point_name'")
            __props__.__dict__["mount_point_name"] = mount_point_name
            if mount_point_status is None and not opts.urn:
                raise TypeError("Missing required property 'mount_point_status'")
            __props__.__dict__["mount_point_status"] = mount_point_status
        super().__init__(
            "tencentcloud:Chdfs/mountPoint:MountPoint", resource_name, __props__, opts
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        file_system_id: pulumi.Input[_builtins.str | None] = None,
        mount_point_name: pulumi.Input[_builtins.str | None] = None,
        mount_point_status: pulumi.Input[_builtins.int | None] = None,
    ) -> "MountPoint":
        """
        Get an existing MountPoint resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] file_system_id: file system id you want to mount.
        :param pulumi.Input[_builtins.str] mount_point_name: mount point name.
        :param pulumi.Input[_builtins.int] mount_point_status: mount status 1:open, 2:close.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _MountPointState.__new__(_MountPointState)

        __props__.__dict__["file_system_id"] = file_system_id
        __props__.__dict__["mount_point_name"] = mount_point_name
        __props__.__dict__["mount_point_status"] = mount_point_status
        return MountPoint(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="fileSystemId")
    def file_system_id(self) -> pulumi.Output[_builtins.str]:
        """
        file system id you want to mount.
        """
        return pulumi.get(self, "file_system_id")

    @_builtins.property
    @pulumi.getter(name="mountPointName")
    def mount_point_name(self) -> pulumi.Output[_builtins.str]:
        """
        mount point name.
        """
        return pulumi.get(self, "mount_point_name")

    @_builtins.property
    @pulumi.getter(name="mountPointStatus")
    def mount_point_status(self) -> pulumi.Output[_builtins.int]:
        """
        mount status 1:open, 2:close.
        """
        return pulumi.get(self, "mount_point_status")
