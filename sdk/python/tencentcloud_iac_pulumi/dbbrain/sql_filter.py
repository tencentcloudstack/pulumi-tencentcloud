# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import Optional, Union, overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ["SqlFilter", "SqlFilterArgs"]


@pulumi.input_type
class SqlFilterArgs:
    def __init__(
        __self__,
        *,
        duration: pulumi.Input[_builtins.int],
        filter_key: pulumi.Input[_builtins.str],
        instance_id: pulumi.Input[_builtins.str],
        max_concurrency: pulumi.Input[_builtins.int],
        session_token: pulumi.Input["SqlFilterSessionTokenArgs"],
        sql_type: pulumi.Input[_builtins.str],
        product: pulumi.Input[_builtins.str | None] = None,
        status: pulumi.Input[_builtins.str | None] = None
    ):
        """
        The set of arguments for constructing a SqlFilter resource.

        :param pulumi.Input[_builtins.int] duration: filter duration.
        :param pulumi.Input[_builtins.str] filter_key: filter key.
        :param pulumi.Input[_builtins.str] instance_id: instance id.
        :param pulumi.Input[_builtins.int] max_concurrency: maximum concurreny.
        :param pulumi.Input['SqlFilterSessionTokenArgs'] session_token: session token.
        :param pulumi.Input[_builtins.str] sql_type: sql type, optional value is SELECT, UPDATE, DELETE, INSERT, REPLACE.
        :param pulumi.Input[_builtins.str] product: product, optional value is &amp;#39;mysql&amp;#39;, &amp;#39;cynosdb&amp;#39;.
        :param pulumi.Input[_builtins.str] status: filter status.
        """
        pulumi.set(__self__, "duration", duration)
        pulumi.set(__self__, "filter_key", filter_key)
        pulumi.set(__self__, "instance_id", instance_id)
        pulumi.set(__self__, "max_concurrency", max_concurrency)
        pulumi.set(__self__, "session_token", session_token)
        pulumi.set(__self__, "sql_type", sql_type)
        if product is not None:
            pulumi.set(__self__, "product", product)
        if status is not None:
            pulumi.set(__self__, "status", status)

    @_builtins.property
    @pulumi.getter
    def duration(self) -> pulumi.Input[_builtins.int]:
        """
        filter duration.
        """
        return pulumi.get(self, "duration")

    @duration.setter
    def duration(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "duration", value)

    @_builtins.property
    @pulumi.getter(name="filterKey")
    def filter_key(self) -> pulumi.Input[_builtins.str]:
        """
        filter key.
        """
        return pulumi.get(self, "filter_key")

    @filter_key.setter
    def filter_key(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "filter_key", value)

    @_builtins.property
    @pulumi.getter(name="instanceId")
    def instance_id(self) -> pulumi.Input[_builtins.str]:
        """
        instance id.
        """
        return pulumi.get(self, "instance_id")

    @instance_id.setter
    def instance_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "instance_id", value)

    @_builtins.property
    @pulumi.getter(name="maxConcurrency")
    def max_concurrency(self) -> pulumi.Input[_builtins.int]:
        """
        maximum concurreny.
        """
        return pulumi.get(self, "max_concurrency")

    @max_concurrency.setter
    def max_concurrency(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "max_concurrency", value)

    @_builtins.property
    @pulumi.getter(name="sessionToken")
    def session_token(self) -> pulumi.Input["SqlFilterSessionTokenArgs"]:
        """
        session token.
        """
        return pulumi.get(self, "session_token")

    @session_token.setter
    def session_token(self, value: pulumi.Input["SqlFilterSessionTokenArgs"]):
        pulumi.set(self, "session_token", value)

    @_builtins.property
    @pulumi.getter(name="sqlType")
    def sql_type(self) -> pulumi.Input[_builtins.str]:
        """
        sql type, optional value is SELECT, UPDATE, DELETE, INSERT, REPLACE.
        """
        return pulumi.get(self, "sql_type")

    @sql_type.setter
    def sql_type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "sql_type", value)

    @_builtins.property
    @pulumi.getter
    def product(self) -> pulumi.Input[_builtins.str | None]:
        """
        product, optional value is &amp;#39;mysql&amp;#39;, &amp;#39;cynosdb&amp;#39;.
        """
        return pulumi.get(self, "product")

    @product.setter
    def product(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "product", value)

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Input[_builtins.str | None]:
        """
        filter status.
        """
        return pulumi.get(self, "status")

    @status.setter
    def status(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "status", value)


@pulumi.input_type
class _SqlFilterState:
    def __init__(
        __self__,
        *,
        duration: pulumi.Input[_builtins.int | None] = None,
        filter_id: pulumi.Input[_builtins.int | None] = None,
        filter_key: pulumi.Input[_builtins.str | None] = None,
        instance_id: pulumi.Input[_builtins.str | None] = None,
        max_concurrency: pulumi.Input[_builtins.int | None] = None,
        product: pulumi.Input[_builtins.str | None] = None,
        session_token: pulumi.Input[Optional["SqlFilterSessionTokenArgs"]] = None,
        sql_type: pulumi.Input[_builtins.str | None] = None,
        status: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering SqlFilter resources.

        :param pulumi.Input[_builtins.int] duration: filter duration.
        :param pulumi.Input[_builtins.int] filter_id: filter id.
        :param pulumi.Input[_builtins.str] filter_key: filter key.
        :param pulumi.Input[_builtins.str] instance_id: instance id.
        :param pulumi.Input[_builtins.int] max_concurrency: maximum concurreny.
        :param pulumi.Input[_builtins.str] product: product, optional value is &amp;#39;mysql&amp;#39;, &amp;#39;cynosdb&amp;#39;.
        :param pulumi.Input['SqlFilterSessionTokenArgs'] session_token: session token.
        :param pulumi.Input[_builtins.str] sql_type: sql type, optional value is SELECT, UPDATE, DELETE, INSERT, REPLACE.
        :param pulumi.Input[_builtins.str] status: filter status.
        """
        if duration is not None:
            pulumi.set(__self__, "duration", duration)
        if filter_id is not None:
            pulumi.set(__self__, "filter_id", filter_id)
        if filter_key is not None:
            pulumi.set(__self__, "filter_key", filter_key)
        if instance_id is not None:
            pulumi.set(__self__, "instance_id", instance_id)
        if max_concurrency is not None:
            pulumi.set(__self__, "max_concurrency", max_concurrency)
        if product is not None:
            pulumi.set(__self__, "product", product)
        if session_token is not None:
            pulumi.set(__self__, "session_token", session_token)
        if sql_type is not None:
            pulumi.set(__self__, "sql_type", sql_type)
        if status is not None:
            pulumi.set(__self__, "status", status)

    @_builtins.property
    @pulumi.getter
    def duration(self) -> pulumi.Input[_builtins.int | None]:
        """
        filter duration.
        """
        return pulumi.get(self, "duration")

    @duration.setter
    def duration(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "duration", value)

    @_builtins.property
    @pulumi.getter(name="filterId")
    def filter_id(self) -> pulumi.Input[_builtins.int | None]:
        """
        filter id.
        """
        return pulumi.get(self, "filter_id")

    @filter_id.setter
    def filter_id(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "filter_id", value)

    @_builtins.property
    @pulumi.getter(name="filterKey")
    def filter_key(self) -> pulumi.Input[_builtins.str | None]:
        """
        filter key.
        """
        return pulumi.get(self, "filter_key")

    @filter_key.setter
    def filter_key(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "filter_key", value)

    @_builtins.property
    @pulumi.getter(name="instanceId")
    def instance_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        instance id.
        """
        return pulumi.get(self, "instance_id")

    @instance_id.setter
    def instance_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "instance_id", value)

    @_builtins.property
    @pulumi.getter(name="maxConcurrency")
    def max_concurrency(self) -> pulumi.Input[_builtins.int | None]:
        """
        maximum concurreny.
        """
        return pulumi.get(self, "max_concurrency")

    @max_concurrency.setter
    def max_concurrency(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "max_concurrency", value)

    @_builtins.property
    @pulumi.getter
    def product(self) -> pulumi.Input[_builtins.str | None]:
        """
        product, optional value is &amp;#39;mysql&amp;#39;, &amp;#39;cynosdb&amp;#39;.
        """
        return pulumi.get(self, "product")

    @product.setter
    def product(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "product", value)

    @_builtins.property
    @pulumi.getter(name="sessionToken")
    def session_token(self) -> pulumi.Input[Optional["SqlFilterSessionTokenArgs"]]:
        """
        session token.
        """
        return pulumi.get(self, "session_token")

    @session_token.setter
    def session_token(self, value: pulumi.Input[Optional["SqlFilterSessionTokenArgs"]]):
        pulumi.set(self, "session_token", value)

    @_builtins.property
    @pulumi.getter(name="sqlType")
    def sql_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        sql type, optional value is SELECT, UPDATE, DELETE, INSERT, REPLACE.
        """
        return pulumi.get(self, "sql_type")

    @sql_type.setter
    def sql_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "sql_type", value)

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Input[_builtins.str | None]:
        """
        filter status.
        """
        return pulumi.get(self, "status")

    @status.setter
    def status(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "status", value)


@pulumi.type_token("tencentcloud:Dbbrain/sqlFilter:SqlFilter")
class SqlFilter(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        duration: pulumi.Input[_builtins.int | None] = None,
        filter_key: pulumi.Input[_builtins.str | None] = None,
        instance_id: pulumi.Input[_builtins.str | None] = None,
        max_concurrency: pulumi.Input[_builtins.int | None] = None,
        product: pulumi.Input[_builtins.str | None] = None,
        session_token: pulumi.Input[
            Union["SqlFilterSessionTokenArgs", "SqlFilterSessionTokenArgsDict"] | None
        ] = None,
        sql_type: pulumi.Input[_builtins.str | None] = None,
        status: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a dbbrain sql_filter.

        ## Example Usage

        ```python
        import pulumi
        import pulumi_tencentcloud as tencentcloud
        import tencentcloud_iac_pulumi as tencentcloud

        config = pulumi.Config()
        availability_zone = config.get("availabilityZone")
        if availability_zone is None:
            availability_zone = "ap-guangzhou-3"
        region = config.get("region")
        if region is None:
            region = "ap-guangzhou"
        mysql = tencentcloud.mysql.get_instance(instance_name="instance_name")
        mysql_id = mysql.instance_lists[0].mysql_id
        sql_filter = tencentcloud.dbbrain.SqlFilter("sql_filter",
            instance_id=mysql_id,
            session_token={
                "user": "test",
                "password": "===password===",
            },
            sql_type="SELECT",
            filter_key="filter_key",
            max_concurrency=10,
            duration=3600)
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.int] duration: filter duration.
        :param pulumi.Input[_builtins.str] filter_key: filter key.
        :param pulumi.Input[_builtins.str] instance_id: instance id.
        :param pulumi.Input[_builtins.int] max_concurrency: maximum concurreny.
        :param pulumi.Input[_builtins.str] product: product, optional value is &amp;#39;mysql&amp;#39;, &amp;#39;cynosdb&amp;#39;.
        :param pulumi.Input[Union['SqlFilterSessionTokenArgs', 'SqlFilterSessionTokenArgsDict']] session_token: session token.
        :param pulumi.Input[_builtins.str] sql_type: sql type, optional value is SELECT, UPDATE, DELETE, INSERT, REPLACE.
        :param pulumi.Input[_builtins.str] status: filter status.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: SqlFilterArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a dbbrain sql_filter.

        ## Example Usage

        ```python
        import pulumi
        import pulumi_tencentcloud as tencentcloud
        import tencentcloud_iac_pulumi as tencentcloud

        config = pulumi.Config()
        availability_zone = config.get("availabilityZone")
        if availability_zone is None:
            availability_zone = "ap-guangzhou-3"
        region = config.get("region")
        if region is None:
            region = "ap-guangzhou"
        mysql = tencentcloud.mysql.get_instance(instance_name="instance_name")
        mysql_id = mysql.instance_lists[0].mysql_id
        sql_filter = tencentcloud.dbbrain.SqlFilter("sql_filter",
            instance_id=mysql_id,
            session_token={
                "user": "test",
                "password": "===password===",
            },
            sql_type="SELECT",
            filter_key="filter_key",
            max_concurrency=10,
            duration=3600)
        ```


        :param str resource_name: The name of the resource.
        :param SqlFilterArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            SqlFilterArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        duration: pulumi.Input[_builtins.int | None] = None,
        filter_key: pulumi.Input[_builtins.str | None] = None,
        instance_id: pulumi.Input[_builtins.str | None] = None,
        max_concurrency: pulumi.Input[_builtins.int | None] = None,
        product: pulumi.Input[_builtins.str | None] = None,
        session_token: pulumi.Input[
            Union["SqlFilterSessionTokenArgs", "SqlFilterSessionTokenArgsDict"] | None
        ] = None,
        sql_type: pulumi.Input[_builtins.str | None] = None,
        status: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = SqlFilterArgs.__new__(SqlFilterArgs)

            if duration is None and not opts.urn:
                raise TypeError("Missing required property 'duration'")
            __props__.__dict__["duration"] = duration
            if filter_key is None and not opts.urn:
                raise TypeError("Missing required property 'filter_key'")
            __props__.__dict__["filter_key"] = filter_key
            if instance_id is None and not opts.urn:
                raise TypeError("Missing required property 'instance_id'")
            __props__.__dict__["instance_id"] = instance_id
            if max_concurrency is None and not opts.urn:
                raise TypeError("Missing required property 'max_concurrency'")
            __props__.__dict__["max_concurrency"] = max_concurrency
            __props__.__dict__["product"] = product
            if session_token is None and not opts.urn:
                raise TypeError("Missing required property 'session_token'")
            __props__.__dict__["session_token"] = session_token
            if sql_type is None and not opts.urn:
                raise TypeError("Missing required property 'sql_type'")
            __props__.__dict__["sql_type"] = sql_type
            __props__.__dict__["status"] = status
            __props__.__dict__["filter_id"] = None
        super().__init__(
            "tencentcloud:Dbbrain/sqlFilter:SqlFilter", resource_name, __props__, opts
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        duration: pulumi.Input[_builtins.int | None] = None,
        filter_id: pulumi.Input[_builtins.int | None] = None,
        filter_key: pulumi.Input[_builtins.str | None] = None,
        instance_id: pulumi.Input[_builtins.str | None] = None,
        max_concurrency: pulumi.Input[_builtins.int | None] = None,
        product: pulumi.Input[_builtins.str | None] = None,
        session_token: pulumi.Input[
            Union["SqlFilterSessionTokenArgs", "SqlFilterSessionTokenArgsDict"] | None
        ] = None,
        sql_type: pulumi.Input[_builtins.str | None] = None,
        status: pulumi.Input[_builtins.str | None] = None,
    ) -> "SqlFilter":
        """
        Get an existing SqlFilter resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.int] duration: filter duration.
        :param pulumi.Input[_builtins.int] filter_id: filter id.
        :param pulumi.Input[_builtins.str] filter_key: filter key.
        :param pulumi.Input[_builtins.str] instance_id: instance id.
        :param pulumi.Input[_builtins.int] max_concurrency: maximum concurreny.
        :param pulumi.Input[_builtins.str] product: product, optional value is &amp;#39;mysql&amp;#39;, &amp;#39;cynosdb&amp;#39;.
        :param pulumi.Input[Union['SqlFilterSessionTokenArgs', 'SqlFilterSessionTokenArgsDict']] session_token: session token.
        :param pulumi.Input[_builtins.str] sql_type: sql type, optional value is SELECT, UPDATE, DELETE, INSERT, REPLACE.
        :param pulumi.Input[_builtins.str] status: filter status.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _SqlFilterState.__new__(_SqlFilterState)

        __props__.__dict__["duration"] = duration
        __props__.__dict__["filter_id"] = filter_id
        __props__.__dict__["filter_key"] = filter_key
        __props__.__dict__["instance_id"] = instance_id
        __props__.__dict__["max_concurrency"] = max_concurrency
        __props__.__dict__["product"] = product
        __props__.__dict__["session_token"] = session_token
        __props__.__dict__["sql_type"] = sql_type
        __props__.__dict__["status"] = status
        return SqlFilter(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter
    def duration(self) -> pulumi.Output[_builtins.int]:
        """
        filter duration.
        """
        return pulumi.get(self, "duration")

    @_builtins.property
    @pulumi.getter(name="filterId")
    def filter_id(self) -> pulumi.Output[_builtins.int]:
        """
        filter id.
        """
        return pulumi.get(self, "filter_id")

    @_builtins.property
    @pulumi.getter(name="filterKey")
    def filter_key(self) -> pulumi.Output[_builtins.str]:
        """
        filter key.
        """
        return pulumi.get(self, "filter_key")

    @_builtins.property
    @pulumi.getter(name="instanceId")
    def instance_id(self) -> pulumi.Output[_builtins.str]:
        """
        instance id.
        """
        return pulumi.get(self, "instance_id")

    @_builtins.property
    @pulumi.getter(name="maxConcurrency")
    def max_concurrency(self) -> pulumi.Output[_builtins.int]:
        """
        maximum concurreny.
        """
        return pulumi.get(self, "max_concurrency")

    @_builtins.property
    @pulumi.getter
    def product(self) -> pulumi.Output[_builtins.str | None]:
        """
        product, optional value is &amp;#39;mysql&amp;#39;, &amp;#39;cynosdb&amp;#39;.
        """
        return pulumi.get(self, "product")

    @_builtins.property
    @pulumi.getter(name="sessionToken")
    def session_token(self) -> pulumi.Output["outputs.SqlFilterSessionToken"]:
        """
        session token.
        """
        return pulumi.get(self, "session_token")

    @_builtins.property
    @pulumi.getter(name="sqlType")
    def sql_type(self) -> pulumi.Output[_builtins.str]:
        """
        sql type, optional value is SELECT, UPDATE, DELETE, INSERT, REPLACE.
        """
        return pulumi.get(self, "sql_type")

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Output[_builtins.str]:
        """
        filter status.
        """
        return pulumi.get(self, "status")
