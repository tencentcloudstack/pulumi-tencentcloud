# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["Wan", "WanArgs"]


@pulumi.input_type
class WanArgs:
    def __init__(
        __self__,
        *,
        cluster_id: pulumi.Input[_builtins.str],
        instance_grp_id: pulumi.Input[_builtins.str]
    ):
        """
        The set of arguments for constructing a Wan resource.

        :param pulumi.Input[_builtins.str] cluster_id: Cluster ID.
        :param pulumi.Input[_builtins.str] instance_grp_id: Instance Group ID.
        """
        pulumi.set(__self__, "cluster_id", cluster_id)
        pulumi.set(__self__, "instance_grp_id", instance_grp_id)

    @_builtins.property
    @pulumi.getter(name="clusterId")
    def cluster_id(self) -> pulumi.Input[_builtins.str]:
        """
        Cluster ID.
        """
        return pulumi.get(self, "cluster_id")

    @cluster_id.setter
    def cluster_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "cluster_id", value)

    @_builtins.property
    @pulumi.getter(name="instanceGrpId")
    def instance_grp_id(self) -> pulumi.Input[_builtins.str]:
        """
        Instance Group ID.
        """
        return pulumi.get(self, "instance_grp_id")

    @instance_grp_id.setter
    def instance_grp_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "instance_grp_id", value)


@pulumi.input_type
class _WanState:
    def __init__(
        __self__,
        *,
        cluster_id: pulumi.Input[_builtins.str | None] = None,
        instance_grp_id: pulumi.Input[_builtins.str | None] = None,
        wan_domain: pulumi.Input[_builtins.str | None] = None,
        wan_ip: pulumi.Input[_builtins.str | None] = None,
        wan_port: pulumi.Input[_builtins.int | None] = None,
        wan_status: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering Wan resources.

        :param pulumi.Input[_builtins.str] cluster_id: Cluster ID.
        :param pulumi.Input[_builtins.str] instance_grp_id: Instance Group ID.
        :param pulumi.Input[_builtins.str] wan_domain: Domain name.
        :param pulumi.Input[_builtins.str] wan_ip: Network ip.
        :param pulumi.Input[_builtins.int] wan_port: Internet port.
        :param pulumi.Input[_builtins.str] wan_status: Internet status.
        """
        if cluster_id is not None:
            pulumi.set(__self__, "cluster_id", cluster_id)
        if instance_grp_id is not None:
            pulumi.set(__self__, "instance_grp_id", instance_grp_id)
        if wan_domain is not None:
            pulumi.set(__self__, "wan_domain", wan_domain)
        if wan_ip is not None:
            pulumi.set(__self__, "wan_ip", wan_ip)
        if wan_port is not None:
            pulumi.set(__self__, "wan_port", wan_port)
        if wan_status is not None:
            pulumi.set(__self__, "wan_status", wan_status)

    @_builtins.property
    @pulumi.getter(name="clusterId")
    def cluster_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Cluster ID.
        """
        return pulumi.get(self, "cluster_id")

    @cluster_id.setter
    def cluster_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "cluster_id", value)

    @_builtins.property
    @pulumi.getter(name="instanceGrpId")
    def instance_grp_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Instance Group ID.
        """
        return pulumi.get(self, "instance_grp_id")

    @instance_grp_id.setter
    def instance_grp_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "instance_grp_id", value)

    @_builtins.property
    @pulumi.getter(name="wanDomain")
    def wan_domain(self) -> pulumi.Input[_builtins.str | None]:
        """
        Domain name.
        """
        return pulumi.get(self, "wan_domain")

    @wan_domain.setter
    def wan_domain(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "wan_domain", value)

    @_builtins.property
    @pulumi.getter(name="wanIp")
    def wan_ip(self) -> pulumi.Input[_builtins.str | None]:
        """
        Network ip.
        """
        return pulumi.get(self, "wan_ip")

    @wan_ip.setter
    def wan_ip(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "wan_ip", value)

    @_builtins.property
    @pulumi.getter(name="wanPort")
    def wan_port(self) -> pulumi.Input[_builtins.int | None]:
        """
        Internet port.
        """
        return pulumi.get(self, "wan_port")

    @wan_port.setter
    def wan_port(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "wan_port", value)

    @_builtins.property
    @pulumi.getter(name="wanStatus")
    def wan_status(self) -> pulumi.Input[_builtins.str | None]:
        """
        Internet status.
        """
        return pulumi.get(self, "wan_status")

    @wan_status.setter
    def wan_status(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "wan_status", value)


@pulumi.type_token("tencentcloud:Cynosdb/wan:Wan")
class Wan(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        cluster_id: pulumi.Input[_builtins.str | None] = None,
        instance_grp_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a cynosdb wan

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        wan = tencentcloud.cynosdb.Wan("wan",
            cluster_id="cynosdbmysql-bws8h88b",
            instance_grp_id="cynosdbmysql-grp-lxav0p9z")
        ```

        ## Import

        cynosdb wan can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Cynosdb/wan:Wan wan cynosdbmysql-bws8h88b#cynosdbmysql-grp-lxav0p9z
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] cluster_id: Cluster ID.
        :param pulumi.Input[_builtins.str] instance_grp_id: Instance Group ID.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: WanArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a cynosdb wan

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        wan = tencentcloud.cynosdb.Wan("wan",
            cluster_id="cynosdbmysql-bws8h88b",
            instance_grp_id="cynosdbmysql-grp-lxav0p9z")
        ```

        ## Import

        cynosdb wan can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Cynosdb/wan:Wan wan cynosdbmysql-bws8h88b#cynosdbmysql-grp-lxav0p9z
        ```


        :param str resource_name: The name of the resource.
        :param WanArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            WanArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        cluster_id: pulumi.Input[_builtins.str | None] = None,
        instance_grp_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = WanArgs.__new__(WanArgs)

            if cluster_id is None and not opts.urn:
                raise TypeError("Missing required property 'cluster_id'")
            __props__.__dict__["cluster_id"] = cluster_id
            if instance_grp_id is None and not opts.urn:
                raise TypeError("Missing required property 'instance_grp_id'")
            __props__.__dict__["instance_grp_id"] = instance_grp_id
            __props__.__dict__["wan_domain"] = None
            __props__.__dict__["wan_ip"] = None
            __props__.__dict__["wan_port"] = None
            __props__.__dict__["wan_status"] = None
        super().__init__("tencentcloud:Cynosdb/wan:Wan", resource_name, __props__, opts)

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        cluster_id: pulumi.Input[_builtins.str | None] = None,
        instance_grp_id: pulumi.Input[_builtins.str | None] = None,
        wan_domain: pulumi.Input[_builtins.str | None] = None,
        wan_ip: pulumi.Input[_builtins.str | None] = None,
        wan_port: pulumi.Input[_builtins.int | None] = None,
        wan_status: pulumi.Input[_builtins.str | None] = None,
    ) -> "Wan":
        """
        Get an existing Wan resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] cluster_id: Cluster ID.
        :param pulumi.Input[_builtins.str] instance_grp_id: Instance Group ID.
        :param pulumi.Input[_builtins.str] wan_domain: Domain name.
        :param pulumi.Input[_builtins.str] wan_ip: Network ip.
        :param pulumi.Input[_builtins.int] wan_port: Internet port.
        :param pulumi.Input[_builtins.str] wan_status: Internet status.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _WanState.__new__(_WanState)

        __props__.__dict__["cluster_id"] = cluster_id
        __props__.__dict__["instance_grp_id"] = instance_grp_id
        __props__.__dict__["wan_domain"] = wan_domain
        __props__.__dict__["wan_ip"] = wan_ip
        __props__.__dict__["wan_port"] = wan_port
        __props__.__dict__["wan_status"] = wan_status
        return Wan(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="clusterId")
    def cluster_id(self) -> pulumi.Output[_builtins.str]:
        """
        Cluster ID.
        """
        return pulumi.get(self, "cluster_id")

    @_builtins.property
    @pulumi.getter(name="instanceGrpId")
    def instance_grp_id(self) -> pulumi.Output[_builtins.str]:
        """
        Instance Group ID.
        """
        return pulumi.get(self, "instance_grp_id")

    @_builtins.property
    @pulumi.getter(name="wanDomain")
    def wan_domain(self) -> pulumi.Output[_builtins.str]:
        """
        Domain name.
        """
        return pulumi.get(self, "wan_domain")

    @_builtins.property
    @pulumi.getter(name="wanIp")
    def wan_ip(self) -> pulumi.Output[_builtins.str]:
        """
        Network ip.
        """
        return pulumi.get(self, "wan_ip")

    @_builtins.property
    @pulumi.getter(name="wanPort")
    def wan_port(self) -> pulumi.Output[_builtins.int]:
        """
        Internet port.
        """
        return pulumi.get(self, "wan_port")

    @_builtins.property
    @pulumi.getter(name="wanStatus")
    def wan_status(self) -> pulumi.Output[_builtins.str]:
        """
        Internet status.
        """
        return pulumi.get(self, "wan_status")
