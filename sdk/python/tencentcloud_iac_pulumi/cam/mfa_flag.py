# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import Optional, Union, overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ["MfaFlag", "MfaFlagArgs"]


@pulumi.input_type
class MfaFlagArgs:
    def __init__(
        __self__,
        *,
        op_uin: pulumi.Input[_builtins.int],
        action_flag: pulumi.Input[Optional["MfaFlagActionFlagArgs"]] = None,
        login_flag: pulumi.Input[Optional["MfaFlagLoginFlagArgs"]] = None
    ):
        """
        The set of arguments for constructing a MfaFlag resource.

        :param pulumi.Input[_builtins.int] op_uin: Operate uin.
        :param pulumi.Input['MfaFlagActionFlagArgs'] action_flag: Action flag setting.
        :param pulumi.Input['MfaFlagLoginFlagArgs'] login_flag: Login flag setting.
        """
        pulumi.set(__self__, "op_uin", op_uin)
        if action_flag is not None:
            pulumi.set(__self__, "action_flag", action_flag)
        if login_flag is not None:
            pulumi.set(__self__, "login_flag", login_flag)

    @_builtins.property
    @pulumi.getter(name="opUin")
    def op_uin(self) -> pulumi.Input[_builtins.int]:
        """
        Operate uin.
        """
        return pulumi.get(self, "op_uin")

    @op_uin.setter
    def op_uin(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "op_uin", value)

    @_builtins.property
    @pulumi.getter(name="actionFlag")
    def action_flag(self) -> pulumi.Input[Optional["MfaFlagActionFlagArgs"]]:
        """
        Action flag setting.
        """
        return pulumi.get(self, "action_flag")

    @action_flag.setter
    def action_flag(self, value: pulumi.Input[Optional["MfaFlagActionFlagArgs"]]):
        pulumi.set(self, "action_flag", value)

    @_builtins.property
    @pulumi.getter(name="loginFlag")
    def login_flag(self) -> pulumi.Input[Optional["MfaFlagLoginFlagArgs"]]:
        """
        Login flag setting.
        """
        return pulumi.get(self, "login_flag")

    @login_flag.setter
    def login_flag(self, value: pulumi.Input[Optional["MfaFlagLoginFlagArgs"]]):
        pulumi.set(self, "login_flag", value)


@pulumi.input_type
class _MfaFlagState:
    def __init__(
        __self__,
        *,
        action_flag: pulumi.Input[Optional["MfaFlagActionFlagArgs"]] = None,
        login_flag: pulumi.Input[Optional["MfaFlagLoginFlagArgs"]] = None,
        op_uin: pulumi.Input[_builtins.int | None] = None
    ):
        """
        Input properties used for looking up and filtering MfaFlag resources.

        :param pulumi.Input['MfaFlagActionFlagArgs'] action_flag: Action flag setting.
        :param pulumi.Input['MfaFlagLoginFlagArgs'] login_flag: Login flag setting.
        :param pulumi.Input[_builtins.int] op_uin: Operate uin.
        """
        if action_flag is not None:
            pulumi.set(__self__, "action_flag", action_flag)
        if login_flag is not None:
            pulumi.set(__self__, "login_flag", login_flag)
        if op_uin is not None:
            pulumi.set(__self__, "op_uin", op_uin)

    @_builtins.property
    @pulumi.getter(name="actionFlag")
    def action_flag(self) -> pulumi.Input[Optional["MfaFlagActionFlagArgs"]]:
        """
        Action flag setting.
        """
        return pulumi.get(self, "action_flag")

    @action_flag.setter
    def action_flag(self, value: pulumi.Input[Optional["MfaFlagActionFlagArgs"]]):
        pulumi.set(self, "action_flag", value)

    @_builtins.property
    @pulumi.getter(name="loginFlag")
    def login_flag(self) -> pulumi.Input[Optional["MfaFlagLoginFlagArgs"]]:
        """
        Login flag setting.
        """
        return pulumi.get(self, "login_flag")

    @login_flag.setter
    def login_flag(self, value: pulumi.Input[Optional["MfaFlagLoginFlagArgs"]]):
        pulumi.set(self, "login_flag", value)

    @_builtins.property
    @pulumi.getter(name="opUin")
    def op_uin(self) -> pulumi.Input[_builtins.int | None]:
        """
        Operate uin.
        """
        return pulumi.get(self, "op_uin")

    @op_uin.setter
    def op_uin(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "op_uin", value)


@pulumi.type_token("tencentcloud:Cam/mfaFlag:MfaFlag")
class MfaFlag(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        action_flag: pulumi.Input[
            Union["MfaFlagActionFlagArgs", "MfaFlagActionFlagArgsDict"] | None
        ] = None,
        login_flag: pulumi.Input[
            Union["MfaFlagLoginFlagArgs", "MfaFlagLoginFlagArgsDict"] | None
        ] = None,
        op_uin: pulumi.Input[_builtins.int | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a CAM mfa flag

        ## Example Usage

        ```python
        import pulumi
        import pulumi_tencentcloud as tencentcloud
        import tencentcloud_iac_pulumi as tencentcloud

        info = tencentcloud.user.get_info()
        example = tencentcloud.cam.MfaFlag("example",
            op_uin=output(info.uin).apply(lambda x: int(x)),
            login_flag={
                "phone": 0,
                "stoken": 1,
                "wechat": 0,
            },
            action_flag={
                "phone": 0,
                "stoken": 1,
                "wechat": 0,
            })
        ```

        ## Import

        CAM mfa flag can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Cam/mfaFlag:MfaFlag example 100037718110
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[Union['MfaFlagActionFlagArgs', 'MfaFlagActionFlagArgsDict']] action_flag: Action flag setting.
        :param pulumi.Input[Union['MfaFlagLoginFlagArgs', 'MfaFlagLoginFlagArgsDict']] login_flag: Login flag setting.
        :param pulumi.Input[_builtins.int] op_uin: Operate uin.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: MfaFlagArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a CAM mfa flag

        ## Example Usage

        ```python
        import pulumi
        import pulumi_tencentcloud as tencentcloud
        import tencentcloud_iac_pulumi as tencentcloud

        info = tencentcloud.user.get_info()
        example = tencentcloud.cam.MfaFlag("example",
            op_uin=output(info.uin).apply(lambda x: int(x)),
            login_flag={
                "phone": 0,
                "stoken": 1,
                "wechat": 0,
            },
            action_flag={
                "phone": 0,
                "stoken": 1,
                "wechat": 0,
            })
        ```

        ## Import

        CAM mfa flag can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Cam/mfaFlag:MfaFlag example 100037718110
        ```


        :param str resource_name: The name of the resource.
        :param MfaFlagArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            MfaFlagArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        action_flag: pulumi.Input[
            Union["MfaFlagActionFlagArgs", "MfaFlagActionFlagArgsDict"] | None
        ] = None,
        login_flag: pulumi.Input[
            Union["MfaFlagLoginFlagArgs", "MfaFlagLoginFlagArgsDict"] | None
        ] = None,
        op_uin: pulumi.Input[_builtins.int | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = MfaFlagArgs.__new__(MfaFlagArgs)

            __props__.__dict__["action_flag"] = action_flag
            __props__.__dict__["login_flag"] = login_flag
            if op_uin is None and not opts.urn:
                raise TypeError("Missing required property 'op_uin'")
            __props__.__dict__["op_uin"] = op_uin
        super().__init__(
            "tencentcloud:Cam/mfaFlag:MfaFlag", resource_name, __props__, opts
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        action_flag: pulumi.Input[
            Union["MfaFlagActionFlagArgs", "MfaFlagActionFlagArgsDict"] | None
        ] = None,
        login_flag: pulumi.Input[
            Union["MfaFlagLoginFlagArgs", "MfaFlagLoginFlagArgsDict"] | None
        ] = None,
        op_uin: pulumi.Input[_builtins.int | None] = None,
    ) -> "MfaFlag":
        """
        Get an existing MfaFlag resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[Union['MfaFlagActionFlagArgs', 'MfaFlagActionFlagArgsDict']] action_flag: Action flag setting.
        :param pulumi.Input[Union['MfaFlagLoginFlagArgs', 'MfaFlagLoginFlagArgsDict']] login_flag: Login flag setting.
        :param pulumi.Input[_builtins.int] op_uin: Operate uin.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _MfaFlagState.__new__(_MfaFlagState)

        __props__.__dict__["action_flag"] = action_flag
        __props__.__dict__["login_flag"] = login_flag
        __props__.__dict__["op_uin"] = op_uin
        return MfaFlag(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="actionFlag")
    def action_flag(self) -> pulumi.Output["outputs.MfaFlagActionFlag"]:
        """
        Action flag setting.
        """
        return pulumi.get(self, "action_flag")

    @_builtins.property
    @pulumi.getter(name="loginFlag")
    def login_flag(self) -> pulumi.Output["outputs.MfaFlagLoginFlag"]:
        """
        Login flag setting.
        """
        return pulumi.get(self, "login_flag")

    @_builtins.property
    @pulumi.getter(name="opUin")
    def op_uin(self) -> pulumi.Output[_builtins.int]:
        """
        Operate uin.
        """
        return pulumi.get(self, "op_uin")
