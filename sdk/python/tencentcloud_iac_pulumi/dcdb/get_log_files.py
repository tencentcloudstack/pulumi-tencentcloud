# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities
from . import outputs

__all__ = [
    'GetLogFilesResult',
    'AwaitableGetLogFilesResult',
    'get_log_files',
    'get_log_files_output',
]

@pulumi.output_type
class GetLogFilesResult:
    """
    A collection of values returned by getLogFiles.
    """
    def __init__(__self__, files=None, id=None, instance_id=None, normal_prefix=None, result_output_file=None, shard_id=None, type=None, vpc_prefix=None):
        if files and not isinstance(files, list):
            raise TypeError("Expected argument 'files' to be a list")
        pulumi.set(__self__, "files", files)
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)
        if instance_id and not isinstance(instance_id, str):
            raise TypeError("Expected argument 'instance_id' to be a str")
        pulumi.set(__self__, "instance_id", instance_id)
        if normal_prefix and not isinstance(normal_prefix, str):
            raise TypeError("Expected argument 'normal_prefix' to be a str")
        pulumi.set(__self__, "normal_prefix", normal_prefix)
        if result_output_file and not isinstance(result_output_file, str):
            raise TypeError("Expected argument 'result_output_file' to be a str")
        pulumi.set(__self__, "result_output_file", result_output_file)
        if shard_id and not isinstance(shard_id, str):
            raise TypeError("Expected argument 'shard_id' to be a str")
        pulumi.set(__self__, "shard_id", shard_id)
        if type and not isinstance(type, int):
            raise TypeError("Expected argument 'type' to be a int")
        pulumi.set(__self__, "type", type)
        if vpc_prefix and not isinstance(vpc_prefix, str):
            raise TypeError("Expected argument 'vpc_prefix' to be a str")
        pulumi.set(__self__, "vpc_prefix", vpc_prefix)

    @_builtins.property
    @pulumi.getter
    def files(self) -> Sequence['outputs.GetLogFilesFileResult']:
        """
        Information such as `uri`, `length`, and `mtime` (modification time).
        """
        return pulumi.get(self, "files")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The provider-assigned unique ID for this managed resource.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="instanceId")
    def instance_id(self) -> _builtins.str:
        return pulumi.get(self, "instance_id")

    @_builtins.property
    @pulumi.getter(name="normalPrefix")
    def normal_prefix(self) -> _builtins.str:
        """
        For an instance in a common network, this prefix plus URI can be used as the download address.
        """
        return pulumi.get(self, "normal_prefix")

    @_builtins.property
    @pulumi.getter(name="resultOutputFile")
    def result_output_file(self) -> Optional[_builtins.str]:
        return pulumi.get(self, "result_output_file")

    @_builtins.property
    @pulumi.getter(name="shardId")
    def shard_id(self) -> _builtins.str:
        return pulumi.get(self, "shard_id")

    @_builtins.property
    @pulumi.getter
    def type(self) -> _builtins.int:
        return pulumi.get(self, "type")

    @_builtins.property
    @pulumi.getter(name="vpcPrefix")
    def vpc_prefix(self) -> _builtins.str:
        """
        For an instance in a VPC, this prefix plus URI can be used as the download address.
        """
        return pulumi.get(self, "vpc_prefix")


class AwaitableGetLogFilesResult(GetLogFilesResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetLogFilesResult(
            files=self.files,
            id=self.id,
            instance_id=self.instance_id,
            normal_prefix=self.normal_prefix,
            result_output_file=self.result_output_file,
            shard_id=self.shard_id,
            type=self.type,
            vpc_prefix=self.vpc_prefix)


def get_log_files(instance_id: Optional[_builtins.str] = None,
                  result_output_file: Optional[_builtins.str] = None,
                  shard_id: Optional[_builtins.str] = None,
                  type: Optional[_builtins.int] = None,
                  opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableGetLogFilesResult:
    """
    Use this data source to query detailed information of dcdb log_files

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    log_files = tencentcloud.Dcdb.get_log_files(instance_id=dcdb_id,
        shard_id="shard-1b5r04az",
        type=1)
    ```


    :param _builtins.str instance_id: Instance ID in the format of `tdsqlshard-ow728lmc`.
    :param _builtins.str result_output_file: Used to save results.
    :param _builtins.str shard_id: Instance shard ID in the format of `shard-rc754ljk`.
    :param _builtins.int type: Requested log type. Valid values: 1 (binlog), 2 (cold backup), 3 (errlog), 4 (slowlog).
    """
    __args__ = dict()
    __args__['instanceId'] = instance_id
    __args__['resultOutputFile'] = result_output_file
    __args__['shardId'] = shard_id
    __args__['type'] = type
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke('tencentcloud:Dcdb/getLogFiles:getLogFiles', __args__, opts=opts, typ=GetLogFilesResult).value

    return AwaitableGetLogFilesResult(
        files=pulumi.get(__ret__, 'files'),
        id=pulumi.get(__ret__, 'id'),
        instance_id=pulumi.get(__ret__, 'instance_id'),
        normal_prefix=pulumi.get(__ret__, 'normal_prefix'),
        result_output_file=pulumi.get(__ret__, 'result_output_file'),
        shard_id=pulumi.get(__ret__, 'shard_id'),
        type=pulumi.get(__ret__, 'type'),
        vpc_prefix=pulumi.get(__ret__, 'vpc_prefix'))
def get_log_files_output(instance_id: Optional[pulumi.Input[_builtins.str]] = None,
                         result_output_file: Optional[pulumi.Input[Optional[_builtins.str]]] = None,
                         shard_id: Optional[pulumi.Input[_builtins.str]] = None,
                         type: Optional[pulumi.Input[_builtins.int]] = None,
                         opts: Optional[Union[pulumi.InvokeOptions, pulumi.InvokeOutputOptions]] = None) -> pulumi.Output[GetLogFilesResult]:
    """
    Use this data source to query detailed information of dcdb log_files

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    log_files = tencentcloud.Dcdb.get_log_files(instance_id=dcdb_id,
        shard_id="shard-1b5r04az",
        type=1)
    ```


    :param _builtins.str instance_id: Instance ID in the format of `tdsqlshard-ow728lmc`.
    :param _builtins.str result_output_file: Used to save results.
    :param _builtins.str shard_id: Instance shard ID in the format of `shard-rc754ljk`.
    :param _builtins.int type: Requested log type. Valid values: 1 (binlog), 2 (cold backup), 3 (errlog), 4 (slowlog).
    """
    __args__ = dict()
    __args__['instanceId'] = instance_id
    __args__['resultOutputFile'] = result_output_file
    __args__['shardId'] = shard_id
    __args__['type'] = type
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output('tencentcloud:Dcdb/getLogFiles:getLogFiles', __args__, opts=opts, typ=GetLogFilesResult)
    return __ret__.apply(lambda __response__: GetLogFilesResult(
        files=pulumi.get(__response__, 'files'),
        id=pulumi.get(__response__, 'id'),
        instance_id=pulumi.get(__response__, 'instance_id'),
        normal_prefix=pulumi.get(__response__, 'normal_prefix'),
        result_output_file=pulumi.get(__response__, 'result_output_file'),
        shard_id=pulumi.get(__response__, 'shard_id'),
        type=pulumi.get(__response__, 'type'),
        vpc_prefix=pulumi.get(__response__, 'vpc_prefix')))
