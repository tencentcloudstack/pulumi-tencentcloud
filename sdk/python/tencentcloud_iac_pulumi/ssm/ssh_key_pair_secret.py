# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Mapping
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["SshKeyPairSecret", "SshKeyPairSecretArgs"]


@pulumi.input_type
class SshKeyPairSecretArgs:
    def __init__(
        __self__,
        *,
        project_id: pulumi.Input[_builtins.int],
        secret_name: pulumi.Input[_builtins.str],
        clean_ssh_key: pulumi.Input[_builtins.bool | None] = None,
        description: pulumi.Input[_builtins.str | None] = None,
        kms_key_id: pulumi.Input[_builtins.str | None] = None,
        ssh_key_name: pulumi.Input[_builtins.str | None] = None,
        status: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None
    ):
        """
        The set of arguments for constructing a SshKeyPairSecret resource.

        :param pulumi.Input[_builtins.int] project_id: ID of the project to which the created SSH key belongs.
        :param pulumi.Input[_builtins.str] secret_name: Secret name, which must be unique in the same region. It can contain 128 bytes of letters, digits, hyphens and underscores and must begin with a letter or digit.
        :param pulumi.Input[_builtins.bool] clean_ssh_key: Specifies whether to delete the SSH key from both the secret and the SSH key list in the CVM console. This field is only take effect when delete SSH key secrets. Valid values: `True`: deletes SSH key from both the secret and SSH key list in the CVM console. Note that the deletion will fail if the SSH key is already bound to a CVM instance.`False`: only deletes the SSH key information in the secret.
        :param pulumi.Input[_builtins.str] description: Description, such as what it is used for. It contains up to 2,048 bytes.
        :param pulumi.Input[_builtins.str] kms_key_id: Specifies a KMS CMK to encrypt the secret.If this parameter is left empty, the CMK created by Secrets Manager by default will be used for encryption.You can also specify a custom KMS CMK created in the same region for encryption.
        :param pulumi.Input[_builtins.str] ssh_key_name: Name of the SSH key pair, which only contains digits, letters and underscores and must start with a digit or letter. The maximum length is 25 characters.
        :param pulumi.Input[_builtins.str] status: Enable or Disable Secret. Valid values is `Enabled` or `Disabled`. Default is `Enabled`.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tags of secret.
        """
        pulumi.set(__self__, "project_id", project_id)
        pulumi.set(__self__, "secret_name", secret_name)
        if clean_ssh_key is not None:
            pulumi.set(__self__, "clean_ssh_key", clean_ssh_key)
        if description is not None:
            pulumi.set(__self__, "description", description)
        if kms_key_id is not None:
            pulumi.set(__self__, "kms_key_id", kms_key_id)
        if ssh_key_name is not None:
            pulumi.set(__self__, "ssh_key_name", ssh_key_name)
        if status is not None:
            pulumi.set(__self__, "status", status)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)

    @_builtins.property
    @pulumi.getter(name="projectId")
    def project_id(self) -> pulumi.Input[_builtins.int]:
        """
        ID of the project to which the created SSH key belongs.
        """
        return pulumi.get(self, "project_id")

    @project_id.setter
    def project_id(self, value: pulumi.Input[_builtins.int]):
        pulumi.set(self, "project_id", value)

    @_builtins.property
    @pulumi.getter(name="secretName")
    def secret_name(self) -> pulumi.Input[_builtins.str]:
        """
        Secret name, which must be unique in the same region. It can contain 128 bytes of letters, digits, hyphens and underscores and must begin with a letter or digit.
        """
        return pulumi.get(self, "secret_name")

    @secret_name.setter
    def secret_name(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "secret_name", value)

    @_builtins.property
    @pulumi.getter(name="cleanSshKey")
    def clean_ssh_key(self) -> pulumi.Input[_builtins.bool | None]:
        """
        Specifies whether to delete the SSH key from both the secret and the SSH key list in the CVM console. This field is only take effect when delete SSH key secrets. Valid values: `True`: deletes SSH key from both the secret and SSH key list in the CVM console. Note that the deletion will fail if the SSH key is already bound to a CVM instance.`False`: only deletes the SSH key information in the secret.
        """
        return pulumi.get(self, "clean_ssh_key")

    @clean_ssh_key.setter
    def clean_ssh_key(self, value: pulumi.Input[_builtins.bool | None]):
        pulumi.set(self, "clean_ssh_key", value)

    @_builtins.property
    @pulumi.getter
    def description(self) -> pulumi.Input[_builtins.str | None]:
        """
        Description, such as what it is used for. It contains up to 2,048 bytes.
        """
        return pulumi.get(self, "description")

    @description.setter
    def description(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "description", value)

    @_builtins.property
    @pulumi.getter(name="kmsKeyId")
    def kms_key_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Specifies a KMS CMK to encrypt the secret.If this parameter is left empty, the CMK created by Secrets Manager by default will be used for encryption.You can also specify a custom KMS CMK created in the same region for encryption.
        """
        return pulumi.get(self, "kms_key_id")

    @kms_key_id.setter
    def kms_key_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "kms_key_id", value)

    @_builtins.property
    @pulumi.getter(name="sshKeyName")
    def ssh_key_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Name of the SSH key pair, which only contains digits, letters and underscores and must start with a digit or letter. The maximum length is 25 characters.
        """
        return pulumi.get(self, "ssh_key_name")

    @ssh_key_name.setter
    def ssh_key_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "ssh_key_name", value)

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Input[_builtins.str | None]:
        """
        Enable or Disable Secret. Valid values is `Enabled` or `Disabled`. Default is `Enabled`.
        """
        return pulumi.get(self, "status")

    @status.setter
    def status(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "status", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]:
        """
        Tags of secret.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(
        self, value: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "tags", value)


@pulumi.input_type
class _SshKeyPairSecretState:
    def __init__(
        __self__,
        *,
        clean_ssh_key: pulumi.Input[_builtins.bool | None] = None,
        create_time: pulumi.Input[_builtins.int | None] = None,
        description: pulumi.Input[_builtins.str | None] = None,
        kms_key_id: pulumi.Input[_builtins.str | None] = None,
        project_id: pulumi.Input[_builtins.int | None] = None,
        resource_id: pulumi.Input[_builtins.str | None] = None,
        secret_name: pulumi.Input[_builtins.str | None] = None,
        secret_type: pulumi.Input[_builtins.int | None] = None,
        ssh_key_id: pulumi.Input[_builtins.str | None] = None,
        ssh_key_name: pulumi.Input[_builtins.str | None] = None,
        status: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None
    ):
        """
        Input properties used for looking up and filtering SshKeyPairSecret resources.

        :param pulumi.Input[_builtins.bool] clean_ssh_key: Specifies whether to delete the SSH key from both the secret and the SSH key list in the CVM console. This field is only take effect when delete SSH key secrets. Valid values: `True`: deletes SSH key from both the secret and SSH key list in the CVM console. Note that the deletion will fail if the SSH key is already bound to a CVM instance.`False`: only deletes the SSH key information in the secret.
        :param pulumi.Input[_builtins.int] create_time: Credential creation time in UNIX timestamp format.
        :param pulumi.Input[_builtins.str] description: Description, such as what it is used for. It contains up to 2,048 bytes.
        :param pulumi.Input[_builtins.str] kms_key_id: Specifies a KMS CMK to encrypt the secret.If this parameter is left empty, the CMK created by Secrets Manager by default will be used for encryption.You can also specify a custom KMS CMK created in the same region for encryption.
        :param pulumi.Input[_builtins.int] project_id: ID of the project to which the created SSH key belongs.
        :param pulumi.Input[_builtins.str] resource_id: The resource ID associated with the secret.
        :param pulumi.Input[_builtins.str] secret_name: Secret name, which must be unique in the same region. It can contain 128 bytes of letters, digits, hyphens and underscores and must begin with a letter or digit.
        :param pulumi.Input[_builtins.int] secret_type: `0`: user-defined secret. `1`: Tencent Cloud services secret. `2`: SSH key secret. `3`: Tencent Cloud API key secret. Note: this field may return `null`, indicating that no valid values can be obtained.
        :param pulumi.Input[_builtins.str] ssh_key_id: The key pair ID is the unique identifier of the key pair in the cloud server.
        :param pulumi.Input[_builtins.str] ssh_key_name: Name of the SSH key pair, which only contains digits, letters and underscores and must start with a digit or letter. The maximum length is 25 characters.
        :param pulumi.Input[_builtins.str] status: Enable or Disable Secret. Valid values is `Enabled` or `Disabled`. Default is `Enabled`.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tags of secret.
        """
        if clean_ssh_key is not None:
            pulumi.set(__self__, "clean_ssh_key", clean_ssh_key)
        if create_time is not None:
            pulumi.set(__self__, "create_time", create_time)
        if description is not None:
            pulumi.set(__self__, "description", description)
        if kms_key_id is not None:
            pulumi.set(__self__, "kms_key_id", kms_key_id)
        if project_id is not None:
            pulumi.set(__self__, "project_id", project_id)
        if resource_id is not None:
            pulumi.set(__self__, "resource_id", resource_id)
        if secret_name is not None:
            pulumi.set(__self__, "secret_name", secret_name)
        if secret_type is not None:
            pulumi.set(__self__, "secret_type", secret_type)
        if ssh_key_id is not None:
            pulumi.set(__self__, "ssh_key_id", ssh_key_id)
        if ssh_key_name is not None:
            pulumi.set(__self__, "ssh_key_name", ssh_key_name)
        if status is not None:
            pulumi.set(__self__, "status", status)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)

    @_builtins.property
    @pulumi.getter(name="cleanSshKey")
    def clean_ssh_key(self) -> pulumi.Input[_builtins.bool | None]:
        """
        Specifies whether to delete the SSH key from both the secret and the SSH key list in the CVM console. This field is only take effect when delete SSH key secrets. Valid values: `True`: deletes SSH key from both the secret and SSH key list in the CVM console. Note that the deletion will fail if the SSH key is already bound to a CVM instance.`False`: only deletes the SSH key information in the secret.
        """
        return pulumi.get(self, "clean_ssh_key")

    @clean_ssh_key.setter
    def clean_ssh_key(self, value: pulumi.Input[_builtins.bool | None]):
        pulumi.set(self, "clean_ssh_key", value)

    @_builtins.property
    @pulumi.getter(name="createTime")
    def create_time(self) -> pulumi.Input[_builtins.int | None]:
        """
        Credential creation time in UNIX timestamp format.
        """
        return pulumi.get(self, "create_time")

    @create_time.setter
    def create_time(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "create_time", value)

    @_builtins.property
    @pulumi.getter
    def description(self) -> pulumi.Input[_builtins.str | None]:
        """
        Description, such as what it is used for. It contains up to 2,048 bytes.
        """
        return pulumi.get(self, "description")

    @description.setter
    def description(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "description", value)

    @_builtins.property
    @pulumi.getter(name="kmsKeyId")
    def kms_key_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Specifies a KMS CMK to encrypt the secret.If this parameter is left empty, the CMK created by Secrets Manager by default will be used for encryption.You can also specify a custom KMS CMK created in the same region for encryption.
        """
        return pulumi.get(self, "kms_key_id")

    @kms_key_id.setter
    def kms_key_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "kms_key_id", value)

    @_builtins.property
    @pulumi.getter(name="projectId")
    def project_id(self) -> pulumi.Input[_builtins.int | None]:
        """
        ID of the project to which the created SSH key belongs.
        """
        return pulumi.get(self, "project_id")

    @project_id.setter
    def project_id(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "project_id", value)

    @_builtins.property
    @pulumi.getter(name="resourceId")
    def resource_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        The resource ID associated with the secret.
        """
        return pulumi.get(self, "resource_id")

    @resource_id.setter
    def resource_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "resource_id", value)

    @_builtins.property
    @pulumi.getter(name="secretName")
    def secret_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Secret name, which must be unique in the same region. It can contain 128 bytes of letters, digits, hyphens and underscores and must begin with a letter or digit.
        """
        return pulumi.get(self, "secret_name")

    @secret_name.setter
    def secret_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "secret_name", value)

    @_builtins.property
    @pulumi.getter(name="secretType")
    def secret_type(self) -> pulumi.Input[_builtins.int | None]:
        """
        `0`: user-defined secret. `1`: Tencent Cloud services secret. `2`: SSH key secret. `3`: Tencent Cloud API key secret. Note: this field may return `null`, indicating that no valid values can be obtained.
        """
        return pulumi.get(self, "secret_type")

    @secret_type.setter
    def secret_type(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "secret_type", value)

    @_builtins.property
    @pulumi.getter(name="sshKeyId")
    def ssh_key_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        The key pair ID is the unique identifier of the key pair in the cloud server.
        """
        return pulumi.get(self, "ssh_key_id")

    @ssh_key_id.setter
    def ssh_key_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "ssh_key_id", value)

    @_builtins.property
    @pulumi.getter(name="sshKeyName")
    def ssh_key_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Name of the SSH key pair, which only contains digits, letters and underscores and must start with a digit or letter. The maximum length is 25 characters.
        """
        return pulumi.get(self, "ssh_key_name")

    @ssh_key_name.setter
    def ssh_key_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "ssh_key_name", value)

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Input[_builtins.str | None]:
        """
        Enable or Disable Secret. Valid values is `Enabled` or `Disabled`. Default is `Enabled`.
        """
        return pulumi.get(self, "status")

    @status.setter
    def status(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "status", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]:
        """
        Tags of secret.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(
        self, value: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "tags", value)


@pulumi.type_token("tencentcloud:Ssm/sshKeyPairSecret:SshKeyPairSecret")
class SshKeyPairSecret(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        clean_ssh_key: pulumi.Input[_builtins.bool | None] = None,
        description: pulumi.Input[_builtins.str | None] = None,
        kms_key_id: pulumi.Input[_builtins.str | None] = None,
        project_id: pulumi.Input[_builtins.int | None] = None,
        secret_name: pulumi.Input[_builtins.str | None] = None,
        ssh_key_name: pulumi.Input[_builtins.str | None] = None,
        status: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a ssm ssh key pair secret

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        example = tencentcloud.kms.Key("example",
            alias="tf-example-kms-key",
            description="example of kms key",
            key_rotation_enabled=False,
            is_enabled=True,
            tags={
                "createdBy": "terraform",
            })
        example_ssh_key_pair_secret = tencentcloud.ssm.SshKeyPairSecret("example",
            secret_name="tf-example",
            project_id=0,
            description="desc.",
            kms_key_id=example.id,
            ssh_key_name="tf_example_ssh",
            status="Enabled",
            clean_ssh_key=True,
            tags={
                "createdBy": "terraform",
            })
        ```

        ## Import

        ssm ssh_key_pair_secret can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Ssm/sshKeyPairSecret:SshKeyPairSecret ssh_key_pair_secret ssh_key_pair_secret_name
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.bool] clean_ssh_key: Specifies whether to delete the SSH key from both the secret and the SSH key list in the CVM console. This field is only take effect when delete SSH key secrets. Valid values: `True`: deletes SSH key from both the secret and SSH key list in the CVM console. Note that the deletion will fail if the SSH key is already bound to a CVM instance.`False`: only deletes the SSH key information in the secret.
        :param pulumi.Input[_builtins.str] description: Description, such as what it is used for. It contains up to 2,048 bytes.
        :param pulumi.Input[_builtins.str] kms_key_id: Specifies a KMS CMK to encrypt the secret.If this parameter is left empty, the CMK created by Secrets Manager by default will be used for encryption.You can also specify a custom KMS CMK created in the same region for encryption.
        :param pulumi.Input[_builtins.int] project_id: ID of the project to which the created SSH key belongs.
        :param pulumi.Input[_builtins.str] secret_name: Secret name, which must be unique in the same region. It can contain 128 bytes of letters, digits, hyphens and underscores and must begin with a letter or digit.
        :param pulumi.Input[_builtins.str] ssh_key_name: Name of the SSH key pair, which only contains digits, letters and underscores and must start with a digit or letter. The maximum length is 25 characters.
        :param pulumi.Input[_builtins.str] status: Enable or Disable Secret. Valid values is `Enabled` or `Disabled`. Default is `Enabled`.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tags of secret.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: SshKeyPairSecretArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a ssm ssh key pair secret

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        example = tencentcloud.kms.Key("example",
            alias="tf-example-kms-key",
            description="example of kms key",
            key_rotation_enabled=False,
            is_enabled=True,
            tags={
                "createdBy": "terraform",
            })
        example_ssh_key_pair_secret = tencentcloud.ssm.SshKeyPairSecret("example",
            secret_name="tf-example",
            project_id=0,
            description="desc.",
            kms_key_id=example.id,
            ssh_key_name="tf_example_ssh",
            status="Enabled",
            clean_ssh_key=True,
            tags={
                "createdBy": "terraform",
            })
        ```

        ## Import

        ssm ssh_key_pair_secret can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Ssm/sshKeyPairSecret:SshKeyPairSecret ssh_key_pair_secret ssh_key_pair_secret_name
        ```


        :param str resource_name: The name of the resource.
        :param SshKeyPairSecretArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            SshKeyPairSecretArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        clean_ssh_key: pulumi.Input[_builtins.bool | None] = None,
        description: pulumi.Input[_builtins.str | None] = None,
        kms_key_id: pulumi.Input[_builtins.str | None] = None,
        project_id: pulumi.Input[_builtins.int | None] = None,
        secret_name: pulumi.Input[_builtins.str | None] = None,
        ssh_key_name: pulumi.Input[_builtins.str | None] = None,
        status: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = SshKeyPairSecretArgs.__new__(SshKeyPairSecretArgs)

            __props__.__dict__["clean_ssh_key"] = clean_ssh_key
            __props__.__dict__["description"] = description
            __props__.__dict__["kms_key_id"] = kms_key_id
            if project_id is None and not opts.urn:
                raise TypeError("Missing required property 'project_id'")
            __props__.__dict__["project_id"] = project_id
            if secret_name is None and not opts.urn:
                raise TypeError("Missing required property 'secret_name'")
            __props__.__dict__["secret_name"] = secret_name
            __props__.__dict__["ssh_key_name"] = ssh_key_name
            __props__.__dict__["status"] = status
            __props__.__dict__["tags"] = tags
            __props__.__dict__["create_time"] = None
            __props__.__dict__["resource_id"] = None
            __props__.__dict__["secret_type"] = None
            __props__.__dict__["ssh_key_id"] = None
        super().__init__(
            "tencentcloud:Ssm/sshKeyPairSecret:SshKeyPairSecret",
            resource_name,
            __props__,
            opts,
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        clean_ssh_key: pulumi.Input[_builtins.bool | None] = None,
        create_time: pulumi.Input[_builtins.int | None] = None,
        description: pulumi.Input[_builtins.str | None] = None,
        kms_key_id: pulumi.Input[_builtins.str | None] = None,
        project_id: pulumi.Input[_builtins.int | None] = None,
        resource_id: pulumi.Input[_builtins.str | None] = None,
        secret_name: pulumi.Input[_builtins.str | None] = None,
        secret_type: pulumi.Input[_builtins.int | None] = None,
        ssh_key_id: pulumi.Input[_builtins.str | None] = None,
        ssh_key_name: pulumi.Input[_builtins.str | None] = None,
        status: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
    ) -> "SshKeyPairSecret":
        """
        Get an existing SshKeyPairSecret resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.bool] clean_ssh_key: Specifies whether to delete the SSH key from both the secret and the SSH key list in the CVM console. This field is only take effect when delete SSH key secrets. Valid values: `True`: deletes SSH key from both the secret and SSH key list in the CVM console. Note that the deletion will fail if the SSH key is already bound to a CVM instance.`False`: only deletes the SSH key information in the secret.
        :param pulumi.Input[_builtins.int] create_time: Credential creation time in UNIX timestamp format.
        :param pulumi.Input[_builtins.str] description: Description, such as what it is used for. It contains up to 2,048 bytes.
        :param pulumi.Input[_builtins.str] kms_key_id: Specifies a KMS CMK to encrypt the secret.If this parameter is left empty, the CMK created by Secrets Manager by default will be used for encryption.You can also specify a custom KMS CMK created in the same region for encryption.
        :param pulumi.Input[_builtins.int] project_id: ID of the project to which the created SSH key belongs.
        :param pulumi.Input[_builtins.str] resource_id: The resource ID associated with the secret.
        :param pulumi.Input[_builtins.str] secret_name: Secret name, which must be unique in the same region. It can contain 128 bytes of letters, digits, hyphens and underscores and must begin with a letter or digit.
        :param pulumi.Input[_builtins.int] secret_type: `0`: user-defined secret. `1`: Tencent Cloud services secret. `2`: SSH key secret. `3`: Tencent Cloud API key secret. Note: this field may return `null`, indicating that no valid values can be obtained.
        :param pulumi.Input[_builtins.str] ssh_key_id: The key pair ID is the unique identifier of the key pair in the cloud server.
        :param pulumi.Input[_builtins.str] ssh_key_name: Name of the SSH key pair, which only contains digits, letters and underscores and must start with a digit or letter. The maximum length is 25 characters.
        :param pulumi.Input[_builtins.str] status: Enable or Disable Secret. Valid values is `Enabled` or `Disabled`. Default is `Enabled`.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tags of secret.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _SshKeyPairSecretState.__new__(_SshKeyPairSecretState)

        __props__.__dict__["clean_ssh_key"] = clean_ssh_key
        __props__.__dict__["create_time"] = create_time
        __props__.__dict__["description"] = description
        __props__.__dict__["kms_key_id"] = kms_key_id
        __props__.__dict__["project_id"] = project_id
        __props__.__dict__["resource_id"] = resource_id
        __props__.__dict__["secret_name"] = secret_name
        __props__.__dict__["secret_type"] = secret_type
        __props__.__dict__["ssh_key_id"] = ssh_key_id
        __props__.__dict__["ssh_key_name"] = ssh_key_name
        __props__.__dict__["status"] = status
        __props__.__dict__["tags"] = tags
        return SshKeyPairSecret(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="cleanSshKey")
    def clean_ssh_key(self) -> pulumi.Output[_builtins.bool | None]:
        """
        Specifies whether to delete the SSH key from both the secret and the SSH key list in the CVM console. This field is only take effect when delete SSH key secrets. Valid values: `True`: deletes SSH key from both the secret and SSH key list in the CVM console. Note that the deletion will fail if the SSH key is already bound to a CVM instance.`False`: only deletes the SSH key information in the secret.
        """
        return pulumi.get(self, "clean_ssh_key")

    @_builtins.property
    @pulumi.getter(name="createTime")
    def create_time(self) -> pulumi.Output[_builtins.int]:
        """
        Credential creation time in UNIX timestamp format.
        """
        return pulumi.get(self, "create_time")

    @_builtins.property
    @pulumi.getter
    def description(self) -> pulumi.Output[_builtins.str | None]:
        """
        Description, such as what it is used for. It contains up to 2,048 bytes.
        """
        return pulumi.get(self, "description")

    @_builtins.property
    @pulumi.getter(name="kmsKeyId")
    def kms_key_id(self) -> pulumi.Output[_builtins.str]:
        """
        Specifies a KMS CMK to encrypt the secret.If this parameter is left empty, the CMK created by Secrets Manager by default will be used for encryption.You can also specify a custom KMS CMK created in the same region for encryption.
        """
        return pulumi.get(self, "kms_key_id")

    @_builtins.property
    @pulumi.getter(name="projectId")
    def project_id(self) -> pulumi.Output[_builtins.int]:
        """
        ID of the project to which the created SSH key belongs.
        """
        return pulumi.get(self, "project_id")

    @_builtins.property
    @pulumi.getter(name="resourceId")
    def resource_id(self) -> pulumi.Output[_builtins.str]:
        """
        The resource ID associated with the secret.
        """
        return pulumi.get(self, "resource_id")

    @_builtins.property
    @pulumi.getter(name="secretName")
    def secret_name(self) -> pulumi.Output[_builtins.str]:
        """
        Secret name, which must be unique in the same region. It can contain 128 bytes of letters, digits, hyphens and underscores and must begin with a letter or digit.
        """
        return pulumi.get(self, "secret_name")

    @_builtins.property
    @pulumi.getter(name="secretType")
    def secret_type(self) -> pulumi.Output[_builtins.int]:
        """
        `0`: user-defined secret. `1`: Tencent Cloud services secret. `2`: SSH key secret. `3`: Tencent Cloud API key secret. Note: this field may return `null`, indicating that no valid values can be obtained.
        """
        return pulumi.get(self, "secret_type")

    @_builtins.property
    @pulumi.getter(name="sshKeyId")
    def ssh_key_id(self) -> pulumi.Output[_builtins.str]:
        """
        The key pair ID is the unique identifier of the key pair in the cloud server.
        """
        return pulumi.get(self, "ssh_key_id")

    @_builtins.property
    @pulumi.getter(name="sshKeyName")
    def ssh_key_name(self) -> pulumi.Output[_builtins.str]:
        """
        Name of the SSH key pair, which only contains digits, letters and underscores and must start with a digit or letter. The maximum length is 25 characters.
        """
        return pulumi.get(self, "ssh_key_name")

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Output[_builtins.str]:
        """
        Enable or Disable Secret. Valid values is `Enabled` or `Disabled`. Default is `Enabled`.
        """
        return pulumi.get(self, "status")

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Output[Mapping[str, _builtins.str] | None]:
        """
        Tags of secret.
        """
        return pulumi.get(self, "tags")
