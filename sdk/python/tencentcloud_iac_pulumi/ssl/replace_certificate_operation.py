# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["ReplaceCertificateOperation", "ReplaceCertificateOperationArgs"]


@pulumi.input_type
class ReplaceCertificateOperationArgs:
    def __init__(
        __self__,
        *,
        certificate_id: pulumi.Input[_builtins.str],
        valid_type: pulumi.Input[_builtins.str],
        cert_csr_encrypt_algo: pulumi.Input[_builtins.str | None] = None,
        cert_csr_key_parameter: pulumi.Input[_builtins.str | None] = None,
        csr_content: pulumi.Input[_builtins.str | None] = None,
        csr_key_password: pulumi.Input[_builtins.str | None] = None,
        csr_type: pulumi.Input[_builtins.str | None] = None,
        reason: pulumi.Input[_builtins.str | None] = None
    ):
        """
        The set of arguments for constructing a ReplaceCertificateOperation resource.

        :param pulumi.Input[_builtins.str] certificate_id: Certificate ID.
        :param pulumi.Input[_builtins.str] valid_type: Verification type: DNS_AUTO = automatic DNS verification (this verification type is only supported for domain names that are resolved by Tencent Cloud and have normal resolution status), DNS = manual DNS verification, FILE = file verification.
        :param pulumi.Input[_builtins.str] cert_csr_encrypt_algo: CSR encryption method, optional: RSA, ECC, SM2. (Selectable only if CsrType is Online), default is RSA.
        :param pulumi.Input[_builtins.str] cert_csr_key_parameter: CSR encryption parameter, when CsrEncryptAlgo is RSA, you can choose 2048, 4096, etc., and the default is 2048; when CsrEncryptAlgo is ECC, you can choose prime256v1, secp384r1, etc., and the default is prime256v1;.
        :param pulumi.Input[_builtins.str] csr_content: CSR Content.
        :param pulumi.Input[_builtins.str] csr_key_password: KEY Password.
        :param pulumi.Input[_builtins.str] csr_type: Type, default Original. Available options: Original = original certificate CSR, Upload = manual upload, Online = online generation.
        :param pulumi.Input[_builtins.str] reason: Reason for reissue.
        """
        pulumi.set(__self__, "certificate_id", certificate_id)
        pulumi.set(__self__, "valid_type", valid_type)
        if cert_csr_encrypt_algo is not None:
            pulumi.set(__self__, "cert_csr_encrypt_algo", cert_csr_encrypt_algo)
        if cert_csr_key_parameter is not None:
            pulumi.set(__self__, "cert_csr_key_parameter", cert_csr_key_parameter)
        if csr_content is not None:
            pulumi.set(__self__, "csr_content", csr_content)
        if csr_key_password is not None:
            pulumi.set(__self__, "csr_key_password", csr_key_password)
        if csr_type is not None:
            pulumi.set(__self__, "csr_type", csr_type)
        if reason is not None:
            pulumi.set(__self__, "reason", reason)

    @_builtins.property
    @pulumi.getter(name="certificateId")
    def certificate_id(self) -> pulumi.Input[_builtins.str]:
        """
        Certificate ID.
        """
        return pulumi.get(self, "certificate_id")

    @certificate_id.setter
    def certificate_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "certificate_id", value)

    @_builtins.property
    @pulumi.getter(name="validType")
    def valid_type(self) -> pulumi.Input[_builtins.str]:
        """
        Verification type: DNS_AUTO = automatic DNS verification (this verification type is only supported for domain names that are resolved by Tencent Cloud and have normal resolution status), DNS = manual DNS verification, FILE = file verification.
        """
        return pulumi.get(self, "valid_type")

    @valid_type.setter
    def valid_type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "valid_type", value)

    @_builtins.property
    @pulumi.getter(name="certCsrEncryptAlgo")
    def cert_csr_encrypt_algo(self) -> pulumi.Input[_builtins.str | None]:
        """
        CSR encryption method, optional: RSA, ECC, SM2. (Selectable only if CsrType is Online), default is RSA.
        """
        return pulumi.get(self, "cert_csr_encrypt_algo")

    @cert_csr_encrypt_algo.setter
    def cert_csr_encrypt_algo(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "cert_csr_encrypt_algo", value)

    @_builtins.property
    @pulumi.getter(name="certCsrKeyParameter")
    def cert_csr_key_parameter(self) -> pulumi.Input[_builtins.str | None]:
        """
        CSR encryption parameter, when CsrEncryptAlgo is RSA, you can choose 2048, 4096, etc., and the default is 2048; when CsrEncryptAlgo is ECC, you can choose prime256v1, secp384r1, etc., and the default is prime256v1;.
        """
        return pulumi.get(self, "cert_csr_key_parameter")

    @cert_csr_key_parameter.setter
    def cert_csr_key_parameter(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "cert_csr_key_parameter", value)

    @_builtins.property
    @pulumi.getter(name="csrContent")
    def csr_content(self) -> pulumi.Input[_builtins.str | None]:
        """
        CSR Content.
        """
        return pulumi.get(self, "csr_content")

    @csr_content.setter
    def csr_content(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "csr_content", value)

    @_builtins.property
    @pulumi.getter(name="csrKeyPassword")
    def csr_key_password(self) -> pulumi.Input[_builtins.str | None]:
        """
        KEY Password.
        """
        return pulumi.get(self, "csr_key_password")

    @csr_key_password.setter
    def csr_key_password(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "csr_key_password", value)

    @_builtins.property
    @pulumi.getter(name="csrType")
    def csr_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Type, default Original. Available options: Original = original certificate CSR, Upload = manual upload, Online = online generation.
        """
        return pulumi.get(self, "csr_type")

    @csr_type.setter
    def csr_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "csr_type", value)

    @_builtins.property
    @pulumi.getter
    def reason(self) -> pulumi.Input[_builtins.str | None]:
        """
        Reason for reissue.
        """
        return pulumi.get(self, "reason")

    @reason.setter
    def reason(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "reason", value)


@pulumi.input_type
class _ReplaceCertificateOperationState:
    def __init__(
        __self__,
        *,
        cert_csr_encrypt_algo: pulumi.Input[_builtins.str | None] = None,
        cert_csr_key_parameter: pulumi.Input[_builtins.str | None] = None,
        certificate_id: pulumi.Input[_builtins.str | None] = None,
        csr_content: pulumi.Input[_builtins.str | None] = None,
        csr_key_password: pulumi.Input[_builtins.str | None] = None,
        csr_type: pulumi.Input[_builtins.str | None] = None,
        reason: pulumi.Input[_builtins.str | None] = None,
        valid_type: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering ReplaceCertificateOperation resources.

        :param pulumi.Input[_builtins.str] cert_csr_encrypt_algo: CSR encryption method, optional: RSA, ECC, SM2. (Selectable only if CsrType is Online), default is RSA.
        :param pulumi.Input[_builtins.str] cert_csr_key_parameter: CSR encryption parameter, when CsrEncryptAlgo is RSA, you can choose 2048, 4096, etc., and the default is 2048; when CsrEncryptAlgo is ECC, you can choose prime256v1, secp384r1, etc., and the default is prime256v1;.
        :param pulumi.Input[_builtins.str] certificate_id: Certificate ID.
        :param pulumi.Input[_builtins.str] csr_content: CSR Content.
        :param pulumi.Input[_builtins.str] csr_key_password: KEY Password.
        :param pulumi.Input[_builtins.str] csr_type: Type, default Original. Available options: Original = original certificate CSR, Upload = manual upload, Online = online generation.
        :param pulumi.Input[_builtins.str] reason: Reason for reissue.
        :param pulumi.Input[_builtins.str] valid_type: Verification type: DNS_AUTO = automatic DNS verification (this verification type is only supported for domain names that are resolved by Tencent Cloud and have normal resolution status), DNS = manual DNS verification, FILE = file verification.
        """
        if cert_csr_encrypt_algo is not None:
            pulumi.set(__self__, "cert_csr_encrypt_algo", cert_csr_encrypt_algo)
        if cert_csr_key_parameter is not None:
            pulumi.set(__self__, "cert_csr_key_parameter", cert_csr_key_parameter)
        if certificate_id is not None:
            pulumi.set(__self__, "certificate_id", certificate_id)
        if csr_content is not None:
            pulumi.set(__self__, "csr_content", csr_content)
        if csr_key_password is not None:
            pulumi.set(__self__, "csr_key_password", csr_key_password)
        if csr_type is not None:
            pulumi.set(__self__, "csr_type", csr_type)
        if reason is not None:
            pulumi.set(__self__, "reason", reason)
        if valid_type is not None:
            pulumi.set(__self__, "valid_type", valid_type)

    @_builtins.property
    @pulumi.getter(name="certCsrEncryptAlgo")
    def cert_csr_encrypt_algo(self) -> pulumi.Input[_builtins.str | None]:
        """
        CSR encryption method, optional: RSA, ECC, SM2. (Selectable only if CsrType is Online), default is RSA.
        """
        return pulumi.get(self, "cert_csr_encrypt_algo")

    @cert_csr_encrypt_algo.setter
    def cert_csr_encrypt_algo(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "cert_csr_encrypt_algo", value)

    @_builtins.property
    @pulumi.getter(name="certCsrKeyParameter")
    def cert_csr_key_parameter(self) -> pulumi.Input[_builtins.str | None]:
        """
        CSR encryption parameter, when CsrEncryptAlgo is RSA, you can choose 2048, 4096, etc., and the default is 2048; when CsrEncryptAlgo is ECC, you can choose prime256v1, secp384r1, etc., and the default is prime256v1;.
        """
        return pulumi.get(self, "cert_csr_key_parameter")

    @cert_csr_key_parameter.setter
    def cert_csr_key_parameter(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "cert_csr_key_parameter", value)

    @_builtins.property
    @pulumi.getter(name="certificateId")
    def certificate_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Certificate ID.
        """
        return pulumi.get(self, "certificate_id")

    @certificate_id.setter
    def certificate_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "certificate_id", value)

    @_builtins.property
    @pulumi.getter(name="csrContent")
    def csr_content(self) -> pulumi.Input[_builtins.str | None]:
        """
        CSR Content.
        """
        return pulumi.get(self, "csr_content")

    @csr_content.setter
    def csr_content(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "csr_content", value)

    @_builtins.property
    @pulumi.getter(name="csrKeyPassword")
    def csr_key_password(self) -> pulumi.Input[_builtins.str | None]:
        """
        KEY Password.
        """
        return pulumi.get(self, "csr_key_password")

    @csr_key_password.setter
    def csr_key_password(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "csr_key_password", value)

    @_builtins.property
    @pulumi.getter(name="csrType")
    def csr_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Type, default Original. Available options: Original = original certificate CSR, Upload = manual upload, Online = online generation.
        """
        return pulumi.get(self, "csr_type")

    @csr_type.setter
    def csr_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "csr_type", value)

    @_builtins.property
    @pulumi.getter
    def reason(self) -> pulumi.Input[_builtins.str | None]:
        """
        Reason for reissue.
        """
        return pulumi.get(self, "reason")

    @reason.setter
    def reason(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "reason", value)

    @_builtins.property
    @pulumi.getter(name="validType")
    def valid_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Verification type: DNS_AUTO = automatic DNS verification (this verification type is only supported for domain names that are resolved by Tencent Cloud and have normal resolution status), DNS = manual DNS verification, FILE = file verification.
        """
        return pulumi.get(self, "valid_type")

    @valid_type.setter
    def valid_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "valid_type", value)


@pulumi.type_token(
    "tencentcloud:Ssl/replaceCertificateOperation:ReplaceCertificateOperation"
)
class ReplaceCertificateOperation(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        cert_csr_encrypt_algo: pulumi.Input[_builtins.str | None] = None,
        cert_csr_key_parameter: pulumi.Input[_builtins.str | None] = None,
        certificate_id: pulumi.Input[_builtins.str | None] = None,
        csr_content: pulumi.Input[_builtins.str | None] = None,
        csr_key_password: pulumi.Input[_builtins.str | None] = None,
        csr_type: pulumi.Input[_builtins.str | None] = None,
        reason: pulumi.Input[_builtins.str | None] = None,
        valid_type: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a ssl replace_certificate

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        replace_certificate = tencentcloud.ssl.ReplaceCertificateOperation("replace_certificate",
            certificate_id="8L6JsWq2",
            valid_type="DNS_AUTO",
            csr_type="online")
        ```

        ## Import

        ssl replace_certificate can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Ssl/replaceCertificateOperation:ReplaceCertificateOperation replace_certificate replace_certificate_id
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] cert_csr_encrypt_algo: CSR encryption method, optional: RSA, ECC, SM2. (Selectable only if CsrType is Online), default is RSA.
        :param pulumi.Input[_builtins.str] cert_csr_key_parameter: CSR encryption parameter, when CsrEncryptAlgo is RSA, you can choose 2048, 4096, etc., and the default is 2048; when CsrEncryptAlgo is ECC, you can choose prime256v1, secp384r1, etc., and the default is prime256v1;.
        :param pulumi.Input[_builtins.str] certificate_id: Certificate ID.
        :param pulumi.Input[_builtins.str] csr_content: CSR Content.
        :param pulumi.Input[_builtins.str] csr_key_password: KEY Password.
        :param pulumi.Input[_builtins.str] csr_type: Type, default Original. Available options: Original = original certificate CSR, Upload = manual upload, Online = online generation.
        :param pulumi.Input[_builtins.str] reason: Reason for reissue.
        :param pulumi.Input[_builtins.str] valid_type: Verification type: DNS_AUTO = automatic DNS verification (this verification type is only supported for domain names that are resolved by Tencent Cloud and have normal resolution status), DNS = manual DNS verification, FILE = file verification.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: ReplaceCertificateOperationArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a ssl replace_certificate

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        replace_certificate = tencentcloud.ssl.ReplaceCertificateOperation("replace_certificate",
            certificate_id="8L6JsWq2",
            valid_type="DNS_AUTO",
            csr_type="online")
        ```

        ## Import

        ssl replace_certificate can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:Ssl/replaceCertificateOperation:ReplaceCertificateOperation replace_certificate replace_certificate_id
        ```


        :param str resource_name: The name of the resource.
        :param ReplaceCertificateOperationArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            ReplaceCertificateOperationArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        cert_csr_encrypt_algo: pulumi.Input[_builtins.str | None] = None,
        cert_csr_key_parameter: pulumi.Input[_builtins.str | None] = None,
        certificate_id: pulumi.Input[_builtins.str | None] = None,
        csr_content: pulumi.Input[_builtins.str | None] = None,
        csr_key_password: pulumi.Input[_builtins.str | None] = None,
        csr_type: pulumi.Input[_builtins.str | None] = None,
        reason: pulumi.Input[_builtins.str | None] = None,
        valid_type: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = ReplaceCertificateOperationArgs.__new__(
                ReplaceCertificateOperationArgs
            )

            __props__.__dict__["cert_csr_encrypt_algo"] = cert_csr_encrypt_algo
            __props__.__dict__["cert_csr_key_parameter"] = cert_csr_key_parameter
            if certificate_id is None and not opts.urn:
                raise TypeError("Missing required property 'certificate_id'")
            __props__.__dict__["certificate_id"] = certificate_id
            __props__.__dict__["csr_content"] = csr_content
            __props__.__dict__["csr_key_password"] = csr_key_password
            __props__.__dict__["csr_type"] = csr_type
            __props__.__dict__["reason"] = reason
            if valid_type is None and not opts.urn:
                raise TypeError("Missing required property 'valid_type'")
            __props__.__dict__["valid_type"] = valid_type
        super().__init__(
            "tencentcloud:Ssl/replaceCertificateOperation:ReplaceCertificateOperation",
            resource_name,
            __props__,
            opts,
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        cert_csr_encrypt_algo: pulumi.Input[_builtins.str | None] = None,
        cert_csr_key_parameter: pulumi.Input[_builtins.str | None] = None,
        certificate_id: pulumi.Input[_builtins.str | None] = None,
        csr_content: pulumi.Input[_builtins.str | None] = None,
        csr_key_password: pulumi.Input[_builtins.str | None] = None,
        csr_type: pulumi.Input[_builtins.str | None] = None,
        reason: pulumi.Input[_builtins.str | None] = None,
        valid_type: pulumi.Input[_builtins.str | None] = None,
    ) -> "ReplaceCertificateOperation":
        """
        Get an existing ReplaceCertificateOperation resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] cert_csr_encrypt_algo: CSR encryption method, optional: RSA, ECC, SM2. (Selectable only if CsrType is Online), default is RSA.
        :param pulumi.Input[_builtins.str] cert_csr_key_parameter: CSR encryption parameter, when CsrEncryptAlgo is RSA, you can choose 2048, 4096, etc., and the default is 2048; when CsrEncryptAlgo is ECC, you can choose prime256v1, secp384r1, etc., and the default is prime256v1;.
        :param pulumi.Input[_builtins.str] certificate_id: Certificate ID.
        :param pulumi.Input[_builtins.str] csr_content: CSR Content.
        :param pulumi.Input[_builtins.str] csr_key_password: KEY Password.
        :param pulumi.Input[_builtins.str] csr_type: Type, default Original. Available options: Original = original certificate CSR, Upload = manual upload, Online = online generation.
        :param pulumi.Input[_builtins.str] reason: Reason for reissue.
        :param pulumi.Input[_builtins.str] valid_type: Verification type: DNS_AUTO = automatic DNS verification (this verification type is only supported for domain names that are resolved by Tencent Cloud and have normal resolution status), DNS = manual DNS verification, FILE = file verification.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _ReplaceCertificateOperationState.__new__(
            _ReplaceCertificateOperationState
        )

        __props__.__dict__["cert_csr_encrypt_algo"] = cert_csr_encrypt_algo
        __props__.__dict__["cert_csr_key_parameter"] = cert_csr_key_parameter
        __props__.__dict__["certificate_id"] = certificate_id
        __props__.__dict__["csr_content"] = csr_content
        __props__.__dict__["csr_key_password"] = csr_key_password
        __props__.__dict__["csr_type"] = csr_type
        __props__.__dict__["reason"] = reason
        __props__.__dict__["valid_type"] = valid_type
        return ReplaceCertificateOperation(
            resource_name, opts=opts, __props__=__props__
        )

    @_builtins.property
    @pulumi.getter(name="certCsrEncryptAlgo")
    def cert_csr_encrypt_algo(self) -> pulumi.Output[_builtins.str | None]:
        """
        CSR encryption method, optional: RSA, ECC, SM2. (Selectable only if CsrType is Online), default is RSA.
        """
        return pulumi.get(self, "cert_csr_encrypt_algo")

    @_builtins.property
    @pulumi.getter(name="certCsrKeyParameter")
    def cert_csr_key_parameter(self) -> pulumi.Output[_builtins.str | None]:
        """
        CSR encryption parameter, when CsrEncryptAlgo is RSA, you can choose 2048, 4096, etc., and the default is 2048; when CsrEncryptAlgo is ECC, you can choose prime256v1, secp384r1, etc., and the default is prime256v1;.
        """
        return pulumi.get(self, "cert_csr_key_parameter")

    @_builtins.property
    @pulumi.getter(name="certificateId")
    def certificate_id(self) -> pulumi.Output[_builtins.str]:
        """
        Certificate ID.
        """
        return pulumi.get(self, "certificate_id")

    @_builtins.property
    @pulumi.getter(name="csrContent")
    def csr_content(self) -> pulumi.Output[_builtins.str | None]:
        """
        CSR Content.
        """
        return pulumi.get(self, "csr_content")

    @_builtins.property
    @pulumi.getter(name="csrKeyPassword")
    def csr_key_password(self) -> pulumi.Output[_builtins.str | None]:
        """
        KEY Password.
        """
        return pulumi.get(self, "csr_key_password")

    @_builtins.property
    @pulumi.getter(name="csrType")
    def csr_type(self) -> pulumi.Output[_builtins.str | None]:
        """
        Type, default Original. Available options: Original = original certificate CSR, Upload = manual upload, Online = online generation.
        """
        return pulumi.get(self, "csr_type")

    @_builtins.property
    @pulumi.getter
    def reason(self) -> pulumi.Output[_builtins.str | None]:
        """
        Reason for reissue.
        """
        return pulumi.get(self, "reason")

    @_builtins.property
    @pulumi.getter(name="validType")
    def valid_type(self) -> pulumi.Output[_builtins.str]:
        """
        Verification type: DNS_AUTO = automatic DNS verification (this verification type is only supported for domain names that are resolved by Tencent Cloud and have normal resolution status), DNS = manual DNS verification, FILE = file verification.
        """
        return pulumi.get(self, "valid_type")
