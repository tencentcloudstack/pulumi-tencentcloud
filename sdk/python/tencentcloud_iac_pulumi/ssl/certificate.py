# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Mapping, Sequence
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["Certificate", "CertificateArgs"]


@pulumi.input_type
class CertificateArgs:
    def __init__(
        __self__,
        *,
        cert: pulumi.Input[_builtins.str],
        type: pulumi.Input[_builtins.str],
        key: pulumi.Input[_builtins.str | None] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        project_id: pulumi.Input[_builtins.int | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None
    ):
        """
        The set of arguments for constructing a Certificate resource.

        :param pulumi.Input[_builtins.str] cert: Content of the SSL certificate. Not allowed newline at the start and end.
        :param pulumi.Input[_builtins.str] type: Type of the SSL certificate. Valid values: `CA` and `SVR`.
        :param pulumi.Input[_builtins.str] key: Key of the SSL certificate and required when certificate type is `SVR`. Not allowed newline at the start and end.
        :param pulumi.Input[_builtins.str] name: Name of the SSL certificate.
        :param pulumi.Input[_builtins.int] project_id: Project ID of the SSL certificate. Default is `0`.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tags of the SSL certificate.
        """
        pulumi.set(__self__, "cert", cert)
        pulumi.set(__self__, "type", type)
        if key is not None:
            pulumi.set(__self__, "key", key)
        if name is not None:
            pulumi.set(__self__, "name", name)
        if project_id is not None:
            pulumi.set(__self__, "project_id", project_id)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)

    @_builtins.property
    @pulumi.getter
    def cert(self) -> pulumi.Input[_builtins.str]:
        """
        Content of the SSL certificate. Not allowed newline at the start and end.
        """
        return pulumi.get(self, "cert")

    @cert.setter
    def cert(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "cert", value)

    @_builtins.property
    @pulumi.getter
    def type(self) -> pulumi.Input[_builtins.str]:
        """
        Type of the SSL certificate. Valid values: `CA` and `SVR`.
        """
        return pulumi.get(self, "type")

    @type.setter
    def type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "type", value)

    @_builtins.property
    @pulumi.getter
    def key(self) -> pulumi.Input[_builtins.str | None]:
        """
        Key of the SSL certificate and required when certificate type is `SVR`. Not allowed newline at the start and end.
        """
        return pulumi.get(self, "key")

    @key.setter
    def key(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "key", value)

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Name of the SSL certificate.
        """
        return pulumi.get(self, "name")

    @name.setter
    def name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "name", value)

    @_builtins.property
    @pulumi.getter(name="projectId")
    def project_id(self) -> pulumi.Input[_builtins.int | None]:
        """
        Project ID of the SSL certificate. Default is `0`.
        """
        return pulumi.get(self, "project_id")

    @project_id.setter
    def project_id(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "project_id", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]:
        """
        Tags of the SSL certificate.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(
        self, value: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "tags", value)


@pulumi.input_type
class _CertificateState:
    def __init__(
        __self__,
        *,
        begin_time: pulumi.Input[_builtins.str | None] = None,
        cert: pulumi.Input[_builtins.str | None] = None,
        create_time: pulumi.Input[_builtins.str | None] = None,
        domain: pulumi.Input[_builtins.str | None] = None,
        end_time: pulumi.Input[_builtins.str | None] = None,
        key: pulumi.Input[_builtins.str | None] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        product_zh_name: pulumi.Input[_builtins.str | None] = None,
        project_id: pulumi.Input[_builtins.int | None] = None,
        status: pulumi.Input[_builtins.int | None] = None,
        subject_names: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        type: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering Certificate resources.

        :param pulumi.Input[_builtins.str] begin_time: Beginning time of the SSL certificate.
        :param pulumi.Input[_builtins.str] cert: Content of the SSL certificate. Not allowed newline at the start and end.
        :param pulumi.Input[_builtins.str] create_time: Creation time of the SSL certificate.
        :param pulumi.Input[_builtins.str] domain: Primary domain of the SSL certificate.
        :param pulumi.Input[_builtins.str] end_time: Ending time of the SSL certificate.
        :param pulumi.Input[_builtins.str] key: Key of the SSL certificate and required when certificate type is `SVR`. Not allowed newline at the start and end.
        :param pulumi.Input[_builtins.str] name: Name of the SSL certificate.
        :param pulumi.Input[_builtins.str] product_zh_name: Certificate authority.
        :param pulumi.Input[_builtins.int] project_id: Project ID of the SSL certificate. Default is `0`.
        :param pulumi.Input[_builtins.int] status: Status of the SSL certificate.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] subject_names: ALL domains included in the SSL certificate. Including the primary domain name.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tags of the SSL certificate.
        :param pulumi.Input[_builtins.str] type: Type of the SSL certificate. Valid values: `CA` and `SVR`.
        """
        if begin_time is not None:
            pulumi.set(__self__, "begin_time", begin_time)
        if cert is not None:
            pulumi.set(__self__, "cert", cert)
        if create_time is not None:
            pulumi.set(__self__, "create_time", create_time)
        if domain is not None:
            pulumi.set(__self__, "domain", domain)
        if end_time is not None:
            pulumi.set(__self__, "end_time", end_time)
        if key is not None:
            pulumi.set(__self__, "key", key)
        if name is not None:
            pulumi.set(__self__, "name", name)
        if product_zh_name is not None:
            pulumi.set(__self__, "product_zh_name", product_zh_name)
        if project_id is not None:
            pulumi.set(__self__, "project_id", project_id)
        if status is not None:
            pulumi.set(__self__, "status", status)
        if subject_names is not None:
            pulumi.set(__self__, "subject_names", subject_names)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)
        if type is not None:
            pulumi.set(__self__, "type", type)

    @_builtins.property
    @pulumi.getter(name="beginTime")
    def begin_time(self) -> pulumi.Input[_builtins.str | None]:
        """
        Beginning time of the SSL certificate.
        """
        return pulumi.get(self, "begin_time")

    @begin_time.setter
    def begin_time(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "begin_time", value)

    @_builtins.property
    @pulumi.getter
    def cert(self) -> pulumi.Input[_builtins.str | None]:
        """
        Content of the SSL certificate. Not allowed newline at the start and end.
        """
        return pulumi.get(self, "cert")

    @cert.setter
    def cert(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "cert", value)

    @_builtins.property
    @pulumi.getter(name="createTime")
    def create_time(self) -> pulumi.Input[_builtins.str | None]:
        """
        Creation time of the SSL certificate.
        """
        return pulumi.get(self, "create_time")

    @create_time.setter
    def create_time(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "create_time", value)

    @_builtins.property
    @pulumi.getter
    def domain(self) -> pulumi.Input[_builtins.str | None]:
        """
        Primary domain of the SSL certificate.
        """
        return pulumi.get(self, "domain")

    @domain.setter
    def domain(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "domain", value)

    @_builtins.property
    @pulumi.getter(name="endTime")
    def end_time(self) -> pulumi.Input[_builtins.str | None]:
        """
        Ending time of the SSL certificate.
        """
        return pulumi.get(self, "end_time")

    @end_time.setter
    def end_time(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "end_time", value)

    @_builtins.property
    @pulumi.getter
    def key(self) -> pulumi.Input[_builtins.str | None]:
        """
        Key of the SSL certificate and required when certificate type is `SVR`. Not allowed newline at the start and end.
        """
        return pulumi.get(self, "key")

    @key.setter
    def key(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "key", value)

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Name of the SSL certificate.
        """
        return pulumi.get(self, "name")

    @name.setter
    def name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "name", value)

    @_builtins.property
    @pulumi.getter(name="productZhName")
    def product_zh_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Certificate authority.
        """
        return pulumi.get(self, "product_zh_name")

    @product_zh_name.setter
    def product_zh_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "product_zh_name", value)

    @_builtins.property
    @pulumi.getter(name="projectId")
    def project_id(self) -> pulumi.Input[_builtins.int | None]:
        """
        Project ID of the SSL certificate. Default is `0`.
        """
        return pulumi.get(self, "project_id")

    @project_id.setter
    def project_id(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "project_id", value)

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Input[_builtins.int | None]:
        """
        Status of the SSL certificate.
        """
        return pulumi.get(self, "status")

    @status.setter
    def status(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "status", value)

    @_builtins.property
    @pulumi.getter(name="subjectNames")
    def subject_names(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        ALL domains included in the SSL certificate. Including the primary domain name.
        """
        return pulumi.get(self, "subject_names")

    @subject_names.setter
    def subject_names(
        self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "subject_names", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]:
        """
        Tags of the SSL certificate.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(
        self, value: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "tags", value)

    @_builtins.property
    @pulumi.getter
    def type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Type of the SSL certificate. Valid values: `CA` and `SVR`.
        """
        return pulumi.get(self, "type")

    @type.setter
    def type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "type", value)


@pulumi.type_token("tencentcloud:Ssl/certificate:Certificate")
class Certificate(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        cert: pulumi.Input[_builtins.str | None] = None,
        key: pulumi.Input[_builtins.str | None] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        project_id: pulumi.Input[_builtins.int | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        type: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a SSL certificate.

        Upload the existing SSL certificate to Tencent Cloud.

        ## Example Usage

        ```python
        import pulumi
        import pulumi_tencentcloud as tencentcloud
        import tencentcloud_iac_pulumi as tencentcloud

        config = pulumi.Config()
        ca = config.get("ca")
        if ca is None:
            ca = \"\"\"-----BEGIN CERTIFICATE-----
        
        -----END CERTIFICATE-----\"\"\"
        ca_certificate = tencentcloud.ssl.Certificate("ca",
            name="ssl-ca",
            type="CA",
            cert=ca)
        ca_get_certificates = tencentcloud.ssl.get_certificates_output(name=ca_certificate.name)
        ```

        ```python
        import pulumi
        import pulumi_tencentcloud as tencentcloud
        import tencentcloud_iac_pulumi as tencentcloud

        config = pulumi.Config()
        cert = config.get("cert")
        if cert is None:
            cert = \"\"\"-----BEGIN CERTIFICATE-----
        
        -----END CERTIFICATE-----\"\"\"
        key = config.get("key")
        if key is None:
            key = \"\"\"Public Key Info:
        \\x09Public Key Algorithm: RSA
        \\x09Key Security Level: High (3072 bits)

        modulus:
        \\x0900:d5:1c:a9:f8:32:8a:48:d1:4a:66:c3:ed:c1:f4:7e
        \\x098f:33:a7:43:c6:05:89:e2:0a:62:06:a4:57:af:3c:77
        \\x09c6:20:a6:89:5e:9d:b9:9e:10:fa:f3:4f:ce:ed:ba:71
        \\x0943:ec:6f:5f:e8:02:c6:34:3f:2b:8f:b9:e9:8f:40:28
        \\x09c3:18:4e:2f:f8:7f:bf:50:fd:bb:45:2c:21:a8:f9:8b
        \\x0961:96:26:ba:8d:02:11:8b:6b:15:a2:10:1f:e1:cb:95
        \\x098e:8b:0c:43:ac:67:a2:81:0a:89:90:53:39:d8:4a:7b
        \\x091b:56:31:ec:f1:c9:98:a1:46:c0:df:45:84:8b:05:8b
        \\x0957:0d:65:f5:51:a2:96:6e:09:ef:d1:30:f3:6e:b0:d5
        \\x0959:68:88:be:a9:c1:1f:7e:c1:35:72:67:e6:b9:29:46
        \\x09dc:0a:74:44:a3:88:c9:1d:1a:8c:31:33:a9:99:fd:ff
        \\x0944:7b:57:05:82:17:de:ea:cb:c4:cd:62:4f:78:2b:fd
        \\x096d:ab:7c:f2:85:5c:d1:67:cd:f1:39:27:eb:4e:a1:1f
        \\x096f:7b:28:dc:05:36:8f:22:a0:61:dc:2b:fc:56:52:ef
        \\x0965:42:1d:2f:08:b2:58:27:a8:95:b8:8b:32:17:66:a8
        \\x0908:2a:61:da:45:9b:4c:37:88:d4:96:88:ba:97:fe:a5
        \\x0992:db:7a:2e:c8:21:f7:11:02:79:cc:73:89:22:6d:3c
        \\x09c6:a8:83:cf:1a:0b:c1:e7:32:a2:49:de:44:9c:18:22
        \\x09f7:e8:66:06:f8:a6:b0:e4:f9:c3:95:65:c1:69:e8:83
        \\x097b:cc:ff:79:62:bd:f9:1a:f6:33:ea:e1:4c:dc:68:b0
        \\x0905:46:7c:32:72:c4:05:f2:89:7e:3d:2d:0d:2f:ed:ff
        \\x096a:cf:07:3f:d0:98:83:6e:fb:14:bf:57:26:e8:e8:29
        \\x09a0:43:0f:f5:25:ea:a8:17:70:1a:a7:8a:6c:3d:bf:da
        \\x0939:4f:e3:cc:65:60:fe:a5:d5:2a:09:1b:71:a0:67:c7
        \\x095b:

        public exponent:
        \\x0901:00:01:

        private exponent:
        \\x0900:b1:56:d0:fa:00:d4:a2:13:c7:5e:0c:dc:e4:f1:97
        \\x09ff:82:74:46:29:9a:a2:4a:bf:69:23:2d:ce:e9:bb:df
        \\x09cf:b7:8b:dd:f4:26:3c:38:14:d9:3f:6f:c2:3a:81:53
        \\x098f:ba:48:53:fe:b5:90:4a:19:e7:1e:0b:0f:18:6d:c3
        \\x097d:d5:d3:fa:87:47:86:e4:d6:bf:e7:a7:f9:ba:ab:2e
        \\x0919:5e:e1:8b:8b:9b:95:0d:f7:66:61:1e:19:e9:c3:88
        \\x0908:be:1c:ce:93:c1:09:b1:68:1b:61:46:60:74:64:46
        \\x095d:51:34:ea:7f:a9:ca:a1:2a:47:85:84:4b:ef:84:05
        \\x0997:c3:46:7d:06:19:ce:24:73:90:64:fb:df:16:d5:80
        \\x0934:8e:90:7c:58:b6:a4:86:ce:30:b3:ab:52:8b:f2:95
        \\x094c:b6:46:5a:77:db:73:c0:0c:3f:6d:12:18:a8:54:7c
        \\x09ff:77:c3:ca:89:9f:63:98:ef:48:2d:c1:09:70:6e:ea
        \\x09cb:bb:78:91:42:8a:22:3e:21:ef:a5:bf:16:ee:66:45
        \\x09e5:f0:26:6a:85:8e:e1:69:62:ac:05:00:a6:44:ba:c8
        \\x09ac:10:00:97:f5:51:65:7f:9a:1f:7b:99:9d:02:d4:87
        \\x0950:ce:74:06:51:67:fa:fb:90:e4:33:79:f2:a8:61:ee
        \\x0945:1d:87:ca:22:5b:ac:e7:32:38:f8:2c:fd:55:92:1e
        \\x093d:60:1e:7c:4b:fd:28:ff:e5:b1:02:6a:aa:22:f7:ae
        \\x09a8:36:90:7b:a6:f7:29:05:14:3a:21:da:36:05:f9:b0
        \\x099d:f7:fb:10:75:d7:2c:21:32:95:e7:f7:17:be:09:cb
        \\x0966:fe:f1:69:71:df:a4:5e:3f:fc:67:6c:37:65:b8:51
        \\x09c6:22:38:fb:07:ce:89:54:50:43:71:44:3d:c3:51:5a
        \\x09bd:e5:c7:87:b2:ea:7b:64:0f:5d:34:9c:a1:52:b3:ce
        \\x0906:86:ba:7a:05:80:48:b3:6c:1b:79:74:9b:49:f2:30
        \\x09c1:

        prime1:
        \\x0900:e4:31:46:59:3d:24:f7:31:d9:22:26:af:c0:3e:f5
        \\x09c1:6d:be:ba:d3:9e:3f:b9:2c:43:a0:d0:47:09:e4:35
        \\x0963:19:a4:33:82:af:f9:76:3c:11:c2:cb:34:f9:a6:ab
        \\x09dd:ab:64:5a:6b:9c:c1:2a:52:89:64:7e:b5:a7:f0:4d
        \\x0929:13:a4:cf:17:f4:f2:0d:a0:6e:b9:5d:95:41:10:df
        \\x09ae:f3:7a:13:49:21:66:73:2a:b7:e2:8d:7c:c2:34:e5
        \\x093f:bd:78:ca:fc:64:c5:1c:3a:66:7a:12:53:96:bd:b0
        \\x09c3:7a:0c:ec:5e:55:c0:c3:3f:7f:25:72:f4:e2:19:94
        \\x099d:65:15:be:c8:82:20:57:12:97:b2:a8:4d:3d:e0:8f
        \\x09e2:1f:d0:c8:49:aa:f4:34:fa:91:d1:d1:cc:98:bc:3d
        \\x098b:b1:9b:8f:fd:ef:03:dd:92:fb:ca:99:45:af:cc:83
        \\x0958:4c:bb:ba:73:9e:23:84:f9:7e:4f:40:fe:00:b5:bf
        \\x096f:

        prime2:
        \\x0900:ef:14:ef:73:fc:0c:fc:e3:87:d9:7f:a6:f8:55:86
        \\x0957:63:8a:86:87:f5:ef:63:20:1f:b2:ae:28:dc:ab:59
        \\x0980:8f:15:64:44:e2:bc:a5:7b:d7:69:ef:30:b1:83:b3
        \\x09bd:09:fd:4a:0c:c6:31:5b:a4:79:d0:e5:d3:a8:31:fd
        \\x0959:ea:52:63:cf:17:a7:c1:54:bf:a8:11:9b:b1:85:47
        \\x095a:08:a6:9c:2f:47:9d:ac:5d:e8:7c:e4:31:6c:99:71
        \\x0904:7d:20:98:be:8b:60:07:66:2d:b9:41:10:ea:dd:5b
        \\x0987:20:65:62:ea:75:a7:a6:04:a2:18:66:6b:db:5b:a4
        \\x099f:12:97:cb:7c:8c:d2:e0:ce:02:ef:1e:df:a1:9d:6a
        \\x09bc:00:38:18:36:a1:c5:97:16:be:7a:df:5f:4f:4f:de
        \\x09a3:cb:25:fe:f6:67:0d:31:aa:0a:d4:1b:be:df:91:2c
        \\x0905:14:20:37:cc:4f:50:33:a6:50:1b:90:f9:b2:08:80
        \\x09d5:

        coefficient:
        \\x0947:d1:7f:ca:93:6a:14:9b:fe:85:8d:c2:15:11:52:a2
        \\x09a5:bc:f5:6b:a2:69:76:49:1e:09:79:f1:15:bf:39:48
        \\x0941:ff:92:78:af:bc:7d:6f:76:3b:32:9e:08:d2:42:06
        \\x0904:5f:36:e0:be:a8:1d:21:5c:ec:39:09:e0:77:c5:86
        \\x0906:e6:ce:98:16:fc:0f:30:de:a7:69:7a:8f:dd:01:42
        \\x092a:22:f5:b7:c2:fc:c8:90:5a:78:dc:b3:e0:4d:e7:2d
        \\x0998:6c:e3:34:1b:d7:e8:f8:90:57:7e:4d:41:d6:4a:29
        \\x0981:92:eb:89:5b:45:85:dd:b9:16:20:63:cb:59:f6:06
        \\x0959:c1:dd:3b:6b:92:0a:5e:5e:63:4a:f1:a7:d5:16:b9
        \\x098b:6c:d8:ad:76:0e:2d:3c:e0:b3:73:e0:6d:af:d4:a2
        \\x09bc:4b:fd:6c:2d:d7:5d:4d:cd:28:03:64:b2:ef:9a:1d
        \\x0982:8d:53:40:c5:f8:fb:f3:63:de:8e:1a:21:b6:35:14
        \\x09

        exp1:
        \\x0900:9c:a5:8a:d2:65:dc:03:69:8f:d2:16:d6:9d:55:5b
        \\x0925:4e:ae:18:d8:7e:90:e6:10:11:d8:ca:41:89:f3:c4
        \\x0906:64:aa:c8:c5:95:01:dd:fd:7c:7f:c9:39:73:8b:cb
        \\x09fd:9e:d3:84:12:cd:87:f9:02:b1:d8:6f:f7:49:f2:f7
        \\x0935:14:8c:15:b2:2f:6f:1e:95:9c:8c:d9:46:45:65:4c
        \\x09f8:6f:a1:c4:ad:76:25:3b:37:ff:05:a1:f5:1b:e8:6d
        \\x09db:64:b9:10:37:55:01:ce:cf:f4:5b:26:4b:85:76:70
        \\x096a:b0:55:40:c9:bd:7a:57:4e:36:7d:41:be:03:9c:65
        \\x09dd:ea:6f:94:09:56:f2:d6:73:27:f9:f7:f9:16:5a:1a
        \\x09cb:b2:e5:83:28:b7:17:6f:6a:f7:41:1f:11:a1:63:cf
        \\x09a8:1e:e3:58:64:8c:78:8d:d9:81:c9:e1:8f:ea:0f:ad
        \\x09b6:a6:ee:54:1f:5c:56:ab:c9:0d:c1:60:2f:3d:d3:86
        \\x0937:

        exp2:
        \\x0964:12:b7:48:2d:30:a2:89:fa:cb:27:8b:94:56:f8:2c
        \\x098c:15:e7:c9:f1:3f:8a:96:5f:ae:43:08:07:96:11:98
        \\x09a6:4b:a5:f4:cf:93:77:11:27:51:c8:34:f1:98:d7:1b
        \\x0941:9b:2b:eb:bc:e9:dc:1a:34:83:24:30:3c:2e:f0:85
        \\x093a:77:d2:1f:55:1f:7a:e5:26:74:0b:2a:c8:5b:a9:4a
        \\x091e:64:de:eb:4b:66:cc:47:62:91:24:53:2b:c9:ee:6c
        \\x099a:93:92:5b:ef:aa:fa:6d:e2:a5:b0:7e:8c:50:ab:87
        \\x091c:20:54:0f:1f:c0:54:d5:8b:a3:fa:fb:1a:8e:79:91
        \\x09bc:0e:9d:b6:3c:9b:e8:4d:53:1d:14:27:37:56:d4:de
        \\x096c:99:0e:49:8f:dd:4d:28:d0:02:4e:8d:6e:7d:58:0b
        \\x09e7:74:b8:0c:1b:86:82:4b:52:cd:05:f0:17:54:84:c0
        \\x097b:74:20:e6:fc:2b:ed:f2:a7:85:62:61:a2:0b:bd:21
        \\x09


        Public Key PIN:
        \\x09pin-sha256:t5OXXC5gYqMNtUMsTqRs3A3vhfK2BiXVOgYzIEYv7Y8=
        Public Key ID:
        \\x09sha256:b793975c2e6062a30db5432c4ea46cdc0def85f2b60625d53a063320462fed8f
        \\x09sha1:2f058a05034b2fdb3771bded4e7c8f560fa6a423

        -----BEGIN RSA PRIVATE KEY-----
        
        -----END RSA PRIVATE KEY-----\"\"\"
        svr_certificate = tencentcloud.ssl.Certificate("svr",
            name="ssl-svr",
            type="SVR",
            cert=cert,
            key=key)
        svr = tencentcloud.ssl.get_certificates_output(name=svr_certificate.name)
        ```

        ## Import

        ssl certificate can be imported using the id, e.g.

        ```sh
          $ pulumi import tencentcloud:Ssl/certificate:Certificate cert GjTNRoK7
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] cert: Content of the SSL certificate. Not allowed newline at the start and end.
        :param pulumi.Input[_builtins.str] key: Key of the SSL certificate and required when certificate type is `SVR`. Not allowed newline at the start and end.
        :param pulumi.Input[_builtins.str] name: Name of the SSL certificate.
        :param pulumi.Input[_builtins.int] project_id: Project ID of the SSL certificate. Default is `0`.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tags of the SSL certificate.
        :param pulumi.Input[_builtins.str] type: Type of the SSL certificate. Valid values: `CA` and `SVR`.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: CertificateArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a SSL certificate.

        Upload the existing SSL certificate to Tencent Cloud.

        ## Example Usage

        ```python
        import pulumi
        import pulumi_tencentcloud as tencentcloud
        import tencentcloud_iac_pulumi as tencentcloud

        config = pulumi.Config()
        ca = config.get("ca")
        if ca is None:
            ca = \"\"\"-----BEGIN CERTIFICATE-----
        
        -----END CERTIFICATE-----\"\"\"
        ca_certificate = tencentcloud.ssl.Certificate("ca",
            name="ssl-ca",
            type="CA",
            cert=ca)
        ca_get_certificates = tencentcloud.ssl.get_certificates_output(name=ca_certificate.name)
        ```

        ```python
        import pulumi
        import pulumi_tencentcloud as tencentcloud
        import tencentcloud_iac_pulumi as tencentcloud

        config = pulumi.Config()
        cert = config.get("cert")
        if cert is None:
            cert = \"\"\"-----BEGIN CERTIFICATE-----
        
        -----END CERTIFICATE-----\"\"\"
        key = config.get("key")
        if key is None:
            key = \"\"\"Public Key Info:
        \\x09Public Key Algorithm: RSA
        \\x09Key Security Level: High (3072 bits)

        modulus:
        \\x0900:d5:1c:a9:f8:32:8a:48:d1:4a:66:c3:ed:c1:f4:7e
        \\x098f:33:a7:43:c6:05:89:e2:0a:62:06:a4:57:af:3c:77
        \\x09c6:20:a6:89:5e:9d:b9:9e:10:fa:f3:4f:ce:ed:ba:71
        \\x0943:ec:6f:5f:e8:02:c6:34:3f:2b:8f:b9:e9:8f:40:28
        \\x09c3:18:4e:2f:f8:7f:bf:50:fd:bb:45:2c:21:a8:f9:8b
        \\x0961:96:26:ba:8d:02:11:8b:6b:15:a2:10:1f:e1:cb:95
        \\x098e:8b:0c:43:ac:67:a2:81:0a:89:90:53:39:d8:4a:7b
        \\x091b:56:31:ec:f1:c9:98:a1:46:c0:df:45:84:8b:05:8b
        \\x0957:0d:65:f5:51:a2:96:6e:09:ef:d1:30:f3:6e:b0:d5
        \\x0959:68:88:be:a9:c1:1f:7e:c1:35:72:67:e6:b9:29:46
        \\x09dc:0a:74:44:a3:88:c9:1d:1a:8c:31:33:a9:99:fd:ff
        \\x0944:7b:57:05:82:17:de:ea:cb:c4:cd:62:4f:78:2b:fd
        \\x096d:ab:7c:f2:85:5c:d1:67:cd:f1:39:27:eb:4e:a1:1f
        \\x096f:7b:28:dc:05:36:8f:22:a0:61:dc:2b:fc:56:52:ef
        \\x0965:42:1d:2f:08:b2:58:27:a8:95:b8:8b:32:17:66:a8
        \\x0908:2a:61:da:45:9b:4c:37:88:d4:96:88:ba:97:fe:a5
        \\x0992:db:7a:2e:c8:21:f7:11:02:79:cc:73:89:22:6d:3c
        \\x09c6:a8:83:cf:1a:0b:c1:e7:32:a2:49:de:44:9c:18:22
        \\x09f7:e8:66:06:f8:a6:b0:e4:f9:c3:95:65:c1:69:e8:83
        \\x097b:cc:ff:79:62:bd:f9:1a:f6:33:ea:e1:4c:dc:68:b0
        \\x0905:46:7c:32:72:c4:05:f2:89:7e:3d:2d:0d:2f:ed:ff
        \\x096a:cf:07:3f:d0:98:83:6e:fb:14:bf:57:26:e8:e8:29
        \\x09a0:43:0f:f5:25:ea:a8:17:70:1a:a7:8a:6c:3d:bf:da
        \\x0939:4f:e3:cc:65:60:fe:a5:d5:2a:09:1b:71:a0:67:c7
        \\x095b:

        public exponent:
        \\x0901:00:01:

        private exponent:
        \\x0900:b1:56:d0:fa:00:d4:a2:13:c7:5e:0c:dc:e4:f1:97
        \\x09ff:82:74:46:29:9a:a2:4a:bf:69:23:2d:ce:e9:bb:df
        \\x09cf:b7:8b:dd:f4:26:3c:38:14:d9:3f:6f:c2:3a:81:53
        \\x098f:ba:48:53:fe:b5:90:4a:19:e7:1e:0b:0f:18:6d:c3
        \\x097d:d5:d3:fa:87:47:86:e4:d6:bf:e7:a7:f9:ba:ab:2e
        \\x0919:5e:e1:8b:8b:9b:95:0d:f7:66:61:1e:19:e9:c3:88
        \\x0908:be:1c:ce:93:c1:09:b1:68:1b:61:46:60:74:64:46
        \\x095d:51:34:ea:7f:a9:ca:a1:2a:47:85:84:4b:ef:84:05
        \\x0997:c3:46:7d:06:19:ce:24:73:90:64:fb:df:16:d5:80
        \\x0934:8e:90:7c:58:b6:a4:86:ce:30:b3:ab:52:8b:f2:95
        \\x094c:b6:46:5a:77:db:73:c0:0c:3f:6d:12:18:a8:54:7c
        \\x09ff:77:c3:ca:89:9f:63:98:ef:48:2d:c1:09:70:6e:ea
        \\x09cb:bb:78:91:42:8a:22:3e:21:ef:a5:bf:16:ee:66:45
        \\x09e5:f0:26:6a:85:8e:e1:69:62:ac:05:00:a6:44:ba:c8
        \\x09ac:10:00:97:f5:51:65:7f:9a:1f:7b:99:9d:02:d4:87
        \\x0950:ce:74:06:51:67:fa:fb:90:e4:33:79:f2:a8:61:ee
        \\x0945:1d:87:ca:22:5b:ac:e7:32:38:f8:2c:fd:55:92:1e
        \\x093d:60:1e:7c:4b:fd:28:ff:e5:b1:02:6a:aa:22:f7:ae
        \\x09a8:36:90:7b:a6:f7:29:05:14:3a:21:da:36:05:f9:b0
        \\x099d:f7:fb:10:75:d7:2c:21:32:95:e7:f7:17:be:09:cb
        \\x0966:fe:f1:69:71:df:a4:5e:3f:fc:67:6c:37:65:b8:51
        \\x09c6:22:38:fb:07:ce:89:54:50:43:71:44:3d:c3:51:5a
        \\x09bd:e5:c7:87:b2:ea:7b:64:0f:5d:34:9c:a1:52:b3:ce
        \\x0906:86:ba:7a:05:80:48:b3:6c:1b:79:74:9b:49:f2:30
        \\x09c1:

        prime1:
        \\x0900:e4:31:46:59:3d:24:f7:31:d9:22:26:af:c0:3e:f5
        \\x09c1:6d:be:ba:d3:9e:3f:b9:2c:43:a0:d0:47:09:e4:35
        \\x0963:19:a4:33:82:af:f9:76:3c:11:c2:cb:34:f9:a6:ab
        \\x09dd:ab:64:5a:6b:9c:c1:2a:52:89:64:7e:b5:a7:f0:4d
        \\x0929:13:a4:cf:17:f4:f2:0d:a0:6e:b9:5d:95:41:10:df
        \\x09ae:f3:7a:13:49:21:66:73:2a:b7:e2:8d:7c:c2:34:e5
        \\x093f:bd:78:ca:fc:64:c5:1c:3a:66:7a:12:53:96:bd:b0
        \\x09c3:7a:0c:ec:5e:55:c0:c3:3f:7f:25:72:f4:e2:19:94
        \\x099d:65:15:be:c8:82:20:57:12:97:b2:a8:4d:3d:e0:8f
        \\x09e2:1f:d0:c8:49:aa:f4:34:fa:91:d1:d1:cc:98:bc:3d
        \\x098b:b1:9b:8f:fd:ef:03:dd:92:fb:ca:99:45:af:cc:83
        \\x0958:4c:bb:ba:73:9e:23:84:f9:7e:4f:40:fe:00:b5:bf
        \\x096f:

        prime2:
        \\x0900:ef:14:ef:73:fc:0c:fc:e3:87:d9:7f:a6:f8:55:86
        \\x0957:63:8a:86:87:f5:ef:63:20:1f:b2:ae:28:dc:ab:59
        \\x0980:8f:15:64:44:e2:bc:a5:7b:d7:69:ef:30:b1:83:b3
        \\x09bd:09:fd:4a:0c:c6:31:5b:a4:79:d0:e5:d3:a8:31:fd
        \\x0959:ea:52:63:cf:17:a7:c1:54:bf:a8:11:9b:b1:85:47
        \\x095a:08:a6:9c:2f:47:9d:ac:5d:e8:7c:e4:31:6c:99:71
        \\x0904:7d:20:98:be:8b:60:07:66:2d:b9:41:10:ea:dd:5b
        \\x0987:20:65:62:ea:75:a7:a6:04:a2:18:66:6b:db:5b:a4
        \\x099f:12:97:cb:7c:8c:d2:e0:ce:02:ef:1e:df:a1:9d:6a
        \\x09bc:00:38:18:36:a1:c5:97:16:be:7a:df:5f:4f:4f:de
        \\x09a3:cb:25:fe:f6:67:0d:31:aa:0a:d4:1b:be:df:91:2c
        \\x0905:14:20:37:cc:4f:50:33:a6:50:1b:90:f9:b2:08:80
        \\x09d5:

        coefficient:
        \\x0947:d1:7f:ca:93:6a:14:9b:fe:85:8d:c2:15:11:52:a2
        \\x09a5:bc:f5:6b:a2:69:76:49:1e:09:79:f1:15:bf:39:48
        \\x0941:ff:92:78:af:bc:7d:6f:76:3b:32:9e:08:d2:42:06
        \\x0904:5f:36:e0:be:a8:1d:21:5c:ec:39:09:e0:77:c5:86
        \\x0906:e6:ce:98:16:fc:0f:30:de:a7:69:7a:8f:dd:01:42
        \\x092a:22:f5:b7:c2:fc:c8:90:5a:78:dc:b3:e0:4d:e7:2d
        \\x0998:6c:e3:34:1b:d7:e8:f8:90:57:7e:4d:41:d6:4a:29
        \\x0981:92:eb:89:5b:45:85:dd:b9:16:20:63:cb:59:f6:06
        \\x0959:c1:dd:3b:6b:92:0a:5e:5e:63:4a:f1:a7:d5:16:b9
        \\x098b:6c:d8:ad:76:0e:2d:3c:e0:b3:73:e0:6d:af:d4:a2
        \\x09bc:4b:fd:6c:2d:d7:5d:4d:cd:28:03:64:b2:ef:9a:1d
        \\x0982:8d:53:40:c5:f8:fb:f3:63:de:8e:1a:21:b6:35:14
        \\x09

        exp1:
        \\x0900:9c:a5:8a:d2:65:dc:03:69:8f:d2:16:d6:9d:55:5b
        \\x0925:4e:ae:18:d8:7e:90:e6:10:11:d8:ca:41:89:f3:c4
        \\x0906:64:aa:c8:c5:95:01:dd:fd:7c:7f:c9:39:73:8b:cb
        \\x09fd:9e:d3:84:12:cd:87:f9:02:b1:d8:6f:f7:49:f2:f7
        \\x0935:14:8c:15:b2:2f:6f:1e:95:9c:8c:d9:46:45:65:4c
        \\x09f8:6f:a1:c4:ad:76:25:3b:37:ff:05:a1:f5:1b:e8:6d
        \\x09db:64:b9:10:37:55:01:ce:cf:f4:5b:26:4b:85:76:70
        \\x096a:b0:55:40:c9:bd:7a:57:4e:36:7d:41:be:03:9c:65
        \\x09dd:ea:6f:94:09:56:f2:d6:73:27:f9:f7:f9:16:5a:1a
        \\x09cb:b2:e5:83:28:b7:17:6f:6a:f7:41:1f:11:a1:63:cf
        \\x09a8:1e:e3:58:64:8c:78:8d:d9:81:c9:e1:8f:ea:0f:ad
        \\x09b6:a6:ee:54:1f:5c:56:ab:c9:0d:c1:60:2f:3d:d3:86
        \\x0937:

        exp2:
        \\x0964:12:b7:48:2d:30:a2:89:fa:cb:27:8b:94:56:f8:2c
        \\x098c:15:e7:c9:f1:3f:8a:96:5f:ae:43:08:07:96:11:98
        \\x09a6:4b:a5:f4:cf:93:77:11:27:51:c8:34:f1:98:d7:1b
        \\x0941:9b:2b:eb:bc:e9:dc:1a:34:83:24:30:3c:2e:f0:85
        \\x093a:77:d2:1f:55:1f:7a:e5:26:74:0b:2a:c8:5b:a9:4a
        \\x091e:64:de:eb:4b:66:cc:47:62:91:24:53:2b:c9:ee:6c
        \\x099a:93:92:5b:ef:aa:fa:6d:e2:a5:b0:7e:8c:50:ab:87
        \\x091c:20:54:0f:1f:c0:54:d5:8b:a3:fa:fb:1a:8e:79:91
        \\x09bc:0e:9d:b6:3c:9b:e8:4d:53:1d:14:27:37:56:d4:de
        \\x096c:99:0e:49:8f:dd:4d:28:d0:02:4e:8d:6e:7d:58:0b
        \\x09e7:74:b8:0c:1b:86:82:4b:52:cd:05:f0:17:54:84:c0
        \\x097b:74:20:e6:fc:2b:ed:f2:a7:85:62:61:a2:0b:bd:21
        \\x09


        Public Key PIN:
        \\x09pin-sha256:t5OXXC5gYqMNtUMsTqRs3A3vhfK2BiXVOgYzIEYv7Y8=
        Public Key ID:
        \\x09sha256:b793975c2e6062a30db5432c4ea46cdc0def85f2b60625d53a063320462fed8f
        \\x09sha1:2f058a05034b2fdb3771bded4e7c8f560fa6a423

        -----BEGIN RSA PRIVATE KEY-----
        
        -----END RSA PRIVATE KEY-----\"\"\"
        svr_certificate = tencentcloud.ssl.Certificate("svr",
            name="ssl-svr",
            type="SVR",
            cert=cert,
            key=key)
        svr = tencentcloud.ssl.get_certificates_output(name=svr_certificate.name)
        ```

        ## Import

        ssl certificate can be imported using the id, e.g.

        ```sh
          $ pulumi import tencentcloud:Ssl/certificate:Certificate cert GjTNRoK7
        ```


        :param str resource_name: The name of the resource.
        :param CertificateArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            CertificateArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        cert: pulumi.Input[_builtins.str | None] = None,
        key: pulumi.Input[_builtins.str | None] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        project_id: pulumi.Input[_builtins.int | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        type: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = CertificateArgs.__new__(CertificateArgs)

            if cert is None and not opts.urn:
                raise TypeError("Missing required property 'cert'")
            __props__.__dict__["cert"] = cert
            __props__.__dict__["key"] = (
                None if key is None else pulumi.Output.secret(key)
            )
            __props__.__dict__["name"] = name
            __props__.__dict__["project_id"] = project_id
            __props__.__dict__["tags"] = tags
            if type is None and not opts.urn:
                raise TypeError("Missing required property 'type'")
            __props__.__dict__["type"] = type
            __props__.__dict__["begin_time"] = None
            __props__.__dict__["create_time"] = None
            __props__.__dict__["domain"] = None
            __props__.__dict__["end_time"] = None
            __props__.__dict__["product_zh_name"] = None
            __props__.__dict__["status"] = None
            __props__.__dict__["subject_names"] = None
        secret_opts = pulumi.ResourceOptions(additional_secret_outputs=["key"])
        opts = pulumi.ResourceOptions.merge(opts, secret_opts)
        super().__init__(
            "tencentcloud:Ssl/certificate:Certificate", resource_name, __props__, opts
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        begin_time: pulumi.Input[_builtins.str | None] = None,
        cert: pulumi.Input[_builtins.str | None] = None,
        create_time: pulumi.Input[_builtins.str | None] = None,
        domain: pulumi.Input[_builtins.str | None] = None,
        end_time: pulumi.Input[_builtins.str | None] = None,
        key: pulumi.Input[_builtins.str | None] = None,
        name: pulumi.Input[_builtins.str | None] = None,
        product_zh_name: pulumi.Input[_builtins.str | None] = None,
        project_id: pulumi.Input[_builtins.int | None] = None,
        status: pulumi.Input[_builtins.int | None] = None,
        subject_names: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        type: pulumi.Input[_builtins.str | None] = None,
    ) -> "Certificate":
        """
        Get an existing Certificate resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] begin_time: Beginning time of the SSL certificate.
        :param pulumi.Input[_builtins.str] cert: Content of the SSL certificate. Not allowed newline at the start and end.
        :param pulumi.Input[_builtins.str] create_time: Creation time of the SSL certificate.
        :param pulumi.Input[_builtins.str] domain: Primary domain of the SSL certificate.
        :param pulumi.Input[_builtins.str] end_time: Ending time of the SSL certificate.
        :param pulumi.Input[_builtins.str] key: Key of the SSL certificate and required when certificate type is `SVR`. Not allowed newline at the start and end.
        :param pulumi.Input[_builtins.str] name: Name of the SSL certificate.
        :param pulumi.Input[_builtins.str] product_zh_name: Certificate authority.
        :param pulumi.Input[_builtins.int] project_id: Project ID of the SSL certificate. Default is `0`.
        :param pulumi.Input[_builtins.int] status: Status of the SSL certificate.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] subject_names: ALL domains included in the SSL certificate. Including the primary domain name.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tags of the SSL certificate.
        :param pulumi.Input[_builtins.str] type: Type of the SSL certificate. Valid values: `CA` and `SVR`.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _CertificateState.__new__(_CertificateState)

        __props__.__dict__["begin_time"] = begin_time
        __props__.__dict__["cert"] = cert
        __props__.__dict__["create_time"] = create_time
        __props__.__dict__["domain"] = domain
        __props__.__dict__["end_time"] = end_time
        __props__.__dict__["key"] = key
        __props__.__dict__["name"] = name
        __props__.__dict__["product_zh_name"] = product_zh_name
        __props__.__dict__["project_id"] = project_id
        __props__.__dict__["status"] = status
        __props__.__dict__["subject_names"] = subject_names
        __props__.__dict__["tags"] = tags
        __props__.__dict__["type"] = type
        return Certificate(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="beginTime")
    def begin_time(self) -> pulumi.Output[_builtins.str]:
        """
        Beginning time of the SSL certificate.
        """
        return pulumi.get(self, "begin_time")

    @_builtins.property
    @pulumi.getter
    def cert(self) -> pulumi.Output[_builtins.str]:
        """
        Content of the SSL certificate. Not allowed newline at the start and end.
        """
        return pulumi.get(self, "cert")

    @_builtins.property
    @pulumi.getter(name="createTime")
    def create_time(self) -> pulumi.Output[_builtins.str]:
        """
        Creation time of the SSL certificate.
        """
        return pulumi.get(self, "create_time")

    @_builtins.property
    @pulumi.getter
    def domain(self) -> pulumi.Output[_builtins.str]:
        """
        Primary domain of the SSL certificate.
        """
        return pulumi.get(self, "domain")

    @_builtins.property
    @pulumi.getter(name="endTime")
    def end_time(self) -> pulumi.Output[_builtins.str]:
        """
        Ending time of the SSL certificate.
        """
        return pulumi.get(self, "end_time")

    @_builtins.property
    @pulumi.getter
    def key(self) -> pulumi.Output[_builtins.str | None]:
        """
        Key of the SSL certificate and required when certificate type is `SVR`. Not allowed newline at the start and end.
        """
        return pulumi.get(self, "key")

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Output[_builtins.str]:
        """
        Name of the SSL certificate.
        """
        return pulumi.get(self, "name")

    @_builtins.property
    @pulumi.getter(name="productZhName")
    def product_zh_name(self) -> pulumi.Output[_builtins.str]:
        """
        Certificate authority.
        """
        return pulumi.get(self, "product_zh_name")

    @_builtins.property
    @pulumi.getter(name="projectId")
    def project_id(self) -> pulumi.Output[_builtins.int | None]:
        """
        Project ID of the SSL certificate. Default is `0`.
        """
        return pulumi.get(self, "project_id")

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Output[_builtins.int]:
        """
        Status of the SSL certificate.
        """
        return pulumi.get(self, "status")

    @_builtins.property
    @pulumi.getter(name="subjectNames")
    def subject_names(self) -> pulumi.Output[Sequence[_builtins.str]]:
        """
        ALL domains included in the SSL certificate. Including the primary domain name.
        """
        return pulumi.get(self, "subject_names")

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Output[Mapping[str, _builtins.str]]:
        """
        Tags of the SSL certificate.
        """
        return pulumi.get(self, "tags")

    @_builtins.property
    @pulumi.getter
    def type(self) -> pulumi.Output[_builtins.str]:
        """
        Type of the SSL certificate. Valid values: `CA` and `SVR`.
        """
        return pulumi.get(self, "type")
