# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs

__all__ = [
    "AwaitableGetTagRetentionExecutionTasksResult",
    "GetTagRetentionExecutionTasksResult",
    "get_tag_retention_execution_tasks",
    "get_tag_retention_execution_tasks_output",
]


@pulumi.output_type
class GetTagRetentionExecutionTasksResult:
    """
    A collection of values returned by getTagRetentionExecutionTasks.
    """

    def __init__(
        __self__,
        execution_id=None,
        id=None,
        registry_id=None,
        result_output_file=None,
        retention_id=None,
        retention_task_lists=None,
    ):
        if execution_id and not isinstance(execution_id, int):
            raise TypeError("Expected argument 'execution_id' to be a int")
        pulumi.set(__self__, "execution_id", execution_id)
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)
        if registry_id and not isinstance(registry_id, str):
            raise TypeError("Expected argument 'registry_id' to be a str")
        pulumi.set(__self__, "registry_id", registry_id)
        if result_output_file and not isinstance(result_output_file, str):
            raise TypeError("Expected argument 'result_output_file' to be a str")
        pulumi.set(__self__, "result_output_file", result_output_file)
        if retention_id and not isinstance(retention_id, int):
            raise TypeError("Expected argument 'retention_id' to be a int")
        pulumi.set(__self__, "retention_id", retention_id)
        if retention_task_lists and not isinstance(retention_task_lists, list):
            raise TypeError("Expected argument 'retention_task_lists' to be a list")
        pulumi.set(__self__, "retention_task_lists", retention_task_lists)

    @_builtins.property
    @pulumi.getter(name="executionId")
    def execution_id(self) -> _builtins.int:
        """
        the rule execution id.
        """
        return pulumi.get(self, "execution_id")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The provider-assigned unique ID for this managed resource.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="registryId")
    def registry_id(self) -> _builtins.str:
        return pulumi.get(self, "registry_id")

    @_builtins.property
    @pulumi.getter(name="resultOutputFile")
    def result_output_file(self) -> _builtins.str | None:
        return pulumi.get(self, "result_output_file")

    @_builtins.property
    @pulumi.getter(name="retentionId")
    def retention_id(self) -> _builtins.int:
        return pulumi.get(self, "retention_id")

    @_builtins.property
    @pulumi.getter(name="retentionTaskLists")
    def retention_task_lists(
        self,
    ) -> Sequence["outputs.GetTagRetentionExecutionTasksRetentionTaskListResult"]:
        """
        list of version retention tasks.
        """
        return pulumi.get(self, "retention_task_lists")


class AwaitableGetTagRetentionExecutionTasksResult(GetTagRetentionExecutionTasksResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetTagRetentionExecutionTasksResult(
            execution_id=self.execution_id,
            id=self.id,
            registry_id=self.registry_id,
            result_output_file=self.result_output_file,
            retention_id=self.retention_id,
            retention_task_lists=self.retention_task_lists,
        )


def get_tag_retention_execution_tasks(
    execution_id: _builtins.int | None = None,
    registry_id: _builtins.str | None = None,
    result_output_file: _builtins.str | None = None,
    retention_id: _builtins.int | None = None,
    opts: pulumi.InvokeOptions | None = None,
) -> AwaitableGetTagRetentionExecutionTasksResult:
    """
    Use this data source to query detailed information of tcr tag_retention_execution_tasks

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    tasks = tencentcloud.tcr.get_tag_retention_execution_tasks(registry_id="tcr_ins_id",
        retention_id=1,
        execution_id=1)
    ```


    :param _builtins.int execution_id: execution id.
    :param _builtins.str registry_id: instance id.
    :param _builtins.str result_output_file: Used to save results.
    :param _builtins.int retention_id: retention id.
    """
    __args__ = dict()
    __args__["executionId"] = execution_id
    __args__["registryId"] = registry_id
    __args__["resultOutputFile"] = result_output_file
    __args__["retentionId"] = retention_id
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke(
        "tencentcloud:Tcr/getTagRetentionExecutionTasks:getTagRetentionExecutionTasks",
        __args__,
        opts=opts,
        typ=GetTagRetentionExecutionTasksResult,
    ).value

    return AwaitableGetTagRetentionExecutionTasksResult(
        execution_id=pulumi.get(__ret__, "execution_id"),
        id=pulumi.get(__ret__, "id"),
        registry_id=pulumi.get(__ret__, "registry_id"),
        result_output_file=pulumi.get(__ret__, "result_output_file"),
        retention_id=pulumi.get(__ret__, "retention_id"),
        retention_task_lists=pulumi.get(__ret__, "retention_task_lists"),
    )


def get_tag_retention_execution_tasks_output(
    execution_id: pulumi.Input[_builtins.int | None] = None,
    registry_id: pulumi.Input[_builtins.str | None] = None,
    result_output_file: pulumi.Input[_builtins.str | None] = None,
    retention_id: pulumi.Input[_builtins.int | None] = None,
    opts: pulumi.InvokeOptions | pulumi.InvokeOutputOptions | None = None,
) -> pulumi.Output[GetTagRetentionExecutionTasksResult]:
    """
    Use this data source to query detailed information of tcr tag_retention_execution_tasks

    ## Example Usage

    ```python
    import pulumi
    import pulumi_tencentcloud as tencentcloud

    tasks = tencentcloud.tcr.get_tag_retention_execution_tasks(registry_id="tcr_ins_id",
        retention_id=1,
        execution_id=1)
    ```


    :param _builtins.int execution_id: execution id.
    :param _builtins.str registry_id: instance id.
    :param _builtins.str result_output_file: Used to save results.
    :param _builtins.int retention_id: retention id.
    """
    __args__ = dict()
    __args__["executionId"] = execution_id
    __args__["registryId"] = registry_id
    __args__["resultOutputFile"] = result_output_file
    __args__["retentionId"] = retention_id
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output(
        "tencentcloud:Tcr/getTagRetentionExecutionTasks:getTagRetentionExecutionTasks",
        __args__,
        opts=opts,
        typ=GetTagRetentionExecutionTasksResult,
    )
    return __ret__.apply(
        lambda __response__: GetTagRetentionExecutionTasksResult(
            execution_id=pulumi.get(__response__, "execution_id"),
            id=pulumi.get(__response__, "id"),
            registry_id=pulumi.get(__response__, "registry_id"),
            result_output_file=pulumi.get(__response__, "result_output_file"),
            retention_id=pulumi.get(__response__, "retention_id"),
            retention_task_lists=pulumi.get(__response__, "retention_task_lists"),
        )
    )
