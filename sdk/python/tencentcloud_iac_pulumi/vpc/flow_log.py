# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Mapping
from typing import Optional, Union, overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ["FlowLog", "FlowLogArgs"]


@pulumi.input_type
class FlowLogArgs:
    def __init__(
        __self__,
        *,
        flow_log_name: pulumi.Input[_builtins.str],
        resource_id: pulumi.Input[_builtins.str],
        resource_type: pulumi.Input[_builtins.str],
        traffic_type: pulumi.Input[_builtins.str],
        cloud_log_id: pulumi.Input[_builtins.str | None] = None,
        cloud_log_region: pulumi.Input[_builtins.str | None] = None,
        flow_log_description: pulumi.Input[_builtins.str | None] = None,
        flow_log_storage: pulumi.Input[Optional["FlowLogFlowLogStorageArgs"]] = None,
        storage_type: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        vpc_id: pulumi.Input[_builtins.str | None] = None
    ):
        """
        The set of arguments for constructing a FlowLog resource.

        :param pulumi.Input[_builtins.str] flow_log_name: The name of the flow log instance.
        :param pulumi.Input[_builtins.str] resource_id: The unique ID of the resource.
        :param pulumi.Input[_builtins.str] resource_type: The type of resource associated with the flow log. Valid values: `VPC`, `SUBNET`, `NETWORKINTERFACE`, `CCN`, `NAT`, and `DCG`.
        :param pulumi.Input[_builtins.str] traffic_type: Type of the flow logs to be collected. Valid values: `ACCEPT`, `REJECT` and `ALL`.
        :param pulumi.Input[_builtins.str] cloud_log_id: The storage ID of the flow log.
        :param pulumi.Input[_builtins.str] cloud_log_region: The region corresponding to the flow log storage ID. If not passed in, this field defaults to the current region.
        :param pulumi.Input[_builtins.str] flow_log_description: The description of the flow log.
        :param pulumi.Input['FlowLogFlowLogStorageArgs'] flow_log_storage: Information of the flow log consumer, required while `storage_type` is `ckafka`.
        :param pulumi.Input[_builtins.str] storage_type: Consumer types: `cls` and `ckafka`.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tag description list.
        :param pulumi.Input[_builtins.str] vpc_id: The VPC ID or unique ID of the resource. We recommend using the unique ID. This parameter is required unless the `ResourceType` is set to `CCN`.
        """
        pulumi.set(__self__, "flow_log_name", flow_log_name)
        pulumi.set(__self__, "resource_id", resource_id)
        pulumi.set(__self__, "resource_type", resource_type)
        pulumi.set(__self__, "traffic_type", traffic_type)
        if cloud_log_id is not None:
            pulumi.set(__self__, "cloud_log_id", cloud_log_id)
        if cloud_log_region is not None:
            pulumi.set(__self__, "cloud_log_region", cloud_log_region)
        if flow_log_description is not None:
            pulumi.set(__self__, "flow_log_description", flow_log_description)
        if flow_log_storage is not None:
            pulumi.set(__self__, "flow_log_storage", flow_log_storage)
        if storage_type is not None:
            pulumi.set(__self__, "storage_type", storage_type)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)
        if vpc_id is not None:
            pulumi.set(__self__, "vpc_id", vpc_id)

    @_builtins.property
    @pulumi.getter(name="flowLogName")
    def flow_log_name(self) -> pulumi.Input[_builtins.str]:
        """
        The name of the flow log instance.
        """
        return pulumi.get(self, "flow_log_name")

    @flow_log_name.setter
    def flow_log_name(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "flow_log_name", value)

    @_builtins.property
    @pulumi.getter(name="resourceId")
    def resource_id(self) -> pulumi.Input[_builtins.str]:
        """
        The unique ID of the resource.
        """
        return pulumi.get(self, "resource_id")

    @resource_id.setter
    def resource_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "resource_id", value)

    @_builtins.property
    @pulumi.getter(name="resourceType")
    def resource_type(self) -> pulumi.Input[_builtins.str]:
        """
        The type of resource associated with the flow log. Valid values: `VPC`, `SUBNET`, `NETWORKINTERFACE`, `CCN`, `NAT`, and `DCG`.
        """
        return pulumi.get(self, "resource_type")

    @resource_type.setter
    def resource_type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "resource_type", value)

    @_builtins.property
    @pulumi.getter(name="trafficType")
    def traffic_type(self) -> pulumi.Input[_builtins.str]:
        """
        Type of the flow logs to be collected. Valid values: `ACCEPT`, `REJECT` and `ALL`.
        """
        return pulumi.get(self, "traffic_type")

    @traffic_type.setter
    def traffic_type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "traffic_type", value)

    @_builtins.property
    @pulumi.getter(name="cloudLogId")
    def cloud_log_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        The storage ID of the flow log.
        """
        return pulumi.get(self, "cloud_log_id")

    @cloud_log_id.setter
    def cloud_log_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "cloud_log_id", value)

    @_builtins.property
    @pulumi.getter(name="cloudLogRegion")
    def cloud_log_region(self) -> pulumi.Input[_builtins.str | None]:
        """
        The region corresponding to the flow log storage ID. If not passed in, this field defaults to the current region.
        """
        return pulumi.get(self, "cloud_log_region")

    @cloud_log_region.setter
    def cloud_log_region(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "cloud_log_region", value)

    @_builtins.property
    @pulumi.getter(name="flowLogDescription")
    def flow_log_description(self) -> pulumi.Input[_builtins.str | None]:
        """
        The description of the flow log.
        """
        return pulumi.get(self, "flow_log_description")

    @flow_log_description.setter
    def flow_log_description(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "flow_log_description", value)

    @_builtins.property
    @pulumi.getter(name="flowLogStorage")
    def flow_log_storage(self) -> pulumi.Input[Optional["FlowLogFlowLogStorageArgs"]]:
        """
        Information of the flow log consumer, required while `storage_type` is `ckafka`.
        """
        return pulumi.get(self, "flow_log_storage")

    @flow_log_storage.setter
    def flow_log_storage(
        self, value: pulumi.Input[Optional["FlowLogFlowLogStorageArgs"]]
    ):
        pulumi.set(self, "flow_log_storage", value)

    @_builtins.property
    @pulumi.getter(name="storageType")
    def storage_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Consumer types: `cls` and `ckafka`.
        """
        return pulumi.get(self, "storage_type")

    @storage_type.setter
    def storage_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "storage_type", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]:
        """
        Tag description list.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(
        self, value: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "tags", value)

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        The VPC ID or unique ID of the resource. We recommend using the unique ID. This parameter is required unless the `ResourceType` is set to `CCN`.
        """
        return pulumi.get(self, "vpc_id")

    @vpc_id.setter
    def vpc_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "vpc_id", value)


@pulumi.input_type
class _FlowLogState:
    def __init__(
        __self__,
        *,
        cloud_log_id: pulumi.Input[_builtins.str | None] = None,
        cloud_log_region: pulumi.Input[_builtins.str | None] = None,
        flow_log_description: pulumi.Input[_builtins.str | None] = None,
        flow_log_name: pulumi.Input[_builtins.str | None] = None,
        flow_log_storage: pulumi.Input[Optional["FlowLogFlowLogStorageArgs"]] = None,
        resource_id: pulumi.Input[_builtins.str | None] = None,
        resource_type: pulumi.Input[_builtins.str | None] = None,
        storage_type: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        traffic_type: pulumi.Input[_builtins.str | None] = None,
        vpc_id: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering FlowLog resources.

        :param pulumi.Input[_builtins.str] cloud_log_id: The storage ID of the flow log.
        :param pulumi.Input[_builtins.str] cloud_log_region: The region corresponding to the flow log storage ID. If not passed in, this field defaults to the current region.
        :param pulumi.Input[_builtins.str] flow_log_description: The description of the flow log.
        :param pulumi.Input[_builtins.str] flow_log_name: The name of the flow log instance.
        :param pulumi.Input['FlowLogFlowLogStorageArgs'] flow_log_storage: Information of the flow log consumer, required while `storage_type` is `ckafka`.
        :param pulumi.Input[_builtins.str] resource_id: The unique ID of the resource.
        :param pulumi.Input[_builtins.str] resource_type: The type of resource associated with the flow log. Valid values: `VPC`, `SUBNET`, `NETWORKINTERFACE`, `CCN`, `NAT`, and `DCG`.
        :param pulumi.Input[_builtins.str] storage_type: Consumer types: `cls` and `ckafka`.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tag description list.
        :param pulumi.Input[_builtins.str] traffic_type: Type of the flow logs to be collected. Valid values: `ACCEPT`, `REJECT` and `ALL`.
        :param pulumi.Input[_builtins.str] vpc_id: The VPC ID or unique ID of the resource. We recommend using the unique ID. This parameter is required unless the `ResourceType` is set to `CCN`.
        """
        if cloud_log_id is not None:
            pulumi.set(__self__, "cloud_log_id", cloud_log_id)
        if cloud_log_region is not None:
            pulumi.set(__self__, "cloud_log_region", cloud_log_region)
        if flow_log_description is not None:
            pulumi.set(__self__, "flow_log_description", flow_log_description)
        if flow_log_name is not None:
            pulumi.set(__self__, "flow_log_name", flow_log_name)
        if flow_log_storage is not None:
            pulumi.set(__self__, "flow_log_storage", flow_log_storage)
        if resource_id is not None:
            pulumi.set(__self__, "resource_id", resource_id)
        if resource_type is not None:
            pulumi.set(__self__, "resource_type", resource_type)
        if storage_type is not None:
            pulumi.set(__self__, "storage_type", storage_type)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)
        if traffic_type is not None:
            pulumi.set(__self__, "traffic_type", traffic_type)
        if vpc_id is not None:
            pulumi.set(__self__, "vpc_id", vpc_id)

    @_builtins.property
    @pulumi.getter(name="cloudLogId")
    def cloud_log_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        The storage ID of the flow log.
        """
        return pulumi.get(self, "cloud_log_id")

    @cloud_log_id.setter
    def cloud_log_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "cloud_log_id", value)

    @_builtins.property
    @pulumi.getter(name="cloudLogRegion")
    def cloud_log_region(self) -> pulumi.Input[_builtins.str | None]:
        """
        The region corresponding to the flow log storage ID. If not passed in, this field defaults to the current region.
        """
        return pulumi.get(self, "cloud_log_region")

    @cloud_log_region.setter
    def cloud_log_region(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "cloud_log_region", value)

    @_builtins.property
    @pulumi.getter(name="flowLogDescription")
    def flow_log_description(self) -> pulumi.Input[_builtins.str | None]:
        """
        The description of the flow log.
        """
        return pulumi.get(self, "flow_log_description")

    @flow_log_description.setter
    def flow_log_description(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "flow_log_description", value)

    @_builtins.property
    @pulumi.getter(name="flowLogName")
    def flow_log_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        The name of the flow log instance.
        """
        return pulumi.get(self, "flow_log_name")

    @flow_log_name.setter
    def flow_log_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "flow_log_name", value)

    @_builtins.property
    @pulumi.getter(name="flowLogStorage")
    def flow_log_storage(self) -> pulumi.Input[Optional["FlowLogFlowLogStorageArgs"]]:
        """
        Information of the flow log consumer, required while `storage_type` is `ckafka`.
        """
        return pulumi.get(self, "flow_log_storage")

    @flow_log_storage.setter
    def flow_log_storage(
        self, value: pulumi.Input[Optional["FlowLogFlowLogStorageArgs"]]
    ):
        pulumi.set(self, "flow_log_storage", value)

    @_builtins.property
    @pulumi.getter(name="resourceId")
    def resource_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        The unique ID of the resource.
        """
        return pulumi.get(self, "resource_id")

    @resource_id.setter
    def resource_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "resource_id", value)

    @_builtins.property
    @pulumi.getter(name="resourceType")
    def resource_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        The type of resource associated with the flow log. Valid values: `VPC`, `SUBNET`, `NETWORKINTERFACE`, `CCN`, `NAT`, and `DCG`.
        """
        return pulumi.get(self, "resource_type")

    @resource_type.setter
    def resource_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "resource_type", value)

    @_builtins.property
    @pulumi.getter(name="storageType")
    def storage_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Consumer types: `cls` and `ckafka`.
        """
        return pulumi.get(self, "storage_type")

    @storage_type.setter
    def storage_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "storage_type", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]:
        """
        Tag description list.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(
        self, value: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "tags", value)

    @_builtins.property
    @pulumi.getter(name="trafficType")
    def traffic_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Type of the flow logs to be collected. Valid values: `ACCEPT`, `REJECT` and `ALL`.
        """
        return pulumi.get(self, "traffic_type")

    @traffic_type.setter
    def traffic_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "traffic_type", value)

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        The VPC ID or unique ID of the resource. We recommend using the unique ID. This parameter is required unless the `ResourceType` is set to `CCN`.
        """
        return pulumi.get(self, "vpc_id")

    @vpc_id.setter
    def vpc_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "vpc_id", value)


@pulumi.type_token("tencentcloud:Vpc/flowLog:FlowLog")
class FlowLog(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        cloud_log_id: pulumi.Input[_builtins.str | None] = None,
        cloud_log_region: pulumi.Input[_builtins.str | None] = None,
        flow_log_description: pulumi.Input[_builtins.str | None] = None,
        flow_log_name: pulumi.Input[_builtins.str | None] = None,
        flow_log_storage: pulumi.Input[
            Union["FlowLogFlowLogStorageArgs", "FlowLogFlowLogStorageArgsDict"] | None
        ] = None,
        resource_id: pulumi.Input[_builtins.str | None] = None,
        resource_type: pulumi.Input[_builtins.str | None] = None,
        storage_type: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        traffic_type: pulumi.Input[_builtins.str | None] = None,
        vpc_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a vpc flow_log

        > **NOTE:** The cloud server instance specifications that support stream log collection include: M6ce, M6p, SA3se, S4m, DA3, ITA3, I6t, I6, S5se, SA2, SK1, S4, S5, SN3ne, S3ne, S2ne, SA2a, S3ne, SW3a, SW3b, SW3ne, ITA3, IT5c, IT5, IT5c, IT3, I3, D3, DA2, D2, M6, MA2, M4, C6, IT3a, IT3b, IT3c, C4ne, CN3ne, C3ne, GI1, PNV4, GNV4v, GNV4, GT4, GI3X, GN7, GN7vw.

        > **NOTE:** The following models no longer support the collection of new stream logs, and the stock stream logs will no longer be reported for data from July 25, 2022: Standard models: S3, SA1, S2, S1;Memory type: M3, M2, M1;Calculation type: C4, CN3, C3, C2;Batch calculation type: BC1, BS1;HPCC: HCCIC5, HCCG5v.

        ## Import

        vpc flow_log can be imported using the flow log Id combine vpc Id, e.g.

        ```sh
        $ pulumi import tencentcloud:Vpc/flowLog:FlowLog example fl-7k59x22l#vpc-n6qjlen5
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] cloud_log_id: The storage ID of the flow log.
        :param pulumi.Input[_builtins.str] cloud_log_region: The region corresponding to the flow log storage ID. If not passed in, this field defaults to the current region.
        :param pulumi.Input[_builtins.str] flow_log_description: The description of the flow log.
        :param pulumi.Input[_builtins.str] flow_log_name: The name of the flow log instance.
        :param pulumi.Input[Union['FlowLogFlowLogStorageArgs', 'FlowLogFlowLogStorageArgsDict']] flow_log_storage: Information of the flow log consumer, required while `storage_type` is `ckafka`.
        :param pulumi.Input[_builtins.str] resource_id: The unique ID of the resource.
        :param pulumi.Input[_builtins.str] resource_type: The type of resource associated with the flow log. Valid values: `VPC`, `SUBNET`, `NETWORKINTERFACE`, `CCN`, `NAT`, and `DCG`.
        :param pulumi.Input[_builtins.str] storage_type: Consumer types: `cls` and `ckafka`.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tag description list.
        :param pulumi.Input[_builtins.str] traffic_type: Type of the flow logs to be collected. Valid values: `ACCEPT`, `REJECT` and `ALL`.
        :param pulumi.Input[_builtins.str] vpc_id: The VPC ID or unique ID of the resource. We recommend using the unique ID. This parameter is required unless the `ResourceType` is set to `CCN`.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: FlowLogArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a vpc flow_log

        > **NOTE:** The cloud server instance specifications that support stream log collection include: M6ce, M6p, SA3se, S4m, DA3, ITA3, I6t, I6, S5se, SA2, SK1, S4, S5, SN3ne, S3ne, S2ne, SA2a, S3ne, SW3a, SW3b, SW3ne, ITA3, IT5c, IT5, IT5c, IT3, I3, D3, DA2, D2, M6, MA2, M4, C6, IT3a, IT3b, IT3c, C4ne, CN3ne, C3ne, GI1, PNV4, GNV4v, GNV4, GT4, GI3X, GN7, GN7vw.

        > **NOTE:** The following models no longer support the collection of new stream logs, and the stock stream logs will no longer be reported for data from July 25, 2022: Standard models: S3, SA1, S2, S1;Memory type: M3, M2, M1;Calculation type: C4, CN3, C3, C2;Batch calculation type: BC1, BS1;HPCC: HCCIC5, HCCG5v.

        ## Import

        vpc flow_log can be imported using the flow log Id combine vpc Id, e.g.

        ```sh
        $ pulumi import tencentcloud:Vpc/flowLog:FlowLog example fl-7k59x22l#vpc-n6qjlen5
        ```


        :param str resource_name: The name of the resource.
        :param FlowLogArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            FlowLogArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        cloud_log_id: pulumi.Input[_builtins.str | None] = None,
        cloud_log_region: pulumi.Input[_builtins.str | None] = None,
        flow_log_description: pulumi.Input[_builtins.str | None] = None,
        flow_log_name: pulumi.Input[_builtins.str | None] = None,
        flow_log_storage: pulumi.Input[
            Union["FlowLogFlowLogStorageArgs", "FlowLogFlowLogStorageArgsDict"] | None
        ] = None,
        resource_id: pulumi.Input[_builtins.str | None] = None,
        resource_type: pulumi.Input[_builtins.str | None] = None,
        storage_type: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        traffic_type: pulumi.Input[_builtins.str | None] = None,
        vpc_id: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = FlowLogArgs.__new__(FlowLogArgs)

            __props__.__dict__["cloud_log_id"] = cloud_log_id
            __props__.__dict__["cloud_log_region"] = cloud_log_region
            __props__.__dict__["flow_log_description"] = flow_log_description
            if flow_log_name is None and not opts.urn:
                raise TypeError("Missing required property 'flow_log_name'")
            __props__.__dict__["flow_log_name"] = flow_log_name
            __props__.__dict__["flow_log_storage"] = flow_log_storage
            if resource_id is None and not opts.urn:
                raise TypeError("Missing required property 'resource_id'")
            __props__.__dict__["resource_id"] = resource_id
            if resource_type is None and not opts.urn:
                raise TypeError("Missing required property 'resource_type'")
            __props__.__dict__["resource_type"] = resource_type
            __props__.__dict__["storage_type"] = storage_type
            __props__.__dict__["tags"] = tags
            if traffic_type is None and not opts.urn:
                raise TypeError("Missing required property 'traffic_type'")
            __props__.__dict__["traffic_type"] = traffic_type
            __props__.__dict__["vpc_id"] = vpc_id
        super().__init__(
            "tencentcloud:Vpc/flowLog:FlowLog", resource_name, __props__, opts
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        cloud_log_id: pulumi.Input[_builtins.str | None] = None,
        cloud_log_region: pulumi.Input[_builtins.str | None] = None,
        flow_log_description: pulumi.Input[_builtins.str | None] = None,
        flow_log_name: pulumi.Input[_builtins.str | None] = None,
        flow_log_storage: pulumi.Input[
            Union["FlowLogFlowLogStorageArgs", "FlowLogFlowLogStorageArgsDict"] | None
        ] = None,
        resource_id: pulumi.Input[_builtins.str | None] = None,
        resource_type: pulumi.Input[_builtins.str | None] = None,
        storage_type: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        traffic_type: pulumi.Input[_builtins.str | None] = None,
        vpc_id: pulumi.Input[_builtins.str | None] = None,
    ) -> "FlowLog":
        """
        Get an existing FlowLog resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] cloud_log_id: The storage ID of the flow log.
        :param pulumi.Input[_builtins.str] cloud_log_region: The region corresponding to the flow log storage ID. If not passed in, this field defaults to the current region.
        :param pulumi.Input[_builtins.str] flow_log_description: The description of the flow log.
        :param pulumi.Input[_builtins.str] flow_log_name: The name of the flow log instance.
        :param pulumi.Input[Union['FlowLogFlowLogStorageArgs', 'FlowLogFlowLogStorageArgsDict']] flow_log_storage: Information of the flow log consumer, required while `storage_type` is `ckafka`.
        :param pulumi.Input[_builtins.str] resource_id: The unique ID of the resource.
        :param pulumi.Input[_builtins.str] resource_type: The type of resource associated with the flow log. Valid values: `VPC`, `SUBNET`, `NETWORKINTERFACE`, `CCN`, `NAT`, and `DCG`.
        :param pulumi.Input[_builtins.str] storage_type: Consumer types: `cls` and `ckafka`.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tag description list.
        :param pulumi.Input[_builtins.str] traffic_type: Type of the flow logs to be collected. Valid values: `ACCEPT`, `REJECT` and `ALL`.
        :param pulumi.Input[_builtins.str] vpc_id: The VPC ID or unique ID of the resource. We recommend using the unique ID. This parameter is required unless the `ResourceType` is set to `CCN`.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _FlowLogState.__new__(_FlowLogState)

        __props__.__dict__["cloud_log_id"] = cloud_log_id
        __props__.__dict__["cloud_log_region"] = cloud_log_region
        __props__.__dict__["flow_log_description"] = flow_log_description
        __props__.__dict__["flow_log_name"] = flow_log_name
        __props__.__dict__["flow_log_storage"] = flow_log_storage
        __props__.__dict__["resource_id"] = resource_id
        __props__.__dict__["resource_type"] = resource_type
        __props__.__dict__["storage_type"] = storage_type
        __props__.__dict__["tags"] = tags
        __props__.__dict__["traffic_type"] = traffic_type
        __props__.__dict__["vpc_id"] = vpc_id
        return FlowLog(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="cloudLogId")
    def cloud_log_id(self) -> pulumi.Output[_builtins.str | None]:
        """
        The storage ID of the flow log.
        """
        return pulumi.get(self, "cloud_log_id")

    @_builtins.property
    @pulumi.getter(name="cloudLogRegion")
    def cloud_log_region(self) -> pulumi.Output[_builtins.str]:
        """
        The region corresponding to the flow log storage ID. If not passed in, this field defaults to the current region.
        """
        return pulumi.get(self, "cloud_log_region")

    @_builtins.property
    @pulumi.getter(name="flowLogDescription")
    def flow_log_description(self) -> pulumi.Output[_builtins.str | None]:
        """
        The description of the flow log.
        """
        return pulumi.get(self, "flow_log_description")

    @_builtins.property
    @pulumi.getter(name="flowLogName")
    def flow_log_name(self) -> pulumi.Output[_builtins.str]:
        """
        The name of the flow log instance.
        """
        return pulumi.get(self, "flow_log_name")

    @_builtins.property
    @pulumi.getter(name="flowLogStorage")
    def flow_log_storage(self) -> pulumi.Output["outputs.FlowLogFlowLogStorage"]:
        """
        Information of the flow log consumer, required while `storage_type` is `ckafka`.
        """
        return pulumi.get(self, "flow_log_storage")

    @_builtins.property
    @pulumi.getter(name="resourceId")
    def resource_id(self) -> pulumi.Output[_builtins.str]:
        """
        The unique ID of the resource.
        """
        return pulumi.get(self, "resource_id")

    @_builtins.property
    @pulumi.getter(name="resourceType")
    def resource_type(self) -> pulumi.Output[_builtins.str]:
        """
        The type of resource associated with the flow log. Valid values: `VPC`, `SUBNET`, `NETWORKINTERFACE`, `CCN`, `NAT`, and `DCG`.
        """
        return pulumi.get(self, "resource_type")

    @_builtins.property
    @pulumi.getter(name="storageType")
    def storage_type(self) -> pulumi.Output[_builtins.str | None]:
        """
        Consumer types: `cls` and `ckafka`.
        """
        return pulumi.get(self, "storage_type")

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Output[Mapping[str, _builtins.str] | None]:
        """
        Tag description list.
        """
        return pulumi.get(self, "tags")

    @_builtins.property
    @pulumi.getter(name="trafficType")
    def traffic_type(self) -> pulumi.Output[_builtins.str]:
        """
        Type of the flow logs to be collected. Valid values: `ACCEPT`, `REJECT` and `ALL`.
        """
        return pulumi.get(self, "traffic_type")

    @_builtins.property
    @pulumi.getter(name="vpcId")
    def vpc_id(self) -> pulumi.Output[_builtins.str | None]:
        """
        The VPC ID or unique ID of the resource. We recommend using the unique ID. This parameter is required unless the `ResourceType` is set to `CCN`.
        """
        return pulumi.get(self, "vpc_id")
