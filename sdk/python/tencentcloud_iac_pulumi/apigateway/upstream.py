# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Mapping, Sequence
from typing import Optional, Union, overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities
from . import outputs
from ._inputs import *

__all__ = ["Upstream", "UpstreamArgs"]


@pulumi.input_type
class UpstreamArgs:
    def __init__(
        __self__,
        *,
        algorithm: pulumi.Input[_builtins.str],
        scheme: pulumi.Input[_builtins.str],
        uniq_vpc_id: pulumi.Input[_builtins.str],
        health_checker: pulumi.Input[Optional["UpstreamHealthCheckerArgs"]] = None,
        k8s_services: pulumi.Input[
            Sequence[pulumi.Input["UpstreamK8sServiceArgs"]] | None
        ] = None,
        nodes: pulumi.Input[Sequence[pulumi.Input["UpstreamNodeArgs"]] | None] = None,
        retries: pulumi.Input[_builtins.int | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        upstream_description: pulumi.Input[_builtins.str | None] = None,
        upstream_host: pulumi.Input[_builtins.str | None] = None,
        upstream_name: pulumi.Input[_builtins.str | None] = None,
        upstream_type: pulumi.Input[_builtins.str | None] = None
    ):
        """
        The set of arguments for constructing a Upstream resource.

        :param pulumi.Input[_builtins.str] algorithm: Load balancing algorithm, value range: ROUND-ROBIN.
        :param pulumi.Input[_builtins.str] scheme: Backend protocol, value range: HTTP, HTTPS, gRPC, gRPCs.
        :param pulumi.Input[_builtins.str] uniq_vpc_id: VPC Unique ID.
        :param pulumi.Input['UpstreamHealthCheckerArgs'] health_checker: Health check configuration, currently only supports VPC channels.
        :param pulumi.Input[Sequence[pulumi.Input['UpstreamK8sServiceArgs']]] k8s_services: Configuration of K8S container service.
        :param pulumi.Input[Sequence[pulumi.Input['UpstreamNodeArgs']]] nodes: Backend nodes.
        :param pulumi.Input[_builtins.int] retries: Request retry count, default to 3 times.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tag description list.
        :param pulumi.Input[_builtins.str] upstream_description: Backend channel description.
        :param pulumi.Input[_builtins.str] upstream_host: Host request header forwarded by gateway to backend.
        :param pulumi.Input[_builtins.str] upstream_name: Backend channel name.
        :param pulumi.Input[_builtins.str] upstream_type: Backend access type, value range: IP_PORT, K8S.
        """
        pulumi.set(__self__, "algorithm", algorithm)
        pulumi.set(__self__, "scheme", scheme)
        pulumi.set(__self__, "uniq_vpc_id", uniq_vpc_id)
        if health_checker is not None:
            pulumi.set(__self__, "health_checker", health_checker)
        if k8s_services is not None:
            pulumi.set(__self__, "k8s_services", k8s_services)
        if nodes is not None:
            pulumi.set(__self__, "nodes", nodes)
        if retries is not None:
            pulumi.set(__self__, "retries", retries)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)
        if upstream_description is not None:
            pulumi.set(__self__, "upstream_description", upstream_description)
        if upstream_host is not None:
            pulumi.set(__self__, "upstream_host", upstream_host)
        if upstream_name is not None:
            pulumi.set(__self__, "upstream_name", upstream_name)
        if upstream_type is not None:
            pulumi.set(__self__, "upstream_type", upstream_type)

    @_builtins.property
    @pulumi.getter
    def algorithm(self) -> pulumi.Input[_builtins.str]:
        """
        Load balancing algorithm, value range: ROUND-ROBIN.
        """
        return pulumi.get(self, "algorithm")

    @algorithm.setter
    def algorithm(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "algorithm", value)

    @_builtins.property
    @pulumi.getter
    def scheme(self) -> pulumi.Input[_builtins.str]:
        """
        Backend protocol, value range: HTTP, HTTPS, gRPC, gRPCs.
        """
        return pulumi.get(self, "scheme")

    @scheme.setter
    def scheme(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "scheme", value)

    @_builtins.property
    @pulumi.getter(name="uniqVpcId")
    def uniq_vpc_id(self) -> pulumi.Input[_builtins.str]:
        """
        VPC Unique ID.
        """
        return pulumi.get(self, "uniq_vpc_id")

    @uniq_vpc_id.setter
    def uniq_vpc_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "uniq_vpc_id", value)

    @_builtins.property
    @pulumi.getter(name="healthChecker")
    def health_checker(self) -> pulumi.Input[Optional["UpstreamHealthCheckerArgs"]]:
        """
        Health check configuration, currently only supports VPC channels.
        """
        return pulumi.get(self, "health_checker")

    @health_checker.setter
    def health_checker(
        self, value: pulumi.Input[Optional["UpstreamHealthCheckerArgs"]]
    ):
        pulumi.set(self, "health_checker", value)

    @_builtins.property
    @pulumi.getter(name="k8sServices")
    def k8s_services(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input["UpstreamK8sServiceArgs"]] | None]:
        """
        Configuration of K8S container service.
        """
        return pulumi.get(self, "k8s_services")

    @k8s_services.setter
    def k8s_services(
        self,
        value: pulumi.Input[Sequence[pulumi.Input["UpstreamK8sServiceArgs"]] | None],
    ):
        pulumi.set(self, "k8s_services", value)

    @_builtins.property
    @pulumi.getter
    def nodes(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input["UpstreamNodeArgs"]] | None]:
        """
        Backend nodes.
        """
        return pulumi.get(self, "nodes")

    @nodes.setter
    def nodes(
        self, value: pulumi.Input[Sequence[pulumi.Input["UpstreamNodeArgs"]] | None]
    ):
        pulumi.set(self, "nodes", value)

    @_builtins.property
    @pulumi.getter
    def retries(self) -> pulumi.Input[_builtins.int | None]:
        """
        Request retry count, default to 3 times.
        """
        return pulumi.get(self, "retries")

    @retries.setter
    def retries(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "retries", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]:
        """
        Tag description list.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(
        self, value: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "tags", value)

    @_builtins.property
    @pulumi.getter(name="upstreamDescription")
    def upstream_description(self) -> pulumi.Input[_builtins.str | None]:
        """
        Backend channel description.
        """
        return pulumi.get(self, "upstream_description")

    @upstream_description.setter
    def upstream_description(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "upstream_description", value)

    @_builtins.property
    @pulumi.getter(name="upstreamHost")
    def upstream_host(self) -> pulumi.Input[_builtins.str | None]:
        """
        Host request header forwarded by gateway to backend.
        """
        return pulumi.get(self, "upstream_host")

    @upstream_host.setter
    def upstream_host(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "upstream_host", value)

    @_builtins.property
    @pulumi.getter(name="upstreamName")
    def upstream_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Backend channel name.
        """
        return pulumi.get(self, "upstream_name")

    @upstream_name.setter
    def upstream_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "upstream_name", value)

    @_builtins.property
    @pulumi.getter(name="upstreamType")
    def upstream_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Backend access type, value range: IP_PORT, K8S.
        """
        return pulumi.get(self, "upstream_type")

    @upstream_type.setter
    def upstream_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "upstream_type", value)


@pulumi.input_type
class _UpstreamState:
    def __init__(
        __self__,
        *,
        algorithm: pulumi.Input[_builtins.str | None] = None,
        health_checker: pulumi.Input[Optional["UpstreamHealthCheckerArgs"]] = None,
        k8s_services: pulumi.Input[
            Sequence[pulumi.Input["UpstreamK8sServiceArgs"]] | None
        ] = None,
        nodes: pulumi.Input[Sequence[pulumi.Input["UpstreamNodeArgs"]] | None] = None,
        retries: pulumi.Input[_builtins.int | None] = None,
        scheme: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        uniq_vpc_id: pulumi.Input[_builtins.str | None] = None,
        upstream_description: pulumi.Input[_builtins.str | None] = None,
        upstream_host: pulumi.Input[_builtins.str | None] = None,
        upstream_name: pulumi.Input[_builtins.str | None] = None,
        upstream_type: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering Upstream resources.

        :param pulumi.Input[_builtins.str] algorithm: Load balancing algorithm, value range: ROUND-ROBIN.
        :param pulumi.Input['UpstreamHealthCheckerArgs'] health_checker: Health check configuration, currently only supports VPC channels.
        :param pulumi.Input[Sequence[pulumi.Input['UpstreamK8sServiceArgs']]] k8s_services: Configuration of K8S container service.
        :param pulumi.Input[Sequence[pulumi.Input['UpstreamNodeArgs']]] nodes: Backend nodes.
        :param pulumi.Input[_builtins.int] retries: Request retry count, default to 3 times.
        :param pulumi.Input[_builtins.str] scheme: Backend protocol, value range: HTTP, HTTPS, gRPC, gRPCs.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tag description list.
        :param pulumi.Input[_builtins.str] uniq_vpc_id: VPC Unique ID.
        :param pulumi.Input[_builtins.str] upstream_description: Backend channel description.
        :param pulumi.Input[_builtins.str] upstream_host: Host request header forwarded by gateway to backend.
        :param pulumi.Input[_builtins.str] upstream_name: Backend channel name.
        :param pulumi.Input[_builtins.str] upstream_type: Backend access type, value range: IP_PORT, K8S.
        """
        if algorithm is not None:
            pulumi.set(__self__, "algorithm", algorithm)
        if health_checker is not None:
            pulumi.set(__self__, "health_checker", health_checker)
        if k8s_services is not None:
            pulumi.set(__self__, "k8s_services", k8s_services)
        if nodes is not None:
            pulumi.set(__self__, "nodes", nodes)
        if retries is not None:
            pulumi.set(__self__, "retries", retries)
        if scheme is not None:
            pulumi.set(__self__, "scheme", scheme)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)
        if uniq_vpc_id is not None:
            pulumi.set(__self__, "uniq_vpc_id", uniq_vpc_id)
        if upstream_description is not None:
            pulumi.set(__self__, "upstream_description", upstream_description)
        if upstream_host is not None:
            pulumi.set(__self__, "upstream_host", upstream_host)
        if upstream_name is not None:
            pulumi.set(__self__, "upstream_name", upstream_name)
        if upstream_type is not None:
            pulumi.set(__self__, "upstream_type", upstream_type)

    @_builtins.property
    @pulumi.getter
    def algorithm(self) -> pulumi.Input[_builtins.str | None]:
        """
        Load balancing algorithm, value range: ROUND-ROBIN.
        """
        return pulumi.get(self, "algorithm")

    @algorithm.setter
    def algorithm(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "algorithm", value)

    @_builtins.property
    @pulumi.getter(name="healthChecker")
    def health_checker(self) -> pulumi.Input[Optional["UpstreamHealthCheckerArgs"]]:
        """
        Health check configuration, currently only supports VPC channels.
        """
        return pulumi.get(self, "health_checker")

    @health_checker.setter
    def health_checker(
        self, value: pulumi.Input[Optional["UpstreamHealthCheckerArgs"]]
    ):
        pulumi.set(self, "health_checker", value)

    @_builtins.property
    @pulumi.getter(name="k8sServices")
    def k8s_services(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input["UpstreamK8sServiceArgs"]] | None]:
        """
        Configuration of K8S container service.
        """
        return pulumi.get(self, "k8s_services")

    @k8s_services.setter
    def k8s_services(
        self,
        value: pulumi.Input[Sequence[pulumi.Input["UpstreamK8sServiceArgs"]] | None],
    ):
        pulumi.set(self, "k8s_services", value)

    @_builtins.property
    @pulumi.getter
    def nodes(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input["UpstreamNodeArgs"]] | None]:
        """
        Backend nodes.
        """
        return pulumi.get(self, "nodes")

    @nodes.setter
    def nodes(
        self, value: pulumi.Input[Sequence[pulumi.Input["UpstreamNodeArgs"]] | None]
    ):
        pulumi.set(self, "nodes", value)

    @_builtins.property
    @pulumi.getter
    def retries(self) -> pulumi.Input[_builtins.int | None]:
        """
        Request retry count, default to 3 times.
        """
        return pulumi.get(self, "retries")

    @retries.setter
    def retries(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "retries", value)

    @_builtins.property
    @pulumi.getter
    def scheme(self) -> pulumi.Input[_builtins.str | None]:
        """
        Backend protocol, value range: HTTP, HTTPS, gRPC, gRPCs.
        """
        return pulumi.get(self, "scheme")

    @scheme.setter
    def scheme(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "scheme", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]:
        """
        Tag description list.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(
        self, value: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "tags", value)

    @_builtins.property
    @pulumi.getter(name="uniqVpcId")
    def uniq_vpc_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        VPC Unique ID.
        """
        return pulumi.get(self, "uniq_vpc_id")

    @uniq_vpc_id.setter
    def uniq_vpc_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "uniq_vpc_id", value)

    @_builtins.property
    @pulumi.getter(name="upstreamDescription")
    def upstream_description(self) -> pulumi.Input[_builtins.str | None]:
        """
        Backend channel description.
        """
        return pulumi.get(self, "upstream_description")

    @upstream_description.setter
    def upstream_description(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "upstream_description", value)

    @_builtins.property
    @pulumi.getter(name="upstreamHost")
    def upstream_host(self) -> pulumi.Input[_builtins.str | None]:
        """
        Host request header forwarded by gateway to backend.
        """
        return pulumi.get(self, "upstream_host")

    @upstream_host.setter
    def upstream_host(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "upstream_host", value)

    @_builtins.property
    @pulumi.getter(name="upstreamName")
    def upstream_name(self) -> pulumi.Input[_builtins.str | None]:
        """
        Backend channel name.
        """
        return pulumi.get(self, "upstream_name")

    @upstream_name.setter
    def upstream_name(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "upstream_name", value)

    @_builtins.property
    @pulumi.getter(name="upstreamType")
    def upstream_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Backend access type, value range: IP_PORT, K8S.
        """
        return pulumi.get(self, "upstream_type")

    @upstream_type.setter
    def upstream_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "upstream_type", value)


@pulumi.type_token("tencentcloud:ApiGateway/upstream:Upstream")
class Upstream(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        algorithm: pulumi.Input[_builtins.str | None] = None,
        health_checker: pulumi.Input[
            Union["UpstreamHealthCheckerArgs", "UpstreamHealthCheckerArgsDict"] | None
        ] = None,
        k8s_services: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union["UpstreamK8sServiceArgs", "UpstreamK8sServiceArgsDict"]
                ]
            ]
            | None
        ] = None,
        nodes: pulumi.Input[
            Sequence[pulumi.Input[Union["UpstreamNodeArgs", "UpstreamNodeArgsDict"]]]
            | None
        ] = None,
        retries: pulumi.Input[_builtins.int | None] = None,
        scheme: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        uniq_vpc_id: pulumi.Input[_builtins.str | None] = None,
        upstream_description: pulumi.Input[_builtins.str | None] = None,
        upstream_host: pulumi.Input[_builtins.str | None] = None,
        upstream_name: pulumi.Input[_builtins.str | None] = None,
        upstream_type: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Provides a resource to create a apigateway upstream

        ## Example Usage

        ### Create a basic VPC channel

        ```python
        import pulumi
        import pulumi_tencentcloud as tencentcloud
        import tencentcloud_iac_pulumi as tencentcloud

        zones = tencentcloud.availability.get_zones_by_product(product="cvm")
        images = tencentcloud.images.get_instance(image_types=["PUBLIC_IMAGE"],
            image_name_regex="Final")
        instance_types = tencentcloud.instance.get_types(filters=[{
                "name": "instance-family",
                "values": ["S5"],
            }],
            cpu_core_count=2,
            exclude_sold_out=True)
        vpc = tencentcloud.vpc.Instance("vpc",
            name="vpc-example",
            cidr_block="10.0.0.0/16")
        subnet = tencentcloud.subnet.Instance("subnet",
            availability_zone=zones.zones[3].name,
            name="subnet-example",
            vpc_id=vpc.id,
            cidr_block="10.0.0.0/16",
            is_multicast=False)
        example = tencentcloud.instance.Instance("example",
            instance_name="tf_example",
            availability_zone=zones.zones[3].name,
            image_id=images.images[0].image_id,
            instance_type=instance_types.instance_types[0].instance_type,
            system_disk_type="CLOUD_PREMIUM",
            system_disk_size=50,
            hostname="terraform",
            project_id=0,
            vpc_id=vpc.id,
            subnet_id=subnet.id,
            data_disks=[{
                "data_disk_type": "CLOUD_PREMIUM",
                "data_disk_size": 50,
                "encrypt": False,
            }],
            tags={
                "tagKey": "tagValue",
            })
        example_upstream = tencentcloud.apigateway.Upstream("example",
            scheme="HTTP",
            algorithm="ROUND-ROBIN",
            uniq_vpc_id=vpc.id,
            upstream_name="tf_example",
            upstream_description="desc.",
            upstream_type="IP_PORT",
            retries=5,
            nodes=[{
                "host": "1.1.1.1",
                "port": 9090,
                "weight": 10,
                "vm_instance_id": example.id,
                "tags": ["tags"],
            }],
            tags={
                "createdBy": "terraform",
            })
        ```

        ### Create a complete VPC channel

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        example = tencentcloud.apigateway.Upstream("example",
            scheme="HTTP",
            algorithm="ROUND-ROBIN",
            uniq_vpc_id=vpc["id"],
            upstream_name="tf_example",
            upstream_description="desc.",
            upstream_type="IP_PORT",
            retries=5,
            nodes=[{
                "host": "1.1.1.1",
                "port": 9090,
                "weight": 10,
                "vm_instance_id": example_tencentcloud_instance["id"],
                "tags": ["tags"],
            }],
            health_checker={
                "enable_active_check": True,
                "enable_passive_check": True,
                "healthy_http_status": "200",
                "unhealthy_http_status": "500",
                "tcp_failure_threshold": 5,
                "timeout_threshold": 5,
                "http_failure_threshold": 3,
                "active_check_http_path": "/",
                "active_check_timeout": 5,
                "active_check_interval": 5,
                "unhealthy_timeout": 30,
            },
            tags={
                "createdBy": "terraform",
            })
        ```

        ## Import

        apigateway upstream can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:ApiGateway/upstream:Upstream upstream upstream_id
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] algorithm: Load balancing algorithm, value range: ROUND-ROBIN.
        :param pulumi.Input[Union['UpstreamHealthCheckerArgs', 'UpstreamHealthCheckerArgsDict']] health_checker: Health check configuration, currently only supports VPC channels.
        :param pulumi.Input[Sequence[pulumi.Input[Union['UpstreamK8sServiceArgs', 'UpstreamK8sServiceArgsDict']]]] k8s_services: Configuration of K8S container service.
        :param pulumi.Input[Sequence[pulumi.Input[Union['UpstreamNodeArgs', 'UpstreamNodeArgsDict']]]] nodes: Backend nodes.
        :param pulumi.Input[_builtins.int] retries: Request retry count, default to 3 times.
        :param pulumi.Input[_builtins.str] scheme: Backend protocol, value range: HTTP, HTTPS, gRPC, gRPCs.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tag description list.
        :param pulumi.Input[_builtins.str] uniq_vpc_id: VPC Unique ID.
        :param pulumi.Input[_builtins.str] upstream_description: Backend channel description.
        :param pulumi.Input[_builtins.str] upstream_host: Host request header forwarded by gateway to backend.
        :param pulumi.Input[_builtins.str] upstream_name: Backend channel name.
        :param pulumi.Input[_builtins.str] upstream_type: Backend access type, value range: IP_PORT, K8S.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: UpstreamArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Provides a resource to create a apigateway upstream

        ## Example Usage

        ### Create a basic VPC channel

        ```python
        import pulumi
        import pulumi_tencentcloud as tencentcloud
        import tencentcloud_iac_pulumi as tencentcloud

        zones = tencentcloud.availability.get_zones_by_product(product="cvm")
        images = tencentcloud.images.get_instance(image_types=["PUBLIC_IMAGE"],
            image_name_regex="Final")
        instance_types = tencentcloud.instance.get_types(filters=[{
                "name": "instance-family",
                "values": ["S5"],
            }],
            cpu_core_count=2,
            exclude_sold_out=True)
        vpc = tencentcloud.vpc.Instance("vpc",
            name="vpc-example",
            cidr_block="10.0.0.0/16")
        subnet = tencentcloud.subnet.Instance("subnet",
            availability_zone=zones.zones[3].name,
            name="subnet-example",
            vpc_id=vpc.id,
            cidr_block="10.0.0.0/16",
            is_multicast=False)
        example = tencentcloud.instance.Instance("example",
            instance_name="tf_example",
            availability_zone=zones.zones[3].name,
            image_id=images.images[0].image_id,
            instance_type=instance_types.instance_types[0].instance_type,
            system_disk_type="CLOUD_PREMIUM",
            system_disk_size=50,
            hostname="terraform",
            project_id=0,
            vpc_id=vpc.id,
            subnet_id=subnet.id,
            data_disks=[{
                "data_disk_type": "CLOUD_PREMIUM",
                "data_disk_size": 50,
                "encrypt": False,
            }],
            tags={
                "tagKey": "tagValue",
            })
        example_upstream = tencentcloud.apigateway.Upstream("example",
            scheme="HTTP",
            algorithm="ROUND-ROBIN",
            uniq_vpc_id=vpc.id,
            upstream_name="tf_example",
            upstream_description="desc.",
            upstream_type="IP_PORT",
            retries=5,
            nodes=[{
                "host": "1.1.1.1",
                "port": 9090,
                "weight": 10,
                "vm_instance_id": example.id,
                "tags": ["tags"],
            }],
            tags={
                "createdBy": "terraform",
            })
        ```

        ### Create a complete VPC channel

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        example = tencentcloud.apigateway.Upstream("example",
            scheme="HTTP",
            algorithm="ROUND-ROBIN",
            uniq_vpc_id=vpc["id"],
            upstream_name="tf_example",
            upstream_description="desc.",
            upstream_type="IP_PORT",
            retries=5,
            nodes=[{
                "host": "1.1.1.1",
                "port": 9090,
                "weight": 10,
                "vm_instance_id": example_tencentcloud_instance["id"],
                "tags": ["tags"],
            }],
            health_checker={
                "enable_active_check": True,
                "enable_passive_check": True,
                "healthy_http_status": "200",
                "unhealthy_http_status": "500",
                "tcp_failure_threshold": 5,
                "timeout_threshold": 5,
                "http_failure_threshold": 3,
                "active_check_http_path": "/",
                "active_check_timeout": 5,
                "active_check_interval": 5,
                "unhealthy_timeout": 30,
            },
            tags={
                "createdBy": "terraform",
            })
        ```

        ## Import

        apigateway upstream can be imported using the id, e.g.

        ```sh
        $ pulumi import tencentcloud:ApiGateway/upstream:Upstream upstream upstream_id
        ```


        :param str resource_name: The name of the resource.
        :param UpstreamArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            UpstreamArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        algorithm: pulumi.Input[_builtins.str | None] = None,
        health_checker: pulumi.Input[
            Union["UpstreamHealthCheckerArgs", "UpstreamHealthCheckerArgsDict"] | None
        ] = None,
        k8s_services: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union["UpstreamK8sServiceArgs", "UpstreamK8sServiceArgsDict"]
                ]
            ]
            | None
        ] = None,
        nodes: pulumi.Input[
            Sequence[pulumi.Input[Union["UpstreamNodeArgs", "UpstreamNodeArgsDict"]]]
            | None
        ] = None,
        retries: pulumi.Input[_builtins.int | None] = None,
        scheme: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        uniq_vpc_id: pulumi.Input[_builtins.str | None] = None,
        upstream_description: pulumi.Input[_builtins.str | None] = None,
        upstream_host: pulumi.Input[_builtins.str | None] = None,
        upstream_name: pulumi.Input[_builtins.str | None] = None,
        upstream_type: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = UpstreamArgs.__new__(UpstreamArgs)

            if algorithm is None and not opts.urn:
                raise TypeError("Missing required property 'algorithm'")
            __props__.__dict__["algorithm"] = algorithm
            __props__.__dict__["health_checker"] = health_checker
            __props__.__dict__["k8s_services"] = k8s_services
            __props__.__dict__["nodes"] = nodes
            __props__.__dict__["retries"] = retries
            if scheme is None and not opts.urn:
                raise TypeError("Missing required property 'scheme'")
            __props__.__dict__["scheme"] = scheme
            __props__.__dict__["tags"] = tags
            if uniq_vpc_id is None and not opts.urn:
                raise TypeError("Missing required property 'uniq_vpc_id'")
            __props__.__dict__["uniq_vpc_id"] = uniq_vpc_id
            __props__.__dict__["upstream_description"] = upstream_description
            __props__.__dict__["upstream_host"] = upstream_host
            __props__.__dict__["upstream_name"] = upstream_name
            __props__.__dict__["upstream_type"] = upstream_type
        super().__init__(
            "tencentcloud:ApiGateway/upstream:Upstream", resource_name, __props__, opts
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        algorithm: pulumi.Input[_builtins.str | None] = None,
        health_checker: pulumi.Input[
            Union["UpstreamHealthCheckerArgs", "UpstreamHealthCheckerArgsDict"] | None
        ] = None,
        k8s_services: pulumi.Input[
            Sequence[
                pulumi.Input[
                    Union["UpstreamK8sServiceArgs", "UpstreamK8sServiceArgsDict"]
                ]
            ]
            | None
        ] = None,
        nodes: pulumi.Input[
            Sequence[pulumi.Input[Union["UpstreamNodeArgs", "UpstreamNodeArgsDict"]]]
            | None
        ] = None,
        retries: pulumi.Input[_builtins.int | None] = None,
        scheme: pulumi.Input[_builtins.str | None] = None,
        tags: pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]] | None] = None,
        uniq_vpc_id: pulumi.Input[_builtins.str | None] = None,
        upstream_description: pulumi.Input[_builtins.str | None] = None,
        upstream_host: pulumi.Input[_builtins.str | None] = None,
        upstream_name: pulumi.Input[_builtins.str | None] = None,
        upstream_type: pulumi.Input[_builtins.str | None] = None,
    ) -> "Upstream":
        """
        Get an existing Upstream resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] algorithm: Load balancing algorithm, value range: ROUND-ROBIN.
        :param pulumi.Input[Union['UpstreamHealthCheckerArgs', 'UpstreamHealthCheckerArgsDict']] health_checker: Health check configuration, currently only supports VPC channels.
        :param pulumi.Input[Sequence[pulumi.Input[Union['UpstreamK8sServiceArgs', 'UpstreamK8sServiceArgsDict']]]] k8s_services: Configuration of K8S container service.
        :param pulumi.Input[Sequence[pulumi.Input[Union['UpstreamNodeArgs', 'UpstreamNodeArgsDict']]]] nodes: Backend nodes.
        :param pulumi.Input[_builtins.int] retries: Request retry count, default to 3 times.
        :param pulumi.Input[_builtins.str] scheme: Backend protocol, value range: HTTP, HTTPS, gRPC, gRPCs.
        :param pulumi.Input[Mapping[str, pulumi.Input[_builtins.str]]] tags: Tag description list.
        :param pulumi.Input[_builtins.str] uniq_vpc_id: VPC Unique ID.
        :param pulumi.Input[_builtins.str] upstream_description: Backend channel description.
        :param pulumi.Input[_builtins.str] upstream_host: Host request header forwarded by gateway to backend.
        :param pulumi.Input[_builtins.str] upstream_name: Backend channel name.
        :param pulumi.Input[_builtins.str] upstream_type: Backend access type, value range: IP_PORT, K8S.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _UpstreamState.__new__(_UpstreamState)

        __props__.__dict__["algorithm"] = algorithm
        __props__.__dict__["health_checker"] = health_checker
        __props__.__dict__["k8s_services"] = k8s_services
        __props__.__dict__["nodes"] = nodes
        __props__.__dict__["retries"] = retries
        __props__.__dict__["scheme"] = scheme
        __props__.__dict__["tags"] = tags
        __props__.__dict__["uniq_vpc_id"] = uniq_vpc_id
        __props__.__dict__["upstream_description"] = upstream_description
        __props__.__dict__["upstream_host"] = upstream_host
        __props__.__dict__["upstream_name"] = upstream_name
        __props__.__dict__["upstream_type"] = upstream_type
        return Upstream(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter
    def algorithm(self) -> pulumi.Output[_builtins.str]:
        """
        Load balancing algorithm, value range: ROUND-ROBIN.
        """
        return pulumi.get(self, "algorithm")

    @_builtins.property
    @pulumi.getter(name="healthChecker")
    def health_checker(
        self,
    ) -> pulumi.Output[Optional["outputs.UpstreamHealthChecker"]]:
        """
        Health check configuration, currently only supports VPC channels.
        """
        return pulumi.get(self, "health_checker")

    @_builtins.property
    @pulumi.getter(name="k8sServices")
    def k8s_services(
        self,
    ) -> pulumi.Output[Sequence["outputs.UpstreamK8sService"] | None]:
        """
        Configuration of K8S container service.
        """
        return pulumi.get(self, "k8s_services")

    @_builtins.property
    @pulumi.getter
    def nodes(self) -> pulumi.Output[Sequence["outputs.UpstreamNode"] | None]:
        """
        Backend nodes.
        """
        return pulumi.get(self, "nodes")

    @_builtins.property
    @pulumi.getter
    def retries(self) -> pulumi.Output[_builtins.int | None]:
        """
        Request retry count, default to 3 times.
        """
        return pulumi.get(self, "retries")

    @_builtins.property
    @pulumi.getter
    def scheme(self) -> pulumi.Output[_builtins.str]:
        """
        Backend protocol, value range: HTTP, HTTPS, gRPC, gRPCs.
        """
        return pulumi.get(self, "scheme")

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Output[Mapping[str, _builtins.str] | None]:
        """
        Tag description list.
        """
        return pulumi.get(self, "tags")

    @_builtins.property
    @pulumi.getter(name="uniqVpcId")
    def uniq_vpc_id(self) -> pulumi.Output[_builtins.str]:
        """
        VPC Unique ID.
        """
        return pulumi.get(self, "uniq_vpc_id")

    @_builtins.property
    @pulumi.getter(name="upstreamDescription")
    def upstream_description(self) -> pulumi.Output[_builtins.str | None]:
        """
        Backend channel description.
        """
        return pulumi.get(self, "upstream_description")

    @_builtins.property
    @pulumi.getter(name="upstreamHost")
    def upstream_host(self) -> pulumi.Output[_builtins.str | None]:
        """
        Host request header forwarded by gateway to backend.
        """
        return pulumi.get(self, "upstream_host")

    @_builtins.property
    @pulumi.getter(name="upstreamName")
    def upstream_name(self) -> pulumi.Output[_builtins.str | None]:
        """
        Backend channel name.
        """
        return pulumi.get(self, "upstream_name")

    @_builtins.property
    @pulumi.getter(name="upstreamType")
    def upstream_type(self) -> pulumi.Output[_builtins.str | None]:
        """
        Backend access type, value range: IP_PORT, K8S.
        """
        return pulumi.get(self, "upstream_type")
