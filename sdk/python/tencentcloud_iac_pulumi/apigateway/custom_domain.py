# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import sys
from collections.abc import Sequence
from typing import overload

import pulumi
import pulumi.runtime

if sys.version_info >= (3, 11):
    pass
else:
    pass
from .. import _utilities

__all__ = ["CustomDomain", "CustomDomainArgs"]


@pulumi.input_type
class CustomDomainArgs:
    def __init__(
        __self__,
        *,
        default_domain: pulumi.Input[_builtins.str],
        net_type: pulumi.Input[_builtins.str],
        protocol: pulumi.Input[_builtins.str],
        service_id: pulumi.Input[_builtins.str],
        sub_domain: pulumi.Input[_builtins.str],
        certificate_id: pulumi.Input[_builtins.str | None] = None,
        is_default_mapping: pulumi.Input[_builtins.bool | None] = None,
        is_forced_https: pulumi.Input[_builtins.bool | None] = None,
        path_mappings: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None] = None
    ):
        """
        The set of arguments for constructing a CustomDomain resource.

        :param pulumi.Input[_builtins.str] default_domain: Default domain name.
        :param pulumi.Input[_builtins.str] net_type: Network type. Valid values: `OUTER`, `INNER`.
        :param pulumi.Input[_builtins.str] protocol: Protocol supported by service. Valid values: `http`, `https`, `http&https`.
        :param pulumi.Input[_builtins.str] service_id: Unique service ID.
        :param pulumi.Input[_builtins.str] sub_domain: Custom domain name to be bound.
        :param pulumi.Input[_builtins.str] certificate_id: Unique certificate ID of the custom domain name to be bound. You can choose to upload for the `protocol` attribute value `https` or `http&https`.
        :param pulumi.Input[_builtins.bool] is_default_mapping: Whether the default path mapping is used. The default value is `true`. When it is `false`, it means custom path mapping. In this case, the `path_mappings` attribute is required.
        :param pulumi.Input[_builtins.bool] is_forced_https: Whether to force HTTP requests to jump to HTTPS, default to false. When the parameter is true, the API gateway will redirect all HTTP protocol requests using the custom domain name to the HTTPS protocol for forwarding.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] path_mappings: Custom domain name path mapping. The data format is: `path#environment`. Optional values for the environment are `test`, `prepub`, and `release`.
        """
        pulumi.set(__self__, "default_domain", default_domain)
        pulumi.set(__self__, "net_type", net_type)
        pulumi.set(__self__, "protocol", protocol)
        pulumi.set(__self__, "service_id", service_id)
        pulumi.set(__self__, "sub_domain", sub_domain)
        if certificate_id is not None:
            pulumi.set(__self__, "certificate_id", certificate_id)
        if is_default_mapping is not None:
            pulumi.set(__self__, "is_default_mapping", is_default_mapping)
        if is_forced_https is not None:
            pulumi.set(__self__, "is_forced_https", is_forced_https)
        if path_mappings is not None:
            pulumi.set(__self__, "path_mappings", path_mappings)

    @_builtins.property
    @pulumi.getter(name="defaultDomain")
    def default_domain(self) -> pulumi.Input[_builtins.str]:
        """
        Default domain name.
        """
        return pulumi.get(self, "default_domain")

    @default_domain.setter
    def default_domain(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "default_domain", value)

    @_builtins.property
    @pulumi.getter(name="netType")
    def net_type(self) -> pulumi.Input[_builtins.str]:
        """
        Network type. Valid values: `OUTER`, `INNER`.
        """
        return pulumi.get(self, "net_type")

    @net_type.setter
    def net_type(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "net_type", value)

    @_builtins.property
    @pulumi.getter
    def protocol(self) -> pulumi.Input[_builtins.str]:
        """
        Protocol supported by service. Valid values: `http`, `https`, `http&https`.
        """
        return pulumi.get(self, "protocol")

    @protocol.setter
    def protocol(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "protocol", value)

    @_builtins.property
    @pulumi.getter(name="serviceId")
    def service_id(self) -> pulumi.Input[_builtins.str]:
        """
        Unique service ID.
        """
        return pulumi.get(self, "service_id")

    @service_id.setter
    def service_id(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "service_id", value)

    @_builtins.property
    @pulumi.getter(name="subDomain")
    def sub_domain(self) -> pulumi.Input[_builtins.str]:
        """
        Custom domain name to be bound.
        """
        return pulumi.get(self, "sub_domain")

    @sub_domain.setter
    def sub_domain(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "sub_domain", value)

    @_builtins.property
    @pulumi.getter(name="certificateId")
    def certificate_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Unique certificate ID of the custom domain name to be bound. You can choose to upload for the `protocol` attribute value `https` or `http&https`.
        """
        return pulumi.get(self, "certificate_id")

    @certificate_id.setter
    def certificate_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "certificate_id", value)

    @_builtins.property
    @pulumi.getter(name="isDefaultMapping")
    def is_default_mapping(self) -> pulumi.Input[_builtins.bool | None]:
        """
        Whether the default path mapping is used. The default value is `true`. When it is `false`, it means custom path mapping. In this case, the `path_mappings` attribute is required.
        """
        return pulumi.get(self, "is_default_mapping")

    @is_default_mapping.setter
    def is_default_mapping(self, value: pulumi.Input[_builtins.bool | None]):
        pulumi.set(self, "is_default_mapping", value)

    @_builtins.property
    @pulumi.getter(name="isForcedHttps")
    def is_forced_https(self) -> pulumi.Input[_builtins.bool | None]:
        """
        Whether to force HTTP requests to jump to HTTPS, default to false. When the parameter is true, the API gateway will redirect all HTTP protocol requests using the custom domain name to the HTTPS protocol for forwarding.
        """
        return pulumi.get(self, "is_forced_https")

    @is_forced_https.setter
    def is_forced_https(self, value: pulumi.Input[_builtins.bool | None]):
        pulumi.set(self, "is_forced_https", value)

    @_builtins.property
    @pulumi.getter(name="pathMappings")
    def path_mappings(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        Custom domain name path mapping. The data format is: `path#environment`. Optional values for the environment are `test`, `prepub`, and `release`.
        """
        return pulumi.get(self, "path_mappings")

    @path_mappings.setter
    def path_mappings(
        self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "path_mappings", value)


@pulumi.input_type
class _CustomDomainState:
    def __init__(
        __self__,
        *,
        certificate_id: pulumi.Input[_builtins.str | None] = None,
        default_domain: pulumi.Input[_builtins.str | None] = None,
        is_default_mapping: pulumi.Input[_builtins.bool | None] = None,
        is_forced_https: pulumi.Input[_builtins.bool | None] = None,
        net_type: pulumi.Input[_builtins.str | None] = None,
        path_mappings: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None,
        protocol: pulumi.Input[_builtins.str | None] = None,
        service_id: pulumi.Input[_builtins.str | None] = None,
        status: pulumi.Input[_builtins.int | None] = None,
        sub_domain: pulumi.Input[_builtins.str | None] = None
    ):
        """
        Input properties used for looking up and filtering CustomDomain resources.

        :param pulumi.Input[_builtins.str] certificate_id: Unique certificate ID of the custom domain name to be bound. You can choose to upload for the `protocol` attribute value `https` or `http&https`.
        :param pulumi.Input[_builtins.str] default_domain: Default domain name.
        :param pulumi.Input[_builtins.bool] is_default_mapping: Whether the default path mapping is used. The default value is `true`. When it is `false`, it means custom path mapping. In this case, the `path_mappings` attribute is required.
        :param pulumi.Input[_builtins.bool] is_forced_https: Whether to force HTTP requests to jump to HTTPS, default to false. When the parameter is true, the API gateway will redirect all HTTP protocol requests using the custom domain name to the HTTPS protocol for forwarding.
        :param pulumi.Input[_builtins.str] net_type: Network type. Valid values: `OUTER`, `INNER`.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] path_mappings: Custom domain name path mapping. The data format is: `path#environment`. Optional values for the environment are `test`, `prepub`, and `release`.
        :param pulumi.Input[_builtins.str] protocol: Protocol supported by service. Valid values: `http`, `https`, `http&https`.
        :param pulumi.Input[_builtins.str] service_id: Unique service ID.
        :param pulumi.Input[_builtins.int] status: Domain name resolution status. `1` means normal analysis, `0` means parsing failed.
        :param pulumi.Input[_builtins.str] sub_domain: Custom domain name to be bound.
        """
        if certificate_id is not None:
            pulumi.set(__self__, "certificate_id", certificate_id)
        if default_domain is not None:
            pulumi.set(__self__, "default_domain", default_domain)
        if is_default_mapping is not None:
            pulumi.set(__self__, "is_default_mapping", is_default_mapping)
        if is_forced_https is not None:
            pulumi.set(__self__, "is_forced_https", is_forced_https)
        if net_type is not None:
            pulumi.set(__self__, "net_type", net_type)
        if path_mappings is not None:
            pulumi.set(__self__, "path_mappings", path_mappings)
        if protocol is not None:
            pulumi.set(__self__, "protocol", protocol)
        if service_id is not None:
            pulumi.set(__self__, "service_id", service_id)
        if status is not None:
            pulumi.set(__self__, "status", status)
        if sub_domain is not None:
            pulumi.set(__self__, "sub_domain", sub_domain)

    @_builtins.property
    @pulumi.getter(name="certificateId")
    def certificate_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Unique certificate ID of the custom domain name to be bound. You can choose to upload for the `protocol` attribute value `https` or `http&https`.
        """
        return pulumi.get(self, "certificate_id")

    @certificate_id.setter
    def certificate_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "certificate_id", value)

    @_builtins.property
    @pulumi.getter(name="defaultDomain")
    def default_domain(self) -> pulumi.Input[_builtins.str | None]:
        """
        Default domain name.
        """
        return pulumi.get(self, "default_domain")

    @default_domain.setter
    def default_domain(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "default_domain", value)

    @_builtins.property
    @pulumi.getter(name="isDefaultMapping")
    def is_default_mapping(self) -> pulumi.Input[_builtins.bool | None]:
        """
        Whether the default path mapping is used. The default value is `true`. When it is `false`, it means custom path mapping. In this case, the `path_mappings` attribute is required.
        """
        return pulumi.get(self, "is_default_mapping")

    @is_default_mapping.setter
    def is_default_mapping(self, value: pulumi.Input[_builtins.bool | None]):
        pulumi.set(self, "is_default_mapping", value)

    @_builtins.property
    @pulumi.getter(name="isForcedHttps")
    def is_forced_https(self) -> pulumi.Input[_builtins.bool | None]:
        """
        Whether to force HTTP requests to jump to HTTPS, default to false. When the parameter is true, the API gateway will redirect all HTTP protocol requests using the custom domain name to the HTTPS protocol for forwarding.
        """
        return pulumi.get(self, "is_forced_https")

    @is_forced_https.setter
    def is_forced_https(self, value: pulumi.Input[_builtins.bool | None]):
        pulumi.set(self, "is_forced_https", value)

    @_builtins.property
    @pulumi.getter(name="netType")
    def net_type(self) -> pulumi.Input[_builtins.str | None]:
        """
        Network type. Valid values: `OUTER`, `INNER`.
        """
        return pulumi.get(self, "net_type")

    @net_type.setter
    def net_type(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "net_type", value)

    @_builtins.property
    @pulumi.getter(name="pathMappings")
    def path_mappings(
        self,
    ) -> pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]:
        """
        Custom domain name path mapping. The data format is: `path#environment`. Optional values for the environment are `test`, `prepub`, and `release`.
        """
        return pulumi.get(self, "path_mappings")

    @path_mappings.setter
    def path_mappings(
        self, value: pulumi.Input[Sequence[pulumi.Input[_builtins.str]] | None]
    ):
        pulumi.set(self, "path_mappings", value)

    @_builtins.property
    @pulumi.getter
    def protocol(self) -> pulumi.Input[_builtins.str | None]:
        """
        Protocol supported by service. Valid values: `http`, `https`, `http&https`.
        """
        return pulumi.get(self, "protocol")

    @protocol.setter
    def protocol(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "protocol", value)

    @_builtins.property
    @pulumi.getter(name="serviceId")
    def service_id(self) -> pulumi.Input[_builtins.str | None]:
        """
        Unique service ID.
        """
        return pulumi.get(self, "service_id")

    @service_id.setter
    def service_id(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "service_id", value)

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Input[_builtins.int | None]:
        """
        Domain name resolution status. `1` means normal analysis, `0` means parsing failed.
        """
        return pulumi.get(self, "status")

    @status.setter
    def status(self, value: pulumi.Input[_builtins.int | None]):
        pulumi.set(self, "status", value)

    @_builtins.property
    @pulumi.getter(name="subDomain")
    def sub_domain(self) -> pulumi.Input[_builtins.str | None]:
        """
        Custom domain name to be bound.
        """
        return pulumi.get(self, "sub_domain")

    @sub_domain.setter
    def sub_domain(self, value: pulumi.Input[_builtins.str | None]):
        pulumi.set(self, "sub_domain", value)


@pulumi.type_token("tencentcloud:ApiGateway/customDomain:CustomDomain")
class CustomDomain(pulumi.CustomResource):
    @overload
    def __init__(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        certificate_id: pulumi.Input[_builtins.str | None] = None,
        default_domain: pulumi.Input[_builtins.str | None] = None,
        is_default_mapping: pulumi.Input[_builtins.bool | None] = None,
        is_forced_https: pulumi.Input[_builtins.bool | None] = None,
        net_type: pulumi.Input[_builtins.str | None] = None,
        path_mappings: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None,
        protocol: pulumi.Input[_builtins.str | None] = None,
        service_id: pulumi.Input[_builtins.str | None] = None,
        sub_domain: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        """
        Use this resource to create custom domain of API gateway.

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        foo = tencentcloud.apigateway.CustomDomain("foo",
            service_id="service-ohxqslqe",
            sub_domain="tic-test.dnsv1.com",
            protocol="http",
            net_type="OUTER",
            is_default_mapping=False,
            default_domain="service-ohxqslqe-1259649581.gz.apigw.tencentcs.com",
            path_mappings=[
                "/good#test",
                "/root#release",
            ])
        ```


        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] certificate_id: Unique certificate ID of the custom domain name to be bound. You can choose to upload for the `protocol` attribute value `https` or `http&https`.
        :param pulumi.Input[_builtins.str] default_domain: Default domain name.
        :param pulumi.Input[_builtins.bool] is_default_mapping: Whether the default path mapping is used. The default value is `true`. When it is `false`, it means custom path mapping. In this case, the `path_mappings` attribute is required.
        :param pulumi.Input[_builtins.bool] is_forced_https: Whether to force HTTP requests to jump to HTTPS, default to false. When the parameter is true, the API gateway will redirect all HTTP protocol requests using the custom domain name to the HTTPS protocol for forwarding.
        :param pulumi.Input[_builtins.str] net_type: Network type. Valid values: `OUTER`, `INNER`.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] path_mappings: Custom domain name path mapping. The data format is: `path#environment`. Optional values for the environment are `test`, `prepub`, and `release`.
        :param pulumi.Input[_builtins.str] protocol: Protocol supported by service. Valid values: `http`, `https`, `http&https`.
        :param pulumi.Input[_builtins.str] service_id: Unique service ID.
        :param pulumi.Input[_builtins.str] sub_domain: Custom domain name to be bound.
        """

    @overload
    def __init__(
        __self__,
        resource_name: str,
        args: CustomDomainArgs,
        opts: pulumi.ResourceOptions | None = None,
    ):
        """
        Use this resource to create custom domain of API gateway.

        ## Example Usage

        ```python
        import pulumi
        import tencentcloud_iac_pulumi as tencentcloud

        foo = tencentcloud.apigateway.CustomDomain("foo",
            service_id="service-ohxqslqe",
            sub_domain="tic-test.dnsv1.com",
            protocol="http",
            net_type="OUTER",
            is_default_mapping=False,
            default_domain="service-ohxqslqe-1259649581.gz.apigw.tencentcs.com",
            path_mappings=[
                "/good#test",
                "/root#release",
            ])
        ```


        :param str resource_name: The name of the resource.
        :param CustomDomainArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """

    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(
            CustomDomainArgs, pulumi.ResourceOptions, *args, **kwargs
        )
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(
        __self__,
        resource_name: str,
        opts: pulumi.ResourceOptions | None = None,
        certificate_id: pulumi.Input[_builtins.str | None] = None,
        default_domain: pulumi.Input[_builtins.str | None] = None,
        is_default_mapping: pulumi.Input[_builtins.bool | None] = None,
        is_forced_https: pulumi.Input[_builtins.bool | None] = None,
        net_type: pulumi.Input[_builtins.str | None] = None,
        path_mappings: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None,
        protocol: pulumi.Input[_builtins.str | None] = None,
        service_id: pulumi.Input[_builtins.str | None] = None,
        sub_domain: pulumi.Input[_builtins.str | None] = None,
        __props__=None,
    ):
        opts = pulumi.ResourceOptions.merge(
            _utilities.get_resource_opts_defaults(), opts
        )
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError(
                "Expected resource options to be a ResourceOptions instance"
            )
        if opts.id is None:
            if __props__ is not None:
                raise TypeError(
                    "__props__ is only valid when passed in combination with a valid opts.id to get an existing resource"
                )
            __props__ = CustomDomainArgs.__new__(CustomDomainArgs)

            __props__.__dict__["certificate_id"] = certificate_id
            if default_domain is None and not opts.urn:
                raise TypeError("Missing required property 'default_domain'")
            __props__.__dict__["default_domain"] = default_domain
            __props__.__dict__["is_default_mapping"] = is_default_mapping
            __props__.__dict__["is_forced_https"] = is_forced_https
            if net_type is None and not opts.urn:
                raise TypeError("Missing required property 'net_type'")
            __props__.__dict__["net_type"] = net_type
            __props__.__dict__["path_mappings"] = path_mappings
            if protocol is None and not opts.urn:
                raise TypeError("Missing required property 'protocol'")
            __props__.__dict__["protocol"] = protocol
            if service_id is None and not opts.urn:
                raise TypeError("Missing required property 'service_id'")
            __props__.__dict__["service_id"] = service_id
            if sub_domain is None and not opts.urn:
                raise TypeError("Missing required property 'sub_domain'")
            __props__.__dict__["sub_domain"] = sub_domain
            __props__.__dict__["status"] = None
        super().__init__(
            "tencentcloud:ApiGateway/customDomain:CustomDomain",
            resource_name,
            __props__,
            opts,
        )

    @staticmethod
    def get(
        resource_name: str,
        id: pulumi.Input[str],
        opts: pulumi.ResourceOptions | None = None,
        certificate_id: pulumi.Input[_builtins.str | None] = None,
        default_domain: pulumi.Input[_builtins.str | None] = None,
        is_default_mapping: pulumi.Input[_builtins.bool | None] = None,
        is_forced_https: pulumi.Input[_builtins.bool | None] = None,
        net_type: pulumi.Input[_builtins.str | None] = None,
        path_mappings: pulumi.Input[
            Sequence[pulumi.Input[_builtins.str]] | None
        ] = None,
        protocol: pulumi.Input[_builtins.str | None] = None,
        service_id: pulumi.Input[_builtins.str | None] = None,
        status: pulumi.Input[_builtins.int | None] = None,
        sub_domain: pulumi.Input[_builtins.str | None] = None,
    ) -> "CustomDomain":
        """
        Get an existing CustomDomain resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] certificate_id: Unique certificate ID of the custom domain name to be bound. You can choose to upload for the `protocol` attribute value `https` or `http&https`.
        :param pulumi.Input[_builtins.str] default_domain: Default domain name.
        :param pulumi.Input[_builtins.bool] is_default_mapping: Whether the default path mapping is used. The default value is `true`. When it is `false`, it means custom path mapping. In this case, the `path_mappings` attribute is required.
        :param pulumi.Input[_builtins.bool] is_forced_https: Whether to force HTTP requests to jump to HTTPS, default to false. When the parameter is true, the API gateway will redirect all HTTP protocol requests using the custom domain name to the HTTPS protocol for forwarding.
        :param pulumi.Input[_builtins.str] net_type: Network type. Valid values: `OUTER`, `INNER`.
        :param pulumi.Input[Sequence[pulumi.Input[_builtins.str]]] path_mappings: Custom domain name path mapping. The data format is: `path#environment`. Optional values for the environment are `test`, `prepub`, and `release`.
        :param pulumi.Input[_builtins.str] protocol: Protocol supported by service. Valid values: `http`, `https`, `http&https`.
        :param pulumi.Input[_builtins.str] service_id: Unique service ID.
        :param pulumi.Input[_builtins.int] status: Domain name resolution status. `1` means normal analysis, `0` means parsing failed.
        :param pulumi.Input[_builtins.str] sub_domain: Custom domain name to be bound.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _CustomDomainState.__new__(_CustomDomainState)

        __props__.__dict__["certificate_id"] = certificate_id
        __props__.__dict__["default_domain"] = default_domain
        __props__.__dict__["is_default_mapping"] = is_default_mapping
        __props__.__dict__["is_forced_https"] = is_forced_https
        __props__.__dict__["net_type"] = net_type
        __props__.__dict__["path_mappings"] = path_mappings
        __props__.__dict__["protocol"] = protocol
        __props__.__dict__["service_id"] = service_id
        __props__.__dict__["status"] = status
        __props__.__dict__["sub_domain"] = sub_domain
        return CustomDomain(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter(name="certificateId")
    def certificate_id(self) -> pulumi.Output[_builtins.str]:
        """
        Unique certificate ID of the custom domain name to be bound. You can choose to upload for the `protocol` attribute value `https` or `http&https`.
        """
        return pulumi.get(self, "certificate_id")

    @_builtins.property
    @pulumi.getter(name="defaultDomain")
    def default_domain(self) -> pulumi.Output[_builtins.str]:
        """
        Default domain name.
        """
        return pulumi.get(self, "default_domain")

    @_builtins.property
    @pulumi.getter(name="isDefaultMapping")
    def is_default_mapping(self) -> pulumi.Output[_builtins.bool | None]:
        """
        Whether the default path mapping is used. The default value is `true`. When it is `false`, it means custom path mapping. In this case, the `path_mappings` attribute is required.
        """
        return pulumi.get(self, "is_default_mapping")

    @_builtins.property
    @pulumi.getter(name="isForcedHttps")
    def is_forced_https(self) -> pulumi.Output[_builtins.bool | None]:
        """
        Whether to force HTTP requests to jump to HTTPS, default to false. When the parameter is true, the API gateway will redirect all HTTP protocol requests using the custom domain name to the HTTPS protocol for forwarding.
        """
        return pulumi.get(self, "is_forced_https")

    @_builtins.property
    @pulumi.getter(name="netType")
    def net_type(self) -> pulumi.Output[_builtins.str]:
        """
        Network type. Valid values: `OUTER`, `INNER`.
        """
        return pulumi.get(self, "net_type")

    @_builtins.property
    @pulumi.getter(name="pathMappings")
    def path_mappings(self) -> pulumi.Output[Sequence[_builtins.str]]:
        """
        Custom domain name path mapping. The data format is: `path#environment`. Optional values for the environment are `test`, `prepub`, and `release`.
        """
        return pulumi.get(self, "path_mappings")

    @_builtins.property
    @pulumi.getter
    def protocol(self) -> pulumi.Output[_builtins.str]:
        """
        Protocol supported by service. Valid values: `http`, `https`, `http&https`.
        """
        return pulumi.get(self, "protocol")

    @_builtins.property
    @pulumi.getter(name="serviceId")
    def service_id(self) -> pulumi.Output[_builtins.str]:
        """
        Unique service ID.
        """
        return pulumi.get(self, "service_id")

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Output[_builtins.int]:
        """
        Domain name resolution status. `1` means normal analysis, `0` means parsing failed.
        """
        return pulumi.get(self, "status")

    @_builtins.property
    @pulumi.getter(name="subDomain")
    def sub_domain(self) -> pulumi.Output[_builtins.str]:
        """
        Custom domain name to be bound.
        """
        return pulumi.get(self, "sub_domain")
